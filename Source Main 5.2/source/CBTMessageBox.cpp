...
int leaf::CBTMessageBox( HWND hWnd, const std::wstring& text, const std::wstring& caption, UINT uType,
                         bool bAlwaysOnTop )
{
...
int leaf::CCBTMessageBox::OpenMessageBox( HWND hWnd, const wchar_t* lpText, const wchar_t* lpCaption, UINT uType,
                                          bool bAlwaysOnTop )
{
...
LRESULT CALLBACK leaf::CCBTMessageBox::CBTProc( INT nCode, WPARAM wParam, LPARAM lParam )
{
  ...
  if ( nCode == HCBT_ACTIVATE && GetInstance()->GetHookHandle() ) {
    ...
    if ( ( hParentWnd != nullptr ) && ( hChildWnd != nullptr ) &&
         ( GetWindowRect( GetDesktopWindow(), &rDesktop ) != 0 ) && ( GetWindowRect( hParentWnd, &rParent ) != 0 ) &&
  ...
  // otherwise, continue with any possible chained hooks
    ...
    CallNextHookEx( GetInstance()->GetHookHandle(), nCode, wParam, lParam );
  }
