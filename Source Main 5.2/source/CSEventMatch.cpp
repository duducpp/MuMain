...
void CSBaseMatch::RenderTime( void )
{
  ...
  if ( m_iMatchCountDownType >= TYPE_MATCH_CASTLE_ENTER_CLOSE && m_iMatchCountDownType <= TYPE_MATCH_CASTLE_END ) {
    ...
    WriteWide( lpszStr, GlobalText[textNum], GlobalText[1'146], remainingSeconds );
    ...
    WriteWide( lpszStr, GlobalText[textNum], GlobalText[1'147], remainingSeconds );
    ...
    int textNum = ( m_iMatchCountDownType == TYPE_MATCH_CURSEDTEMPLE_GAME_START ) ? 2'386 : 2'384;
    WriteWide( lpszStr, GlobalText[textNum], remainingSeconds );
  } else if ( m_iMatchCountDownType >= TYPE_MATCH_DOPPELGANGER_ENTER_CLOSE &&
              m_iMatchCountDownType <= TYPE_MATCH_DOPPELGANGER_CLOSE ) {
    const int textNum = 2'860 + m_iMatchCountDownType - TYPE_MATCH_DOPPELGANGER_ENTER_CLOSE;
    WriteWide( lpszStr, GlobalText[textNum], remainingSeconds );
  } else {
