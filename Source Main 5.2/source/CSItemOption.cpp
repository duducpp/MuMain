...
struct ITEM_SET_OPTION_FILE
{
  ...
  std::array<std::array<std::uint8_t, MAX_ITEM_SET_STANDARD_OPTION_PER_ITEM_COUNT>, MAX_ITEM_SET_STANDARD_OPTION_COUNT>
      byStandardOption{};
  std::array<std::array<std::uint8_t, MAX_ITEM_SET_STANDARD_OPTION_PER_ITEM_COUNT>, MAX_ITEM_SET_STANDARD_OPTION_COUNT>
...
bool CSItemOption::IsDisableSkill( ActionSkillType Type, int Energy, int Charisma )
{
  ...
  if ( Type >= AT_SKILL_STUN && Type <= AT_SKILL_REMOVAL_BUFF ) {
    SkillEnergy = 0;
  } else if ( ( Type >= 18 && Type <= 23 ) || ( Type >= 41 && Type <= 43 ) || ( Type >= 47 && Type <= 49 ) ||
              Type == 24 || Type == 51 || Type == 52 || Type == 55 || Type == 56 ) {
    SkillEnergy = 0;
...
bool CSItemOption::isClassRequirementFulfilled( const ITEM_SET_OPTION& setOptions, const int firstClass,
                                                int secondClass )
{
  ...
  for ( int i = 0; i < m_SetSearchResultCount; ++i ) {
    ...
    for ( int o = 0; o < standardOptionCount; ++o ) {
      for ( int n = 0; n < MAX_ITEM_SET_STANDARD_OPTION_PER_ITEM_COUNT; ++n ) {
        TryAddSetOption( setOptions.byStandardOption[o][n], setOptions.byStandardOptionValue[o][n], o, set, setOptions,
                         isThisSetComplete, false, false, requireClass, firstClass, secondClass );
      }
    }

    for ( int o = 0; o < MAX_ITEM_SET_EXT_OPTION_COUNT; ++o ) {
      TryAddSetOption( setOptions.byExtOption[o], setOptions.byExtOptionValue[o], 0, set, setOptions, isThisSetComplete,
                       false, true, requireClass, firstClass, secondClass );
    }
...
bool CSItemOption::getExplainText( wchar_t* text, std::uint8_t option, int value )
{
  ...
  switch ( option + AT_SET_OPTION_IMPROVE_STRENGTH ) {
    ...
    case AT_SET_OPTION_EARTH_MASTERY:
      swprintf( text, GlobalText[971 + ( option + AT_SET_OPTION_IMPROVE_STRENGTH - AT_SET_OPTION_IMPROVE_ATTACK_2 )],
                value );
      return true;
    default:
      return false;
...
int CSItemOption::RenderDefaultOptionText( const ITEM* ip, int TextNum )
{
  ...
  if ( GetDefaultOptionText( ip, TextList[TNum] ) ) {
    ...
    if ( ( ip->Type >= ITEM_RING_OF_ICE && ip->Type <= ITEM_RING_OF_POISON ) ||
      ...
      swprintf( TextList[TNum], GlobalText[1'165] ); // "Increase Attribute Damage"
      TextListColor[TNum] = TEXT_COLOR_BLUE;
      TNum++;
    }
  }
...
void CSItemOption::RenderSetOptionList( const int StartX, const int StartY )
{
  if ( m_bViewOptionList && m_SetSearchResultCount > 0 ) {
    ...
    for ( int i = 0; i < m_SetSearchResultCount; i++ ) {
      ...
      // print set name:
      swprintf( TextList[TextNum], L"%ls %ls", set.SetName, GlobalText[1'089] );
...
void CSItemOption::RenderOptionHelper( void )
{
  ...
  swprintf( TextList[TextNum], L"\n" );
  TextNum++;
  swprintf( TextList[TextNum], L"%ls %ls %ls", setOption.strSetName, GlobalText[1'089], GlobalText[159] );
  TextListColor[TextNum] = TEXT_COLOR_YELLOW;
  TextNum++;
...
int CSItemOption::RenderSetOptionListInItem( const ITEM* ip, int TextNum, bool bIsEquippedItem )
{
  ...
  TNum++;
  swprintf( TextList[TNum], L"%ls %ls", GlobalText[1'089], GlobalText[159] );
