    ...
    if ( pQuest->QuestAct[i].byRequestClass[m_byClass] != 0 ) {
      ...
      for ( int j = 0; j < pQuest->shQuestRequestNum; ++j ) {
        if ( pQuest->QuestRequest[j].byType == iRequestType || pQuest->QuestRequest[j].byType == 255 ) {
          if ( pQuest->QuestRequest[j].wCompleteQuestIndex != 65'535 ) {
...
void CSQuest::RenderDevilSquare( void )
{
  ...
  g_pRenderText->RenderText( m_iStartX + 95 - 60, m_iStartY + 12, GlobalText[1'145], 120, 0, RT3_SORT_CENTER );
...
void CSQuest::RenderBloodCastle( void )
{
  ...
  g_pRenderText->SetTextColor( 230, 230, 230, 255 );
  g_pRenderText->SetBgColor( 20, 20, 20, 255 );
  g_pRenderText->RenderText( m_iStartX + 95 - 60, m_iStartY + 12, GlobalText[1'146], 120, 0, RT3_SORT_CENTER );

  g_pRenderText->SetTextColor( 223, 191, 103, 255 );
  g_pRenderText->SetBgColor( 0 );
  swprintf_s( Text, std::size( Text ), GlobalText[869], BLOODCASTLE_QUEST_NUM, GlobalText[1'146], GlobalText[1'140] );
