...
void CCharInfoBalloon::Render()
{
  ...
  g_pRenderText->RenderText( nTextPosX, int( ( spriteY + 6 ) / g_fScreenRate_y ), m_szName, spriteW / g_fScreenRate_x,
                             0, RT3_SORT_CENTER );

  g_pRenderText->SetTextColor( CLRDW_WHITE );
  g_pRenderText->RenderText( nTextPosX, int( ( spriteY + 22 ) / g_fScreenRate_y ), m_szGuild, spriteW / g_fScreenRate_x,
                             0, RT3_SORT_CENTER );

  g_pRenderText->SetTextColor( CLRDW_BR_ORANGE );
  g_pRenderText->RenderText( nTextPosX, int( ( spriteY + 38 ) / g_fScreenRate_y ), m_szClass, spriteW / g_fScreenRate_x,
                             0, RT3_SORT_CENTER );
}
...
void CCharInfoBalloon::SetInfo()
{
  ...
  swprintf_s( m_szClass, L"%ls %d", gCharacterManager.GetCharacterClassText( m_pCharInfo->Class ), m_pCharInfo->Level );
}
