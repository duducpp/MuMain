...
public:
  ...
  /// </summary>
  /// <remarks>
  /// Is sent by the client when: This packet is sent by the client (below season 1) after it connected and received the
  /// 'Hello' message. Causes reaction on server side: The server will send a ServerListResponseOld back to the client.
  /// </remarks>
  ...
  /// </summary>
  /// <param name="patchVersion">The patch version.</param>
  /// <param name="patchAddress">The patch address, usually to a ftp server. The address is usually "encrypted" with the
  /// 3-byte XOR key (FC CF AB).</param> <remarks> Is sent by the server when: This packet is sent by the server after
  /// the client (launcher) requested to check the patch version and it requires an update. Causes reaction on client
  /// side: The launcher will download the required patches and then activate the start button.
  /// </remarks>
