...
public:
  ...
  template<class T>
  xstreambuf& operator<<( const T& data )
  {
    return write( (void*)( &data ), sizeof( data ) );
  }

  xstreambuf& operator<<( char* sz ) { return write( sz, strlen( sz ) + 1 ); }
  xstreambuf& operator<<( const char* sz ) { return write( sz, strlen( sz ) + 1 ); }
  xstreambuf& operator<<( std::string str ) { return write( str.data(), str.size() + 1 ); }

  template<class T>
  xstreambuf& operator>>( T& data )
  {
    return read( (char*)&data, sizeof( data ) );
  }

  xstreambuf& operator>>( char* sz ) { return read( sz, strlen( (const char*)( m_pBuffer ) + m_offset ) + 1 ); }
