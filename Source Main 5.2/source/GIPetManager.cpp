...
bool RenderPetCmdInfo( int sx, int sy, int Type )
{
  ...
  if ( gCharacterManager.GetBaseClass( Hero->Class ) == CLASS_DARK_LORD ) {
    ...
    swprintf( TextList[TextNum], GlobalText[1'219 + cmdType] );
    TextNum++;
    SkipNum++;
    ...
    switch ( cmdType ) {
      case PET_CMD_DEFAULT:
        swprintf( TextList[TextNum], GlobalText[1'223] );
        TextNum++;
        SkipNum++;
        break;
      case PET_CMD_RANDOM:
        swprintf( TextList[TextNum], GlobalText[1'224] );
        TextNum++;
        SkipNum++;
        break;
      case PET_CMD_OWNER:
        swprintf( TextList[TextNum], GlobalText[1'225] );
        TextNum++;
        SkipNum++;
        break;
      case PET_CMD_TARGET:
        swprintf( TextList[TextNum], GlobalText[1'226] );
        TextNum++;
        SkipNum++;
        break;
    }
...
void CalcPetInfo( PET_INFO* pPetInfo )
{
  ...
  switch ( pPetInfo->m_dwPetType ) {
    case PET_TYPE_DARK_SPIRIT:
      ...
      pPetInfo->m_wAttackSuccess = ( 1'000 + pPetInfo->m_wLevel ) + ( pPetInfo->m_wLevel * 15 );
      break;
...
std::uint32_t GetPetItemValue( PET_INFO* pPetInfo )
{
  ...
  switch ( pPetInfo->m_dwPetType ) {
    case PET_TYPE_DARK_HORSE:
      gold = static_cast<std::uint32_t>( pPetInfo->m_wLevel ) * 2'000'000u;
      break;

    case PET_TYPE_DARK_SPIRIT:
      gold = static_cast<std::uint32_t>( pPetInfo->m_wLevel ) * 1'000'000u;
      break;
  }
...
bool RenderPetItemInfo( int sx, int sy, ITEM* pItem, int iInvenType )
{
  ...
  if ( g_pNewUISystem->IsVisible( SEASON3B::INTERFACE_NPCSHOP ) ) {
    ...
    if ( GetPersonalItemPrice( indexInv, price, g_IsPurchaseShop ) ) {
      ...
      appendLine( TEXT_COLOR_RED, true, false, GlobalText[1'101] );
      appendEmptyLine();
    }
  }
  ...
  if ( pItem->Type == ITEM_DARK_HORSE_ITEM ) {
    ...
    appendLine( TEXT_COLOR_BLUE, true, true, GlobalText[1'187] );
  } else if ( pItem->Type == ITEM_DARK_RAVEN_ITEM ) {
    RequireCharisma = ( 185 + ( pPetInfo->m_wLevel * 15 ) );
    appendLine( TEXT_COLOR_BLUE, true, true, GlobalText[1'214] );
  }
  ...
  if ( pItem->Type == ITEM_DARK_HORSE_ITEM ) {
    ...
    appendLine( TEXT_COLOR_BLUE, false, false, GlobalText[1'188], 2 );
  }

  SIZE TextSize = { 0, 0 };
  GetTextExtentPoint32( g_pRenderText->GetFontDC(), TextList[0], 1, &TextSize );
  int Height =
      static_cast<int>( ( ( TextNum - SkipNum ) * TextSize.cy + SkipNum * TextSize.cy / 2 ) / g_fScreenRate_y );
  if ( sy - Height >= 0 ) {
    sy -= Height;
  }
