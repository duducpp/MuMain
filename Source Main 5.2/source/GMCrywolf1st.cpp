  ...
  switch ( o->Type ) {
    ...
    case 56: {
      b->RenderBody( RENDER_TEXTURE, o->Alpha, o->BlendMesh, o->BlendMeshLight, o->BlendMeshTexCoordU,
                     o->BlendMeshTexCoordV, o->HiddenMesh );
      if ( m_OccupationState == CRYWOLF_OCCUPATION_STATE_PEACE ) {
        float Color = (int)rand() % 10'000 * 0.0001f;
  ...
  switch ( o->Type ) {
    ...
    case MODEL_DEATH_SPIRIT: {
      ...
      for ( int i = 0; i < Models[o->Type].NumMeshs; i++ ) {
        ...
        o->BlendMeshTexCoordU = (int)WorldTime % 10'000 * 0.0005f;
        b->RenderMesh( i, RENDER_TEXTURE, o->Alpha, o->BlendMesh, o->BlendMeshLight, o->BlendMeshTexCoordU,
                       o->BlendMeshTexCoordV );
      }
      b->EndRender();
...
void M34CryWolf1st::RenderBaseSmoke( void )
{
  ...
  float WindX2 = (float)( (int)WorldTime % 100'000 ) * 0.0005f;
...
void M34CryWolf1st::Set_Message_Box( int Str, int Num, int Key, int ObjNum )
{
  ...
  if ( Str == 56 ) {
    ...
    swprintf( Box_String[Num], GlobalText[1'950 + Str], State );
  } else {
    wcscpy( Box_String[Num], GlobalText[1'950 + Str] );
  }
  if ( Str == 56 || Str == 57 ) {
    Message_Box = 1;
  } else {
...
bool M34CryWolf1st::Render_Mvp_Interface()
{
  ...
  float Number[5][6] = {
    { 565.f, 280.f, 13.f, 13.f, 12.f / 16.f, 12.f / 16.f },
  ...
  if ( View_End_Result == true ) {
    ...
    swprintf( Text, L"%ls", GlobalText[1'973] );
    g_pRenderText->RenderText( 333, 160, Text, 0, 0, RT3_WRITE_CENTER );
    swprintf( Text, L"%ls", GlobalText[1'977] );
    ...
    if ( View_Suc_Or_Fail == 1 ) {
      g_pRenderText->RenderText( 330, 175 + icntIndex * 17, GlobalText[2'000], 0, 0, RT3_WRITE_CENTER );
      icntIndex++;
      g_pRenderText->RenderText( 328, 175 + icntIndex * 17, GlobalText[2'001], 0, 0, RT3_WRITE_CENTER );
    } else {
      g_pRenderText->RenderText( 330, 175 + icntIndex * 17, GlobalText[2'009], 0, 0, RT3_WRITE_CENTER );
    }
  ...
  if ( Suc_Or_Fail >= 0 ) {
    ...
    if ( ( Delay * 15 ) > 479 ) {
      ...
      g_pCryWolfInterface->Render( -329 + ( Delay * 15 ), 50, 329, 94, 0.f, 0.f, 329.f / 512.f, 94.f / 128.f, Add_Num,
                                   false, false, A_Value );
    }
  ...
  g_pRenderText->SetTextColor( 255, 148, 21, 255 );
  g_pRenderText->SetBgColor( 0 );
  swprintf( Text, GlobalText[1'948], Dark_elf_Num );
  ...
  if ( View_Bal == true ) {
    if ( Deco_Insert < 21.f ) {
      ...
      g_pCryWolfInterface->Render( Val_Icon[0], Val_Icon[1], Val_Icon[2], Val_Icon[3], 0.f, 0.f, Val_Icon[4],
                                   Val_Icon[5], 4 );
      swprintf( Text, GlobalText[1'949] );
  ...
  if ( TimeStart = true ) {
    ...
    if ( m_iMinute > 0 ) {
      ...
      nPastMinute    = m_iMinute - ( GetTickCount() - m_dwSyncTime ) / 60'000;

      Time_Add = ( Tim / 60'000 ) * 60;
