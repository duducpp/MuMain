...
static const int g_iKalimaLevel[14][2] = {
  { 40,  999 },
  { 131, 999 },
  { 181, 999 },
  { 231, 999 },
...
bool GetUseLostMap( bool bDrawAlert )
{
  ...
  if ( bDrawAlert && Hero->SafeZone ) {
    g_pSystemLogBox->AddText( GlobalText[1'238], SEASON3B::TYPE_ERROR_MESSAGE );
    return false;
  }
  ...
  if ( bDrawAlert ) {
    wchar_t Text[100];
    swprintf( Text, GlobalText[1'123], g_iKalimaLevel[startIndex][0] );
...
int RenderHellasItemInfo( ITEM* ip, int textNum )
{
  ...
  switch ( ip->Type ) {
    case ITEM_LOST_MAP: {
      ...
      swprintf( TextList[TextNum], L"%ls %ls       %ls    ", GlobalText[58], GlobalText[368], GlobalText[935] );
      TextListColor[TextNum] = TEXT_COLOR_WHITE;
      TextBold[TextNum]      = false;
      TextNum++;
      for ( int i = 0; i < NUM_HELLAS; i++ ) {
        swprintf( TextList[TextNum], L"        %d             %3d~%3d     ", i + 1, g_iKalimaLevel[startIndex + i][0],
                  std::min<int>( 400, g_iKalimaLevel[startIndex + i][1] ) );
      ...
      TextNum++;
      swprintf( TextList[TextNum], GlobalText[1'184] );
      ...
      if ( HeroLevel < g_iKalimaLevel[startIndex][0] ) {
        swprintf( TextList[TextNum], L"\n" );
        TextNum++;
        swprintf( TextList[TextNum], GlobalText[1'123], g_iKalimaLevel[startIndex][0] );
    ...
    case ITEM_SYMBOL_OF_KUNDUN: {
      ...
      TextNum++;
      if ( ip->Durability >= 5 ) {
        swprintf( TextList[TextNum], GlobalText[1'182] );
        TextListColor[TextNum] = TEXT_COLOR_YELLOW;
        TextNum++;
      } else {
        swprintf( TextList[TextNum], GlobalText[1'183], ( 5 - ip->Durability ) );
        TextListColor[TextNum] = TEXT_COLOR_YELLOW;
        TextNum++;
      }
    } break;
...
int CreateBigMon( OBJECT* o )
{
  ...
  if ( LastBigMonCreation < WorldTime - BigMonInterval ) {
    ...
    Vector( Hero->Object.Position[0] - 1'000 - rand() % 200, Hero->Object.Position[1] - 500 + rand() % 200,
        ...
        if ( o->CurrentAction == MONSTER01_SHOCK ) {
          ...
          if ( (int)o->LifeTime == 100 && o->AnimationFrame > 4.0f ) {
            ...
            if ( gMapManager.InHellas() ) {
              AddWaterWave( ( c->PositionX ), ( c->PositionY ), 2, 2'000 );
            }
          }
...
bool RenderHellasMonsterObjectMesh( OBJECT* o, BMD* b )
{
  ...
  if ( o->Type == MODEL_DEATH_ANGEL ) {
    ...
    b->RenderMesh( 0, RENDER_CHROME | RENDER_BRIGHT, o->Alpha, o->BlendMesh, o->BlendMeshLight, o->BlendMeshTexCoordU,
                   o->BlendMeshTexCoordV, o->HiddenMesh );
    b->RenderMesh( 2, RENDER_CHROME | RENDER_BRIGHT, o->Alpha, o->BlendMesh, o->BlendMeshLight, o->BlendMeshTexCoordU,
                   o->BlendMeshTexCoordV, o->HiddenMesh );
    success = true;
    ...
    if ( o->SubType == 9 ) {
      ...
      b->RenderMesh( 1, RENDER_CHROME | RENDER_BRIGHT, 0.5f, o->BlendMesh, o->BlendMeshLight, o->BlendMeshTexCoordU,
                     o->BlendMeshTexCoordV, o->HiddenMesh );
      Vector( 1.f, 1.f, 1.f, b->BodyLight );
