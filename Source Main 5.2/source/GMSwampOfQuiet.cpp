...
bool GMSwampOfQuiet::RenderMonsterVisual( CHARACTER* pCharacter, OBJECT* pObject, BMD* pModel )
{
  ...
  switch ( pObject->Type ) {
    ...
    case MODEL_MEDUSA: {
      ...
      CreateJointFpsChecked( BITMAP_JOINT_ENERGY, vPos, pObject->Position, pObject->Angle, 57, pObject, 10.0f, 67, 0, 0,
                             15, vColor ); // 57 - pk:node , ichaindex:maxtail
      CreateJointFpsChecked( BITMAP_JOINT_ENERGY, vPos, pObject->Position, pObject->Angle, 57, pObject, 10.0f, 70, 0, 0,
                             15, vColor ); // 57 - pk:node , ichaindex:maxtail
