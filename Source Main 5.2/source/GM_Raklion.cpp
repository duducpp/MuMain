    ...
    case MODEL_SELUPAN: {
      if ( o->CurrentAction == MONSTER01_ATTACK1 ) {
        ...
        if ( o->AnimationFrame <= 3.f ) {
          ...
          for ( int i = 0; i < 1; ++i ) {
            VectorCopy( o->Position, vPos );
            vPos[0] += ( rand() % 2'000 - 1000.f );
            vPos[1] += ( rand() % 2'000 - 1000.f );
...
bool CGM_Raklion::RenderObjectMesh( OBJECT* o, BMD* b, bool ExtraMon )
{
  ...
  if ( o->Type >= 6 && o->Type <= 12 ) {
    ...
    b->RenderMesh( 1, RENDER_CHROME | RENDER_BRIGHT, o->Alpha, o->BlendMesh, o->BlendMeshLight, o->BlendMeshTexCoordU,
                   o->BlendMeshTexCoordV, BITMAP_CHROME8 );
  ...
  switch ( o->Type ) {
    ...
    case MODEL_SELUPAN: {
      ...
      b->RenderMesh( 5, RENDER_TEXTURE | RENDER_BRIGHT, o->Alpha, 5, o->BlendMeshLight,
                     (int)WorldTime % 10'000 * 0.0002f, (int)WorldTime % 10'000 * 0.0002f, o->HiddenMesh );
...
void CGM_Raklion::RenderBaseSmoke()
{
  ...
  float WindX = (float)( (int)WorldTime % 100'000 ) * 0.0001f;
...
void CGM_Raklion::CreateMapEffect()
{
  if ( m_byState <= RAKLION_STATE_NOTIFY_1 ) {
    ...
    for ( int i = 0; i < 5; ++i ) {
      ...
      vPos[0] += ( rand() % 1'600 - 800.f );
      vPos[1] += ( rand() % 1'600 - 800.f );
      ...
      for ( int k = 0; k < 3; ++k ) {
        VectorCopy( pObject->Position, vPos );
        vPos[0] += ( rand() % 1'000 - 500.f );
        vPos[1] += ( rand() % 1'000 - 500.f );
