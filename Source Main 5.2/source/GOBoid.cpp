      ...
      case MODEL_DARK_HORSE:
        ...
        if ( o->Owner->CurrentAction == PLAYER_ATTACK_DARKHORSE ) {
          ...
          if ( gMapManager.WorldActive == WD_10HEAVEN || g_Direction.m_CKanturu.IsMayaScene() ) {
            ...
            if ( bWave ) {
              Vector( Position[0] + (float)( rand() % 16 - 8 ), Position[1] + (float)( rand() % 16 - 8 ), Position[2],
                      p );
              CreateEffect( BITMAP_SHOCK_WAVE, p, o->Angle, Light, 1 );
...
int CreateDragon( OBJECT* o, int index )
{
  ...
  if ( index < 3 ) {
    ...
    Vector( Hero->Object.Position[0] + (float)( rand() % 4'000 - 2'000 ),
            Hero->Object.Position[1] + (float)( rand() % 4'000 - 2'000 ), Hero->Object.Position[2] - 600.f,
            o->Position );
  } else {
    ...
    Vector( Hero->Object.Position[0] + (float)( rand() % 1'024 - 512 ),
            Hero->Object.Position[1] + (float)( rand() % 1'024 - 512 ), Hero->Object.Position[2], o->Position );
...
void MoveHeavenBug( OBJECT* o, int index )
{
  ...
  o->Angle[2] += 0.01f * cosf( (float)( 34'571 + iFrame + index * 41'273 ) * 0.0003f ) *
                 sinf( (float)( 17'732 + iFrame + index * 5'161 ) * 0.0003f ) * FPS_ANIMATION_FACTOR;
  ...
  for ( int i = 0; i < MAX_FISHS; i++ ) {
    ...
    if ( o->Live ) {
      if ( o->Type != -1 ) {
        ...
        b->PlayAnimation( &o->AnimationFrame, &o->PriorAnimationFrame, &o->PriorAction, o->Velocity * 0.5f, o->Position,
                          o->Angle );
      }
      if ( o->Type == MODEL_FISH01 + 7 || o->Type == MODEL_FISH01 + 8 ) {
