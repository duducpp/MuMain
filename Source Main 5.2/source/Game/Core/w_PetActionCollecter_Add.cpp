...
bool PetActionCollecterAdd::Move( OBJECT* obj, CHARACTER* Owner, int targetKey, double tick, bool bForceRender )
{
  ...
  m_fRadWidthStand = ( ( 2 * Q_PI ) / 4000.0f ) * fmod( tick, 4'000 );
  m_fRadWidthGet   = ( ( 2 * Q_PI ) / 2000.0f ) * fmod( tick, 2'000 );
  ...
  switch ( m_state ) {
    ...
    case eAction_Move: {
      ...
      if ( 0 == Speed || CompTimeControl( 100'000, m_dwRootingTime ) ) {
    ...
    {
      if ( !m_isRooting || SEARCH_LENGTH < Distance || CompTimeControl( 3'000, m_dwRootingTime ) ) {
        m_isRooting     = false;
        m_dwRootingTime = GetTickCount();
        m_state         = eAction_Return;
        break;
      ...
      if ( CompTimeControl( 1'000, m_dwSendDelayTime ) && &Hero->Object == obj->Owner && SendGetItem == -1 ) {
    ...
    case eAction_Return: {
      ...
      if ( 0 == Speed || CompTimeControl( 3'000, m_dwRootingTime ) ) {
        m_state = eAction_Stand;
      }
    } break;
  }
...
bool PetActionCollecterAdd::Effect( OBJECT* obj, CHARACTER* Owner, int targetKey, double tick, bool bForceRender )
{
  ...
  float fRad1 = ( ( Q_PI / 3000.0f ) * fmodf( tick, 3'000 ) );
...
bool PetActionCollecterSkeleton::Move( OBJECT* obj, CHARACTER* Owner, int targetKey, double tick, bool bForceRender )
{
  ...
  vec3_t targetPos, Range, Direction;
  bool   _isMove    = false;
  float  fRadHeight = ( ( 2 * Q_PI ) / 15000.0f ) * fmodf( tick, 15'000 );
  m_fRadWidthStand  = ( ( 2 * Q_PI ) / 4000.0f ) * fmodf( tick, 4'000 );
  m_fRadWidthGet    = ( ( 2 * Q_PI ) / 2000.0f ) * fmodf( tick, 2'000 );
