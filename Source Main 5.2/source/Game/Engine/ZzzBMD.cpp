...
void BMD::Transform( float ( *BoneMatrix )[3][4], vec3_t BoundingBoxMin, vec3_t BoundingBoxMax, OBB_t* OBB,
                                                               ...
                                                               bool    bApplyTMtoVertices )
{
  ...
  for ( int i_ = 0; i_ < NumBones; ++i_ ) {
    ...
    Vector( BoneTransform[i_][0][3], BoneTransform[i_][1][3], BoneTransform[i_][2][3], arrOutSetfAllBonePositions[i_] );
  }
                                      ...
                                      const float fFrameArea, const float fWeight )
{
  ...
  if ( nullptr == oRefAnimation ) {
    ...
    float fAnimationFrame = oRefAnimation->AnimationFrame, fPiriorAnimationFrame = oRefAnimation->PriorAnimationFrame;
    ...
    for ( int j = 1; j < Width - 1; j++ ) {
      ...
      for ( int k = 0; k < 3; k++ ) {
        Buffer[Index] = ( Buffer[Index - RowStride - 3] + Buffer[Index - RowStride] + Buffer[Index - RowStride + 3] +
                          Buffer[Index - 3] + Buffer[Index + 3] + Buffer[Index + RowStride - 3] +
                      ...
                      float blendMeshTextureCoordU, float blendMeshTextureCoordV, int explicitTextureIndex )
{
  ...
  float wave = static_cast<long>( WorldTime ) % 10'000 * 0.0001f;
  ...
  if ( ( renderFlags & RENDER_COLOR ) == RENDER_COLOR ) {
    ...
    float Wave2 = (int)WorldTime % 5'000 * 0.00024f - 0.4f;
                                 ...
                                 int MeshTexture )
{
  ...
  float Wave = (int)WorldTime % 10'000 * 0.0001f;
    ...
    if ( m->m_csTScript != nullptr ) {
      if ( m->m_csTScript->getHiddenMesh() == false && i != HiddenMesh ) {
        ...
        if ( shadowType == SHADOW_RENDER_COLOR ) {
          ...
          RenderMesh( i, RENDER_COLOR | RENDER_SHADOWMAP, Alpha, iBlendMesh, BlendMeshLight, BlendMeshTexCoordU,
                      BlendMeshTexCoordV );
          ...
          RenderMesh( i, RENDER_TEXTURE | RENDER_SHADOWMAP, Alpha, iBlendMesh, BlendMeshLight, BlendMeshTexCoordU,
                      BlendMeshTexCoordV );
          glColor3f( 1.f, 1.f, 1.f );
        }
      }
...
void BMD::FindTriangleForEdge( int iMesh, int iTri1, int iIndex11 )
{
  ...
  for ( int iTri2 = 0; iTri2 < iNumTriangles; ++iTri2 ) {
    ...
    for ( int iIndex21 = 0; iIndex21 < 3; ++iIndex21 ) {
      ...
      if ( pTri2->EdgeTriangleIndex[iIndex21] == -1 && pTri1->VertexIndex[iIndex11] == pTri2->VertexIndex[iIndex22] &&
...
{
public:
  BMDReader( unsigned char* data, size_t size )
      : data( data )
      , size( size )
      , ptr( 0 )
  {
...
bool BMD::Save2( wchar_t* DirName, wchar_t* ModelFileName )
{
  ...
  auto* pbyBuffer = new BYTE[1'024 * 1'024];
