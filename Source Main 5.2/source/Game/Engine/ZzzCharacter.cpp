...
void SetAttackSpeed()
{
  ...
  } else if ( CharacterAttribute->MagicSpeed >= 855 && CharacterAttribute->MagicSpeed <= 1'040 ) {
    MagicSpeed2 = MagicSpeed2 * 0.0016300f;
  } else if ( CharacterAttribute->MagicSpeed >= 1'041 && CharacterAttribute->MagicSpeed <= 1'104 ) {
    MagicSpeed2 = MagicSpeed2 * 0.0015500f;
  } else if ( CharacterAttribute->MagicSpeed >= 1'301 && CharacterAttribute->MagicSpeed <= 1'500 ) {
    MagicSpeed2 = MagicSpeed2 * 0.0017500f;
  } else if ( CharacterAttribute->MagicSpeed >= 1'501 && CharacterAttribute->MagicSpeed <= 1'524 ) {
    MagicSpeed2 = MagicSpeed2 * 0.0015000f;
  } else if ( CharacterAttribute->MagicSpeed >= 1'525 && CharacterAttribute->MagicSpeed <= 1'800 ) {
    MagicSpeed2 = MagicSpeed2 * 0.0014500f;
  } else if ( CharacterAttribute->MagicSpeed >= 1'801 && CharacterAttribute->MagicSpeed <= 1'999 ) {
    MagicSpeed2 = MagicSpeed2 * 0.0013000f;
  } else if ( CharacterAttribute->MagicSpeed >= 2'000 && CharacterAttribute->MagicSpeed <= 2'167 ) {
    MagicSpeed2 = MagicSpeed2 * 0.0012500f;
  } else if ( CharacterAttribute->MagicSpeed >= 2'168 && CharacterAttribute->MagicSpeed <= 2'354 ) {
    MagicSpeed2 = MagicSpeed2 * 0.0011500f;
  } else if ( CharacterAttribute->MagicSpeed >= 2'855 && CharacterAttribute->MagicSpeed <= 3'011 ) {
    MagicSpeed2 = MagicSpeed2 * 0.0009000f;
  } else if ( CharacterAttribute->MagicSpeed >= 3'011 ) {
    ...
    case MONSTER_MAGIC_SKELETON_7:
      if ( ( c->Skill ) == AT_SKILL_BOSS ) {
        if ( rand_fps_check( 2 ) ) {
          ...
          Vector( o->Position[0] + rand() % 1'024 - 512, o->Position[1] + rand() % 1'024 - 512, o->Position[2],
                  Position );
      ...
      if ( c->Object.CurrentAction == MONSTER01_ATTACK1 ) {
        if ( c->CheckAttackTime( 11 ) ) {
          ...
          for ( int k = 0; k < 5; ++k ) {
            ...
            Angle[0] += 45.f;
            VectorCopy( o->Position, Position );
            Position[0] += (float)( rand() % 1'001 - 500 );
            Position[1] += (float)( rand() % 1'001 - 500 );
            Position[2] += 500.f;
...
bool AttackStage( CHARACTER* c, OBJECT* o )
{
  ...
  switch ( iSkill ) {
    ...
    case AT_SKILL_DEATHSTAB_STR: {
      ...
      if ( c->AttackTime <= 8 && rand_fps_check( 1 ) ) { // 기 모일 곳 위치
      ...
      if ( 6 <= c->AttackTime && c->AttackTime <= 12 ) { // 꼬깔 만들기
        if ( rand_fps_check( 2 ) ) {
          vec3_t Position;
    ...
    case AT_SKILL_IMPALE: // 창찌르기
    {
      ...
      if ( c->CheckAttackTime( 10 ) ) {
        ...
        c->SetLastAttackEffectTime();
      } else if ( c->CheckAttackTime( 4 ) ) { // 준비동작
        ...
        c->SetLastAttackEffectTime();
      } else if ( c->CheckAttackTime( 8 ) ) { // 꼬깔 만들기
        vec3_t Position;
        memcpy( Position, o->Position, sizeof( vec3_t ) );
      ...
      if ( 13 <= c->AttackTime && c->AttackTime <= 14 && rand_fps_check( 1 ) ) { // 현란한 창술
        for ( int i = 0; i < 3; ++i ) {
          vec3_t Position;
          memcpy( Position, o->Position, sizeof( vec3_t ) );
    ...
    case AT_SKILL_PENETRATION_STR:
      if ( o->Type == MODEL_PLAYER && o->CurrentAction >= PLAYER_ATTACK_FIST &&
    ...
    switch ( o->Type ) {
      case MODEL_MERCHANT_GIRL:
        if ( gMapManager.InBattleCastle() == false ) {
          CreateChat( c->ID, GlobalText[1'974], c );
        }
        break;
      case MODEL_ELF_WIZARD:
        CreateChat( c->ID, GlobalText[1'975], c );
        break;
      case MODEL_MASTER:
        CreateChat( c->ID, GlobalText[1'976], c );
        break;
      case MODEL_PLAYER:
        if ( c->MonsterIndex == MONSTER_ELF_SOLDIER ) {
          CreateChat( c->ID, GlobalText[1'827], c );
        }
        break;
    }
  }
...
void MoveCharacter( CHARACTER* c, OBJECT* o )
{
  if ( o->Type == MODEL_WARCRAFT ) {
    ...
    swprintf( Text, GlobalText[1'176], c->Level );
    wcscat( ID, Text );
    ...
    switch ( ( c->Skill ) ) {
      ...
      case AT_SKILL_HELL_FIRE_STR:
        ...
        if ( gMapManager.InHellas() == true ) {
          AddWaterWave( ( c->PositionX ), ( c->PositionY ), 2, -1'500 );
        }
        ...
        if ( c->m_iFenrirSkillTarget != -1 && c->m_iFenrirSkillTarget < MAX_CHARACTERS_CLIENT ) {
          ...
          for ( int i = 0; i < iMonsterNum; i++ ) {
            for ( int j = 0; j < 2; j++ ) {
              ...
              CreateJoint( MODEL_FENRIR_SKILL_THUNDER, Position, p_o[i]->Position, vAngle, 0 + fenrirType, p_o[i],
                           100.f );
              CreateJoint( MODEL_FENRIR_SKILL_THUNDER, Position, p_o[i]->Position, vAngle, 4 + fenrirType, p_o[i],
                           80.f );
            }
          }
  ...
  if ( o->Visible ) {
    ...
    switch ( o->Type ) {
      case MODEL_PLAYER:
        if ( SceneFlag == MAIN_SCENE &&
             ( gMapManager.WorldActive == WD_7ATLANSE || gMapManager.WorldActive == WD_67DOPPLEGANGER3 ) &&
             (int)WorldTime % 10'000 < 1'000 && rand_fps_check( 1 ) ) {
      ...
      case MODEL_CRUST:
        o->BlendMeshTexCoordU = -(float)( (int)( WorldTime ) % 10'000 ) * 0.0004f;
        break;
      case MODEL_DARK_PHEONIX_SHIELD:
        o->BlendMeshTexCoordV = (float)( (int)( WorldTime ) % 10'000 ) * 0.0001f;
        break;
      case MODEL_DRAKAN:
        break;
      ...
      case MODEL_BALROG:
        ...
        break;
      case MODEL_DEVIL:
        o->BlendMeshTexCoordU = -(float)( (int)( WorldTime ) % 10'000 ) * 0.0001f;
        break;
      case MODEL_BALI:
        Vector( 0.f, 0.f, 0.f, p );
      ...
      case MODEL_WEDDING_NPC:
        if ( o->CurrentAction == 1 &&
             ( o->AnimationFrame > 4.5f && o->AnimationFrame <= 4.8f && rand_fps_check( 1 ) ) ) {
                       ...
                       bool Translate, int RenderType, bool bRightHandItem )
{
  ...
  switch ( Type ) {
    ...
    case MODEL_ABSOLUTE_SCEPTER: {
      ...
      CreateSprite( BITMAP_LIGHTNING_MEGA1 + iRandomTexure1, vPosZx01, ( ( ( rand() % 11 ) - 20 ) / 100.f ) + 0.5f,
                    vLight2, o, rand() % 380 );
      ...
      CreateSprite( BITMAP_LIGHTNING_MEGA1 + iRandomTexure1, vPosZx02, ( ( ( rand() % 11 ) - 20 ) / 50.f ) + 0.8f,
                    vLight2, o, rand() % 380 );
    } break;
...
void RenderCharacter( CHARACTER* c, OBJECT* o, int Select )
{
  ...
  if ( c->MonsterIndex == MONSTER_BALROG || c->MonsterIndex == MONSTER_GOLDEN_BUDGE_DRAGON ||
    ...
    if ( c->MonsterIndex == MONSTER_GOLDEN_BUDGE_DRAGON ||
      ...
      if ( c->MonsterIndex == MONSTER_GOLDEN_GREAT_DRAGON ) {
        ...
        if ( rand_fps_check( 1 ) ) {
          b->TransformPosition( o->BoneTransform[57], p, v3EffectPosition, true );
          CreateEffect( MODEL_EFFECT_FIRE_HIK3_MONO, v3EffectPosition, o->Angle, v3EffectLightColor, 1, NULL, -1, 0, 0,
                        0, fEffectScale );
    ...
    if ( c->MonsterIndex == MONSTER_METAL_BALROG ) {
      RenderPartObjectBodyColor( &Models[o->Type], o, o->Type, o->Alpha, RENDER_CHROME | RENDER_BRIGHT | RENDER_EXTRA,
                                 Bright );
    } else {
    ...
    float fSin = 0.5f * ( 1.0f + sinf( (float)( (int)WorldTime % 10'000 ) * 0.001f ) );
  ...
  if ( o->Type == MODEL_NPC_CHERRYBLOSSOM ) {
    ...
    if ( rand_fps_check( 2 ) ) {
      ...
      CreateParticle( BITMAP_CHERRYBLOSSOM_EVENT_PETAL, vtaWorldPos, o->Angle, rand_fps_check( 3 ) ? vLight : vLight1,
                      1, 0.3f );
    }
  ...
  if ( o->Kind == KIND_NPC && gMapManager.WorldActive == WD_0LORENCIA && o->Type == MODEL_PLAYER &&
    ...
    if ( ( o->Kind == KIND_PLAYER && gMapManager.InChaosCastle() == true ) ) {
      ...
      if ( ( o->CurrentAction == PLAYER_SKILL_DRAGONKICK ) &&
        ...
        OBJECT* pObj = &c->Object;
        if ( pObj->m_sTargetIndex < 0 || c->JumpTime > 0 ) {
          RenderPartObject( &c->Object, MODEL_ANGEL, p, c->Light, o->Alpha, 0, 0, 0, false, false, Translate, Select,
                            RenderType );
        } else {
          p->Type = MODEL_ANGEL;
          g_CMonkSystem.DarksideRendering( c, p, Translate, Select );
      ...
      if ( gCharacterManager.GetBaseClass( c->Class ) == CLASS_DARK_LORD ) {
        ...
        if ( c->BodyPart[BODYPART_ARMOR].Type == MODEL_BODY_ARMOR + SKIN_CLASS_DARK_LORD ) {
          pCloth[3].Create( o, 18, 0.0f, 10.0f, -5.0f, 5, 5, 50.0f, 90.0f, BITMAP_DARK_LOAD_SKIRT,
                            BITMAP_DARK_LOAD_SKIRT, PCT_MASK_ALPHA | PCT_HEAVY | PCT_STICKED | PCT_SHORT_SHOULDER );
          ...
          pCloth[3].Create( o, 18, 0.0f, 10.0f, -5.0f, 5, 5, 50.0f, 90.0f, BITMAP_DARKLOAD_SKIRT_3RD,
                            BITMAP_DARKLOAD_SKIRT_3RD, PCT_MASK_ALPHA | PCT_HEAVY | PCT_STICKED | PCT_SHORT_SHOULDER );
          pCloth[3].AddCollisionSphere( 0.0f, -15.0f, -20.0f, 30.0f, 2 );
        }

        if ( c->Wing.Type == MODEL_CAPE_OF_EMPEROR ) {
          pCloth[4].Create( o, 19, 30.0f, 15.0f, 10.0f, 2, 5, 12.0f, 200.0f, BITMAP_ROBE + 10, BITMAP_ROBE + 10,
          ...
          pCloth[5].Create( o, 19, -30.0f, 20.0f, 10.0f, 2, 5, 12.0f, 200.0f, BITMAP_ROBE + 10, BITMAP_ROBE + 10,
                            PCT_FLAT | PCT_SHAPE_NORMAL | PCT_COTTON | PCT_MASK_ALPHA );
        ...
        if ( c->MonsterIndex == MONSTER_DEATH_KING ) {
          ...
          pCloth[0].Create( o, 2, 0.0f, 0.0f, 0.0f, 6, 6, 180.0f, 100.0f, BITMAP_NIGHTMARE_ROBE, BITMAP_NIGHTMARE_ROBE,
                            PCT_CYLINDER | PCT_MASK_ALPHA );
        } else {
          if ( c->Wing.Type == MODEL_WING_OF_RUIN ) {
            pCloth[0].Create( o, 19, 0.0f, 15.0f, 0.0f, 10, 10, 120.0f, 120.0f, BITMAP_ROBE + 8, BITMAP_ROBE + 8,
                              PCT_CURVED | PCT_SHORT_SHOULDER | PCT_HEAVY | PCT_MASK_ALPHA );
          } else {
            pCloth[0].Create( o, 19, 0.0f, 10.0f, 0.0f, 10, 10, /*45.0f*/ 75.0f, 120.0f, BITMAP_ROBE, BITMAP_ROBE + 1,
                              PCT_CURVED | PCT_SHORT_SHOULDER | PCT_HEAVY );
          }
        }
        o->m_pCloth     = (void*)pCloth;
        o->m_byNumCloth = 1;
        ...
        if ( c->PK < PVP_MURDERER2 && c->Level != 4 ) {
          ...
          switch ( w->Type ) {
            ...
            case MODEL_STAFF_OF_RESURRECTION:
              ...
              for ( int j = 0; j < 4; j++ ) {
                Vector( (float)( rand() % 20 - 10 ), (float)( rand() % 20 - 10 - 90.f ), (float)( rand() % 20 - 10 ),
                        Position );
                Models[o->Type].TransformPosition( o->BoneTransform[w->LinkBone], Position, p, true );
                if ( rand_fps_check( 1 ) ) {
    ...
    case MODEL_PLAYER:
      ...
      if ( gMapManager.InChaosCastle() == false ) {
        ...
        if ( fullset && g_pOption->GetRenderLevel() >= 2 ) {
          ...
          if ( EquipmentLevelSet > 9 ) {
            if ( rand_fps_check( 20 ) ) //(o->CurrentAction<PLAYER_WALK_MALE || o->CurrentAction>PLAYER_RUN_RIDE_WEAPON)
                                        //&& rand_fps_check(6))
            {
              VectorCopy( o->Light, Light );
              Vector( 1.f, 1.f, 1.f, o->Light );
...
void SetCharacterScale( CHARACTER* c )
{
  ...
  if ( c->BodyPart[BODYPART_HELM].Type == MODEL_HELM || c->BodyPart[BODYPART_HELM].Type == MODEL_PAD_HELM ||
       ...
       ( c->BodyPart[BODYPART_HELM].Type >= MODEL_VINE_HELM &&
         c->BodyPart[BODYPART_HELM].Type <= MODEL_SPIRIT_HELM ) ) {
