...
bool CreateDevilSquareRain( PARTICLE* o, int Index )
{
  ...
  if ( Index < 300 ) {
    ...
    Vector( Hero->Object.Position[0] + randomX, Hero->Object.Position[1] + randomY, Hero->Object.Position[2] + randomZ,
            o->Position );
  } else {
    const float randomX = Random::RangeFloat( -800, 799 );
    const float randomY = Random::RangeFloat( 1'000, 1'299 ) - RainPosition;
    const float randomZ = Random::RangeFloat( 300, 499 );
    Vector( Hero->Object.Position[0] + randomX, Hero->Object.Position[1] + randomY, Hero->Object.Position[2] + randomZ,
            o->Position );
  }
