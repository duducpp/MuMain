...
int64_t ItemValue( ITEM* ip, int goldType )
{
  ...
  if ( ip->Type == ITEM_BOLT ) {
    ...
    Gold = (long long)( 200'000 + 20'000 * ( Level - 1 ) );
    if ( Level == 1 ) {
      Gold = 50'000;
    }
    Gold *= 3;
    ...
    Gold = (long long)( ip->Durability * 10'000 ) * 3;
    ...
    Gold = (long long)( (long long)ip->Durability * 10'000 ) * 3;
    ...
    if ( Level == 0 ) {
      ...
      Gold = (long long)ip->Durability * 3'900;
    }
  } else if ( ip->Type == ITEM_DEVILS_EYE ) {
    int iValue[8] = { 30'000, 10'000, 50'000, 100'000, 300'000, 500'000, 800'000, 1'000'000 };
    Gold          = iValue[std::min<int>( std::max<int>( 0, Level ), 7 )];
  } else if ( ip->Type == ITEM_DEVILS_KEY ) {
    int iValue[8] = { 30'000, 15'000, 75'000, 150'000, 450'000, 750'000, 1'200'000, 1'500'000 };
    Gold          = iValue[std::min<int>( std::max<int>( 0, Level ), 7 )];
  } else if ( ip->Type == ITEM_DEVILS_INVITATION ) {
    int iValue[8] = { 120'000, 60'000, 84'000, 120'000, 180'000, 240'000, 300'000, 180'000 };
    Gold          = iValue[std::min<int>( std::max<int>( 0, Level ), 7 )];
    ...
    switch ( Level ) {
      case 0:
        Gold = (long long)900'000 * ip->Durability;
        break;
      case 1:
        Gold = (long long)450'000 * ip->Durability;
        break;
    }
    ...
    if ( ( Type == 12 && ip->Type <= ITEM_WINGS_OF_DARKNESS ) || ip->Type == ITEM_CAPE_OF_LORD ||
         ( ip->Type >= ITEM_WING_OF_STORM && ip->Type <= ITEM_WING_OF_DIMENSION ) ||
         ( ip->Type == ITEM_CAPE_OF_OVERRULE ) ) {
      Gold = (long long)( 40'000'000 + ( ( 40 + Level2 ) * Level2 * Level2 * 11 ) );
    } else {
  ...
  Gold = std::min<int64_t>( Gold, 3'000'000'000LL );
  ...
  if ( ip->Type == ITEM_SPLINTER_OF_ARMOR ) {
    ...
    Gold = (long long)ip->Durability * 1'000;
  } else if ( ip->Type == ITEM_FRAGMENT_OF_HORN ) {
    Gold = (long long)ip->Durability * 10'000;
...
void CHARACTER_MACHINE::CalculateDamage()
{
  ...
  if ( Equipment[EQUIPMENT_WING].Type != -1 ) {
    ...
    if ( p->Durability != 0 ) {
      float percent =
          CalcDurabilityPercent( Equipment[EQUIPMENT_WING].Durability, p->Durability, Equipment[EQUIPMENT_WING].Level,
                                 0, 0 ); // Equipment[EQUIPMENT_WING].Option1);
...
void CHARACTER_MACHINE::CalculateNextExperince()
{
  ...
  if ( Character.Level > 255 ) {
    ...
    Character.NextExperience += ( 9 + LevelOver_N ) * LevelOver_N * LevelOver_N * 1'000;
  }
}
...
void CHARACTER_MACHINE::CalulateMasterLevelNextExperience()
{
  ...
  int64_t iData_Master =
      ...
      ( ( (int64_t)9 + (int64_t)iTOverLevel ) * (int64_t)iTOverLevel * (int64_t)iTOverLevel * (int64_t)1'000 );

  Master_Level_Data.lNext_MasterLevel_Experince = ( iData_Master - (int64_t)3'892'250'000 ) / (int64_t)2;
}
...
void CHARACTER_MACHINE::CalculateAll()
{
  ...
  WORD wAddStrength = 0, wAddDexterity = 0, wAddEnergy = 0, wAddVitality = 0, wStrengthResult = 0, wDexterityResult = 0,
       wEnergyResult = 0, wVitalityResult = 0, wAddCharisma = 0, wCharismaResult = 0;
