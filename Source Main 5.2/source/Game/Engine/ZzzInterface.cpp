...
constexpr uint64_t MacroCooldownMs = 4'000; // 4 Seconds
...
void PrintPKLog( CHARACTER* pCha )
{
  ...
  if ( pCha->Key == sKey && ( timeGetTime() - dwStartTime < 1'000 ) ) {
    return;
  }
...
void SetBooleanPosition( CHAT* c )
{
  ...
  if ( lstrlen( c->szShopTitle ) > 0 ) {
    ...
    if ( GetTextExtentPoint32( g_pRenderText->GetFontDC(), c->szShopTitle, lstrlen( c->szShopTitle ), &sizeT[0] ) &&
         GetTextExtentPoint32( g_pRenderText->GetFontDC(), GlobalText[1'104], GlobalText.GetStringSize( 1'104 ),
                               &sizeT[1] ) ) {
...
void RenderBoolean( int x, int y, CHAT* c )
{
  ...
  if ( IsShopInViewport( c->Owner ) ) {
    ...
    g_pRenderText->RenderText( RenderPos.x, RenderPos.y, GlobalText[1'104], 0, iLineHeight, RT3_SORT_LEFT, &TextSize );
      ...
      switch ( c->Owner->m_byGensInfluence ) {
        case 1:
          g_pNewUIGensRanking->RanderMark(
              tempX, y, (SEASON3B::CNewUIGensRanking::GENS_TYPE)c->Owner->m_byGensInfluence, c->Owner->GensRanking,
              SEASON3B::CNewUIGensRanking::MARK_BOOLEAN, (float)RenderPos.y );
          return;
        case 2:
          g_pNewUIGensRanking->RanderMark(
              tempX, y, (SEASON3B::CNewUIGensRanking::GENS_TYPE)c->Owner->m_byGensInfluence, c->Owner->GensRanking,
...
void AddGuildName( CHAT* c, CHARACTER* Owner )
{
  ...
  if ( Owner->GuildMarkIndex >= 0 && GuildMark[Owner->GuildMarkIndex].UnionName[0] ) {
    if ( Owner->GuildRelationShip == GR_UNION ) {
      swprintf( c->Union, L"<%ls> %ls", GuildMark[Owner->GuildMarkIndex].UnionName, GlobalText[1'295] );
    }
    if ( Owner->GuildRelationShip == GR_UNIONMASTER ) {
      if ( Owner->GuildStatus == G_MASTER ) {
        swprintf( c->Union, L"<%ls> %ls", GuildMark[Owner->GuildMarkIndex].UnionName, GlobalText[1'296] );
      } else {
        swprintf( c->Union, L"<%ls> %ls", GuildMark[Owner->GuildMarkIndex].UnionName, GlobalText[1'295] );
      }
    } else if ( Owner->GuildRelationShip == GR_RIVAL ) {
      if ( Owner->GuildStatus == G_MASTER ) {
        swprintf( c->Union, L"<%ls> %ls", GuildMark[Owner->GuildMarkIndex].UnionName, GlobalText[1'298] );
      } else {
        swprintf( c->Union, L"<%ls> %ls", GuildMark[Owner->GuildMarkIndex].UnionName, GlobalText[1'297] );
      }
    } else if ( Owner->GuildRelationShip == GR_RIVALUNION ) {
      swprintf( c->Union, L"<%ls> %ls", GuildMark[Owner->GuildMarkIndex].UnionName, GlobalText[1'299] );
    } else {
  ...
  if ( Owner->GuildMarkIndex >= 0 ) {
    ...
    if ( Owner->GuildStatus == G_PERSON ) {
      swprintf( c->Guild, L"[%ls] %ls", GuildMark[Owner->GuildMarkIndex].GuildName, GlobalText[1'330] );
    } else if ( Owner->GuildStatus == G_MASTER ) {
      swprintf( c->Guild, L"[%ls] %ls", GuildMark[Owner->GuildMarkIndex].GuildName, GlobalText[1'300] );
    } else if ( Owner->GuildStatus == G_SUB_MASTER ) {
      swprintf( c->Guild, L"[%ls] %ls", GuildMark[Owner->GuildMarkIndex].GuildName, GlobalText[1'301] );
    } else if ( Owner->GuildStatus == G_BATTLE_MASTER ) {
      swprintf( c->Guild, L"[%ls] %ls", GuildMark[Owner->GuildMarkIndex].GuildName, GlobalText[1'302] );
    } else {
...
bool CheckAttack_Fenrir( CHARACTER* c )
{
  ...
  if ( gMapManager.InChaosCastle() == true && c != Hero ) {
    return true;
  } else if ( ::IsStrifeMap( gMapManager.WorldActive ) && c != Hero &&
    ...
    if ( ( ( wcscmp( GuildMark[Hero->GuildMarkIndex].GuildName, GuildMark[c->GuildMarkIndex].GuildName ) == NULL ) ||
      ...
      return true;
    } else if ( ( wcscmp( GuildMark[Hero->GuildMarkIndex].GuildName, GuildMark[c->GuildMarkIndex].GuildName ) !=
                  NULL ) &&
                !g_pPartyManager->IsPartyMember( SelectedCharacter ) ) {
      return true;
    }
...
void UseSkillWarrior( CHARACTER* c, OBJECT* o )
{
  ...
  if ( Skill != AT_SKILL_FORCE && Skill != AT_SKILL_FORCE_WAVE && Skill != AT_SKILL_FORCE_WAVE_STR &&
       ...
       Skill != AT_SKILL_KILLING_BLOW && Skill != AT_SKILL_KILLING_BLOW_STR && Skill != AT_SKILL_KILLING_BLOW_MASTERY &&
...
void UseSkillSummon( CHARACTER* pCha, OBJECT* pObj )
{
  ...
  switch ( iSkill ) {
    ...
    case AT_SKILL_ALICE_ENERVATION:
      LetHeroStop();
      SendRequestMagicContinue( iSkill, pCha->PositionX, pCha->PositionY, (BYTE)( pObj->Angle[2] / 360.f * 256.f ), 0,
                                0, 0xffff, 0 );
      switch ( pCha->Helper.Type ) {
        case MODEL_HORN_OF_UNIRIA:
          SetAction( pObj, PLAYER_SKILL_SLEEP_UNI );
  ...
  switch ( iSkill ) {
    ...
    case AT_SKILL_PLASMA_STORM_FENRIR: {
      ...
      SendRequestMagicContinue( iSkill, ( pCha->PositionX ), ( pCha->PositionY ),
                                (BYTE)( pObj->Angle[2] / 360.f * 256.f ), 0, pos, TKey, &pObj->m_bySkillSerialNum );
      pCha->Movement = 0;
      ...
      if ( monsterIndex >= npcIndex ) {
        ...
        if ( isCryWolfElf ) {
          ...
          if ( monsterIndex == MONSTER_LITTLE_SANTA_RED ) {
            swprintf( temp, GlobalText[2'596], 100 );
          } else if ( monsterIndex == MONSTER_LITTLE_SANTA_BLUE ) {
            swprintf( temp, GlobalText[2'597], 100 );
          }
...
bool CheckMacroLimit( wchar_t* Text )
{
  ...
  if ( wcscmp( string, GlobalText[258] ) == 0 || wcscmp( string, GlobalText[259] ) == 0 ||
  ...
  if ( bMacroText == false && LogOut == false ) {
    ...
    if ( !g_pNewUISystem->IsVisible( SEASON3B::INTERFACE_STORAGE ) ) {
      if ( wcscmp( Name, GlobalText[258] ) == NULL || wcscmp( Name, GlobalText[259] ) == NULL ||
        ...
        if ( gMapManager.InChaosCastle() == true ) {
          g_pSystemLogBox->AddText( GlobalText[1'150], SEASON3B::TYPE_SYSTEM_MESSAGE );

          return false;
        }

        if ( ::IsStrifeMap( gMapManager.WorldActive ) ) {
          g_pSystemLogBox->AddText( GlobalText[3'147], SEASON3B::TYPE_SYSTEM_MESSAGE );
          return false;
        }
    ...
    if ( wcscmp( Text, GlobalText[1'117] ) == 0 || wcsicmp( Text, L"/personalshop" ) == 0 ) {
      if ( gMapManager.InChaosCastle() == true ) {
        g_pSystemLogBox->AddText( GlobalText[1'150], SEASON3B::TYPE_SYSTEM_MESSAGE );
        return false;
      }
      ...
      if ( level >= 6 ) {
        ...
        swprintf( szError, GlobalText[1'123], 6 );
        g_pSystemLogBox->AddText( szError, SEASON3B::TYPE_SYSTEM_MESSAGE );
      }
      return true;
    }
    if ( wcsstr( Text, GlobalText[1'118] ) != nullptr || wcsstr( Text, L"/purchase" ) != nullptr ) {
      if ( gMapManager.InChaosCastle() == true ) {
        g_pSystemLogBox->AddText( GlobalText[1'150], SEASON3B::TYPE_SYSTEM_MESSAGE );
        return false;
      }
      ...
      if ( g_pNewUISystem->IsVisible( SEASON3B::INTERFACE_NPCSHOP ) ||
        ...
        g_pSystemLogBox->AddText( GlobalText[1'121], SEASON3B::TYPE_SYSTEM_MESSAGE );
        return false;
      }
      wchar_t szCmd[24];
    ...
    if ( wcscmp( Text, GlobalText[1'136] ) == 0 ) {
      ShowShopTitles();
      g_pSystemLogBox->AddText( GlobalText[1'138], SEASON3B::TYPE_SYSTEM_MESSAGE );
    }

    if ( wcscmp( Text, GlobalText[1'137] ) == 0 ) {
      HideShopTitles();
      g_pSystemLogBox->AddText( GlobalText[1'139], SEASON3B::TYPE_ERROR_MESSAGE );
    }
    if ( wcscmp( Text, GlobalText[908] ) == 0 || wcsicmp( Text, L"/duelstart" ) == 0 ) {
      ...
      if ( !g_DuelMgr.IsDuelEnabled() ) {
        ...
        if ( iLevel < 30 ) {
          wchar_t szError[48] = L"";
          swprintf( szError, GlobalText[2'704], 30 );
    ...
    if ( !wcscmp( Text, GlobalText[1'354] ) || !wcsicmp( Text, L"/union" ) || !wcscmp( Text, GlobalText[1'356] ) ||
         !wcsicmp( Text, L"/rival" ) || !wcscmp( Text, GlobalText[1'357] ) || !wcsicmp( Text, L"/rivaloff" ) ) {
      if ( gMapManager.InChaosCastle() == true ) {
        g_pSystemLogBox->AddText( GlobalText[1'150], SEASON3B::TYPE_SYSTEM_MESSAGE );
        return false;
      }
      if ( Hero->GuildStatus == G_NONE ) {
        g_pSystemLogBox->AddText( GlobalText[1'355], SEASON3B::TYPE_SYSTEM_MESSAGE );
        return true;
      }
      ...
      if ( SelectedCharacter >= 0 && SelectedCharacter < MAX_CHARACTERS_CLIENT ) {
        ...
        if ( o->Kind == KIND_PLAYER && c != Hero && ( o->Type == MODEL_PLAYER || c->Change ) &&
             abs( ( c->PositionX ) - ( Hero->PositionX ) ) <= 1 &&
             abs( ( c->PositionY ) - ( Hero->PositionY ) ) <= 1 ) {
          if ( !wcscmp( Text, GlobalText[1'354] ) || !wcsicmp( Text, L"/union" ) ) {
            ...
            SocketClient->ToGameServer()->SendGuildRelationshipChangeRequest( 0x01, 0x01,
                                                                              CharactersClient[SelectedCharacter].Key );
          } else if ( !wcscmp( Text, GlobalText[1'356] ) || !wcsicmp( Text, L"/rival" ) ) {
            // SendRequestGuildRelationShip(0x02, 0x01, HIBYTE(CharactersClient[SelectedCharacter].Key),
            // LOBYTE(CharactersClient[SelectedCharacter].Key));
            SocketClient->ToGameServer()->SendGuildRelationshipChangeRequest( 0x02, 0x01,
                                                                              CharactersClient[SelectedCharacter].Key );
          } else {
            ...
            // LOBYTE(CharactersClient[SelectedCharacter].Key));
            SocketClient->ToGameServer()->SendGuildRelationshipChangeRequest( 0x02, 0x02,
                                                                              CharactersClient[SelectedCharacter].Key );
          }
        }
      } else {
    ...
    if ( wcscmp( Text, GlobalText[256] ) == NULL || wcsicmp( Text, L"/party" ) == NULL ||
         wcsicmp( Text, L"/pt" ) == NULL ) {
      if ( gMapManager.InChaosCastle() == true ) {
        g_pSystemLogBox->AddText( GlobalText[1'150], SEASON3B::TYPE_SYSTEM_MESSAGE );
        return false;
      }
...
void CheckChatText( wchar_t* Text )
{
  ...
  if ( FindText( Text, GlobalText[270] ) || FindText( Text, GlobalText[271] ) || FindText( Text, GlobalText[272] ) ||
  ...
  } else if ( FindText( Text, GlobalText[278] ) || FindText( Text, GlobalText[279] ) ||
              FindText( Text, GlobalText[280] ) ) {
    SetActionClass( c, o, PLAYER_GOODBYE1, AT_GOODBYE1 );
    SendRequestAction( Hero->Object, AT_GOODBYE1 );
  } else if ( FindText( Text, GlobalText[281] ) || FindText( Text, GlobalText[282] ) ||
              FindText( Text, GlobalText[283] ) || FindText( Text, GlobalText[284] ) ||
              FindText( Text, GlobalText[285] ) || FindText( Text, GlobalText[286] ) ) {
    SetActionClass( c, o, PLAYER_CLAP1, AT_CLAP1 );
    SendRequestAction( Hero->Object, AT_CLAP1 );
  } else if ( FindText( Text, GlobalText[287] ) || FindText( Text, GlobalText[288] ) ||
              FindText( Text, GlobalText[289] ) || FindText( Text, GlobalText[290] ) ) {
    SetActionClass( c, o, PLAYER_GESTURE1, AT_GESTURE1 );
    SendRequestAction( Hero->Object, AT_GESTURE1 );
  } else if ( FindText( Text, GlobalText[292] ) || FindText( Text, GlobalText[293] ) ||
              FindText( Text, GlobalText[294] ) || FindText( Text, GlobalText[295] ) ) {
    SetActionClass( c, o, PLAYER_DIRECTION1, AT_DIRECTION1 );
    SendRequestAction( Hero->Object, AT_DIRECTION1 );
  } else if ( FindText( Text, GlobalText[296] ) || FindText( Text, GlobalText[297] ) ||
              FindText( Text, GlobalText[298] ) || FindText( Text, GlobalText[299] ) ||
              FindText( Text, GlobalText[300] ) || FindText( Text, GlobalText[301] ) ||
    ...
    SendRequestAction( Hero->Object, AT_AWKWARD1 );
              ...
              FindText( Text, GlobalText[306] ) || FindText( Text, GlobalText[307] ) ||
              FindText( Text, GlobalText[308] ) || FindText( Text, GlobalText[309] ) ) {
    ...
    SendRequestAction( Hero->Object, AT_SEE1 );
              ...
              FindText( Text, GlobalText[312] ) || FindText( Text, GlobalText[313] ) ||
              FindText( Text, GlobalText[314] ) || FindText( Text, GlobalText[315] ) ||
              FindText( Text, GlobalText[316] ) ) {
    SetActionClass( c, o, PLAYER_SMILE1, AT_SMILE1 );
    SendRequestAction( Hero->Object, AT_SMILE1 );
  } else if ( FindText( Text, GlobalText[318] ) || FindText( Text, GlobalText[319] ) ||
              FindText( Text, GlobalText[320] ) || FindText( Text, GlobalText[321] ) ) {
    SetActionClass( c, o, PLAYER_CHEER1, AT_CHEER1 );
    SendRequestAction( Hero->Object, AT_CHEER1 );
  } else if ( FindText( Text, GlobalText[322] ) || FindText( Text, GlobalText[323] ) ||
              FindText( Text, GlobalText[324] ) || FindText( Text, GlobalText[325] ) ) {
    SetActionClass( c, o, PLAYER_WIN1, AT_WIN1 );
    SendRequestAction( Hero->Object, AT_WIN1 );
  } else if ( FindText( Text, GlobalText[326] ) || FindText( Text, GlobalText[327] ) ||
              FindText( Text, GlobalText[328] ) || FindText( Text, GlobalText[329] ) ) {
    SetActionClass( c, o, PLAYER_SLEEP1, AT_SLEEP1 );
    SendRequestAction( Hero->Object, AT_SLEEP1 );
  } else if ( FindText( Text, GlobalText[330] ) || FindText( Text, GlobalText[331] ) ||
              FindText( Text, GlobalText[332] ) || FindText( Text, GlobalText[333] ) ||
              FindText( Text, GlobalText[334] ) ) {
    SetActionClass( c, o, PLAYER_COLD1, AT_COLD1 );
    SendRequestAction( Hero->Object, AT_COLD1 );
  } else if ( FindText( Text, GlobalText[335] ) || FindText( Text, GlobalText[336] ) ||
              FindText( Text, GlobalText[337] ) || FindText( Text, GlobalText[338] ) ) {
    SetActionClass( c, o, PLAYER_AGAIN1, AT_AGAIN1 );
    SendRequestAction( Hero->Object, AT_AGAIN1 );
  } else if ( FindText( Text, GlobalText[339] ) || FindText( Text, GlobalText[340] ) ||
              FindText( Text, GlobalText[341] ) ) {
  ...
  } else if ( FindText( Text, GlobalText[344] ) || FindText( Text, GlobalText[345] ) ||
              FindText( Text, GlobalText[346] ) || FindText( Text, GlobalText[347] ) ) {
  ...
  } else if ( FindText( Text, GlobalText[2'228] ) ) {
  ...
  } else if ( FindText( Text, GlobalText[2'243] ) ) {
    ITEM* pItem_rr = &CharacterMachine->Equipment[EQUIPMENT_RING_RIGHT];
    ITEM* pItem_rl = &CharacterMachine->Equipment[EQUIPMENT_RING_LEFT];
...
void AttackKnight( CHARACTER* c, ActionSkillType Skill, float Distance )
{
  ...
  if ( ( ( iTypeR != -1 && ( iTypeR < ITEM_STAFF || iTypeR >= ITEM_STAFF + MAX_ITEM_INDEX ) &&
    ...
    if ( gMapManager.InChaosCastle() ) {
      if ( Skill == AT_SKILL_EARTHSHAKE || Skill == AT_SKILL_EARTHSHAKE_STR || Skill == AT_SKILL_EARTHSHAKE_MASTERY ||
           Skill == AT_SKILL_RIDER || ( Skill >= AT_PET_COMMAND_DEFAULT && Skill <= AT_PET_COMMAND_TARGET ) ||
        ...
        case AT_SKILL_PLASMA_STORM_FENRIR: {
          ...
          if ( CheckTile( c, o, Distance ) ) {
            ...
            if ( g_MovementSkill.m_iTarget != -1 ) {
              ...
              c->m_iFenrirSkillTarget = g_MovementSkill.m_iTarget;
              SendRequestMagicContinue( Skill, ( c->PositionX ), ( c->PositionY ),
                                        (BYTE)( o->Angle[2] / 360.f * 256.f ), 0, pos, TKey, &o->m_bySkillSerialNum );
              c->Movement = 0;
      ...
      case AT_SKILL_TELEPORT: {
        ...
        if ( Success && o->Teleport != TELEPORT_BEGIN && o->Teleport != TELEPORT && o->Alpha >= 0.7f ) {
          ...
          if ( Wall == 0 ) {
            ...
            if ( g_bWhileMovingZone || ( GetTickCount() - g_dwLatestZoneMoving < 3'000 ) ) {
              bResult = false;
            } else {
...
void Attack( CHARACTER* c )
{
  ...
  if ( !EnableFastInput ) {
    ...
    if ( g_pOption->IsAutoAttack() && gMapManager.WorldActive != WD_6STADIUM && gMapManager.InChaosCastle() == false &&
         Attacking == 2 && SelectedCharacter != -1 ) {
      Success = true;
    }
  ...
  if ( c->SafeZone ) {
    if ( ( gMapManager.InBloodCastle() == true ) || gMapManager.InChaosCastle() == true ) {
      if ( Skill != AT_SKILL_HEALING && Skill != AT_SKILL_HEALING_STR && Skill != AT_SKILL_DEFENSE &&
           ...
           Skill != AT_SKILL_RECOVER && Skill != AT_SKILL_ALICE_BERSERKER && Skill != AT_SKILL_ALICE_BERSERKER_STR &&
           Skill != AT_SKILL_IMPROVE_AG && Skill != AT_SKILL_ADD_CRITICAL && Skill != AT_SKILL_ADD_CRITICAL_STR1 &&
  ...
  if ( o->Type == MODEL_PLAYER ) {
    if ( o->CurrentAction < PLAYER_STOP_MALE ||
         o->CurrentAction > PLAYER_STOP_RIDE_WEAPON && o->CurrentAction != PLAYER_STOP_TWO_HAND_SWORD_TWO &&
             ...
             o->CurrentAction != PLAYER_FENRIR_STAND_TWO_SWORD && o->CurrentAction != PLAYER_FENRIR_STAND_ONE_RIGHT &&
...
void MoveHero()
{
  ...
  if ( LoadingWorld > 0 ) {
    LoadingWorld--;
    if ( LoadingWorld == 9'999'998 || LoadingWorld == 29 ) {
    ...
    if ( g_iFollowCharacter >= 0 && g_iFollowCharacter < MAX_CHARACTERS_CLIENT ) {
      ...
      if ( !pPickedItem && RightType == -1 &&
        ...
        if ( g_pMyInventory->IsEquipable( EQUIPMENT_WEAPON_LEFT,
          ...
          SendRequestEquipmentItem( STORAGE_TYPE::INVENTORY, SrcInventoryIndex, &PickItem, STORAGE_TYPE::INVENTORY,
                                    DstInventoryIndex );
        }
      }
      MouseUpdateTime = 0;
          ...
          if ( !c->Movement || ( abs( ( c->PositionX ) - xPos ) < 2 && abs( ( c->PositionY ) - yPos ) < 2 ) ) {
            if ( ( ( c->PositionX ) != TargetX || ( c->PositionY ) != TargetY || !c->Movement ) &&
              ...
              if ( ( TerrainWall[iTerrainIndex] & TW_NOMOVE ) != TW_NOMOVE ) {
                CreateEffect( MODEL_MOVE_TARGETPOSITION_EFFECT, vPos, pHeroObj->Angle, vLight, 0, pHeroObj, -1, 0, 0, 0,
                              0.6f );
              }
            } else {
              MouseUpdateTime = MouseUpdateTimeMax;
        ...
        if ( CollisionDetectLineToOBB( MousePosition, MouseTarget, o->OBB ) ) {
          ...
          if ( fNewDist < fNearestDist ) {
            ...
            if ( gMapManager.WorldActive == WD_0LORENCIA || gMapManager.WorldActive == WD_2DEVIAS ) {
              int Index =
                  ( (int)o->Position[1] / (int)TERRAIN_SCALE ) * 256 + ( (int)o->Position[0] / (int)TERRAIN_SCALE );
...
void GetTime( DWORD time, std::wstring& timeText, bool isSecond )
{
  ...
  if ( isSecond ) {
    DWORD day     = time / ( 1'440 * 60 );
    DWORD oClock  = ( time - ( day * ( 1'440 * 60 ) ) ) / 3'600;
    DWORD minutes = ( time - ( ( oClock * 3'600 ) + ( day * ( 1'440 * 60 ) ) ) ) / 60;
    DWORD second  = time % 60;

    if ( day != 0 ) {
      swprintf( buff, L"%d %ls %d %ls %d %ls %d %ls", day, GlobalText[2'298], oClock, GlobalText[2'299], minutes,
                GlobalText[2'300], second, GlobalText[2'301] );
      timeText = buff;
    } else if ( day == 0 && oClock != 0 ) {
      swprintf( buff, L"%d %ls %d %ls %d %ls", oClock, GlobalText[2'299], minutes, GlobalText[2'300], second,
                GlobalText[2'301] );
      timeText = buff;
    } else if ( day == 0 && oClock == 0 && minutes != 0 ) {
      swprintf( buff, L"%d %ls %d %ls", minutes, GlobalText[2'300], second, GlobalText[2'301] );
      timeText = buff;
    } else if ( day == 0 && oClock == 0 && minutes == 0 ) {
      timeText = GlobalText[2'308];
    }
  } else {
    DWORD day     = time / 1'440;
    DWORD oClock  = ( time - ( day * 1'440 ) ) / 60;
    DWORD minutes = time % 60;

    if ( day != 0 ) {
      swprintf( buff, L"%d %ls %d %ls %d %ls", day, GlobalText[2'298], oClock, GlobalText[2'299], minutes,
                GlobalText[2'300] );
      timeText = buff;
    } else if ( day == 0 && oClock != 0 ) {
      swprintf( buff, L"%d %ls %d %ls", oClock, GlobalText[2'299], minutes, GlobalText[2'300] );
      timeText = buff;
    } else if ( day == 0 && oClock == 0 && minutes != 0 ) {
      swprintf( buff, L"%d %ls", minutes, GlobalText[2'300] );
      timeText = buff;
    }
  }
}
...
void RenderSwichState()
{
  ...
  for ( int i = 0; i < 2; i++ ) {
    if ( Switch_Info[i].m_bySwitchState > 0 ) {
      swprintf( Buff, L"%ls%d / %ls / %ls", GlobalText[1'981], i + 1, Switch_Info[i].m_szGuildName,
...
void RenderOutSides()
{
  if ( gMapManager.WorldActive == WD_8TARKAN ) {
    ...
    float WindX = (float)( (int)WorldTime % 100'000 ) * 0.0002f;
    RenderBitmapUV( BITMAP_CHROME + 2, 0.f, 0.f, 640.f, 480.f - 45.f, WindX, 0.f, 0.3f, 0.3f );
    float WindX2 = (float)( (int)WorldTime % 100'000 ) * 0.001f;
    ...
    EnableAlphaBlend();
    glColor3f( 0.3f, 0.3f, 0.25f );
    float fWindX = (float)( (int)WorldTime % 100'000 ) * 0.004f;
...
void RenderTournamentInterface()
{
  ...
  if ( g_wtMatchTimeLeft.m_Time ) {
    ...
    if ( t_valueMin <= 10 ) {
      ...
      if ( g_wtMatchTimeLeft.m_Type == 3 ) {
        ...
        swprintf( t_Str, GlobalText[1'392], t_valueSec );
      } else {
        if ( g_wtMatchTimeLeft.m_Time < 60 ) {
          g_pRenderText->SetTextColor( 255, 255, 10, 255 );
        }
        if ( t_valueSec < 10 ) {
          swprintf( t_Str, GlobalText[1'390], t_valueMin, t_valueSec );
        } else {
          swprintf( t_Str, GlobalText[1'391], t_valueMin, t_valueSec );
        }
      }
  ...
  swprintf( t_Str, GlobalText[1'393] );
  g_pRenderText->RenderText( WindowX + Width / 2 - 50, WindowY + 20, t_Str );
  g_pRenderText->SetTextColor( 255, 255, 255, 255 );

  swprintf( t_Str, GlobalText[1'394] );
  g_pRenderText->SetTextColor( 255, 255, 10, 255 );
  ...
  if ( g_wtMatchResult.m_Score1 == g_wtMatchResult.m_Score2 ) {
    g_pRenderText->SetFont( g_hFontBig );
    g_pRenderText->SetTextColor( 255, 255, 10, 255 );
    swprintf( t_Str, GlobalText[1'395] );
    ...
    swprintf( t_Str, GlobalText[1'396] );
    g_pRenderText->RenderText( WindowX + 47, WindowY + 115, t_Str );
    g_pRenderText->SetTextColor( 255, 10, 10, 255 );
    swprintf( t_Str, GlobalText[1'397] );
    ...
    if ( MouseLButtonPush ) {
      ...
      if ( !PickObject ) {
        if ( MouseX < 100 && MouseY < 100 ) {
          ...
          if ( PickObject ) {
            PickObjectHeight =
                PickObject->Position[2] - RequestTerrainHeight( PickObject->Position[0], PickObject->Position[1] );
          }
        }
      }
    }
      ...
      if ( MouseLButton ) {
        ...
        if ( EnableRandomObject ) {
          ...
          for ( int i = 0; i < 9; i++ ) {
            ...
            Position[0] += (float)( rand() % 2'000 - 1'000 );
            Position[1] += (float)( rand() % 2'000 - 1'000 );
    ...
    if ( MouseRButton ) {
      ...
      for ( int y = my - 2; y <= my + 2; y++ ) {
        for ( int x = mx - 2; x <= mx + 2; x++ ) {
          ...
          for ( int i = 0; i < 3; i++ ) {
            TerrainLight[Index1][i] = ( TerrainLight[Index1][i] + TerrainLight[Index2][i] + TerrainLight[Index3][i] +
