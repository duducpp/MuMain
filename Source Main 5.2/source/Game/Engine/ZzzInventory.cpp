...
static const int DEFAULT_DEVILSQUARELEVEL[6][2] = {
  { 15,  130    },
  { 131, 180    },
  { 181, 230    },
  { 231, 280    },
...
static const int DARKCLASS_DEVILSQUARELEVEL[6][2] = {
  { 15,  110    },
  { 111, 160    },
  { 161, 210    },
  { 211, 260    },
...
bool SendRequestEquipmentItem( STORAGE_TYPE iSrcType, int iSrcIndex, ITEM* pItem, STORAGE_TYPE iDstType, int iDstIndex )
{
  ...
  g_ConsoleDebug->Write( MCD_SEND, L"0x24 [SendRequestEquipmentItem(%d %d %d %d %d %d %d)]", iSrcIndex, iDstIndex,
                         iSrcType, iDstType, ( pItem->Type & 0x1FFF ), (BYTE)( pItem->Level ),
                         (BYTE)( pItem->Durability ) );
    ...
    switch ( i ) {
      case CLASS_WIZARD: {
        if ( byRequireClass == 1 ) {
          ...
          swprintf( TextList[TextNum], GlobalText[61], GlobalText[1'669] );
          TextListColor[TextNum] = iTextColor;
        }
      ...
      case CLASS_KNIGHT: {
        if ( byRequireClass == 1 ) {
          ...
          swprintf( TextList[TextNum], GlobalText[61], GlobalText[1'668] );
          TextListColor[TextNum] = iTextColor;
        }
      ...
      case CLASS_ELF: {
        if ( byRequireClass == 1 ) {
          ...
          swprintf( TextList[TextNum], GlobalText[61], GlobalText[1'670] );
          TextListColor[TextNum] = iTextColor;
        }
      ...
      case CLASS_DARK: {
        if ( byRequireClass == 1 ) {
          ...
          swprintf( TextList[TextNum], GlobalText[61], GlobalText[1'671] );
          TextListColor[TextNum] = iTextColor;
        }
      ...
      case CLASS_DARK_LORD: {
        if ( byRequireClass == 1 ) {
          ...
          swprintf( TextList[TextNum], GlobalText[61], GlobalText[1'672] );
          TextListColor[TextNum] = iTextColor;
        }
      ...
      case CLASS_SUMMONER: {
        if ( byRequireClass == 1 ) {
          swprintf( TextList[TextNum], GlobalText[61], GlobalText[1'687] );
          TextListColor[TextNum] = iTextColor;
        } else if ( byRequireClass == 2 ) {
          swprintf( TextList[TextNum], GlobalText[61], GlobalText[1'688] );
          TextListColor[TextNum] = iTextColor;
        } else if ( byRequireClass == 3 ) {
          swprintf( TextList[TextNum], GlobalText[61], GlobalText[1'689] );
          TextListColor[TextNum] = iTextColor;
        }
      ...
      case CLASS_RAGEFIGHTER: {
        if ( byRequireClass == 1 ) {
          swprintf( TextList[TextNum], GlobalText[61], GlobalText[3'150] );
          TextListColor[TextNum] = iTextColor;
        } else if ( byRequireClass == 3 ) {
          swprintf( TextList[TextNum], GlobalText[61], GlobalText[3'151] );
          TextListColor[TextNum] = iTextColor;
        }
        TextBold[TextNum] = false;
        TextNum++;
...
void RenderHelpCategory( int iColumnType, int Pos_x, int Pos_y )
{
  ...
  switch ( iColumnType ) {
    ...
    case _COLUMN_TYPE_CURSE:
      pText = GlobalText[1'144];
      break;
    case _COLUMN_TYPE_PET_ATTACK:
      pText = GlobalText[1'239];
      break;
...
unsigned int getGoldColor( DWORD Gold )
{
  ...
  } else if ( Gold >= 1'000'000 ) {
    return ( 255 << 24 ) + ( 0 << 16 ) + ( 150 << 8 ) + ( 255 );
  } else if ( Gold >= 100'000 ) {
...
void ConvertGold( double dGold, wchar_t* szText, int iDecimals /*= 0*/ )
{
  ...
  while ( dwValueTemp / 1'000 > 0 ) {
    iCipherCnt  = iCipherCnt + 3;
    dwValueTemp = dwValueTemp / 1'000;
  }
...
{
  int Gold1 = Gold % 1'000;
  int Gold2 = Gold % 1'000'000 / 1'000;
  int Gold3 = Gold % 1'000'000'000 / 1'000'000;
  int Gold4 = Gold % 1'000'000'000'000 / 1'000'000'000;
  int Gold5 = Gold % 1'000'000'000'000'000 / 1'000'000'000'000;
  int Gold6 = Gold / 1'000'000'000'000'000;
  if ( Gold >= 1'000'000'000'000'000 ) {
    swprintf( Text, L"%d,%03d,%03d,%03d,%03d,%03d", Gold6, Gold5, Gold4, Gold3, Gold2, Gold1 );
  } else if ( Gold >= 1'000'000'000'000 ) {
    swprintf( Text, L"%d,%03d,%03d,%03d,%03d", Gold5, Gold4, Gold3, Gold2, Gold1 );
  } else if ( Gold >= 1'000'000'000 ) {
...
void ConvertTaxGold( DWORD Gold, wchar_t* Text )
{
  ...
  int Gold2 = Gold % 1'000'000 / 1'000;
  int Gold3 = Gold % 1'000'000'000 / 1'000'000;
  int Gold4 = Gold / 1'000'000'000;
  if ( Gold >= 1'000'000'000 ) {
    swprintf( Text, L"%d,%03d,%03d,%03d", Gold4, Gold3, Gold2, Gold1 );
  } else if ( Gold >= 1'000'000 ) {
    swprintf( Text, L"%d,%03d,%03d", Gold3, Gold2, Gold1 );
  } else if ( Gold >= 1'000 ) {
...
int64_t CalcRepairCost( int64_t ItemValue, int Durability, int MaxDurability, short Type, bool SelfRepair )
{
  ...
  int64_t repairGold = std::min<int64_t>( ItemValue, 400'000'000LL );
...
wchar_t ChaosEventName[][100] = { L"히돼?고향 여행권", L"펜티엄4 컴퓨터",
                                  L"디지탈카메라",     L"로지텍 무선 마우스+키보드 세트",
                                  L"256M 램",          L"6개?잡지 구독권",
                                  L"문화상품권(만원)", L"뮤 머그컵",
...
void GetItemName( int iType, int iLevel, wchar_t* Text )
{
  ...
  if ( iType >= ITEM_SCROLL_OF_EMPEROR_RING_OF_HONOR && iType <= ITEM_SOUL_SHARD_OF_WIZARD ) {
    if ( iType == ITEM_SCROLL_OF_EMPEROR_RING_OF_HONOR ) {
      switch ( iLevel ) {
        case 0:
          ...
          break;
        case 1:
          swprintf( Text, L"%ls", GlobalText[906] );
          break;
      }
      ...
      switch ( iLevel ) {
        case 0:
          ...
          break;
        case 1:
          swprintf( Text, L"%ls", GlobalText[907] );
          break;
      }
    } else {
      swprintf( Text, L"%ls", p->Name );
    ...
    switch ( iLevel ) {
      case 0:
        swprintf( Text, L"%ls", GlobalText[100] );
        break;
      case 1:
        swprintf( Text, L"%ls", GlobalText[101] );
        break;
      case 2:
        swprintf( Text, L"%ls", ChaosEventName[p->Durability] );
        break;
    }
    ...
    switch ( iLevel ) {
      case 0:
        ...
        break;
      case 1:
        swprintf( Text, L"%ls", GlobalText[105] );
        break;
      case 2:
        swprintf( Text, L"%ls", GlobalText[106] );
        break;
      case 3:
        swprintf( Text, L"%ls", GlobalText[107] );
        break;
      case 5:
        swprintf( Text, L"%ls", GlobalText[109] );
        break;
      case 6:
        swprintf( Text, L"%ls", GlobalText[110] );
        break;
      case 7:
        swprintf( Text, L"%ls", GlobalText[111] );
        break;
        break;
      case 8:
      case 9:
      ...
      case 12:
        ...
        break;
      case 13:
        swprintf( Text, GlobalText[117] );
        break;
      case 14:
        swprintf( Text, GlobalText[1'650] );
        break;
        break;

      case 15:
        swprintf( Text, GlobalText[1'651] );
        break;
        break;
    }
    ...
    switch ( iLevel ) {
      case 0:
        swprintf( Text, L"%ls %ls", GlobalText[168], p->Name );
        break;
      case 1:
        swprintf( Text, L"%ls %ls", GlobalText[169], p->Name );
        break;
      case 2:
        swprintf( Text, L"%ls %ls", GlobalText[167], p->Name );
        break;
      case 3:
        swprintf( Text, L"%ls %ls", GlobalText[166], p->Name );
        break;
      case 4:
        swprintf( Text, L"%ls %ls", GlobalText[1'900], p->Name );
        break;
    }
    ...
    switch ( iLevel ) {
      case 0:
        ...
        break;
      case 1:
        swprintf( Text, L"%ls", GlobalText[1'235] );
        break;
    }
  } else if ( iType == ITEM_SPIRIT ) {
    switch ( iLevel ) {
      case 0:
        swprintf( Text, L"%ls %ls", GlobalText[1'187], p->Name );
        break;
      case 1:
        swprintf( Text, L"%ls %ls", GlobalText[1'214], p->Name );
        break;
    }
    ...
    switch ( iLevel ) {
      case 0:
        ...
        break;
      case 1:
        swprintf( Text, L"%ls", GlobalText[810] );
        break;
      case 2:
        swprintf( Text, L"%ls", GlobalText[1'098] );
        break;
      case 3:
        swprintf( Text, L"%ls", GlobalText[1'290] );
        break;
    }
    ...
    switch ( iLevel ) {
      case 0:
        ...
        break;
      case 1:
        swprintf( Text, L"%ls", GlobalText[922] );
        break;
      case 2:
        swprintf( Text, L"%ls", GlobalText[928] );
        break;
      case 3:
        swprintf( Text, L"%ls", GlobalText[929] );
        break;
    }
    ...
    switch ( iLevel ) {
      case 0:
        ...
        break;
      case 1:
        swprintf( Text, L"%ls", GlobalText[108] );
        break;
    }
    ...
    switch ( iLevel ) {
      case 0:
        ...
        break;
      case 1:
        swprintf( Text, L"%ls", GlobalText[2'012] );
        break;
    }
    ...
    switch ( iLevel ) {
      case 0:
        ...
        break;
      case 1:
        swprintf( Text, L"%ls", GlobalText[2'013] );
        break;
    }
    ...
    switch ( iLevel ) {
      case 0:
        ...
        break;
      case 1:
        swprintf( Text, L"%ls", GlobalText[2'014] );
        break;
    }
    ...
    swprintf( Text, L"%ls +%d", GlobalText[1'806], iLevel + 1 );
  } else if ( iType == INDEX_COMPILED_SOUL ) {
    swprintf( Text, L"%ls +%d", GlobalText[1'807], iLevel + 1 );
    ...
    int iTextIndex = 0;
    iTextIndex     = ( iLevel == 0 ) ? 1'413 : 1'414;
...
void GetSpecialOptionText( int Type, wchar_t* Text, WORD Option, BYTE Value, int iMana )
{
  switch ( Option ) {
    ...
    case AT_IMPROVE_CURSE:
      swprintf( Text, GlobalText[1'697], Value );
      break;
...
void RenderItemInfo( int sx, int sy, ITEM* ip, bool Sell, int Inventype, bool bItemTextListBoxUse )
{
  ...
  if ( g_pNewUISystem->IsVisible( SEASON3B::INTERFACE_NPCSHOP ) && !IsSellingBan( ip ) ) {
    wchar_t Text[100];
    {
      if ( Sell ) {
        ...
        swprintf( TextList[TextNum], GlobalText[1'620], Text2, Text );
      } else {
  ...
  if ( ( Inventype == SEASON3B::TOOLTIP_TYPE_MY_SHOP || Inventype == SEASON3B::TOOLTIP_TYPE_PURCHASE_SHOP ) &&
       !IsPersonalShopBan( ip ) ) {
    {
      ...
      if ( GetPersonalItemPrice( indexInv, price, g_IsPurchaseShop ) ) {
        ...
        TextListColor[TextNum] = TEXT_COLOR_RED;
        TextBold[TextNum]      = true;
        swprintf( TextList[TextNum], GlobalText[1'101] );
        TextNum++;
        swprintf( TextList[TextNum], L"\n" );
        TextNum++;
        SkipNum++;
  ...
  if ( ip->Type >= ITEM_SCROLL_OF_EMPEROR_RING_OF_HONOR && ip->Type <= ITEM_SOUL_SHARD_OF_WIZARD ) {
    if ( ip->Type == ITEM_SCROLL_OF_EMPEROR_RING_OF_HONOR ) {
      ...
      switch ( Level ) {
        case 0:
          ...
          break;
        case 1:
          swprintf( TextList[TextNum], GlobalText[906] );
          break;
      }
      ...
      switch ( Level ) {
        case 0:
          ...
          break;
        case 1:
          swprintf( TextList[TextNum], GlobalText[907] );
          break;
      }
    } else {
      swprintf( TextList[TextNum], L"%ls", p->Name );
    ...
    switch ( Level ) {
      case 0:
        swprintf( TextList[TextNum], GlobalText[100] );
        break;
      case 1:
        swprintf( TextList[TextNum], GlobalText[101] );
        break;
      case 2:
        swprintf( TextList[TextNum], L"%ls", ChaosEventName[ip->Durability] );
        break;
    }
    ...
    switch ( Level ) {
      case 0:
        ...
        break;
      case 1:
        swprintf( TextList[TextNum], GlobalText[105] );
        break;
      case 2:
        swprintf( TextList[TextNum], GlobalText[106] );
        break;
      case 3:
        swprintf( TextList[TextNum], GlobalText[107] );
        break;
      case 5:
        swprintf( TextList[TextNum], GlobalText[109] );
        break;
      case 6:
        swprintf( TextList[TextNum], GlobalText[110] );
        break;
      case 7:
        swprintf( TextList[TextNum], GlobalText[111] );
        break;
      case 8:
      case 9:
      case 10:
      ...
      case 13:
        ...
        break;
      case 14:
        swprintf( TextList[TextNum], GlobalText[1'650] );
        break;
      case 15:
        swprintf( TextList[TextNum], GlobalText[1'651] );
        break;
    }
  } else if ( ip->Type == ITEM_POTION + 12 ) {
    switch ( Level ) {
    ...
    switch ( Level ) {
      case 0:
        swprintf( TextList[TextNum], L"%ls %ls", GlobalText[168], p->Name );
        break;
      case 1:
        swprintf( TextList[TextNum], L"%ls %ls", GlobalText[169], p->Name );
        break;
      case 2:
        swprintf( TextList[TextNum], L"%ls %ls", GlobalText[167], p->Name );
        break;
      case 3:
        swprintf( TextList[TextNum], L"%ls %ls", GlobalText[166], p->Name );
        break;
      case 4:
        swprintf( TextList[TextNum], L"%ls %ls", GlobalText[1'900], p->Name );
        break;
    }
    ...
    switch ( Level ) {
      case 0:
        ...
        break;
      case 1:
        swprintf( TextList[TextNum], L"%ls", GlobalText[1'235] );
        break;
    }
    ...
    switch ( Level ) {
      case 0:
        ...
        break;
      case 1:
        swprintf( TextList[TextNum], GlobalText[810] );
        break;
      case 2:
        swprintf( TextList[TextNum], GlobalText[1'098] );
        break;
      case 3:
        swprintf( TextList[TextNum], GlobalText[1'290] );
        break;
    }
    ...
    switch ( Level ) {
      case 0:
        swprintf( TextList[TextNum], GlobalText[811] );
        break;
      case 1:
        swprintf( TextList[TextNum], GlobalText[812] );
        break;
      case 2:
        swprintf( TextList[TextNum], GlobalText[817] );
        break;
    }
    ...
    switch ( Level ) {
      case 0:
        ...
        break;
      case 1:
        swprintf( TextList[TextNum], GlobalText[922] );
        break;
      case 2:
        swprintf( TextList[TextNum], GlobalText[928] );
        break;
      case 3:
        swprintf( TextList[TextNum], GlobalText[929] );
        break;
    }
    ...
    switch ( Level ) {
      case 0:
        swprintf( TextList[TextNum], GlobalText[1'413] );
        break;
      case 1:
        swprintf( TextList[TextNum], GlobalText[1'414] );
        break;
    }
  } else if ( ip->Type == ITEM_HELPER + 7 ) {
    switch ( Level ) {
      case 0:
        swprintf( TextList[TextNum], GlobalText[1'460] );
        break;
      case 1:
        swprintf( TextList[TextNum], GlobalText[1'461] );
        break;
    }
  } else if ( ip->Type == ITEM_LIFE_STONE_ITEM ) {
    switch ( Level ) {
      case 0:
        swprintf( TextList[TextNum], GlobalText[1'416] );
        break;
      case 1:
        swprintf( TextList[TextNum], GlobalText[1'462] );
        break;
    }
    ...
    switch ( Level ) {
      case 0:
        ...
        break;
      case 1:
        swprintf( TextList[TextNum], GlobalText[108] );
        break;
    }
    ...
    switch ( Level ) {
      case 0:
        swprintf( TextList[TextNum], L"%ls of %ls", p->Name, GlobalText[1'187] );
        break;
      case 1:
        swprintf( TextList[TextNum], L"%ls of %ls", p->Name, GlobalText[1'214] );
        break;
    }
    ...
    swprintf( TextList[TextNum], GlobalText[1'180], Level );
    ...
    switch ( Level ) {
      case 0:
        ...
        break;
      case 1:
        swprintf( TextList[TextNum], L"%ls", GlobalText[2'012] );
        break;
    }
    ...
    switch ( Level ) {
      case 0:
        ...
        break;
      case 1:
        swprintf( TextList[TextNum], L"%ls", GlobalText[2'013] );
        break;
    }
    ...
    switch ( Level ) {
      case 0:
        ...
        break;
      case 1:
        swprintf( TextList[TextNum], L"%ls", GlobalText[2'014] );
        break;
    }
    ...
    if ( ip->Type == ITEM_HORN_OF_FENRIR ) {
      ...
      if ( ( ip->ExcellentFlags & 63 ) == 0x01 ) {
        swprintf( TextList[TextNum], L"%ls %ls", p->Name, GlobalText[1'863] );
      } else if ( ( ip->ExcellentFlags & 63 ) == 0x02 ) {
        swprintf( TextList[TextNum], L"%ls %ls", p->Name, GlobalText[1'864] );
      } else if ( ( ip->ExcellentFlags & 63 ) == 0x04 ) {
        swprintf( TextList[TextNum], L"%ls %ls", p->Name, GlobalText[1'866] );
      } else {
        swprintf( TextList[TextNum], L"%ls", p->Name );
      }
    } else {
  ...
  if ( ip->Type == ITEM_WEAPON_OF_ARCHANGEL ) {
    ...
    swprintf( TextList[TextNum], GlobalText[815] );
    TextBold[TextNum] = false;
    TextNum++;
    swprintf( TextList[TextNum], L"\n" );
    TextBold[TextNum] = false;
    ...
    switch ( Level ) {
      case 0:
        swprintf( TextList[TextNum], L"%ls: %d ~ %d", GlobalText[42], 107, 110 );
        TextListColor[TextNum] = TEXT_COLOR_WHITE;
        TextBold[TextNum]      = false;
        TextNum++;
        iWeaponSpeed  = 20;
        ...
        break;
      case 1:
        swprintf( TextList[TextNum], L"%ls: %d ~ %d", GlobalText[40], 110, 120 );
        TextListColor[TextNum] = TEXT_COLOR_WHITE;
        TextBold[TextNum]      = false;
        TextNum++;
        iWeaponSpeed  = 35;
        ...
        break;
      case 2:
        swprintf( TextList[TextNum], L"%ls: %d ~ %d", GlobalText[41], 120, 140 );
        TextListColor[TextNum] = TEXT_COLOR_WHITE;
        TextBold[TextNum]      = false;
        TextNum++;
        iWeaponSpeed  = 35;
    ...
    swprintf( TextList[TextNum], GlobalText[64], iWeaponSpeed );
    TextListColor[TextNum] = TEXT_COLOR_WHITE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[73], iNeedStrength );
    TextListColor[TextNum] = TEXT_COLOR_WHITE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[75], iNeedDex );
    TextListColor[TextNum] = TEXT_COLOR_WHITE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], L"\n" );
    ...
    TextNum++;
    SkipNum++;
    swprintf( TextList[TextNum], GlobalText[87] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[94], 20 );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    ...
    switch ( Level ) {
      case 0: {
        swprintf( TextList[TextNum], GlobalText[79], 53 );
        TextListColor[TextNum] = TEXT_COLOR_BLUE;
        TextBold[TextNum]      = true;
        TextNum++;
        swprintf( TextList[TextNum], GlobalText[631] );
        TextListColor[TextNum] = TEXT_COLOR_BLUE;
        TextBold[TextNum]      = false;
        TextNum++;
        swprintf( TextList[TextNum], GlobalText[632], 2 );
        TextListColor[TextNum] = TEXT_COLOR_BLUE;
        TextBold[TextNum]      = false;
        TextNum++;
      } break;
      case 1: {
        ...
        swprintf( TextList[TextNum], GlobalText[84], iMana );
        TextListColor[TextNum] = TEXT_COLOR_BLUE;
        TextBold[TextNum]      = false;
        TextNum++;
        swprintf( TextList[TextNum], GlobalText[629] );
        TextListColor[TextNum] = TEXT_COLOR_BLUE;
        TextBold[TextNum]      = false;
        TextNum++;
        swprintf( TextList[TextNum], GlobalText[630], 2 );
        TextListColor[TextNum] = TEXT_COLOR_BLUE;
        TextBold[TextNum]      = false;
        TextNum++;
      } break;
      case 2: {
        ...
        swprintf( TextList[TextNum], GlobalText[86], iMana );
        TextListColor[TextNum] = TEXT_COLOR_BLUE;
        TextBold[TextNum]      = false;
        TextNum++;
        swprintf( TextList[TextNum], GlobalText[629] );
    ...
    swprintf( TextList[TextNum], GlobalText[628] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[633], 7 );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[634] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
  ...
  if ( ip->Type >= ITEM_SCROLL_OF_EMPEROR_RING_OF_HONOR && ip->Type <= ITEM_SOUL_SHARD_OF_WIZARD ) {
    ...
    swprintf( TextList[TextNum], GlobalText[2'259], GlobalText[iMap] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[2'270] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], L"\n" );
    ...
    swprintf( TextList[TextNum], GlobalText[2'261] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
  } else if ( ip->Type >= ITEM_POTION + 58 && ip->Type <= ITEM_POTION + 62 ) {
    swprintf( TextList[TextNum], GlobalText[2'269] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    ...
    swprintf( TextList[TextNum], GlobalText[2'250] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
  } else if ( ip->Type == ITEM_HELPER + 43 ) {
    swprintf( TextList[TextNum], GlobalText[2'256] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[2'297] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[2'567] );
    TextListColor[TextNum] = TEXT_COLOR_YELLOW;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[2'568] );
    TextListColor[TextNum] = TEXT_COLOR_YELLOW;
    TextBold[TextNum]      = false;
    TextNum++;
  } else if ( ip->Type == ITEM_HELPER + 44 ) {
    swprintf( TextList[TextNum], GlobalText[2'257] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[2'297] );
    ...
    swprintf( TextList[TextNum], GlobalText[2'258] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[2'297] );
    ...
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[2'566] );
    TextListColor[TextNum] = TEXT_COLOR_YELLOW;
    TextBold[TextNum]      = false;
    TextNum++;
    ...
    swprintf( TextList[TextNum], GlobalText[2'500 + index] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    ...
    swprintf( TextList[TextNum], GlobalText[2'503 + ( ip->Type - ( ITEM_POTION + 72 ) )], Item_data.m_byValue1 );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;

    swprintf( TextList[TextNum], GlobalText[2'502] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
  } else if ( ip->Type == ITEM_HELPER + 59 ) {
    swprintf( TextList[TextNum], GlobalText[2'509] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    ...
    swprintf( TextList[TextNum], GlobalText[2'511], statpoint );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;

    swprintf( TextList[TextNum], GlobalText[2'510] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    ...
    TextListColor[TextNum] = TEXT_COLOR_DARKRED;
    swprintf( TextList[TextNum], GlobalText[1'908] );
    TextNum++;
    ...
    swprintf( TextList[TextNum], GlobalText[2'512 + index], value );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[2'517] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[2'518] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;

    if ( ip->Type == ITEM_POTION + 82 ) {
      swprintf( TextList[TextNum], GlobalText[3'115] );
      TextListColor[TextNum] = TEXT_COLOR_YELLOW;
      TextBold[TextNum]      = false;
      TextNum++;
    }
    ...
    swprintf( TextList[TextNum], GlobalText[2'302] );
    TextListColor[TextNum] = TEXT_COLOR_WHITE;
    TextBold[TextNum]      = false;
    TextNum++;
  } else if ( ip->Type == ITEM_HELPER + 60 ) {
    swprintf( TextList[TextNum], GlobalText[2'519] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    ...
    swprintf( TextList[TextNum], GlobalText[2'253], Item_data.m_byValue1 );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[2'569], Item_data.m_byValue2 );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[2'570] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[2'566] );
    ...
    swprintf( TextList[TextNum], GlobalText[2'254], Item_data.m_byValue1 );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[2'571], Item_data.m_byValue2 );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[2'572] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    ...
    swprintf( TextList[TextNum], GlobalText[2'580] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    ...
    swprintf( TextList[TextNum], GlobalText[2'581] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[2'502] );
    ...
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[2'188], 100 );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
  } else if ( ip->Type == ITEM_POTION + 96 ) {
    swprintf( TextList[TextNum], GlobalText[2'573] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[2'574] );
    TextListColor[TextNum] = TEXT_COLOR_PURPLE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[2'708] );
    TextListColor[TextNum] = TEXT_COLOR_PURPLE;
    TextBold[TextNum]      = false;
    TextNum++;
  } else if ( ip->Type == ITEM_DEMON ) {
    swprintf( TextList[TextNum], GlobalText[2'575] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[2'576] );
    TextListColor[TextNum] = TEXT_COLOR_PURPLE;
    TextBold[TextNum]      = false;
    TextNum++;
  } else if ( ip->Type == ITEM_SPIRIT_OF_GUARDIAN ) {
    swprintf( TextList[TextNum], GlobalText[2'577] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[2'578] );
    TextListColor[TextNum] = TEXT_COLOR_PURPLE;
    TextBold[TextNum]      = false;
    TextNum++;
  } else if ( ip->Type == ITEM_PET_RUDOLF ) {
    swprintf( TextList[TextNum], GlobalText[2'600] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    ...
    swprintf( TextList[TextNum], GlobalText[3'068] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;

    swprintf( TextList[TextNum], GlobalText[3'069] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;

    swprintf( TextList[TextNum], GlobalText[3'070] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
  } else if ( ip->Type == ITEM_PET_PANDA ) {
    swprintf( TextList[TextNum], GlobalText[2'746] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[2'747] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[2'748] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    ...
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[2'744] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[2'748] );
    ...
    swprintf( TextList[TextNum], GlobalText[2'968] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
  } else if ( ip->Type == ITEM_HELPER + 105 ) {
    swprintf( TextList[TextNum], GlobalText[2'969] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
  } else if ( ip->Type == ITEM_HELPER + 103 ) {
    swprintf( TextList[TextNum], GlobalText[2'970], 170 );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
  } else if ( ip->Type == ITEM_HELPER + 69 ) {
    swprintf( TextList[TextNum], GlobalText[2'602] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    if ( g_PortalMgr.IsRevivePositionSaved() ) {
      swprintf( TextList[TextNum], GlobalText[2'603] );
      TextListColor[TextNum] = TEXT_COLOR_BLUE;
      TextBold[TextNum]      = false;
      TextNum++;
    ...
    swprintf( TextList[TextNum], GlobalText[2'604] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
  } else if ( ip->Type == ITEM_HELPER + 81 ) {
    swprintf( TextList[TextNum], GlobalText[2'714] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum++]    = false;
    swprintf( TextList[TextNum], GlobalText[2'729] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum++]    = false;
    swprintf( TextList[TextNum], GlobalText[3'084] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum++]    = false;
  } else if ( ip->Type == ITEM_HELPER + 82 ) {
    swprintf( TextList[TextNum], GlobalText[2'715] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum++]    = false;
    swprintf( TextList[TextNum], GlobalText[2'730] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum++]    = false;
    swprintf( TextList[TextNum], GlobalText[2'716] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum++]    = false;
    ...
    swprintf( TextList[TextNum], GlobalText[2'259], GlobalText[2'369] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[2'270] );
    ...
    swprintf( TextList[TextNum], GlobalText[2'551] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    ...
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[2'553] );
    TextListColor[TextNum] = TEXT_COLOR_YELLOW;
    TextBold[TextNum]      = false;
    TextNum++;
    ...
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[2'556] );
    TextListColor[TextNum] = TEXT_COLOR_YELLOW;
    TextBold[TextNum]      = false;
    TextNum++;
    ...
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[2'552] );
    TextListColor[TextNum] = TEXT_COLOR_YELLOW;
    TextBold[TextNum]      = false;
    TextNum++;
    ...
    swprintf( TextList[TextNum], GlobalText[2'559] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
  } else if ( ip->Type == ITEM_CHERRY_BLOSSOM_PLAYBOX ) {
    swprintf( TextList[TextNum], GlobalText[2'011] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
  } else if ( ip->Type == ITEM_CHERRY_BLOSSOM_WINE ) {
    swprintf( TextList[TextNum], GlobalText[2'549] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    ...
    swprintf( TextList[TextNum], GlobalText[2'550] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    std::wstring           timetext;
    ...
    swprintf( TextList[TextNum], GlobalText[2'532] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    ...
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[2'534] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[2'535] );
    ...
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[2'536] );
    TextListColor[TextNum] = TEXT_COLOR_DARKYELLOW;
    TextBold[TextNum]      = false;
    TextNum++;
    ...
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[2'537] );
    TextListColor[TextNum] = TEXT_COLOR_DARKYELLOW;
    TextBold[TextNum]      = false;
    TextNum++;
  } else if ( ip->Type == ITEM_OLD_SCROLL ) {
    swprintf( TextList[TextNum], GlobalText[2'397] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
  } else if ( ip->Type == ITEM_ILLUSION_SORCERER_COVENANT ) {
    swprintf( TextList[TextNum], GlobalText[2'398] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
  } else if ( ip->Type == ITEM_SCROLL_OF_BLOOD ) {
    swprintf( TextList[TextNum], GlobalText[2'399] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
  } else if ( ip->Type == ITEM_POTION + 64 ) {
    swprintf( TextList[TextNum], GlobalText[2'420] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    ...
    swprintf( TextList[TextNum], GlobalText[1'665] );
    TextListColor[TextNum] = TEXT_COLOR_WHITE;
    TextBold[TextNum]      = false;
    TextNum++;
    ...
    swprintf( TextList[TextNum], GlobalText[2'709] );
    TextListColor[TextNum] = TEXT_COLOR_WHITE;
    TextBold[TextNum]      = false;
    TextNum++;
    ...
    const ITEM_ADD_OPTION& Item_data = g_pItemAddOptioninfo->GetItemAddOtioninfo( ip->Type );
    swprintf( TextList[TextNum], GlobalText[2'717] );
    TextBold[TextNum] = false;
    TextNum++;

    switch ( ip->Type ) {
      case ITEM_TYPE_CHARM_MIXWING + EWS_KNIGHT_1_CHARM: {
        swprintf( TextList[TextNum], GlobalText[2'718], Item_data.m_byValue1 );
      } break;
      case ITEM_TYPE_CHARM_MIXWING + EWS_MAGICIAN_1_CHARM: {
        swprintf( TextList[TextNum], GlobalText[2'720], Item_data.m_byValue1 );
      } break;
      case ITEM_TYPE_CHARM_MIXWING + EWS_ELF_1_CHARM: {
        swprintf( TextList[TextNum], GlobalText[2'722], Item_data.m_byValue1 );
      } break;
      case ITEM_TYPE_CHARM_MIXWING + EWS_SUMMONER_1_CHARM: {
        swprintf( TextList[TextNum], GlobalText[2'724], Item_data.m_byValue1 );
      } break;
      case ITEM_TYPE_CHARM_MIXWING + EWS_DARKLORD_1_CHARM: {
        swprintf( TextList[TextNum], GlobalText[2'727], Item_data.m_byValue1 );
      } break;
      case ITEM_TYPE_CHARM_MIXWING + EWS_KNIGHT_2_CHARM: {
        swprintf( TextList[TextNum], GlobalText[2'719], Item_data.m_byValue1 );
      } break;
      case ITEM_TYPE_CHARM_MIXWING + EWS_MAGICIAN_2_CHARM: {
        swprintf( TextList[TextNum], GlobalText[2'721], Item_data.m_byValue1 );
      } break;
      case ITEM_TYPE_CHARM_MIXWING + EWS_ELF_2_CHARM: {
        swprintf( TextList[TextNum], GlobalText[2'723], Item_data.m_byValue1 );
      } break;
      case ITEM_TYPE_CHARM_MIXWING + EWS_SUMMONER_2_CHARM: {
        swprintf( TextList[TextNum], GlobalText[2'725], Item_data.m_byValue1 );
      } break;
      case ITEM_TYPE_CHARM_MIXWING + EWS_DARKKNIGHT_2_CHARM: {
        swprintf( TextList[TextNum], GlobalText[2'726], Item_data.m_byValue1 );
      } break;
    }

    swprintf( TextList[TextNum], GlobalText[2'732 + ( ip->Type - ( ITEM_TYPE_CHARM_MIXWING + EWS_BEGIN ) )],
    ...
    swprintf( TextList[TextNum], GlobalText[2'773] );
    TextListColor[TextNum] = TEXT_COLOR_WHITE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[2'774] );
    TextListColor[TextNum] = TEXT_COLOR_WHITE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[2'775] );
    TextListColor[TextNum] = TEXT_COLOR_WHITE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], L"\n" );
    TextNum++;
    SkipNum++;
    swprintf( TextList[TextNum], GlobalText[1'181], ip->Durability, 5 );
    TextListColor[TextNum] = TEXT_COLOR_WHITE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[2'776], 5 - ip->Durability );
    TextListColor[TextNum] = TEXT_COLOR_YELLOW;
    TextBold[TextNum]      = false;
    TextNum++;
  } else if ( ip->Type == ITEM_POTION + 111 ) {
    swprintf( TextList[TextNum], GlobalText[2'777] );
    TextListColor[TextNum] = TEXT_COLOR_DARKBLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[2'778] );
    TextListColor[TextNum] = TEXT_COLOR_DARKBLUE;
    TextBold[TextNum]      = false;
    TextNum++;
  } else if ( ITEM_SUSPICIOUS_SCRAP_OF_PAPER <= ip->Type && ip->Type <= ITEM_COMPLETE_SECROMICON ) {
    switch ( ip->Type ) {
      case ITEM_SUSPICIOUS_SCRAP_OF_PAPER: {
        ...
        swprintf( TextList[TextNum], GlobalText[2'788] );
        TextListColor[TextNum] = TEXT_COLOR_WHITE;
        TextBold[TextNum]      = false;
        TextNum++;
      } break;
      case ITEM_GAIONS_ORDER: {
        swprintf( TextList[TextNum], GlobalText[2'784] );
        TextListColor[TextNum] = TEXT_COLOR_WHITE;
        TextBold[TextNum]      = false;
        TextNum++;
        swprintf( TextList[TextNum], GlobalText[2'785] );
        TextListColor[TextNum] = TEXT_COLOR_WHITE;
        TextBold[TextNum]      = false;
        TextNum++;
        swprintf( TextList[TextNum], GlobalText[2'786] );
        TextListColor[TextNum] = TEXT_COLOR_WHITE;
        TextBold[TextNum]      = false;
        TextNum++;
      } break;
      ...
      case ITEM_SIXTH_SECROMICON_FRAGMENT: {
        swprintf( TextList[TextNum], GlobalText[2'790] );
        TextListColor[TextNum] = TEXT_COLOR_WHITE;
        TextBold[TextNum]      = false;
        TextNum++;
      } break;
      case ITEM_COMPLETE_SECROMICON: {
        swprintf( TextList[TextNum], GlobalText[2'792] );
        TextListColor[TextNum] = TEXT_COLOR_WHITE;
        TextBold[TextNum]      = false;
        TextNum++;
        swprintf( TextList[TextNum], GlobalText[2'793] );
        TextListColor[TextNum] = TEXT_COLOR_WHITE;
        TextBold[TextNum]      = false;
        TextNum++;
      } break;
    ...
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[3'058], 4 );
    TextListColor[TextNum] = TEXT_COLOR_WHITE;
    TextBold[TextNum]      = false;
    TextNum++;
    ...
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[2'229], 7 );
    TextListColor[TextNum] = TEXT_COLOR_WHITE;
    TextBold[TextNum]      = false;
    TextNum++;
    ...
    swprintf( TextList[TextNum], GlobalText[2'876] );
    TextListColor[TextNum] = TEXT_COLOR_WHITE;
    TextBold[TextNum]      = false;
    TextNum++;
  } else if ( ITEM_POTION + 113 == ip->Type ) {
    swprintf( TextList[TextNum], GlobalText[2'875] );
    TextListColor[TextNum] = TEXT_COLOR_WHITE;
    TextBold[TextNum]      = false;
    TextNum++;
  } else if ( ip->Type == ITEM_POTION + 120 ) {
    swprintf( TextList[TextNum], GlobalText[2'971] );
    TextListColor[TextNum] = TEXT_COLOR_WHITE;
    TextBold[TextNum]      = false;
    TextNum++;
  } else if ( ITEM_POTION + 121 == ip->Type ) {
    swprintf( TextList[TextNum], GlobalText[2'877] );
    TextListColor[TextNum] = TEXT_COLOR_WHITE;
    TextBold[TextNum]      = false;
    TextNum++;
  } else if ( ITEM_POTION + 122 == ip->Type ) {
    swprintf( TextList[TextNum], GlobalText[2'878] );
    TextListColor[TextNum] = TEXT_COLOR_WHITE;
    TextBold[TextNum]      = false;
    TextNum++;
  } else if ( ITEM_POTION + 123 == ip->Type ) {
    swprintf( TextList[TextNum], GlobalText[2'879] );
    TextListColor[TextNum] = TEXT_COLOR_WHITE;
    TextBold[TextNum]      = false;
    TextNum++;
  } else if ( ITEM_POTION + 124 == ip->Type ) {
    swprintf( TextList[TextNum], GlobalText[2'880] );
    TextListColor[TextNum] = TEXT_COLOR_WHITE;
    TextBold[TextNum]      = false;
    TextNum++;
  } else if ( ITEM_POTION + 134 <= ip->Type && ITEM_POTION + 139 >= ip->Type ) {
    swprintf( TextList[TextNum], GlobalText[2'972] );
    TextListColor[TextNum] = TEXT_COLOR_WHITE;
    TextBold[TextNum]      = false;
    TextNum++;
  } else if ( ITEM_HELPER + 116 == ip->Type ) {
    swprintf( TextList[TextNum], GlobalText[3'018] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    ...
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[2'260], ip->Durability );
    TextListColor[TextNum] = TEXT_COLOR_WHITE;
    TextBold[TextNum]      = false;
    TextNum++;
  } else if ( ip->Type == ITEM_HELPER + 124 ) {
    swprintf( TextList[TextNum], GlobalText[3'116] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[3'127] );
    TextListColor[TextNum] = TEXT_COLOR_YELLOW;
    TextBold[TextNum]      = true;
    TextNum++;
    ...
    swprintf( TextList[TextNum], GlobalText[3'267] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
  }
    ...
    if ( ip->Type >> 4 == 15 ) {
      ...
      if ( ip->Type >= ITEM_ETC && ip->Type < ITEM_ETC + MAX_ITEM_INDEX ) {
        ...
        if ( DamageMin + minindex >= DamageMax + maxindex ) {
          swprintf( TextList[TextNum], L"%ls: %d ~ %d", GlobalText[40 + p->TwoHand], DamageMax + maxindex,
                    DamageMax + maxindex );
        } else {
          swprintf( TextList[TextNum], L"%ls: %d ~ %d", GlobalText[40 + p->TwoHand], DamageMin + minindex,
                    DamageMax + maxindex );
        }
      }
    } else {
  ...
  if ( ip->Type >= ITEM_PUMPKIN_OF_LUCK && ip->Type <= ITEM_JACK_OLANTERN_DRINK ) // Halloween event
  {
    ...
    switch ( ip->Type ) {
      ...
      case ITEM_JACK_OLANTERN_BLESSINGS:
        swprintf( Text_data, GlobalText[2'229], Item_data.m_byValue1 );
        swprintf( TextList[TextNum], Text_data );
        TextListColor[TextNum] = TEXT_COLOR_DARKYELLOW;
        break;
      case ITEM_JACK_OLANTERN_WRATH:
        swprintf( Text_data, GlobalText[2'230], Item_data.m_byValue1 );
        swprintf( TextList[TextNum], Text_data );
        TextListColor[TextNum] = TEXT_COLOR_DARKYELLOW;
        break;
      case ITEM_JACK_OLANTERN_CRY:
        swprintf( Text_data, GlobalText[2'231], Item_data.m_byValue1 );
        swprintf( TextList[TextNum], Text_data );
        TextListColor[TextNum] = TEXT_COLOR_DARKYELLOW;
        break;
  ...
  if ( ip->Type == ITEM_BOX_OF_LUCK ) {
    if ( Level == 7 ) {
      ...
      swprintf( TextList[TextNum], GlobalText[1'652] );
      TextListColor[TextNum] = TEXT_COLOR_WHITE;
      TextBold[TextNum]      = false;
      TextNum++;
      swprintf( TextList[TextNum], GlobalText[1'653] );
      TextListColor[TextNum] = TEXT_COLOR_WHITE;
      TextBold[TextNum]      = false;
      TextNum++;
    } else {
  ...
  if ( tCount > 0 ) {
    ...
    if ( nJewelIndex != COMGEM::NOGEM ) {
      swprintf( TextList[TextNum], GlobalText[1'819], tCount,
                GlobalText[COMGEM::GetJewelIndex( nJewelIndex, COMGEM::eGEM_NAME )] );
      TextListColor[TextNum] = TEXT_COLOR_WHITE;
      TextBold[TextNum]      = false;
      TextNum++;
      swprintf( TextList[TextNum], GlobalText[1'820] );
      TextListColor[TextNum] = TEXT_COLOR_WHITE;
      TextBold[TextNum]      = false;
      TextNum++;
    }
  ...
  if ( ip->Type == ITEM_JEWEL_OF_GUARDIAN ) {
    swprintf( TextList[TextNum], GlobalText[1'289] );
    TextListColor[TextNum] = TEXT_COLOR_WHITE;
    TextBold[TextNum]      = false;
    TextNum++;
  }
  ...
  if ( ( ip->Type >= ITEM_WING && ip->Type <= ITEM_WINGS_OF_SATAN ) || ip->Type == ITEM_WING_OF_CURSE ) {
    ...
    swprintf( TextList[TextNum], GlobalText[2'207] );
    TextListColor[TextNum] = TEXT_COLOR_WHITE;
    TextBold[TextNum]      = false;
    TextNum++;
  } else if ( ip->Type == ITEM_GEMSTONE ) {
    swprintf( TextList[TextNum], GlobalText[2'208] );
    TextListColor[TextNum] = TEXT_COLOR_WHITE;
    TextBold[TextNum]      = false;
    TextNum++;
  } else if ( ip->Type == ITEM_JEWEL_OF_HARMONY ) {
    swprintf( TextList[TextNum], GlobalText[2'209] );
    TextListColor[TextNum] = TEXT_COLOR_WHITE;
    TextBold[TextNum]      = false;
    TextNum++;
  } else if ( ip->Type == ITEM_LOWER_REFINE_STONE ) {
    swprintf( TextList[TextNum], GlobalText[2'210] );
    TextListColor[TextNum] = TEXT_COLOR_WHITE;
    TextBold[TextNum]      = false;
    TextNum++;
    ...
    // 연장의 보석
    swprintf( TextList[TextNum], GlobalText[3'305] );
    TextListColor[TextNum] = TEXT_COLOR_WHITE;
    TextBold[TextNum]      = false;
    TextNum++;
              ...
              ip->Type == ITEM_WINGS_OF_DESPAIR ) // 날개
  {
    ...
    switch ( Level ) {
      case 0:
        swprintf( TextList[TextNum], GlobalText[1'215] );
        TextNum++;
        break;
      case 1:
        swprintf( TextList[TextNum], GlobalText[1'216] );
        TextNum++;
        break;
    }
  } else if ( ip->Type == ITEM_LOCHS_FEATHER ) {
    ...
    switch ( Level ) {
      case 0:
        swprintf( TextList[TextNum], L"%ls", GlobalText[748] );
        TextNum++;
        break;
      case 1:
        swprintf( TextList[TextNum], L"%ls", GlobalText[1'236] );
        TextNum++;
        break;
    }
  }
    ...
    switch ( Level ) {
      case 0:
        swprintf( TextList[TextNum], L"%ls %ls", GlobalText[168], GlobalText[636] );
        break;
      case 1:
        swprintf( TextList[TextNum], L"%ls %ls", GlobalText[169], GlobalText[636] );
        break;
      case 2:
        swprintf( TextList[TextNum], L"%ls %ls", GlobalText[167], GlobalText[636] );
        break;
      case 3:
        swprintf( TextList[TextNum], L"%ls %ls", GlobalText[166], GlobalText[636] );
        break;
      case 4:
        swprintf( TextList[TextNum], L"%ls %ls", GlobalText[1'900], GlobalText[636] );
        break;
    }
    TextNum++;
    TextListColor[TextNum] = TEXT_COLOR_WHITE;
    switch ( Level ) {
      case 0:
        swprintf( TextList[TextNum], L"%ls %ls", GlobalText[168], GlobalText[1'910] );
        break;
      case 1:
        swprintf( TextList[TextNum], L"%ls %ls", GlobalText[169], GlobalText[1'910] );
        break;
      case 2:
        swprintf( TextList[TextNum], L"%ls %ls", GlobalText[167], GlobalText[1'910] );
        break;
      case 3:
        swprintf( TextList[TextNum], L"%ls %ls", GlobalText[166], GlobalText[1'910] );
        break;
      case 4:
        swprintf( TextList[TextNum], L"%ls %ls", GlobalText[1'900], GlobalText[1'910] );
        break;
    }
    TextNum++;
    TextListColor[TextNum] = TEXT_COLOR_DARKRED;
    ...
    switch ( Level ) {
      case 0: {
        swprintf( TextList[TextNum], GlobalText[1'417] );
        TextListColor[TextNum] = TEXT_COLOR_DARKRED;
        TextBold[TextNum]      = false;
        TextNum++;
        swprintf( TextList[TextNum], GlobalText[1'418] );
        TextListColor[TextNum] = TEXT_COLOR_DARKRED;
        TextBold[TextNum]      = false;
        TextNum++;
        swprintf( TextList[TextNum], GlobalText[1'419] );
        TextListColor[TextNum] = TEXT_COLOR_DARKRED;
        TextBold[TextNum]      = false;
        TextNum++;
      } break;
      case 1: {
        swprintf( TextList[TextNum], GlobalText[1'638] );
        TextListColor[TextNum] = TEXT_COLOR_DARKRED;
        TextBold[TextNum]      = false;
        TextNum++;
        swprintf( TextList[TextNum], GlobalText[1'639] );
        TextListColor[TextNum] = TEXT_COLOR_DARKRED;
        TextBold[TextNum]      = false;
        TextNum++;
        swprintf( TextList[TextNum], GlobalText[1'472] );
        TextListColor[TextNum] = TEXT_COLOR_DARKRED;
        TextBold[TextNum]      = false;
        TextNum++;
      } break;
    ...
    TextListColor[TextNum] = TEXT_COLOR_WHITE;
    swprintf( TextList[TextNum], L"%ls %ls    %ls      %ls    ", GlobalText[1'147], GlobalText[368], GlobalText[935],
              GlobalText[936] );
    TextListColor[TextNum] = TEXT_COLOR_WHITE;
    TextBold[TextNum]      = false;
    TextNum++;
    ...
    for ( int i = 0; i < 6; i++ ) {
      ...
      swprintf( TextList[TextNum], L"        %d             %3d~%3d     %3d,000", i + 1,
                g_iChaosCastleLevel[startIndex + i][0], std::min<int>( 400, g_iChaosCastleLevel[startIndex + i][1] ),
                Zen );
    ...
    swprintf( TextList[TextNum], L"         %d          %ls   %3d,000", 7, GlobalText[737], 1'000 );
    ...
    TextNum++;
    SkipNum++;
    swprintf( TextList[TextNum], GlobalText[1'157] );
    TextListColor[TextNum] = TEXT_COLOR_DARKBLUE;
    TextNum++;
  } else if ( ip->Type == ITEM_POTION + 21 ) {
    TextListColor[TextNum] = TEXT_COLOR_WHITE;
    switch ( Level ) {
      case 1:
        swprintf( TextList[TextNum], GlobalText[813] );
        break;
      case 2:
        swprintf( TextList[TextNum], GlobalText[1'099] );
        break;
      case 3:
        swprintf( TextList[TextNum], GlobalText[1'291] );
        break;
      default:
        break;
    }
  ...
  if ( ( bDurExist || ip->Durability ) && ( ip->Type < ITEM_LOCHS_FEATHER || ip->Type > ITEM_WEAPON_OF_ARCHANGEL ) &&
    ...
    if ( ip->Type >= ITEM_POTION && ip->Type <= ITEM_ANTIDOTE ) {
      ...
      swprintf( TextList[TextNum], GlobalText[1'181], ip->Durability, 20 );
      Success = true;
    } else if ( ip->Type == ITEM_CLAW_OF_BEAST ) {
      swprintf( TextList[TextNum], GlobalText[1'181], ip->Durability, 10 );
      Success = true;
      ...
      if ( ip->Type == ITEM_HELPER + 126 ) {
        TextNum++;
        swprintf( TextList[TextNum], GlobalText[3'105] );
      } else if ( ip->Type == ITEM_HELPER + 127 ) {
        TextNum++;
        swprintf( TextList[TextNum], GlobalText[3'106] );
      }
      Success = true;
    } else if ( ip->Type == ITEM_POTION + 53 ) {
      swprintf( TextList[TextNum], GlobalText[2'296], ip->Durability );
      Success = true;
    } else if ( ip->Type == ITEM_HELPER + 61 ) {
      swprintf( TextList[TextNum], GlobalText[2'260], ip->Durability );
      Success = true;
      ...
      if ( ip->Durability == 2 ) {
        swprintf( TextList[TextNum], GlobalText[2'605] );
        TextListColor[TextNum] = TEXT_COLOR_BLUE;
        TextBold[TextNum]      = false;
        TextNum++;
      } else if ( ip->Durability == 1 ) {
        swprintf( TextList[TextNum], GlobalText[2'606] );
        TextListColor[TextNum] = TEXT_COLOR_BLUE;
        TextBold[TextNum]      = false;
        TextNum++;
      ...
      swprintf( TextList[TextNum], GlobalText[2'732 + ( ip->Type - ( MODEL_TYPE_CHARM_MIXWING + EWS_BEGIN ) )] );
  ...
  {
    swprintf( TextList[TextNum], GlobalText[1'930], ip->RequireVitality );
  ...
  if ( ( ip->Type >= MODEL_STAFF - MODEL_ITEM && ip->Type < MODEL_STAFF + MAX_ITEM_INDEX - MODEL_ITEM ) ||
    ...
    int nText = ITEM_BOOK_OF_SAHAMUTT <= ip->Type && ip->Type <= ITEM_STAFF + 29 ? 1'691 : 79;
  ...
  if ( IsCepterItem( ip->Type ) == true ) {
    ...
    TextNum++;
    SkipNum++;
    swprintf( TextList[TextNum], GlobalText[1'234], ip->MagicPower );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = true;
    TextNum++;
  }
    ...
    if ( type < SI_None ) {
      if ( g_pUIJewelHarmonyinfo->IsHarmonyJewelOption( type, ip->Jewel_Of_Harmony_Option ) ) {
        ...
        if ( type == SI_Defense && ip->Jewel_Of_Harmony_Option == 7 ) {
          swprintf( TextList[TextNum], L"%ls +%d%%", harmonyjewel.Name,
        ...
        swprintf( TextList[TextNum], L"%ls : %d %d %d", GlobalText[2'204], (int)type, (int)ip->Jewel_Of_Harmony_Option,
        ...
        swprintf( TextList[TextNum], GlobalText[2'205] );
  ...
  for ( int i = 0; i < ip->SpecialNum; i++ ) {
    ...
    if ( ip->Special[i] == AT_LUCK ) {
      ...
      swprintf( TextList[TextNum], GlobalText[1'201] );
      TextListColor[TextNum] = TEXT_COLOR_DARKRED;
      TextBold[TextNum]      = false;
      TextNum++;
  ...
  if ( ip->Type == ITEM_SPLINTER_OF_ARMOR || ip->Type == ITEM_BLESS_OF_GUARDIAN ) {
    swprintf( TextList[TextNum], GlobalText[1'917] );
    TextListColor[TextNum] = TEXT_COLOR_YELLOW;
    TextNum++;
  } else if ( ip->Type == ITEM_CLAW_OF_BEAST || ip->Type == ITEM_FRAGMENT_OF_HORN ) {
    swprintf( TextList[TextNum], GlobalText[1'918] );
    TextListColor[TextNum] = TEXT_COLOR_YELLOW;
    TextNum++;
  } else if ( ip->Type == ITEM_BROKEN_HORN ) {
    swprintf( TextList[TextNum], GlobalText[1'919] );
    TextListColor[TextNum] = TEXT_COLOR_YELLOW;
    TextNum++;
  } else if ( ip->Type == ITEM_HORN_OF_FENRIR ) {
    ...
    if ( ip->ExcellentFlags == 0x01 ) {
      swprintf( TextList[TextNum], GlobalText[1'860], 10 );
      TextListColor[TextNum] = TEXT_COLOR_BLUE;
      TextNum++;
      ...
      swprintf( TextList[TextNum], GlobalText[1'861], 10 );
      TextListColor[TextNum] = TEXT_COLOR_BLUE;
      TextNum++;
      ...
      swprintf( TextList[TextNum], GlobalText[1'867], ( wLevel / 2 ) );
      TextListColor[TextNum] = TEXT_COLOR_BLUE;
      TextNum++;
      swprintf( TextList[TextNum], GlobalText[1'868], ( wLevel / 2 ) );
      TextListColor[TextNum] = TEXT_COLOR_BLUE;
      TextNum++;
      swprintf( TextList[TextNum], GlobalText[1'869], ( wLevel / 12 ) );
      TextListColor[TextNum] = TEXT_COLOR_BLUE;
      TextNum++;
      swprintf( TextList[TextNum], GlobalText[1'870], ( wLevel / 25 ) );
      TextListColor[TextNum] = TEXT_COLOR_BLUE;
      TextNum++;
      swprintf( TextList[TextNum], L"\n" );
      TextNum++;
      swprintf( TextList[TextNum], GlobalText[1'871], ( Hero->Level / 2 ) );
      TextListColor[TextNum] = TEXT_COLOR_GREEN;
      TextNum++;
      swprintf( TextList[TextNum], GlobalText[1'872], ( Hero->Level / 2 ) );
      TextListColor[TextNum] = TEXT_COLOR_GREEN;
      TextNum++;
    }
    ...
    swprintf( TextList[TextNum], GlobalText[1'920] );
    TextListColor[TextNum] = TEXT_COLOR_YELLOW;
    TextNum++;

    if ( ip->ExcellentFlags == 0x00 ) {
      swprintf( TextList[TextNum], GlobalText[1'929] );
      TextListColor[TextNum] = TEXT_COLOR_YELLOW;
      TextNum++;
    }
  } else if ( ip->Type == ITEM_TRANSFORMATION_RING ) {
    swprintf( TextList[TextNum], GlobalText[3'088] );
    TextListColor[TextNum] = TEXT_COLOR_RED;
    TextBold[TextNum]      = false;
    TextNum++;
    ...
    WORD wlevel = CharacterAttribute->Level;
    swprintf( TextList[TextNum], GlobalText[2'225], wlevel );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextNum++;
    ...
    TextListColor[TextNum] = TEXT_COLOR_WHITE;
    swprintf( TextList[TextNum], GlobalText[2'244] );
    TextNum++;
  } else if ( ip->Type == ITEM_GM_GIFT ) {
    TextListColor[TextNum] = TEXT_COLOR_WHITE;
    swprintf( TextList[TextNum], GlobalText[2'323] );
    TextNum++;
    TextListColor[TextNum] = TEXT_COLOR_WHITE;
    swprintf( TextList[TextNum], GlobalText[2'011] );
    TextNum++;
  } else if ( ip->Type == ITEM_JACK_OLANTERN_TRANSFORMATION_RING ) {
    swprintf( TextList[TextNum], L"%ls", GlobalText[2'232] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    ...
    TextNum++;
    SkipNum++;
    swprintf( TextList[TextNum], L"%ls", GlobalText[2'248] );
    TextListColor[TextNum] = TEXT_COLOR_RED;
    TextNum++;
    swprintf( TextList[TextNum], L"\n" );
    TextNum++;
    ...
    swprintf( TextList[TextNum], GlobalText[2'743] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[2'744] );
    ...
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[2'745] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], L"\n" );
    ...
    swprintf( TextList[TextNum], GlobalText[3'065] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[3'066] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], L"\n" );
    TextNum++;
    SkipNum++;
    swprintf( TextList[TextNum], GlobalText[3'067] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[3'072] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], L"\n" );
    ...
    swprintf( TextList[TextNum], L"%ls", GlobalText[2'244] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextNum++;
    ...
    TextListColor[TextNum] = TEXT_COLOR_RED;
    TextNum++;
    swprintf( TextList[TextNum], L"%ls", GlobalText[2'589] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextNum++;
  } else if ( ip->Type == ITEM_POTION + 100 ) {
    swprintf( TextList[TextNum], GlobalText[1'887], ip->Durability );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextNum++;
  } else if ( ip->Type == ITEM_PET_SKELETON ) {
    TextNum--;
    ...
    swprintf( TextList[TextNum], GlobalText[3'071] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
    ...
    if ( ip->Durability == 254 ) {
      swprintf( TextList[TextNum], GlobalText[3'143] );
      TextListColor[TextNum] = TEXT_COLOR_BLUE;
      TextBold[TextNum]      = false;
      TextNum++;
      swprintf( TextList[TextNum], L"\n" );
    ...
    switch ( ip->Type ) {
      ...
      case ITEM_HELPER + 129:
        swprintf( TextList[TextNum], GlobalText[3'121] );
        break;
      case ITEM_HELPER + 134:
        swprintf( TextList[TextNum], GlobalText[3'123] );
        break;
      ...
      case ITEM_HELPER + 133:
        swprintf( TextList[TextNum], GlobalText[3'122] );
        break;
    ...
    switch ( ip->Type ) {
      ...
      case ITEM_HELPER + 129:
        ...
        break;
      case ITEM_HELPER + 134:
        swprintf( TextList[TextNum], GlobalText[3'126], 20 );
        TextListColor[TextNum] = TEXT_COLOR_BLUE;
        TextBold[TextNum]      = false;
        TextNum++;
        break;
...
#ifdef LJH_ADD_ITEMS_EQUIPPED_FROM_INVENTORY_SYSTEM_PART_2
      case ITEM_HELPER + 130:
        swprintf( TextList[TextNum], GlobalText[3'132], 50 );
        TextListColor[TextNum] = TEXT_COLOR_BLUE;
        TextBold[TextNum]      = false;
        TextNum++;
        break;
      case ITEM_HELPER + 131:
        swprintf( TextList[TextNum], GlobalText[3'134], 50 );
        TextListColor[TextNum] = TEXT_COLOR_BLUE;
        TextBold[TextNum]      = false;
        TextNum++;
        break;
      case ITEM_HELPER + 132:
        ...
        swprintf( TextList[TextNum], GlobalText[3'132], 100 );
#else  // LJH_MOD_CHANGED_GOLDEN_OAK_CHARM_STAT
        swprintf( TextList[TextNum], GlobalText[3'132], 150 );
#endif // LJH_MOD_CHANGED_GOLDEN_OAK_CHARM_STAT
        TextListColor[TextNum] = TEXT_COLOR_BLUE;
        TextBold[TextNum]      = false;
        TextNum++;
#ifdef LJH_MOD_CHANGED_GOLDEN_OAK_CHARM_STAT
        swprintf( TextList[TextNum], GlobalText[3'133], 500 );
#else  // LJH_MOD_CHANGED_GOLDEN_OAK_CHARM_STAT
        swprintf( TextList[TextNum], GlobalText[3'133], 50 );
#endif // LJH_MOD_CHANGED_GOLDEN_OAK_CHARM_STAT
        TextListColor[TextNum] = TEXT_COLOR_BLUE;
        TextBold[TextNum]      = false;
        TextNum++;
      ...
      case ITEM_HELPER + 133: // 골든메이플참
        swprintf( TextList[TextNum], GlobalText[3'134], 150 );
        TextListColor[TextNum] = TEXT_COLOR_BLUE;
        TextBold[TextNum]      = false;
        TextNum++;
        swprintf( TextList[TextNum], GlobalText[3'135], 50 );
        TextListColor[TextNum] = TEXT_COLOR_BLUE;
        TextBold[TextNum]      = false;
        TextNum++;
        break;
    ...
    swprintf( TextList[TextNum], GlobalText[3'124] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
  }
  ...
  if ( ip->bPeriodItem == true ) {
    if ( ip->bExpiredPeriod == true ) {
      swprintf( TextList[TextNum], GlobalText[3'266] );
      TextListColor[TextNum] = TEXT_COLOR_RED;
    } else {
      swprintf( TextList[TextNum], GlobalText[3'265] );
      TextListColor[TextNum] = TEXT_COLOR_ORANGE;
      TextNum++;
      SkipNum++;

      swprintf( TextList[TextNum], L"%d-%02d-%02d  %02d:%02d", ExpireTime->tm_year + 1'900, ExpireTime->tm_mon + 1,
...
void RenderSkillInfo( int sx, int sy, int Type, int SkillNum, int iRenderPoint /*= STRP_NONE*/ )
{
  ...
  if ( CharacterAttribute->Skill[Type] == AT_SKILL_FORCE && Hero->Weapon[0].Type != -1 ) {
    for ( int i = 0; i < CharacterMachine->Equipment[EQUIPMENT_WEAPON_RIGHT].SpecialNum; i++ ) {
      if ( CharacterMachine->Equipment[0].Special[i] == AT_SKILL_FORCE_WAVE ) {
        swprintf( lpszName, L"%ls", GlobalText[1'200] );
        break;
      }
    }
  }
    ...
    if ( CharacterAttribute->Skill[Type] == AT_SKILL_SOUL_BARRIER ||
      ...
      if ( !( AT_SKILL_STUN <= bySkill && bySkill <= AT_SKILL_MANA ) &&
        ...
        if ( AT_SKILL_SUMMON_EXPLOSION <= bySkill && bySkill <= AT_SKILL_SUMMON_POLLUTION ) {
          ...
          swprintf( TextList[TextNum], GlobalText[1'692], iMinDamage, iMaxDamage );
        } else {
          swprintf( TextList[TextNum], GlobalText[170], iMinDamage + skillattackpowerRate,
  ...
  if ( HeroClass == CLASS_KNIGHT || HeroClass == CLASS_DARK || HeroClass == CLASS_ELF || HeroClass == CLASS_DARK_LORD ||
    ...
    switch ( CharacterAttribute->Skill[Type] ) {
      ...
      case AT_SKILL_EARTHSHAKE_STR:
      case AT_SKILL_EARTHSHAKE_MASTERY:
        swprintf( TextList[TextNum], GlobalText[1'237] );
        TextListColor[TextNum] = TEXT_COLOR_DARKRED;
        TextBold[TextNum]      = false;
        TextNum++;
        break;
  ...
  if ( gCharacterManager.GetBaseClass( Hero->Class ) == CLASS_ELF ) {
    ...
    switch ( CharacterAttribute->Skill[Type] ) {
      ...
      case AT_SKILL_RECOVER: {
        int Cal = Energy / 4;
        swprintf( TextList[TextNum], GlobalText[1'782], (int)( (float)Cal + (float)CharacterAttribute->Level ) );
      } break;
      default:
        Success = false;
    }
  ...
  if ( gCharacterManager.GetBaseClass( Hero->Class ) == CLASS_KNIGHT ) {
    ...
    if ( Hero->byExtensionSkill == 1 && CharacterAttribute->Level >= 220 ) {
      ...
      if ( ( CharacterAttribute->Skill[Type] >= AT_SKILL_FALLING_SLASH &&
        ...
        swprintf( TextList[TextNum], GlobalText[2'115] );
        TextListColor[TextNum] = TEXT_COLOR_DARKRED;
        TextBold[TextNum]      = false;
        TextNum++;
      }
  ...
  if ( MasteryType != 255 ) {
    swprintf( TextList[TextNum], GlobalText[1'080 + MasteryType] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
  }
  ...
  if ( SkillUseType == SKILL_USE_TYPE_BRAND ) {
    swprintf( TextList[TextNum], GlobalText[1'480], SkillAttribute[BrandType].Name );
    TextListColor[TextNum] = TEXT_COLOR_DARKRED;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[1'481], SkillAttribute[BrandType].Damage );
    TextListColor[TextNum] = TEXT_COLOR_DARKRED;
    TextBold[TextNum]      = false;
    TextNum++;
  }
  ...
  if ( SkillUseType == SKILL_USE_TYPE_MASTER ) {
    swprintf( TextList[TextNum], GlobalText[1'482] );
    TextListColor[TextNum] = TEXT_COLOR_DARKRED;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[1'483], SkillAttribute[SkillType].KillCount );
    TextListColor[TextNum] = TEXT_COLOR_DARKRED;
    TextBold[TextNum]      = false;
    TextNum++;
  }
  ...
  if ( gCharacterManager.GetBaseClass( Hero->Class ) == CLASS_DARK_LORD ) {
    if ( CharacterAttribute->Skill[Type] == AT_SKILL_PARTY_TELEPORT && PartyNumber <= 0 ) {
      swprintf( TextList[TextNum], GlobalText[1'185] );
      TextListColor[TextNum] = TEXT_COLOR_DARKRED;
      TextBold[TextNum]      = false;
      TextNum++;
    }
  ...
  if ( CharacterAttribute->Skill[Type] == AT_SKILL_PLASMA_STORM_FENRIR ) {
    swprintf( TextList[TextNum], GlobalText[1'926] );
    TextListColor[TextNum] = TEXT_COLOR_DARKRED;
    TextBold[TextNum]      = false;
    TextNum++;
    swprintf( TextList[TextNum], GlobalText[1'927] );
    TextListColor[TextNum] = TEXT_COLOR_DARKRED;
    TextBold[TextNum]      = false;
    TextNum++;
  }
  ...
  if ( CharacterAttribute->Skill[Type] == AT_SKILL_INFINITY_ARROW ||
    ...
    swprintf( TextList[3], GlobalText[2'040] );
  ...
  if ( CharacterAttribute->Skill[Type] == AT_SKILL_RUSH || CharacterAttribute->Skill[Type] == AT_SKILL_SPACE_SPLIT ||
       ...
       CharacterAttribute->Skill[Type] == AT_SKILL_DEATH_CANNON ||
       CharacterAttribute->Skill[Type] == AT_SKILL_OCCUPY ) {
    swprintf( TextList[TextNum], GlobalText[2'047] );
    TextListColor[TextNum] = TEXT_COLOR_DARKRED;
    TextBold[TextNum]      = false;
    TextNum++;
  }
  if ( CharacterAttribute->Skill[Type] == AT_SKILL_STUN || CharacterAttribute->Skill[Type] == AT_SKILL_REMOVAL_STUN ||
    ...
    swprintf( TextList[TextNum], GlobalText[2'048] );
    TextListColor[TextNum] = TEXT_COLOR_DARKRED;
    TextBold[TextNum]      = false;
    TextNum++;
  }
  if ( CharacterAttribute->Skill[Type] == AT_SKILL_IMPALE ) {
    swprintf( TextList[TextNum], GlobalText[2'049] );
    TextListColor[TextNum] = TEXT_COLOR_DARKRED;
    TextBold[TextNum]      = false;
    TextNum++;
  }
  ...
  if ( SkillType == AT_SKILL_EXPANSION_OF_WIZARDRY || SkillType == AT_SKILL_EXPANSION_OF_WIZARDRY_STR ||
       SkillType == AT_SKILL_EXPANSION_OF_WIZARDRY_MASTERY ) {
    swprintf( TextList[TextNum], GlobalText[2'054] );
    TextListColor[TextNum] = TEXT_COLOR_BLUE;
    TextBold[TextNum]      = false;
    TextNum++;
  }
...
void RenderItemName( int i, OBJECT* o, ITEM* ip, bool Sort )
{
  ...
  if ( whiteTextItems.count( o->Type ) > 0 ) {
    ...
    switch ( ItemLevel ) {
      case 0:
        swprintf( Name, GlobalText[100] );
        break;
      case 1:
        swprintf( Name, GlobalText[101] );
        break;
      case 2:
        swprintf( Name, GlobalText[104] );
        break;
    }
  } else if ( o->Type == MODEL_FRUITS ) {
    switch ( ItemLevel ) {
      case 0:
        swprintf( Name, L"%ls %ls", GlobalText[168], ItemAttribute[o->Type - MODEL_ITEM].Name );
        break;
      case 1:
        swprintf( Name, L"%ls %ls", GlobalText[169], ItemAttribute[o->Type - MODEL_ITEM].Name );
        break;
      case 2:
        swprintf( Name, L"%ls %ls", GlobalText[167], ItemAttribute[o->Type - MODEL_ITEM].Name );
        break;
      case 3:
        swprintf( Name, L"%ls %ls", GlobalText[166], ItemAttribute[o->Type - MODEL_ITEM].Name );
        break;
      case 4:
        swprintf( Name, L"%ls %ls", GlobalText[1'900], ItemAttribute[o->Type - MODEL_ITEM].Name );
        break;
    }
  } else if ( o->Type == MODEL_SPIRIT ) {
    switch ( ItemLevel ) {
      case 0:
        swprintf( Name, L"%ls of %ls", ItemAttribute[o->Type - MODEL_ITEM].Name, GlobalText[1'187] );
        break;
      case 1:
        swprintf( Name, L"%ls of %ls", ItemAttribute[o->Type - MODEL_ITEM].Name, GlobalText[1'214] );
        break;
    }
  } else if ( o->Type == MODEL_EVENT + 16 ) {
    swprintf( Name, GlobalText[1'235] );
  } else if ( o->Type == MODEL_EVENT + 4 ) {
    swprintf( Name, GlobalText[105] );
  } else if ( o->Type == MODEL_EVENT + 5 ) {
    switch ( ItemLevel ) {
      case 14:
        swprintf( Name, GlobalText[1'650] );
        break;

      case 15:
        swprintf( Name, GlobalText[1'651] );
        break;
    ...
    if ( ItemLevel == 0 ) {
      ...
      SetTextColor( 1.f, 0.3f, 1.f );
      swprintf( Name, GlobalText[2'012] );
    }
    ...
    if ( ItemLevel == 0 ) {
      ...
      SetTextColor( 1.0f, 0.3f, 0.3f );
      swprintf( Name, GlobalText[2'013] );
    }
    ...
    if ( ItemLevel == 0 ) {
      ...
      SetTextColor( 0.3f, 0.3f, 1.f );
      swprintf( Name, GlobalText[2'014] );
    }
    ...
    SetYellowTextColor();
    swprintf( Name, GlobalText[1'290] );
  } else if ( o->Type == MODEL_SIEGE_POTION ) {
    switch ( ItemLevel ) {
      case 0:
        swprintf( Name, GlobalText[1'413] );
        break;
      case 1:
        swprintf( Name, GlobalText[1'414] );
        break;
    }
  } else if ( o->Type == MODEL_HELPER + 7 ) {
    switch ( ItemLevel ) {
      case 0:
        swprintf( Name, GlobalText[1'460] );
        break;
      case 1:
        swprintf( Name, GlobalText[1'461] );
        break;
    }
  } else if ( o->Type == MODEL_LIFE_STONE_ITEM ) {
    switch ( ItemLevel ) {
      case 0:
        swprintf( Name, GlobalText[1'416] );
        break;
      case 1:
        swprintf( Name, GlobalText[1'462] );
        break;
    }
...
bool IsSellingBan( ITEM* pItem )
{
  ...
  if ( pItem->Type == ITEM_BOX_OF_LUCK || ( pItem->Type == ITEM_POTION + 21 && Level == 1 ) ||
       ...
       pItem->Type == ITEM_WEAPON_OF_ARCHANGEL || ( pItem->Type == ITEM_POTION + 20 && Level >= 1 && Level <= 5 ) ||
       IsPartChargeItem( pItem ) ||
       ( ( pItem->Type >= ITEM_TYPE_CHARM_MIXWING + EWS_BEGIN ) &&
         ( pItem->Type <= ITEM_TYPE_CHARM_MIXWING + EWS_END ) ) ||
       pItem->Type == ITEM_PET_PANDA || pItem->Type == ITEM_PANDA_TRANSFORMATION_RING ||
                   ...
                   int ancientDiscriminator, bool PickUp )
{
  ...
  if ( Type == ITEM_BOX_OF_LUCK && Level == 1 ) // 성탄의별
  {
    ...
    RenderObjectScreen( MODEL_JACK_OLANTERN_BLESSINGS, Level, excellentFlags, ancientDiscriminator, Position, Success,
                        PickUp );
    ...
    switch ( Level ) {
      ...
      case 1:
        ...
        break;
      case 2:
        RenderObjectScreen( MODEL_DIVINE_CB_OF_ARCHANGEL, -1, excellentFlags, ancientDiscriminator, Position, Success,
                            PickUp );
        break;
    }
  ...
  if ( ( g_pUIManager->IsOpen( INTERFACE_PERSONALSHOPSALE ) ||
    ...
    if ( MouseX >= ButtonX && MouseX < ButtonX + Width && MouseY >= ButtonY && MouseY < ButtonY + Height &&
      ...
      if ( !IsExistUndecidedPrice() && wcslen( g_szPersonalShopTitle ) > 0 ) {
        ...
        g_pSystemLogBox->AddText( GlobalText[1'119], SEASON3B::TYPE_ERROR_MESSAGE );
      }
    }
    ...
    for ( int j = 0; j < Width; ++j ) {
      if ( j >= ( Width / 2 - 16 ) && j < ( Width / 2 + 16 ) && i >= ( Height / 2 - 16 ) && i < ( Height / 2 + 16 ) ) {
        ...
        *( (unsigned int*)( Buffer + offset ) ) =
            ( 255 << 24 ) + ( 0 << 16 ) + ( (int)( 50 + i / 100.f * 160 ) << 8 ) + ( 50 + i / 100.f * 255 );
      } else {
