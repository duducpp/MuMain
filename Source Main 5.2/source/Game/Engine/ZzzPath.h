                            ...
                            float fDistance )
{
  ...
  if ( 0.0f == fDistance ) {
    ...
    if ( bErrorCheck &&
         ( iWall <= m_pbyMap[iEndIndex] && ( m_pbyMap[GetIndex( xEnd, yEnd )] & TW_ACTION ) != TW_ACTION ) ) {
      return false;
    }
...
inline int PATH::EstimateCostToGoal( int xStart, int yStart, int xNew, int yNew )
{
  ...
  return ( abs( xDist - yDist ) * FACTOR_PATH_DIST + std::min<int>( xDist, yDist ) * FACTOR_PATH_DIST_DIAG + 1 ) * 3 /
         4;
}
