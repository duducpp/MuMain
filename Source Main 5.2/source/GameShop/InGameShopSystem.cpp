...
bool CInGameShopSystem::ScriptDownload()
{
  ...
  if ( !res.IsSuccess() ) {
    ...
    swprintf( szText, GlobalText[3'029], m_ScriptVerInfo.Zone, m_ScriptVerInfo.year, m_ScriptVerInfo.yearId,
              res.GetErrorMessage() );
    CMsgBoxIGSCommon* pMsgBox = NULL;
    CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
    pMsgBox->Initialize( GlobalText[3'028], szText );
    return false;
  }
  ...
  g_ConsoleDebug->Write( MCD_NORMAL, L"InGameShopStatue.Txt - Ver %d.%d.%d", m_ScriptVerInfo.Zone, m_ScriptVerInfo.year,
...
bool CInGameShopSystem::BannerDownload()
{
  ...
  if ( !res.IsSuccess() ) {
    ...
    swprintf( szText, GlobalText[3'030], m_BannerVerInfo.Zone, m_BannerVerInfo.year, m_BannerVerInfo.yearId,
              res.GetErrorMessage() );
    CMsgBoxIGSCommon* pMsgBox = NULL;
    CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
    pMsgBox->Initialize( GlobalText[3'028], szText );
      ...
      if ( ( pProduct->PropertySeq == 2 ) || ( pProduct->PropertySeq == 28 ) || ( pProduct->PropertySeq == 12 ) ||
        ...
        switch ( pProduct->UnitType ) {
          case 386: {
            if ( iValue >= 86'400 ) {
              iValue /= 86'400;
              wcscpy( pszUnitName, GlobalText[2'298] );
            } else if ( iValue >= 3'600 ) {
              iValue /= 3'600;
              wcscpy( pszUnitName, GlobalText[2'299] );
            } else if ( iValue >= 60 ) {
              iValue /= 60;
              wcscpy( pszUnitName, GlobalText[2'300] );
            } else {
              wcscpy( pszUnitName, GlobalText[2'301] );
            }
          } break;
          case 174: {
            if ( iValue >= 1'440 ) {
