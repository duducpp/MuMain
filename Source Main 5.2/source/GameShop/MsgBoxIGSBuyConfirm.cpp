                                       ...
                                       wchar_t* pszName, wchar_t* pszPrice, wchar_t* pszPeriod )
{
  ...
  swprintf( m_szItemName, GlobalText[3'037], pszName );
  swprintf( m_szItemPrice, GlobalText[3'038], pszPrice );
  swprintf( m_szItemPeriod, GlobalText[3'039], pszPeriod );

  // int m_iNumNoticeLine = SeparateTextIntoLines( GlobalText[2898], txtline[0], NUM_LINE_CMB, MAX_LENGTH_CMB);
  m_iNumNoticeLine = ::DivideStringByPixel( &m_szNotice[0][0], NUM_LINE_CMB, MAX_TEXT_LENGTH, GlobalText[2'898],
...
void CMsgBoxIGSBuyConfirm::RenderTexts()
{
  ...
  g_pRenderText->RenderText( GetPos().x, GetPos().y + IGS_TEXT_TITLE_POS_Y, GlobalText[2'896], IMAGE_IGS_FRAME_WIDTH, 0,
                             RT3_SORT_CENTER );
  g_pRenderText->SetFont( g_hFont );
  g_pRenderText->RenderText( GetPos().x, GetPos().y + IGS_TEXT_QUESTION_POS_Y, GlobalText[2'897], IMAGE_IGS_FRAME_WIDTH,
                             0, RT3_SORT_CENTER );
  ...
  if ( m_wItemCode == 65'535 ) {
    swprintf( szText, L"Bad Item index" );
  } else {
    swprintf( szText, L"ItemCode : %d (%d, %d)", m_wItemCode, m_wItemCode / MAX_ITEM_INDEX,
