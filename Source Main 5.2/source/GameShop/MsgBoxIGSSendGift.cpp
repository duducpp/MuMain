                                     ...
                                     wchar_t* pszName, wchar_t* pszPrice, wchar_t* pszPeriod )
{
  ...
  swprintf( m_szName, GlobalText[3'037], pszName );
  swprintf( m_szPrice, GlobalText[3'038], pszPrice );
  swprintf( m_szPeriod, GlobalText[3'039], pszPeriod );

  m_iNumNoticeLine = ::DivideStringByPixel( &m_szNotice[0][0], NUM_LINE_CMB, MAX_TEXT_LENGTH, GlobalText[2'920],
...
CALLBACK_RESULT CMsgBoxIGSSendGift::OKButtonDown( class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam )
{
  ...
  if ( pOwnMsgBox->m_szID[0] == '\0' ) {
    ...
    pMsgBox->Initialize( GlobalText[3'028], GlobalText[3'031] );
  } else if ( wcscmp( pOwnMsgBox->m_szID, Hero->ID ) == 0 ) {
    CMsgBoxIGSCommon* pMsgBox = NULL;
    CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
    pMsgBox->Initialize( GlobalText[3'028], GlobalText[3'032] );
  } else {
...
void CMsgBoxIGSSendGift::RenderTexts()
{
  ...
  g_pRenderText->RenderText( GetPos().x, GetPos().y + IGS_TEXT_TITLE_POS_Y, GlobalText[2'916], IMAGE_IGS_FRAME_WIDTH, 0,
                             RT3_SORT_CENTER );

  g_pRenderText->RenderText( GetPos().x + IGS_TEXT_ID_TITLE_POS_X, GetPos().y + IGS_TEXT_ID_TITLE_POS_Y,
                             GlobalText[2'918], IGS_TEXT_ID_TITLE_WIDTH, 0, RT3_SORT_LEFT );

  g_pRenderText->SetTextColor( 0, 0, 0, 255 );
  g_pRenderText->RenderText( GetPos().x, GetPos().y + IGS_TEXT_MESSAGE_TITLE_POS_Y, GlobalText[2'919],
