...
void RenderText( const wchar_t* text, int x, int y, int sx, int sy, HFONT hFont, DWORD color, DWORD backcolor,
                 int sort )
{
...
bool SEASON3B::CNewUIButton::Render( bool RendOption )
{
  ...
  if ( m_TooltipText.size() != 0 ) {
    if ( CheckMouseIn( m_Pos.x, m_Pos.y, m_Size.x, m_Size.y ) ) {
      ...
      // RenderText( m_TooltipText.c_str(), x, y, Fontsize.cx+6, 0, m_hToolTipFont, m_TooltipTextColor, RGBA(0, 0, 0,
      // 180), RT3_SORT_CENTER );
    }
  }
...
void CNewUIRadioGroupButton::ChangeButtonState( int iBtnIndex, int iImgIndex, BUTTON_STATE eventstate,
                                                int iButtonState )
{
  int i = 0;
  for ( auto iter = m_RadioList.begin(); iter != m_RadioList.end(); ) {
