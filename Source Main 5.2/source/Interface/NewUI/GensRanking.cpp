...
#define TEMP_MAX_TEXT_LENGTH 1'024
...
void CNewUIGensRanking::RenderTexts()
{
  ...
  swprintf( szText, GlobalText[3'095] );
  _y += 24;
  ...
  if ( GetNextContribution() > 0 ) {
    swprintf( _szTempText, GlobalText[3'097], GetNextContribution() );
  ...
  g_pRenderText->SetFont( g_hFontBold );
  g_pRenderText->SetTextColor( 230, 230, 0, 255 );
  swprintf( szText, GlobalText[3'100] );
...
bool CNewUIGensRanking::Update()
{
  ...
  if ( m_pTextBox ) {
    ...
    m_pTextBox->AddText( GlobalText[3'101] );
    m_pTextBox->AddText( GlobalText[3'102] );
...
bool CNewUIGensRanking::SetGensInfo()
{
  ...
  if ( ( m_byGensInfluence & GENSTYPE_DUPRIAN ) == GENSTYPE_DUPRIAN ) {
    SetGensTeamName( GlobalText[3'092] );
    return true;
  } else if ( ( m_byGensInfluence & GENSTYPE_BARNERT ) == GENSTYPE_BARNERT ) {
    SetGensTeamName( GlobalText[3'093] );
    return true;
  } else {
    g_pSystemLogBox->AddText( GlobalText[3'094], SEASON3B::TYPE_SYSTEM_MESSAGE );
    return false;
  }
  return false;
}
...
void CNewUIGensRanking::SetTitleName()
{
  ...
  swprintf( _szTempText, GlobalText[3'104] );
