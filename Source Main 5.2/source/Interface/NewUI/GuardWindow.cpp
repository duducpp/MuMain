...
bool CNewUIGuardWindow::Create( CNewUIManager* pNewUIMng, int x, int y )
{
  ...
  ltext.push_back( GlobalText[1'448] );
  ltext.push_back( GlobalText[1'439] );
  ltext.push_back( GlobalText[1'449] );
  ...
  InitButton( &m_BtnProclaim, m_Pos.x + INVENTORY_WIDTH / 2 - 27, m_Pos.y + 120, GlobalText[1'435] );
  InitButton( &m_BtnRegister, m_Pos.x + INVENTORY_WIDTH / 2 - 27, m_Pos.y + 200, GlobalText[1'439] );
  InitButton( &m_BtnGiveUp, m_Pos.x + INVENTORY_WIDTH / 2 - 27, m_Pos.y + 370, GlobalText[1'549] );
...
bool CNewUIGuardWindow::Render()
{
  ...
  if ( m_eTimeType == CASTLESIEGE_STATE_REGSIEGE ) {
    ltext.push_back( GlobalText[1'448] );
    ltext.push_back( GlobalText[1'435] );
    ltext.push_back( GlobalText[1'449] );
  } else {
    ltext.push_back( GlobalText[1'448] );
    ltext.push_back( GlobalText[1'439] );
...
void CNewUIGuardWindow::RenderFrame()
{
  ...
  swprintf( szText, L"%ls", GlobalText[1'445] );
  ...
  if ( m_szOwnerGuildMaster[0] ) {
    swprintf( szText, GlobalText[1'446], m_szOwnerGuildMaster );
  } else {
    swprintf( szText, GlobalText[1'446], GlobalText[1'361] );
  }
  g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, szText, 190, 0, RT3_SORT_CENTER );

  ptOrigin.y += 15;
  if ( m_szOwnerGuild[0] ) {
    swprintf( szText, GlobalText[1'447], m_szOwnerGuild );
  } else {
    swprintf( szText, GlobalText[1'447], GlobalText[1'361] );
  }
...
void CNewUIGuardWindow::RenderSeigeInfoTab()
{
  ...
  swprintf( szTemp, GlobalText[1'533], m_wStartYear, m_byStartMonth, m_byStartDay, m_byStartHour, m_byStartMinute );
  g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, szTemp, 190, 0, RT3_SORT_CENTER );

  ptOrigin.y += 14;
  swprintf( szTemp, GlobalText[1'534], m_wEndYear, m_byEndMonth, m_byEndDay, m_byEndHour, m_byEndMinute );
  g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, szTemp, 190, 0, RT3_SORT_CENTER );
  ...
  switch ( m_eTimeType ) {
    ...
    case CASTLESIEGE_STATE_IDLE_1:
      g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, GlobalText[1'535], 190, 0, RT3_SORT_CENTER );
      break;
    case CASTLESIEGE_STATE_REGSIEGE:
      g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, GlobalText[1'536], 190, 0, RT3_SORT_CENTER );
      break;
    case CASTLESIEGE_STATE_IDLE_2:
      g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, GlobalText[1'537], 190, 0, RT3_SORT_CENTER );
      break;
    case CASTLESIEGE_STATE_REGMARK:
      g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, GlobalText[1'538], 190, 0, RT3_SORT_CENTER );
      break;
    case CASTLESIEGE_STATE_IDLE_3:
      g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, GlobalText[1'539], 190, 0, RT3_SORT_CENTER );
      break;
    case CASTLESIEGE_STATE_NOTIFY:
      g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, GlobalText[1'540], 190, 0, RT3_SORT_CENTER );
      break;
    case CASTLESIEGE_STATE_READYSIEGE:
      g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, GlobalText[1'541], 190, 0, RT3_SORT_CENTER );
      break;
    case CASTLESIEGE_STATE_STARTSIEGE:
      g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, GlobalText[1'542], 190, 0, RT3_SORT_CENTER );
      break;
    case CASTLESIEGE_STATE_ENDSIEGE:
      g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, GlobalText[1'543], 190, 0, RT3_SORT_CENTER );
      break;
    case CASTLESIEGE_STATE_ENDCYCLE:
      g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, GlobalText[1'544], 190, 0, RT3_SORT_CENTER );
      break;
  }

  if ( m_eTimeType < CASTLESIEGE_STATE_STARTSIEGE ) {
    ptOrigin.y += 35;
    g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, GlobalText[1'545], 190, 0, RT3_SORT_CENTER );

    ptOrigin.y += 14;
    swprintf( szTemp, GlobalText[1'546], m_wSiegeStartYear, m_bySiegeStartMonth, m_bySiegeStartDay, m_bySiegeStartHour,
              m_bySiegeStartMinute );
    g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, szTemp, 190, 0, RT3_SORT_CENTER );

    ptOrigin.y += 35;
    swprintf( szTemp, GlobalText[1'421], m_dwStateLeftSec / 3'600, ( m_dwStateLeftSec % 3'600 ) / 60,
              ( m_dwStateLeftSec % 3'600 ) % 60 );
    g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, szTemp, 190, 0, RT3_SORT_CENTER );
  }
}
...
void CNewUIGuardWindow::RenderRegisterTab()
{
  ...
  switch ( m_eTimeType ) {
    ...
    case CASTLESIEGE_STATE_REGSIEGE:
      if ( Hero->GuildStatus == G_MASTER ) {
        if ( !g_GuardsMan.HasRegistered() ) {
          ...
          g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, GlobalText[1'547], 190, 0, RT3_SORT_CENTER );
        }
      } else {
        g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, GlobalText[1'320], 190, 0, RT3_SORT_CENTER );
      }
      break;
    case CASTLESIEGE_STATE_IDLE_2:
      g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, GlobalText[1'548], 190, 0, RT3_SORT_CENTER );
      break;
    case CASTLESIEGE_STATE_REGMARK: {
      if ( g_GuardsMan.HasRegistered() ) {
        g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, GlobalText[1'436], 190, 0, RT3_SORT_CENTER );
        ptOrigin.y += 30;

        int     nMarkCount = g_GuardsMan.GetMyMarkCount();
        wchar_t szBuffer[256];
        swprintf( szBuffer, GlobalText[1'437], nMarkCount );
        g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, szBuffer, 190, 0, RT3_SORT_CENTER );

        ptOrigin.y += 14;
        swprintf( szBuffer, GlobalText[1'438], g_GuardsMan.GetRegMarkCount() );
        g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, szBuffer, 190, 0, RT3_SORT_CENTER );
        ...
        g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, GlobalText[1'514], 190, 0, RT3_SORT_CENTER );
      }
    } break;
    case CASTLESIEGE_STATE_IDLE_3:
      g_pRenderText->SetFont( g_hFontBold );
      g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, GlobalText[1'440], 190, 0, RT3_SORT_CENTER );
      ptOrigin.y += 14;
      g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, GlobalText[1'441], 190, 0, RT3_SORT_CENTER );
      break;
    case CASTLESIEGE_STATE_NOTIFY:
      g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, GlobalText[1'540], 190, 0, RT3_SORT_CENTER );
      break;
    ...
    case CASTLESIEGE_STATE_ENDSIEGE:
      g_pRenderText->SetFont( g_hFontBold );
      g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, GlobalText[1'442], 190, 0, RT3_SORT_CENTER );
      break;
    case CASTLESIEGE_STATE_ENDCYCLE:
      g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, GlobalText[1'544], 190, 0, RT3_SORT_CENTER );
      break;
...
void CNewUIGuardWindow::RenderRegisterInfoTab()
{
  ...
  if ( m_eTimeType == CASTLESIEGE_STATE_REGSIEGE || m_eTimeType == CASTLESIEGE_STATE_REGMARK ) {
    ...
    wchar_t szBuffer[256];
    swprintf( szBuffer, GlobalText[1'443], 1, 1 );
    g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, szBuffer, 190, 0, RT3_SORT_CENTER );
    ptOrigin.y += 14;
    g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, GlobalText[1'444], 190, 0, RT3_SORT_CENTER );
