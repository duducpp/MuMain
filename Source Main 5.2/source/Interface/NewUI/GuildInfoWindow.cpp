...
#include "Interface/UI/UIPopup.h"
...
bool SEASON3B::CNewUIGuildInfoWindow::Create( CNewUIManager* pNewUIMng, int x, int y )
{
  ...
  m_Button[BUTTON_GET_POSITION].ChangeText( GlobalText[1'307] );
  m_Button[BUTTON_FREE_POSITION].ChangeText( GlobalText[1'308] );
  m_Button[BUTTON_GET_OUT].ChangeText( GlobalText[1'309] );
      ...
      if ( GUILDLIST_TEXT* pText = m_GuildMember.GetSelectedText() ) {
        if ( pText->m_GuildStatus == G_SUB_MASTER || pText->m_GuildStatus == G_BATTLE_MASTER ) {
          if ( GUILDLIST_TEXT* pText = m_GuildMember.GetSelectedText() ) {
            ...
            if ( pMsgBox != NULL ) {
              wchar_t strText[256];
              swprintf( strText, GlobalText[1'367], pText->m_szID );
              pMsgBox->AddMsg( strText );
              pMsgBox->AddMsg( GlobalText[1'368] );
            }
          }
        }
      }
...
void SEASON3B::CNewUIGuildInfoWindow::Render_Text()
{
  ...
  swprintf( Text, GlobalText[1'330] );
  ...
  swprintf( Text, GlobalText[1'352] );
  ...
  if ( m_nCurrentTab == 0 ) {
    ...
    swprintf( Text, GlobalText[1'323] );
    ...
    swprintf( Text, L"%ls :", GlobalText[1'332] );
    RenderText( Text, m_Pos.x + 22, Nm_Loc, 40, 0, 0xFFFFFFFF, 0x00000000, RT3_SORT_LEFT );
    Nm_Loc += 13;

    swprintf( Text, GlobalText[1'256], GuildTotalScore );
    ...
    if ( Hero->GuildStatus == G_MASTER ) {
      ...
      if ( Class == CLASS_DARK_LORD ) {
        ...
        swprintf( Text, GlobalText[1'362], g_nGuildMemberCount, nCount );
      } else {
        swprintf( Text, GlobalText[1'362], g_nGuildMemberCount, CharacterAttribute->Level / 10 );
      }
    } else {
      swprintf( Text, GlobalText[1'310], g_nGuildMemberCount );
    }
    RenderText( Text, m_Pos.x + 22, Nm_Loc, 80, 0, 0xFFFFFFFF, 0x00000000, RT3_SORT_LEFT );

    Nm_Loc += 13;
    swprintf( Text, L"%ls : %ls", GlobalText[1'321], m_RivalGuildName[0] ? m_RivalGuildName : GlobalText[1'361] );
    RenderText( Text, m_Pos.x + 22, Nm_Loc, 0, 0, 0xFFFFFFFF, 0x00000000, RT3_SORT_LEFT );
  } else if ( m_nCurrentTab == 1 ) {
    glColor4f( 1.f, 1.f, 1.f, 1.f );
    RenderText( (wchar_t*)GlobalText[1'389], m_Pos.x + 24, m_Pos.y + 112, 40, 0, 0xFFFFFFFF, 0x00000000,
                RT3_SORT_LEFT );
    RenderText( (wchar_t*)GlobalText[1'307], m_Pos.x + 89, m_Pos.y + 112, 40, 0, 0xFFFFFFFF, 0x00000000,
                RT3_SORT_LEFT );
    RenderText( (wchar_t*)GlobalText[1'022], m_Pos.x + 126, m_Pos.y + 112, 40, 0, 0xFFFFFFFF, 0x00000000,
                RT3_SORT_LEFT );
    ...
    m_Button[BUTTON_UNION_CREATE].ChangeText( GlobalText[1'422] );
    m_Button[BUTTON_UNION_OUT].ChangeText( GlobalText[1'324] );
    if ( GuildMark[Hero->GuildMarkIndex].UnionName[0] != NULL ) {
      RenderText( (wchar_t*)GlobalText[182], m_Pos.x + 34, m_Pos.y + 115, 40, 0, 0xFFFFFFFF, 0x00000000,
                  RT3_SORT_LEFT );
      RenderText( (wchar_t*)GlobalText[1'330], m_Pos.x + 140, m_Pos.y + 115, 40, 0, 0xFFFFFFFF, 0x00000000,
                  RT3_SORT_LEFT );
    }
  }
}
                                                                ...
                                                                BYTE byTargetUserIndexH, BYTE byTargetUserIndexL )
{
  if ( !g_MessageBox->IsEmpty() ) {
    ...
    if ( m_MessageInfo.s_byRelationShipType == 0x01 ) // Union
    {
      ...
      {
        swprintf( szText[0], GlobalText[1'280], pPlayer->ID );
        swprintf( szText[1], GlobalText[1'281] );
        swprintf( szText[2], GlobalText[1'283] );
      } else // Break Off
      {
        swprintf( szText[0], GlobalText[1'280], pPlayer->ID );
        swprintf( szText[1], GlobalText[1'282] );
        swprintf( szText[2], GlobalText[1'283] );
      }
    } else if ( m_MessageInfo.s_byRelationShipType == 0x02 ) // Rival
    {
      if ( m_MessageInfo.s_byRelationShipRequestType == 0x01 ) // Join
      {
        swprintf( szText[0], GlobalText[1'284], pPlayer->ID );
        swprintf( szText[1], GlobalText[1'286] );
        swprintf( szText[2], GlobalText[1'283] );
      } else {
        swprintf( szText[0], GlobalText[1'284], pPlayer->ID );
        swprintf( szText[1], GlobalText[1'285] );
        swprintf( szText[2], GlobalText[1'283] );
      }
    }
