...
#include "Interface/NewUI/MainFrameWindow.h" // self
...
void SEASON3B::CNewUIMainFrameWindow::SetButtonInfo()
{
  ...
  m_BtnCShop.ChangeToolTipText( GlobalText[2'277], true );
  ...
  m_BtnFriend.ChangeToolTipText( GlobalText[1'043], true );
  ...
  m_BtnWindow.ChangeToolTipText( GlobalText[1'744], true );
}
...
void SEASON3B::CNewUIMainFrameWindow::RenderGuageAG()
{
  ...
  RenderBitmap( IMAGE_GAUGE_AG, x, fY, width, fH, 0.f, fV * height / 64.f, width / 16.f,
                ( 1.0f - fV ) * height / 64.f );
...
void SEASON3B::CNewUIMainFrameWindow::RenderGuageSD()
{
  ...
  RenderBitmap( IMAGE_GAUGE_SD, x, fY, width, fH, 0.f, fV * height / 64.f, width / 16.f,
                ( 1.0f - fV ) * height / 64.f );
  ...
  if ( SEASON3B::CheckMouseIn( x, y, width, height ) == true ) {
    ...
    swprintf( strTipText, GlobalText[2'037], wShield, wMaxShield );
    RenderTipText( (int)x - 20, (int)418, strTipText );
  }
}
...
void SEASON3B::CNewUIMainFrameWindow::RenderExperience()
{
  ...
  if ( gCharacterManager.IsMasterLevel( CharacterAttribute->Class ) == true ) {
    ...
    if ( SEASON3B::CheckMouseIn( x, y, width, height ) == true ) {
      ...
      swprintf( strTipText, GlobalText[1'748], dwExperience, dwNexExperience );
    ...
    if ( wPriorLevel > 0 ) {
      ...
      if ( wPriorLevel > 255 ) {
        ...
        dwPriorExperience += ( 9 + iLevelOverN ) * iLevelOverN * iLevelOverN * 1'000;
      }
    }
...
{
  if ( m_bExpEffect == true ) {
    if ( timeGetTime() - m_dwExpEffectTime > 2'000 ) {
      m_bExpEffect      = false;
      m_dwExpEffectTime = 0;
      m_dwGetExp        = 0;
    }
  ...
  if ( iIndex != -1 ) {
    ...
    if ( ( pItem->Type >= ITEM_POTION + 78 && pItem->Type <= ITEM_POTION + 82 ) ) {
      ...
      if ( g_isCharacterBufflist( ( &Hero->Object ), secretPotionbufflist ) != eBuffNone ) {
        SEASON3B::CreateOkMessageBox( GlobalText[2'530], RGBA( 255, 30, 0, 255 ) );
      } else {
    ...
    if ( bItemCount ) {
      for ( j = 0; j < iNumberofItems; ++j ) {
        ...
        if ( ( pItem->Type == i && pItem->Level == m_iHotKeyItemLevel[iType] ) ||
          ...
          if ( pItem->Type == ITEM_ALE || pItem->Type == ITEM_TOWN_PORTAL_SCROLL || pItem->Type == ITEM_POTION + 20 ) {
            iItemCount++;
          } else {
