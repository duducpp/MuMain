    ...
    if ( m_Mini_Map_Data[i].Kind > 0 ) {
      ...
      if ( m_Mini_Map_Data[i].Kind == 1 ) // npc
      {
        if ( !( gMapManager.WorldActive == WD_34CRYWOLF_1ST && m_OccupationState > 0 ) ||
          ...
          RenderPointRotate( IMAGE_MINIMAP_INTERFACE + 5, Tx1, Ty1, NpcWidth, NpcWidth, m_Lenth[m_MiniPos].x - Tx,
                             m_Lenth[m_MiniPos].y - Ty, m_Lenth[m_MiniPos].x, m_Lenth[m_MiniPos].y, Rot, Rot_Loc,
                             17.5f / 32.f, 17.5f / 32.f, i );
        }
      } else if ( m_Mini_Map_Data[i].Kind == 2 ) {
        RenderPointRotate( IMAGE_MINIMAP_INTERFACE + 4, Tx1, Ty1, NpcWidthP, NpcWidthP, m_Lenth[m_MiniPos].x - Tx,
                           m_Lenth[m_MiniPos].y - Ty, m_Lenth[m_MiniPos].x, m_Lenth[m_MiniPos].y, Rot, Rot_Loc,
                           17.5f / 32.f, 17.5f / 32.f, 100 + i );
      }
    } else {
      break;
    }
...
void SEASON3B::CNewUIMiniMap::LoadImages( const wchar_t* Filename )
{
  ...
  swprintf( Fname, L"Data\\Local\\%ls\\Minimap\\Minimap_%ls_%ls.bmd", g_strSelectedML.c_str(), Filename,
