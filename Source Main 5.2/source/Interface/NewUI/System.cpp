...
bool CNewUISystem::LoadMainSceneInterface()
{
  ...
  if ( false ==
       m_pNewChatInputBox->Create( m_pNewUIMng, m_pNewChatLogWindow, m_pNewSystemLogWindow, 0, 480 - 51 - 47 ) ) {
    return false;
  }
...
bool CNewUISystem::IsImpossibleHideInterface( DWORD dwKey )
{
  if ( dwKey == INTERFACE_MAINFRAME || dwKey == INTERFACE_SKILL_LIST || dwKey == INTERFACE_SLIDEWINDOW ||
       ...
       dwKey == INTERFACE_PARTY_INFO_WINDOW || dwKey == INTERFACE_KANTURU_INFO || dwKey == INTERFACE_BLOODCASTLE_TIME ||
       dwKey == INTERFACE_CHAOSCASTLE_TIME || dwKey == INTERFACE_BATTLE_SOCCER_SCORE ||
       dwKey == INTERFACE_DUEL_WINDOW || dwKey == INTERFACE_CRYWOLF || dwKey == INTERFACE_HERO_POSITION_INFO ||
       dwKey == INTERFACE_NAME_WINDOW || dwKey == INTERFACE_SIEGEWARFARE || dwKey == INTERFACE_ITEM_TOOLTIP ||
       dwKey == INTERFACE_HOTKEY || dwKey == INTERFACE_CURSEDTEMPLE_GAMESYSTEM ||
       ...
       dwKey == INTERFACE_DOPPELGANGER_FRAME || dwKey == INTERFACE_GOLD_BOWMAN || dwKey == INTERFACE_GOLD_BOWMAN_LENA ||
