...
int CutStr( const wchar_t* pszSrcText, wchar_t* pTextOut, const int iTargetPixelWidth, const int iMaxOutLine,
...
int CutText3( const wchar_t* pszText, wchar_t* pTextOut, const int TargetWidth, const int iMaxOutLine,
              const int iOutStrLength, const int iFirstLineTab, const BOOL bReverseWrite )
{
...
BOOL CheckMouseIn( int iPos_x, int iPos_y, int iWidth, int iHeight, int CoordType )
{
  if ( CoordType == COORDINATE_TYPE_LEFT_DOWN ) {
    if ( MouseX >= iPos_x && MouseX < iPos_x + iWidth && MouseY >= iPos_y - iHeight && MouseY < iPos_y ) {
      return TRUE;
    } else {
      return FALSE;
    }
  } else {
    if ( MouseX >= iPos_x && MouseX < iPos_x + iWidth && MouseY >= iPos_y && MouseY < iPos_y + iHeight ) {
      return TRUE;
    } else {
      return FALSE;
    }
...
void CUIButton::Render()
{
  ...
  if ( GetState() == UISTATE_DISABLE ) {
    ...
    RenderBitmap( BITMAP_INTERFACE_EX + 9, m_iPos_x, m_iPos_y, (float)m_iWidth, (float)m_iHeight, 0.f, 0.f, 49.f / 64.f,
                  16.f / 16.f );
  ...
  if ( m_bMouseState == TRUE ) {
    RenderBitmap( BITMAP_INTERFACE_EX + 9, m_iPos_x + 1, m_iPos_y + 1, (float)m_iWidth - 1, (float)m_iHeight - 1, 0.f,
                  0.f, 48.f / 64.f, 15.f / 16.f );
  } else {
    RenderBitmap( BITMAP_INTERFACE_EX + 9, m_iPos_x, m_iPos_y, (float)m_iWidth, (float)m_iHeight, 0.f, 0.f, 49.f / 64.f,
                  16.f / 16.f );
  }
...
typename std::deque<T>::iterator CUITextListBox<T>::SLGetSelectLine()
{
  ...
  int iLineCount = 1;
  for ( typename std::deque<T>::iterator resultIter = m_TextList.begin(); resultIter != m_TextList.end();
        ++resultIter, ++iLineCount ) {
...
void CUITextListBox<T>::ComputeScrollBar()
{
  if ( m_bUseNewUIScrollBar == TRUE ) {
    ...
    if ( GetLineNum() - GetBoxSize() <= 0 ) {
      ...
      fPosRate = ( GetLineNum() > 0 ? (float)m_iCurrentRenderEndLine / ( GetLineNum() - GetBoxSize() ) : 1 );
    }
    ...
    m_fScrollBarPos_y =
        m_fScrollBarRange_bottom -
        ( ( m_fScrollBarRange_bottom - m_fScrollBarRange_top - ( m_fScrollBarHeight > 2 ? 0 : m_fScrollBarHeight ) ) *
          fPosRate ) -
        m_fScrollBarHeight;
  }
}
    ...
    if ( MouseLButtonPush && GetState() == UISTATE_NORMAL ) {
      ...
      if ( m_bUseNewUIScrollBar == TRUE ) {
        if ( GetLineNum() < m_iNumRenderLine )
          ;
        else if ( ::CheckMouseIn( m_iPos_x + m_iWidth - m_fScrollBarWidth, m_fScrollBarPos_y, m_fScrollBarWidth,
                                  m_fScrollBarHeight ) ) {
        ...
        } else if ( ::CheckMouseIn( m_iPos_x + m_iWidth - m_fScrollBarWidth, m_fScrollBarRange_top, m_fScrollBarWidth,
          ...
          if ( GetParentUIID() > 0 && g_pWindowMgr->IsRenderFrame() == FALSE )
            ...
            if ( GetParentUIID() == 0 || g_pWindowMgr->IsRenderFrame() == TRUE ) {
              m_bScrollBarClick++;
                                    ...
                                    m_fScrollBarWidth,
                                    m_fScrollBarRange_bottom - m_fScrollBarPos_y - m_fScrollBarHeight ) ) {
          if ( GetParentUIID() > 0 && g_pWindowMgr->IsRenderFrame() == FALSE )
            ;
          else if ( m_bScrollBarClick == FALSE ) {
        ...
        else if ( ::CheckMouseIn( m_iPos_x + m_iWidth - 19 + iNewTypePos_x, m_fScrollBarPos_y, m_fScrollBarWidth,
                                  m_fScrollBarHeight ) ) {
          if ( GetState() == UISTATE_NORMAL && g_dwActiveUIID == 0 ) {
            g_dwActiveUIID = GetUIID();
            SetState( UISTATE_SCROLL );
        ...
        else if ( ::CheckMouseIn( m_iPos_x + m_iWidth - 19 + iNewTypePos_x, m_fScrollBarRange_top, m_fScrollBarWidth,
                                  m_fScrollBarPos_y - m_fScrollBarRange_top ) ) {
          if ( GetParentUIID() > 0 && g_pWindowMgr->IsRenderFrame() == FALSE )
            ;
          else if ( m_bScrollBarClick == FALSE ) {
...
void CUIGuildListBox::RenderInterface()
{
  ...
  if ( GetLineNum() >= m_iNumRenderLine ) {
    RenderBitmap( BITMAP_INTERFACE_EX + 4, (float)m_iPos_x + m_iWidth - 19, (float)m_iPos_y - m_iHeight + 8, 13.0f,
                  13.0f, 0.0f, 0.0f, 13.0f / 16.0f, 13.0f / 16.0f );
    EnableAlphaTest();
    RenderBitmap( BITMAP_INTERFACE_EX + 4, (float)m_iPos_x + m_iWidth - 19, (float)m_iPos_y - 4, 13.0f, -13.0f, 0.0f,
                  0.0f, 13.0f / 16.0f, 13.0f / 16.0f );
    DisableAlphaBlend();

    RenderBitmap( BITMAP_INTERFACE_EX + 3, (float)m_iPos_x + m_iWidth - m_fScrollBarWidth - 6, m_fScrollBarRange_top,
                  m_fScrollBarWidth, m_fScrollBarRange_bottom - m_fScrollBarRange_top, 0.0f, 0.0f, 13.0f / 16.0f,
                  13.0f / 16.0f );
...
void CUISimpleChatListBox::RenderInterface()
{
  ...
  // if (GetLineNum() >= m_iNumRenderLine)
  {
    if ( MouseLButtonPush &&
      ...
      RenderBitmap( BITMAP_INTERFACE_EX + 12, (float)m_iPos_x + m_iWidth - 12, (float)m_iPos_y - m_iHeight - 1, 13.0f,
                    13.0f, 13.0f / 16.0f, 29.0f / 32.0f, -13.0f / 16.0f, -13.0f / 32.0f );
    } else {
      RenderBitmap( BITMAP_INTERFACE_EX + 12, (float)m_iPos_x + m_iWidth - 12, (float)m_iPos_y - m_iHeight - 1, 13.0f,
                    13.0f, 0.0f, 3.0f / 32.0f, 13.0f / 16.0f, 13.0f / 32.0f );
    }

    if ( MouseLButtonPush && ::CheckMouseIn( m_iPos_x + m_iWidth - 12, m_iPos_y - 12, 13.0f, 13.0f ) == TRUE ) {
      RenderBitmap( BITMAP_INTERFACE_EX + 12, (float)m_iPos_x + m_iWidth - 12, (float)m_iPos_y - 12, 13.0f, 13.0f,
                    13.0f / 16.0f, 16.0f / 32.0f, -13.0f / 16.0f, -13.0f / 32.0f );
    } else {
      RenderBitmap( BITMAP_INTERFACE_EX + 12, (float)m_iPos_x + m_iWidth - 12, (float)m_iPos_y - 12, 13.0f, 13.0f, 0.0f,
                    16.0f / 32.0f, 13.0f / 16.0f, 13.0f / 32.0f );
    }

    SetLineColor( 2 );
    RenderColor( (float)m_iPos_x + m_iWidth - m_fScrollBarWidth + 1, m_fScrollBarRange_top, (float)1,
    ...
    if ( GetLineNum() >= m_iNumRenderLine ) {
      ...
      RenderBitmap( BITMAP_INTERFACE_EX + 12, (float)m_iPos_x + m_iWidth - m_fScrollBarWidth + 2,
                    m_fScrollBarPos_y + m_fScrollBarHeight - 1, m_fScrollBarWidth - 2, 1, 0.0f, 2.0f / 32.0f,
      ...
      RenderBitmap( BITMAP_INTERFACE_EX + 12, (float)m_iPos_x + m_iWidth - m_fScrollBarWidth + 2, m_fScrollBarRange_top,
                    m_fScrollBarWidth - 2, m_fScrollBarRange_bottom - m_fScrollBarRange_top, 0.0f, 1.0f / 32.0f,
                    11.0f / 16.0f, 1.0f / 32.0f );
      RenderBitmap( BITMAP_INTERFACE_EX + 12, (float)m_iPos_x + m_iWidth - m_fScrollBarWidth + 2, m_fScrollBarRange_top,
                    m_fScrollBarWidth - 2, 1, 0.0f, 0.0f / 32.0f, 11.0f / 16.0f, 1.0f / 32.0f );
      RenderBitmap( BITMAP_INTERFACE_EX + 12, (float)m_iPos_x + m_iWidth - m_fScrollBarWidth + 2,
                    m_fScrollBarRange_top + m_fScrollBarHeight - 1, m_fScrollBarWidth - 2, 1, 0.0f, 2.0f / 32.0f,
                    11.0f / 16.0f, 1.0f / 32.0f );
    }
  }
}
...
void CUISimpleChatListBox::AddTextToRenderList( const wchar_t* pszID, const wchar_t* pszText, int iType, int iColor )
{
  ...
  if ( wcslen( pszID ) + wcslen( pszText ) >= 20 ) {
    ...
    int     iLine =
        CutText3( pszText, Text1[0], m_iWidth - 30, 10, MAX_TEXT_LENGTH + 1, ( nameSize.cx + 5 ) / g_fScreenRate_x );
...
CUIChatPalListBox::CUIChatPalListBox()
{
  ...
  GetTextExtentPoint32( g_pRenderText->GetFontDC(), GlobalText[1'022], GlobalText.GetStringSize( 1'022 ), &TextSize );
...
BOOL CUIChatPalListBox::RenderDataLine( int iLineNumber )
{
  ...
  if ( m_iLayoutType == 1 ) {
    ...
    //		}
      ...
      swprintf( Text, GlobalText[1'042], m_TextListIter->m_Server + 1 );
    }
...
CUILetterListBox::CUILetterListBox()
{
  ...
  GetTextExtentPoint32( g_pRenderText->GetFontDC(), GlobalText[1'028], GlobalText.GetStringSize( 1'028 ), &TextSize );
  SetColumnWidth( 1, TextSize.cx / g_fScreenRate_x + 8 );
  GetTextExtentPoint32( g_pRenderText->GetFontDC(), GlobalText[1'029], GlobalText.GetStringSize( 1'029 ), &TextSize );
  SetColumnWidth( 2, TextSize.cx / g_fScreenRate_x + 8 );
  GetTextExtentPoint32( g_pRenderText->GetFontDC(), GlobalText[1'030], GlobalText.GetStringSize( 1'030 ), &TextSize );
...
BOOL CUILetterListBox::RenderDataLine( int iLineNumber )
{
  ...
  if ( m_TextListIter->m_bIsRead == TRUE ) {
    RenderBitmap( BITMAP_INTERFACE_EX + 14, iPos_x + 1 + 10, iPos_y - 2, 13.0f, 10.0f, 0.f, 9.f / 32.f, 13.f / 16.f,
                  10.f / 32.f );
  } else {
    RenderBitmap( BITMAP_INTERFACE_EX + 14, iPos_x + 1 + 10, iPos_y - 1, 13.0f, 9.0f, 0.f, 0.f, 13.f / 16.f,
                  9.f / 32.f );
  }

  wcsncpy( Text, m_TextListIter->m_szID, MAX_TEXT_LENGTH );
  g_pRenderText->RenderText( iPos_x + 4 + GetColumnPos_x( 1 ), iPos_y, Text, GetColumnWidth( 1 ) - 4, 0,
                             RT3_SORT_LEFT_CLIP );
  ...
  g_pRenderText->RenderText( iPos_x + 4 + GetColumnPos_x( 3 ), iPos_y, m_TextListIter->m_szText, iMaxWidth, 0,
                             RT3_SORT_LEFT_CLIP );
  DisableAlphaBlend();
...
void CUIRenderText::RenderText( int iPos_x, int iPos_y, const wchar_t* pszText, int iBoxWidth /* = 0 */,
                                int iBoxHeight /* = 0 */, int iSort /* = RT3_SORT_LEFT */,
...
void CUIRenderTextOriginal::UploadText( int sx, int sy, int Width, int Height )
{
  ...
  if ( Width > 0 && Height > 0 && sx + Width > 0 && sy + Height > 0 ) {
    ...
    glTexImage2D( GL_TEXTURE_2D, 0, b->Components, (int)b->Width, (int)b->Height, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                  b->Buffer );

    float TextureUWidth  = ( Width + 0.01f ) / b->Width;
    float TextureVHeight = ( Height + 0.01f ) / b->Height;
    RenderBitmap( BITMAP_FONT, (float)sx, (float)sy, (float)Width, (float)Height, TextureU, TextureV, TextureUWidth,
                  TextureVHeight, false, false );
  }
}

/// \brief Renders the text with GDI to the location of m_hFontDC/m_pFontBuffer as black/white picture. Text is white.
void CUIRenderTextOriginal::RenderText( int iPos_x, int iPos_y, const wchar_t* pszText, int iBoxWidth /* = 0 */,
                                        int iBoxHeight /* = 0 */, int iSort /* = RT3_SORT_LEFT */,
                                        OUT SIZE* lpTextSize /* = NULL */ )
{
  if ( pszText == nullptr || ( pszText[0] == '\0' && iBoxWidth == 0 ) ) {
    return;
  }
  if ( wcslen( pszText ) <= 0 && iBoxWidth == 0 ) {
    return;
  }
  ...
  if ( m_dwBackColor != 0 ) {
    ...
    RenderColor( RealBoxPos.x / g_fScreenRate_x, RealBoxPos.y / g_fScreenRate_y, RealBoxSize.cx / g_fScreenRate_x,
                 RealBoxSize.cy / g_fScreenRate_y );
    EndRenderColor();
  }
...
LRESULT CALLBACK EditWndProc( HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam )
{
  ...
  if ( HIBYTE( GetAsyncKeyState( VK_UP ) ) == 128 || HIBYTE( GetAsyncKeyState( VK_DOWN ) ) == 128 ||
       HIBYTE( GetAsyncKeyState( VK_LEFT ) ) == 128 || HIBYTE( GetAsyncKeyState( VK_RIGHT ) ) == 128 ) {
  ...
  switch ( msg ) {
    ...
    case WM_CHAR:
      ...
      switch ( wParam ) {
        ...
        case VK_RETURN:
          ...
          break;
        case VK_TAB:
          if ( pTextInputBox->GetTabTarget() != nullptr &&
        ...
        default: {
          ...
          if ( pTextInputBox->CheckOption( UIOPTION_NUMBERONLY ) ) {
            if ( wParam == VK_BACK )
              ;
            else if ( wParam == 0x03 || wParam == 0x18 )
              ; // Ctrl+C, Ctrl+X
    ...
    case WM_IME_NOTIFY:
      ...
      if ( msg == WM_IME_NOTIFY && ( pTextInputBox->CheckOption( UIOPTION_SERIALNUMBER ) ||
                                     pTextInputBox->CheckOption( UIOPTION_NUMBERONLY ) ) ) {
        switch ( wParam ) {
...
{
  if ( m_bIsReady == FALSE || m_hEditWnd == nullptr ) {
    return;
  }
...
void CUITextInputBox::SetSize( int iWidth, int iHeight )
{
  ...
  if ( m_hEditWnd != nullptr ) {
    SetWindowPos( m_hEditWnd, nullptr, 0, 0, iWidth * g_fScreenRate_x, iHeight * g_fScreenRate_y,
...
void CUITextInputBox::Init( HWND hWnd, int iWidth, int iHeight, int iMaxLength, BOOL bIsPassword )
{
  ...
  m_hEditWnd = CreateWindowW( L"edit", NULL, WS_CHILD | WS_VISIBLE | dwOptionFlag, m_iRealWindowPos_x,
                              m_iRealWindowPos_y, iWidth * g_fScreenRate_x, iHeight * g_fScreenRate_y, m_hParentWnd,
                              (HMENU)ID_UICEDIT, g_hInst, NULL );

  SetSize( iWidth, iHeight );
  if ( m_hEditWnd ) {
    SetTextLimit( iMaxLength );
    m_hOldProc = reinterpret_cast<WNDPROC>(
        SetWindowLongPtrW( m_hEditWnd, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>( EditWndProc ) ) );
...
void CUITextInputBox::GiveFocus( BOOL SelectText )
{
  ...
  if ( g_iChatInputType == 1 && GetFocus() == g_hWnd && !CheckOption( UIOPTION_SERIALNUMBER ) &&
       !CheckOption( UIOPTION_NUMBERONLY ) ) {
    SetFocus( m_hEditWnd );
    RestoreIMEStatus();
  } else {
    SetFocus( m_hEditWnd );
...
void CUITextInputBox::WriteText( int iOffset, int iWidth, int iHeight )
{
  ...
  RECT       rcCaret    = { pt.x - LIMIT_WIDTH * iSectionX, pt.y - LIMIT_HEIGHT * iSectionY,
                            (int)( pt.x - LIMIT_WIDTH * iSectionX + m_fCaretWidth ),
                            (int)( pt.y - LIMIT_HEIGHT * iSectionY + m_fCaretHeight ) };
  ...
  if ( !m_bUseMultiLine ) {
    ...
    for ( int y = 0; y < iNumberOfYSections; y++ ) {
      ...
      for ( int x = 0; x < iNumberOfXSections; x++ ) {
        ...
        UploadText( RealWndPos.x + LIMIT_WIDTH * x, RealWndPos.y + LIMIT_HEIGHT * y, RealSectionLine.cx,
                    RealSectionLine.cy );
      }
    }
#ifdef PBG_ADD_INGAMESHOPMSGBOX
...
void CUITextInputBox::RenderScrollbar()
{
  ...
  m_fScrollBarPos_y =
      m_fScrollBarRange_top + ( m_fScrollBarRange_bottom - m_fScrollBarRange_top ) * ( fPosRate > 1 ? 1 : fPosRate );
  ...
  // if (GetLineNum() >= m_iNumRenderLine)
  {
    ...
    if ( MouseLButtonPush &&
         ::CheckMouseIn( m_iPos_x + m_iWidth - m_fScrollBarWidth, m_iPos_y + m_iHeight - 9, 13.0f, 13.0f ) == TRUE ) {
      RenderBitmap( BITMAP_INTERFACE_EX + 12, (float)m_iPos_x + m_iWidth - m_fScrollBarWidth,
                    (float)m_iPos_y + m_iHeight - 9, 13.0f, 13.0f, 13.0f / 16.0f, 16.0f / 32.0f, -13.0f / 16.0f,
                    -13.0f / 32.0f );
    } else {
      RenderBitmap( BITMAP_INTERFACE_EX + 12, (float)m_iPos_x + m_iWidth - m_fScrollBarWidth,
                    (float)m_iPos_y + m_iHeight - 9, 13.0f, 13.0f, 0.0f, 16.0f / 32.0f, 13.0f / 16.0f,
                    13.0f / 32.0f ); // ▼
    }
    ...
    RenderBitmap( BITMAP_INTERFACE_EX + 12, (float)m_iPos_x + m_iWidth - m_fScrollBarWidth + 1,
                  m_fScrollBarPos_y + m_fScrollBarHeight, m_fScrollBarWidth - 2, 1, 0.0f, 2.0f / 32.0f, 11.0f / 16.0f,
                  1.0f / 32.0f );
  }
}
...
BOOL CUITextInputBox::DoMouseAction()
{
  ...
  if ( ::CheckMouseIn( m_iPos_x, m_iPos_y - 4, m_iWidth, m_iHeight + 8 ) == TRUE ) {
    if ( MouseLButtonPush && GetState() == UISTATE_NORMAL ) {
      ...
      if ( SendMessageW( m_hEditWnd, EM_GETLINECOUNT, 0, 0 ) < m_iNumLines )
        ;
      else if ( ::CheckMouseIn( m_iPos_x + m_iWidth - 14, m_fScrollBarPos_y, m_fScrollBarWidth, m_fScrollBarHeight ) ) {
        if ( GetState() == UISTATE_NORMAL && g_dwActiveUIID == 0 ) {
          g_dwActiveUIID = GetUIID();
          SetState( UISTATE_SCROLL );
          m_fScrollBarClickPos_y = MouseY - m_fScrollBarPos_y;
        }
      } else if ( ::CheckMouseIn( m_iPos_x + m_iWidth - 14, m_fScrollBarRange_top, m_fScrollBarWidth,
                                  m_fScrollBarPos_y - m_fScrollBarRange_top ) ) {
        if ( GetParentUIID() > 0 && g_pWindowMgr->IsRenderFrame() == FALSE )
          ;
        else if ( m_bScrollBarClick == FALSE ) {
      ...
      } else if ( ::CheckMouseIn( m_iPos_x + m_iWidth - 14, m_fScrollBarPos_y + m_fScrollBarHeight, m_fScrollBarWidth,
                                  m_fScrollBarRange_bottom - m_fScrollBarPos_y - m_fScrollBarHeight ) ) {
        if ( GetParentUIID() > 0 && g_pWindowMgr->IsRenderFrame() == FALSE )
          ;
        else if ( m_bScrollBarClick == FALSE ) {
  ...
  if ( GetState() == UISTATE_SCROLL ) {
    if ( MouseLButtonPush ) {
      ...
      SendMessageW( m_hEditWnd, EM_LINESCROLL, 0, -10'000 );
...
{
  if ( pszText == nullptr || pszText[0] == NULL ) {
    return;
  }
    ...
    for ( int i = 0; i < iDegree; ++i ) {
      if ( m_CurrentHistoryLine != m_HistoryList.begin() ) {
        ...
        if ( m_bHistoryMode == TRUE ) {
          ...
          SendMessage( m_TextInputBox.GetHandle(), EM_SETSEL, (WPARAM)10'000, (LPARAM)-1 );
...
{
  if ( pszNewText == nullptr || pszNewText[0] == '\0' ) {
    return TRUE;
  }
...
{
  if ( pszSource == nullptr || pszSource[0] == L'\0' ) {
    return 0;
  }
...
void CUISlideHelp::AddSlide( int iLoopCount, int iLoopDelay, const wchar_t* pszText, int iType, float fSpeed,
                             DWORD dwTextColor )
{
...
void CUISlideHelp::CheckTime()
{
  ...
  if ( dwCheckTime > m_dwTimer + 1'000 ) {
    m_dwTimer = timeGetTime();
    ++m_dwCurrentSecond;
  }
}
  ...
  for ( m_SlideQueueIter = m_SlideQueue.begin(); m_SlideQueueIter != m_SlideQueue.end(); ++m_SlideQueueIter ) {
    if ( m_SlideQueueIter->first > m_dwCurrentSecond ) {
      ...
      if ( m_SlideQueueIter->second.m_iType == -1 && m_SlideQueueIter->first + 60 < m_dwCurrentSecond )
        ;
      else if ( AddSlideText( m_SlideQueueIter->second.m_pszText, m_SlideQueueIter->second.m_dwTextColor ) == FALSE ) {
        break;
      }
...
void CSlideHelpMgr::Init()
{
  ...
  SetTimer( g_hWnd, SLIDEHELP_TIMER, m_iCreateDelay * 1'000, nullptr );
  CreateSlideText();
}
...
void CSlideHelpMgr::ClearSlideText()
{
  for ( int i = 0; i < SLIDE_LEVEL_MAX; ++i ) {
    m_iLevelCap[i]   = 0;
    m_iTextNumber[i] = 0;
    for ( m_SlideTextListIter = m_SlideTextList[i].begin(); m_SlideTextListIter != m_SlideTextList[i].end();
          ++m_SlideTextListIter ) {
...
void CSlideHelpMgr::AddSlide( int iLoopCount, int iLoopDelay, const wchar_t* pszText, int iType, float fSpeed,
                              DWORD dwTextColor )
{
  if ( SceneFlag != MAIN_SCENE ) {
    return;
...
void CUINewGuildMemberListBox::RenderInterface()
{
  ...
  g_pRenderText->RenderText( m_iPos_x + 14, m_iPos_y - m_iHeight - 12, GlobalText[1'389] );
  g_pRenderText->RenderText( m_iPos_x + 65, m_iPos_y - m_iHeight - 12, GlobalText[1'307] );
  g_pRenderText->RenderText( m_iPos_x + 106, m_iPos_y - m_iHeight - 12, GlobalText[1'022] );
}
...
BOOL CUINewGuildMemberListBox::RenderDataLine( int iLineNumber )
{
  ...
  if ( iCharacterLevel == 0 ) {
    g_pRenderText->RenderText( iPos_x + 45, iPos_y, GlobalText[1'300], 70, 0, RT3_SORT_CENTER );
  } else if ( iCharacterLevel == 1 ) {
    g_pRenderText->RenderText( iPos_x + 45, iPos_y, GlobalText[1'301], 70, 0, RT3_SORT_CENTER );
  } else if ( iCharacterLevel == 2 ) {
    g_pRenderText->RenderText( iPos_x + 45, iPos_y, GlobalText[1'302], 70, 0, RT3_SORT_CENTER );
  }
...
void CUIUnionGuildListBox::RenderInterface()
{
  ...
  g_pRenderText->RenderText( m_iPos_x + 113, m_iPos_y - m_iHeight - 12, GlobalText[1'330] );
}
...
{
  if ( iIndex < 0 || iIndex > MAX_INVENTORY || cComType == COMGEM::NOCOM ) {
    return;
  }
...
BOOL CUIUnmixgemList::RenderDataLine( int iLineNumber )
{
  ...
  if ( pItem ) {
    ...
    swprintf( oText, L"%ls,  %d", GlobalText[COMGEM::GetJewelIndex( nIdx, COMGEM::eGEM_NAME )],
...
void CUIBCDeclareGuildListBox::RenderInterface()
{
  ...
  g_pRenderText->RenderText( m_iPos_x + 50, m_iPos_y - m_iHeight - 12, GlobalText[1'528] );
  g_pRenderText->RenderText( m_iPos_x + 98, m_iPos_y - m_iHeight - 12, GlobalText[1'529] );
  g_pRenderText->RenderText( m_iPos_x + 123, m_iPos_y - m_iHeight - 12, GlobalText[1'530] );
}
...
BOOL CUIBCDeclareGuildListBox::RenderDataLine( int iLineNumber )
{
  ...
  if ( ( wcscmp( GuildMark[Hero->GuildMarkIndex].UnionName, Text ) != NULL &&
         wcscmp( GuildMark[Hero->GuildMarkIndex].GuildName, Text ) != NULL ) ) {
    return FALSE;
  }
  ...
  if ( m_TextListIter->byIsGiveUp ) {
    swprintf( Text, L"%ls", GlobalText[1'531] );
  } else {
    swprintf( Text, L"%ls", GlobalText[1'532] );
  }
...
void CUIBCGuildListBox::AddText( const wchar_t* szGuildName, BYTE byJoinSide, BYTE byGuildInvolved, int iGuildScore )
{
  if ( szGuildName == nullptr || szGuildName[0] == '\0' ) {
    return;
  }
...
void CUIBCGuildListBox::RenderInterface()
{
  ...
  g_pGuardWindow->RenderScrollBar( m_iPos_x + m_iWidth - 12, m_fScrollBarPos_y,
                                   ( GetState() == UISTATE_SCROLL && MouseLButtonPush ) );
  g_pRenderText->RenderText( m_iPos_x + 5, m_iPos_y - m_iHeight - 12, GlobalText[182] );
  g_pRenderText->RenderText( m_iPos_x + 80, m_iPos_y - m_iHeight - 12, GlobalText[1'603] );
  g_pRenderText->RenderText( m_iPos_x + 120, m_iPos_y - m_iHeight - 12, GlobalText[1'604] );
  g_pRenderText->RenderText( m_iPos_x + 18, m_iPos_y + 31 - 1, GlobalText[1'977] );
}
...
BOOL CUIBCGuildListBox::RenderDataLine( int iLineNumber )
{
  ...
  if ( m_TextListIter->byJoinSide == 1 ) {
    swprintf( Text, L"%ls", GlobalText[1'606] );
  } else {
    swprintf( Text, L"%ls", GlobalText[1'605] );
  }
  g_pRenderText->RenderText( iPos_x + 100, iPos_y, Text, 0, 0, RT3_WRITE_RIGHT_TO_LEFT );

  if ( m_TextListIter->byGuildInvolved == 1 ) {
    swprintf( Text, L"%ls", GlobalText[1'607] );
  } else {
    swprintf( Text, L"%ls", GlobalText[1'608] );
  }
...
void RenderGoldRect( float fPos_x, float fPos_y, float fWidth, float fHeight, int iFillType = 0 )
{
  ...
  RenderBitmap( BITMAP_INVENTORY + 19, fPos_x, fPos_y + fHeight, fWidth + 1, 2, 10 / 256.f, 5 / 16.f, 170.f / 256.f,
                2.f / 16.f );
  RenderBitmap( BITMAP_INVENTORY, fPos_x, fPos_y, 2, fHeight, 1.f / 256.f, 5 / 16.f, 2.f / 256.f, 125.f / 256.f );
  RenderBitmap( BITMAP_INVENTORY, fPos_x + fWidth, fPos_y, 2, fHeight, 1.f / 256.f, 5 / 16.f, 2.f / 256.f,
                125.f / 256.f );
}
...
void CUIMoveCommandListBox::AddText( int iIndex, const wchar_t* szMapName, const wchar_t* szSubMapName, int iReqLevel,
                                     int iReqZen, int iGateNum )
{
...
void CUIQuestContentsListBox::RenderCoveredInterface()
{
  ...
  // 아이템인가?
  if ( QUEST_REQUEST_ITEM == m_TextListIter->m_dwType || QUEST_REWARD_ITEM == m_TextListIter->m_dwType ) {
...
{
  if ( GetLineNum() > m_iNumRenderLine ) {
    return ( m_iPos_y - m_iHeight + ( m_iNumRenderLine - 1 ) * TEXT_HEIGHTSIZE - nLine * TEXT_HEIGHTSIZE +
             TEXT_HEIGHTSIZE * 0.1f );
  } else {
    return ( m_iPos_y - m_iHeight + ( GetLineNum() - 1 ) * TEXT_HEIGHTSIZE - nLine * TEXT_HEIGHTSIZE +
             TEXT_HEIGHTSIZE * 0.1f );
  }
}
...
void CRadioButton::RadiobuttonBoxRender()
{
  switch ( m_byMouseState ) {
    case LBTN_DEFAULT:
      ...
      break;
    case LBTN_UP:
      m_bCheckState ? RenderImage( IMAGE_CHECKBTN, m_rtCheckBtn.left, m_rtCheckBtn.top, BTN_WIDTH, BTN_HEIGHT, 0,
                                   BTN_HEIGHT + BTN_SPACE )
                    : RenderImage( IMAGE_CHECKBTN, m_rtCheckBtn.left, m_rtCheckBtn.top, BTN_WIDTH, BTN_HEIGHT, 0, 0 );
      break;
    case LBTN_DOWN:
      RenderImage( IMAGE_CHECKBTN, m_rtCheckBtn.left, m_rtCheckBtn.top, BTN_WIDTH, BTN_HEIGHT, 0,
                   ( BTN_HEIGHT + BTN_SPACE ) * 2.0f );
      break;
  }
}
