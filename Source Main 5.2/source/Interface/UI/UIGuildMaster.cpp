...
#include "Interface/UI/UIGuildMaster.h"
...
CUIGuildMaster::CUIGuildMaster()
{
  ...
  m_NextButton.Init( 5, GlobalText[1'305] );
...
void CUIGuildMaster::DoGuildMasterMainAction()
{
  ...
  if ( m_EditGuildMarkButton.DoMouseAction() ) {
    wchar_t szText[50];
    wcscpy( szText, GlobalText[1'269] );
...
void CUIGuildMaster::RenderGuildMasterMain()
{
  ...
  if ( CheckMouseIn( x, y, Width, Height ) ) {
    ...
    RenderTipText( x, y - 13, GlobalText[1'002] );
  }
}
                                               ...
                                               BYTE byTargetUserIndexL )
{
  ...
  if ( m_byRelationShipType == 0x01 ) // Union
  {
    ...
    {
      swprintf( szText[0], GlobalText[1'280] );
      swprintf( szText[1], GlobalText[1'281], pPlayer->ID );
      swprintf( szText[2], GlobalText[1'283] );
    } else // Break Off
    {
      swprintf( szText[0], GlobalText[1'280] );
      swprintf( szText[1], GlobalText[1'282], pPlayer->ID );
      swprintf( szText[2], GlobalText[1'283] );
    }
...
void CUIGuildMaster::Render()
{
  ...
  switch ( m_eCurrStep ) {
    case STEP_MAIN:
      ...
      break;
    case STEP_CREATE_GUILDINFO:
      g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, GlobalText[1'303], 120, 0, RT3_SORT_CENTER );
      RenderCreateGuild();
      break;
    case STEP_EDIT_GUILD_MARK:
      g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, GlobalText[1'304], 120, 0, RT3_SORT_CENTER );
      RenderEditGuildMark();
      break;
    case STEP_CONFIRM_GUILDINFO:
