...
void CUIPopup::Init()
{
  ...
  ZeroMemory( m_szInputText, sizeof( char ) * 1'024 );
}
...
wchar_t* CUIPopup::GetInputText()
{
  if ( g_iChatInputType == 1 ) {
    g_pSingleTextInputBox->GetText( m_szInputText, 1'024 );
  } else {
    if ( InputText[0] ) {
...
void CUIPopup::Render()
{
  ...
  if ( PopupRenderFuncPointer ) {
    ...
    if ( m_PopupType & POPUP_INPUT ) {
      RenderBitmap( BITMAP_INVENTORY + 11, 320 - m_nInputSize / 2, fPosY, m_nInputSize, 18, 0.f, 0.f, 113.f / 128.f,
                    18.f / 32.f );
