    ...
    if ( iNameLine == 1 ) {
      switch ( g_MixRecipeMgr.GetMixInventoryType() ) {
        case MIXTYPE_TRAINER:
          swprintf( pszNameOut, GlobalText[1'213], GlobalText[1'212] );
          break;
        case MIXTYPE_OSBOURNE:
          swprintf( pszNameOut, GlobalText[1'213], GlobalText[2'061] );
          break;
        case MIXTYPE_JERRIDON:
          swprintf( pszNameOut, GlobalText[1'213], GlobalText[2'062] );
          break;
        case MIXTYPE_ELPIS:
          swprintf( pszNameOut, GlobalText[1'213], GlobalText[2'063] );
          break;
        default:
...
BOOL CMixRecipes::GetRecipeName( MIX_RECIPE* pRecipe, wchar_t* pszNameOut, int iNameLine, BOOL bSimilarRecipe )
{
  ...
  if ( pRecipe->m_bMixOption == 'C' ) {
    ...
    if ( optionTextlist.empty() || bSimilarRecipe ) {
      if ( iNameLine == 1 ) {
        swprintf( pszNameOut, L"%ls", GlobalText[2'194] );
        return TRUE;
      }
      return FALSE;
    }
    ...
    if ( iNameLine == 1 ) {
      if ( pRecipe->m_iMixName[1] == 0 ) {
        ...
        swprintf( pszNameOut, L"%ls %ls %ls", GlobalText[pRecipe->m_iMixName[0]], GlobalText[pRecipe->m_iMixName[1]],
                  GlobalText[pRecipe->m_iMixName[2]] );
      }
      return TRUE;
    }
    return FALSE;
...
int CMixRecipes::GetSourceName( int iItemNum, wchar_t* pszNameOut, int iNumMixItems, CMixItem* pMixItems )
{
  ...
  if ( pMixRecipeItem->m_sTypeMin == pMixRecipeItem->m_sTypeMax &&
    ...
    if ( pMixRecipeItem->m_iLevelMin == 0 && pMixRecipeItem->m_iLevelMax == 255 )
      ...
      swprintf( szTempName, L"%ls +%d%ls", szTempName, pMixRecipeItem->m_iLevelMax, GlobalText[2'342] );
    } else if ( pMixRecipeItem->m_iLevelMax == 255 ) {
      swprintf( szTempName, L"%ls +%d%ls", szTempName, pMixRecipeItem->m_iLevelMin, GlobalText[2'341] );
    } else {
    ...
    if ( pMixRecipeItem->m_iOptionMin == 0 && pMixRecipeItem->m_iOptionMax == 255 )
      ...
      swprintf( szTempName, L"%ls +%d%ls", szTempName, pMixRecipeItem->m_iOptionMin, GlobalText[2'343] );
    } else if ( pMixRecipeItem->m_iOptionMin == 0 ) {
      swprintf( szTempName, L"%ls +%d%ls%ls", szTempName, pMixRecipeItem->m_iOptionMax, GlobalText[2'343],
                GlobalText[2'342] );
    } else if ( pMixRecipeItem->m_iOptionMax == 255 ) {
      swprintf( szTempName, L"%ls +%d%ls%ls", szTempName, pMixRecipeItem->m_iOptionMin, GlobalText[2'343],
                GlobalText[2'341] );
    } else {
      swprintf( szTempName, L"%ls +%d~%d%ls", szTempName, pMixRecipeItem->m_iOptionMin, pMixRecipeItem->m_iOptionMax,
                GlobalText[2'343] );
    }
  }

  if ( pMixRecipeItem->m_iCountMin == 0 && pMixRecipeItem->m_iCountMax == 255 ) {
    swprintf( szTempName, L"%ls (%ls)", szTempName, GlobalText[2'344] );
  } else if ( pMixRecipeItem->m_iCountMin == pMixRecipeItem->m_iCountMax ) {
    swprintf( szTempName, L"%ls %d%ls", szTempName, pMixRecipeItem->m_iCountMin, GlobalText[2'345] );
  } else if ( pMixRecipeItem->m_iCountMin == 0 ) {
    swprintf( szTempName, L"%ls %d%ls %ls", szTempName, pMixRecipeItem->m_iCountMax, GlobalText[2'345],
              GlobalText[2'342] );
  } else if ( pMixRecipeItem->m_iCountMax == 255 ) {
    swprintf( szTempName, L"%ls %d%ls %ls", szTempName, pMixRecipeItem->m_iCountMin, GlobalText[2'345],
              GlobalText[2'341] );
  } else {
    swprintf( szTempName, L"%ls %d~%d%ls", szTempName, pMixRecipeItem->m_iCountMin, pMixRecipeItem->m_iCountMax,
              GlobalText[2'345] );
  }
  ...
  if ( pMixRecipeItem->m_dwSpecialItem & RCP_SP_SETITEM ) {
    swprintf( pszNameOut, L"%ls %ls", GlobalText[1'089], szTempName );
    bPreName = TRUE;
  }
  if ( pMixRecipeItem->m_dwSpecialItem & RCP_SP_HARMONY ) {
    swprintf( pszNameOut, L"%ls %ls", GlobalText[1'550], szTempName );
    bPreName = TRUE;
  }
  if ( pMixRecipeItem->m_dwSpecialItem & RCP_SP_SOCKETITEM ) {
    swprintf( pszNameOut, L"%ls %ls", GlobalText[2'650], szTempName );
    bPreName = TRUE;
  }
  if ( bPreName == FALSE ) {
...
void CMixRecipeMgr::OpenRecipeFile( const wchar_t* szFileName )
{
  ...
  for ( j = 0; j < MAX_MIX_TYPES; ++j ) {
    if ( feof( fp ) || iNumMixRecipes[j] > 1'000 ) {
      wchar_t Text[256];
