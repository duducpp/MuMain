...
void CMsgWin::PopUp( int nMsgCode, wchar_t* pszMsg )
{
  ...
  switch ( m_nMsgCode ) {
    ...
    case MESSAGE_DELETE_CHARACTER_GUILDWARNING:
      lpszMsg = GlobalText[1'654];
      break;
    case MESSAGE_DELETE_CHARACTER_WARNING:
      swprintf( szTempMsg, GlobalText[1'711], CHAR_DEL_LIMIT_LV );
      lpszMsg = szTempMsg;
      break;
    case MESSAGE_DELETE_CHARACTER_CONFIRM:
      swprintf( szTempMsg, GlobalText[1'712], CharactersClient[SelectedHero].ID );
      lpszMsg = szTempMsg;
      eType   = MWT_BTN_BOTH;
      break;
    case MESSAGE_DELETE_CHARACTER_RESIDENT:
      lpszMsg = GlobalText[1'713];
      eType   = MWT_STR_INPUT;
