...
void CNewChaosCastleSystem::RenderMatchResult( void )
{
  ...
  if ( m_iNumResult ) {
    g_pRenderText->RenderText( x, yPos, GlobalText[1'151], 0, 0, RT3_WRITE_CENTER );
    yPos += 16;
    swprintf( lpszStr, L"%ls %ls", GlobalText[1'152], GlobalText[858] );
    g_pRenderText->RenderText( x, yPos, lpszStr, 0, 0, RT3_WRITE_CENTER );
  } else {
    swprintf( lpszStr, L"%ls %ls", GlobalText[1'152], GlobalText[860] );
    g_pRenderText->RenderText( x, yPos, lpszStr, 0, 0, RT3_WRITE_CENTER );
    yPos += 16;
    g_pRenderText->RenderText( x, yPos, GlobalText[1'153], 0, 0, RT3_WRITE_CENTER );
  }
  yPos += 30;
  ...
  swprintf( lpszStr, GlobalText[1'162], pResult->m_iScore );
  g_pRenderText->RenderText( x, yPos, lpszStr, 0, 0, RT3_WRITE_CENTER );
  yPos += 20;

  swprintf( lpszStr, GlobalText[1'163], pResult->m_iZen );
  g_pRenderText->RenderText( x, yPos, lpszStr, 0, 0, RT3_WRITE_CENTER );
  yPos += 24;
