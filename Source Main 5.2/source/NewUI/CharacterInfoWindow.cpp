...
#include "stdafx.h"
#include "NewUI/CharacterInfoWindow.h"
...
void SEASON3B::CNewUICharacterInfoWindow::SetButtonInfo()
{
  ...
  swprintf( strText, L"%ls(%ls)", GlobalText[1'140], L"T" );
  ...
  m_BtnPet.ChangeToolTipText( GlobalText[1'217], true );
  ...
  m_BtnMasterLevel.ChangeToolTipText( GlobalText[1'749], true );
}
...
void SEASON3B::CNewUICharacterInfoWindow::RenderSubjectTexts()
{
  ...
  const wchar_t* apszGlobalText[4] = { GlobalText[461], GlobalText[460], GlobalText[3'130], GlobalText[3'131] };
...
void SEASON3B::CNewUICharacterInfoWindow::RenderTableTexts()
{
  ...
  if ( CharacterAttribute->Level > 9 ) {
    ...
    swprintf( strPoint, L"%ls %d/%d | %ls %d/%d", GlobalText[1'412], CharacterAttribute->AddPoint,
              CharacterAttribute->MaxAddPoint, GlobalText[1'903], iMinus, iMaxMinus );
  } else {
    swprintf( strPoint, L"%ls %d/%d | %ls %d/%d", GlobalText[1'412], 0, 0, GlobalText[1'903], 0, 0 );
  }
  ...
  swprintf( strPointProbability, GlobalText[1'907], iAddPoint, iMinusPoint );
  g_pRenderText->SetFont( g_hFont );
...
void SEASON3B::CNewUICharacterInfoWindow::RenderAttribute()
{
  ...
  if ( iAttackRating > 0 ) {
    if ( iAttackDamageMin + iMinIndex >= iAttackDamageMax + iMaxIndex ) {
      swprintf( strAttakMamage, GlobalText[203], iAttackDamageMax + iMaxIndex, iAttackDamageMax + iMaxIndex,
                iAttackRating );
    } else {
      swprintf( strAttakMamage, GlobalText[203], iAttackDamageMin + iMinIndex, iAttackDamageMax + iMaxIndex,
                iAttackRating );
    }
  } else {
    if ( iAttackDamageMin + iMinIndex >= iAttackDamageMax + iMaxIndex ) {
  ...
  if ( iAttackRatingPK > 0 ) {
    if ( itemoption380Attack != 0 || iAttackPowerRate != 0 ) {
      swprintf( strAttakMamage, GlobalText[2'109], iAttackRatingPK, itemoption380Attack + iAttackPowerRate );
    } else {
      swprintf( strAttakMamage, GlobalText[2'044], iAttackRatingPK );
    }
  ...
  if ( bDexSuccess ) {
    ...
    if ( CharacterAttribute->SuccessfulBlocking > 0 ) {
      ...
      swprintf( strBlocking, GlobalText[207], t_adjdef + maxdefense + iChangeRingAddDefense,
    ...
    if ( CharacterAttribute->SuccessfulBlocking > 0 ) {
      if ( nAdd_FulBlocking ) {
        ...
        swprintf( strBlocking, GlobalText[208], t_adjdef + maxdefense + iChangeRingAddDefense,
                  CharacterAttribute->SuccessfulBlocking );
      }
    } else {
      // 209
      swprintf( strBlocking, GlobalText[209], t_adjdef + maxdefense + iChangeRingAddDefense );
    }
  }
  ...
  if ( itemoption380Defense != 0 || iDefenseRate != 0 ) {
    swprintf( strBlocking, GlobalText[2'110], CharacterAttribute->SuccessfulBlockingPK + add_defense_success_rate_pvp,
              itemoption380Defense + iDefenseRate );
  } else {
    swprintf( strBlocking, GlobalText[2'045], CharacterAttribute->SuccessfulBlockingPK + add_defense_success_rate_pvp );
  }
  ...
  if ( iBaseClass == CLASS_RAGEFIGHTER ) {
    iY += 13;
    // 물리공격력
    swprintf( strVitality, GlobalText[3'155], 50 + ( wVitality / 10 ) );
    g_pRenderText->RenderText( m_Pos.x + 20, m_Pos.y + iY, strVitality );
  }
  ...
  if ( iBaseClass == CLASS_WIZARD || iBaseClass == CLASS_DARK || iBaseClass == CLASS_SUMMONER ) {
    ...
    if ( ( pWeaponRight->Type >= MODEL_STAFF - MODEL_ITEM &&
      ...
      swprintf( strEnergy, GlobalText[215], iMagicDamageMin + maxMg, iMagicDamageMax + maxMg,
  ...
  if ( iBaseClass == CLASS_SUMMONER ) {
    ...
    if ( ITEM_BOOK_OF_SAHAMUTT <= pWeaponLeft->Type && pWeaponLeft->Type <= ITEM_STAFF + 29 ) {
      ...
      swprintf( strEnergy, GlobalText[1'693], iCurseDamageMin, iCurseDamageMax,
                (int)( (iCurseDamageMax)*fCursePercent ) );
    } else {
      swprintf( strEnergy, GlobalText[1'694], iCurseDamageMin, iCurseDamageMax );
    }
  ...
  if ( iBaseClass == CLASS_RAGEFIGHTER ) {
    // 마법공격력
    swprintf( strEnergy, GlobalText[3'156], 50 + ( wEnergy / 10 ) );
    g_pRenderText->RenderText( m_Pos.x + 20, m_Pos.y + iY, strEnergy );
    iY += 13;
    // 범위공격력
    swprintf( strEnergy, GlobalText[3'157], 100 + ( wDexterity / 8 + wEnergy / 10 ) );
    g_pRenderText->RenderText( m_Pos.x + 20, m_Pos.y + iY, strEnergy );
  }
  ...
  if ( iBaseClass == CLASS_DARK_LORD ) {
    ...
    g_pRenderText->RenderText( m_Pos.x + 12, m_Pos.y + HEIGHT_CHARISMA + 6, GlobalText[1'900], 74, 0, RT3_SORT_CENTER );
