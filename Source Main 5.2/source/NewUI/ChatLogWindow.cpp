      ...
      if ( SEASON3B::IsRepeat( VK_LBUTTON ) ) {
        if ( GetNumberOfLines( GetCurrentMsgType() ) > GetNumberOfShowingLines() ) {
          ...
          if ( MouseY - m_iGrapRelativePosY < m_WndPos.y - m_WndSize.cy + WND_TOP_BOTTOM_EDGE ) {
            ...
            float fScrollRate =
                (float)( ( MouseY - m_iGrapRelativePosY ) - ( m_WndPos.y - m_WndSize.cy + WND_TOP_BOTTOM_EDGE ) ) /
                (float)( m_WndSize.cy - WND_TOP_BOTTOM_EDGE * 2 - SCROLL_BTN_HEIGHT );
            Scrolling( GetNumberOfShowingLines() +
                       (float)( GetNumberOfLines( GetCurrentMsgType() ) - GetNumberOfShowingLines() ) * fScrollRate );
      ...
      if ( MouseLButtonPush ) {
        ...
        for ( int i = 0; i < nTopSections; i++ ) {
          if ( SEASON3B::CheckMouseIn(
                   0, ptResizingBtn.y - RESIZING_BTN_HEIGHT - ( ( i + 1 ) * SCROLL_MIDDLE_PART_HEIGHT * 3 * 2 ), 640,
        ...
        if ( SEASON3B::CheckMouseIn( 0,
                                     m_WndPos.y - ( SCROLL_MIDDLE_PART_HEIGHT * 3 + SCROLL_TOP_BOTTOM_PART_HEIGHT * 2 ),
                                                  ...
                                                  OUT type_string& strText1, OUT type_string& strText2 )
{
  ...
  while ( ( required_size > max_first_line_size ) && ( iLocToken > -1 ) ) {
    ...
    GetTextExtentPoint32( g_pRenderText->GetFontDC(), ( strText.substr( 0, iLocToken ) ).c_str(), iLocToken,
                          &TextSize );
