...
void SEASON3B::CNewUICursedTempleEnter::SetButtonInfo()
{
  ...
  // 2147 "입장하기"
  m_Button[CURSEDTEMPLEENTER_OPEN].ChangeText( GlobalText[2'147] );
  ...
  // 1002 "닫기"
  m_Button[CURSEDTEMPLEENTER_EXIT].ChangeText( GlobalText[1'002] );
}
...
bool SEASON3B::CNewUICursedTempleEnter::UpdateMouseEvent()
{
  if ( m_Button[CURSEDTEMPLEENTER_OPEN].UpdateMouseEvent() ) {
    ...
    if ( Result ) {
      ...
      g_pSystemLogBox->AddText( GlobalText[2'367], SEASON3B::TYPE_ERROR_MESSAGE );
    }
...
void SEASON3B::CNewUICursedTempleEnter::RenderText()
{
  ...
  swprintf( Text, GlobalText[2'358] );
  ...
  if ( CheckEnterLevel( enterlevel ) ) {
    ...
    swprintf( Text, GlobalText[2'370], enterlevel );
    ...
    for ( int i = 0; i < EnterLevelCount + 1; ++i ) {
      ...
      if ( i == 5 ) {
        ...
        swprintf( Text, GlobalText[2'371], EnterMinLevel[i], EnterMaxLevel[i] );
      }

      if ( enterlevel == i + 1 ) {
        DisableAlphaBlend();
        swprintf( Text, L"%ls %ls", Text, GlobalText[2'412] );
        DrawText( Text, m_Pos.x + 3, m_Pos.y + 67 + ( i * 15 ), 0xffffffff, 0xff0000ff, RT3_SORT_CENTER,
                  CURSEDTEMPLE_ENTER_WINDOW_WIDTH - 10, false );
        EnableAlphaTest();
      } else {
        swprintf( Text, L"%ls %ls", Text, GlobalText[2'413] );
    ...
    swprintf( Text, GlobalText[2'373], m_EnterCount );
    ...
    memset( &Text, 0, sizeof( char ) );
    swprintf( Text, GlobalText[2'366] );
