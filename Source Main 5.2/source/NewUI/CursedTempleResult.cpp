...
void SEASON3B::CNewUICursedTempleResult::SetButtonInfo()
{
  ...
  m_Button[CURSEDTEMPLERESULT_CLOSE].ChangeButtonImgState( true, CNewUIMessageBoxMng::IMAGE_MSGBOX_BTN_EMPTY_VERY_SMALL,
                                                           true );
  m_Button[CURSEDTEMPLERESULT_CLOSE].ChangeButtonInfo( x, m_Pos.y + CURSEDTEMPLE_RESULT_WINDOW_HEIGHT - 37, 54, 23 );
  m_Button[CURSEDTEMPLERESULT_CLOSE].ChangeText( GlobalText[1'002] );
}
                                                         ...
                                                         DWORD color, DWORD backcolor )
{
  ...
  if ( SEASON3A::eTeam_Allied == resultinfo.s_team ) {
    swprintf( Text, GlobalText[2'387] );
  } else {
    swprintf( Text, GlobalText[2'388] );
  }
...
void SEASON3B::CNewUICursedTempleResult::RenderText()
{
  ...
  memset( &Text, 0, sizeof( wchar_t ) * 200 );
  swprintf( Text, GlobalText[2'414] );
  DrawText( Text, m_Pos.x, m_Pos.y + 13, 0xFF49B0FF, 0x00000000, RT3_SORT_CENTER, CURSEDTEMPLE_RESULT_WINDOW_WIDTH,
            false );

  memset( &Text, 0, sizeof( wchar_t ) * 200 );
  swprintf( Text, L"  %ls           %ls        %ls     %ls    %ls", GlobalText[2'415], GlobalText[681],
            GlobalText[1'973], GlobalText[683], GlobalText[682] );
  ...
  memset( &Text, 0, sizeof( wchar_t ) * 200 );
  swprintf( Text, GlobalText[2'416] );
