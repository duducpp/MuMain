    ...
    if ( progressnpcindex == npcindex ) {
      if ( progressnpcindex == AlliedHolyItemBoxNpc || progressnpcindex == IllusionHolyItemBoxNpc ) {
        ...
        if ( CheckInventoryHolyItem( Hero ) ) {
          ...
          g_pSystemLogBox->AddText( GlobalText[2'092], SEASON3B::TYPE_ERROR_MESSAGE );
        }
      } else {
        SEASON3B::CCursedTempleProgressMsgBox* pMsgBox = NULL;
  ...
  if ( npcindex == AlliedNpc || npcindex == IllusionNpc ) {
    if ( g_MessageBox->IsEmpty() ) {
      if ( npcindex == AlliedNpc ) {
        SEASON3B::CreateOkMessageBox( GlobalText[2'359] );
      }

      if ( npcindex == IllusionNpc ) {
        SEASON3B::CreateOkMessageBox( GlobalText[2'362] );
      }
    }
...
void SEASON3B::CNewUICursedTempleSystem::UpdateScore()
{
  ...
  switch ( m_ScoreEffectState ) {
    ...
    case 1: {
      ...
      if ( curTime - m_StartScoreEffectTime >= 5'000 ) {
...
void SEASON3B::CNewUICursedTempleSystem::UpdateTutorialStep()
{
  ...
  DWORD curTime = timeGetTime();
  if ( curTime - m_TutorialStepTime >= 10'000 ) {
...
void SEASON3B::CNewUICursedTempleSystem::RenderSkill()
{
  ...
  if ( CheckMouseIn( x, y, Width, Height ) ) {
    ...
    swprintf( TextList[TextNum], L"%ls",
              GlobalText[2'379 + ( CursedTempleCurSkillType - AT_SKILL_CURSED_TEMPLE_PRODECTION )] );
  ...
  if ( CheckMouseIn( x, y, Width, Height ) ) {
    ...
    swprintf( TextList[TextNum], L"%ls", GlobalText[2'378] );
  ...
  if ( CheckMouseIn( x, y, Width, Height ) ) {
    ...
    swprintf( TextList[TextNum], L"%ls", GlobalText[2'377] );
    TextListColor[TextNum] = TEXT_COLOR_WHITE;
    TextNum++;
  ...
  for ( int k = 0; k < m_CursedTempleMyTeamCount; ++k ) {
    ...
    if ( p->wPartyUserIndex != Hero->Key && p->wPartyUserIndex != m_HolyItemPlayerIndex ) {
      ...
      if ( m_MyTeam == SEASON3A::eTeam_Allied ) {
        ...
        RenderBitmap( IMAGE_CURSEDTEMPLESYSTEM_MINIMAPICON_ILLUSION_PC, pcX - 3.f, pcY - 3.f, 7.0f, 7.0f, 0.f, 0.f,
                      7.f / 8.f, 7.f / 8.f );
      }
    }
  }
...
void SEASON3B::CNewUICursedTempleSystem::RenderTutorialStep()
{
  ...
  if ( m_TutorialStepState == 0 ) {
    wcscpy( TextList[TextNum], GlobalText[2'400] );
    TextListColor[TextNum] = 0xFF49B0FF;
    ++TextNum;
    swprintf( TextList[TextNum], L"" );
    TextListColor[TextNum] = 0xFF000000;
    ++TextNum;
    wcscpy( TextList[TextNum], GlobalText[2'401] );
    TextListColor[TextNum] = 0xFFffffff;
    ++TextNum;
    wcscpy( TextList[TextNum], GlobalText[2'402] );
    TextListColor[TextNum] = 0xFFffffff;
    ++TextNum;
    wcscpy( TextList[TextNum], GlobalText[2'403] );
    TextListColor[TextNum] = 0xFFffffff;
    ++TextNum;
  } else if ( m_TutorialStepState == 1 ) {
    wcscpy( TextList[TextNum], GlobalText[2'404] );
    TextListColor[TextNum] = 0xFF49B0FF;
    ++TextNum;
    swprintf( TextList[TextNum], L"" );
    TextListColor[TextNum] = 0xFF000000;
    ++TextNum;
    wcscpy( TextList[TextNum], GlobalText[2'405] );
    TextListColor[TextNum] = 0xFFffffff;
    ++TextNum;
    wcscpy( TextList[TextNum], GlobalText[2'406] );
    TextListColor[TextNum] = 0xFFffffff;
    ++TextNum;
    wcscpy( TextList[TextNum], GlobalText[2'407] );
    TextListColor[TextNum] = 0xFFffffff;
    ++TextNum;
  } else if ( m_TutorialStepState == 2 ) {
    wcscpy( TextList[TextNum], GlobalText[2'408] );
    TextListColor[TextNum] = 0xFF49B0FF;
    ++TextNum;
    swprintf( TextList[TextNum], L"" );
    TextListColor[TextNum] = 0xFF000000;
    ++TextNum;
    wcscpy( TextList[TextNum], GlobalText[2'409] );
    TextListColor[TextNum] = 0xFFffffff;
    ++TextNum;
    wcscpy( TextList[TextNum], GlobalText[2'410] );
    TextListColor[TextNum] = 0xFFffffff;
    ++TextNum;
    wcscpy( TextList[TextNum], GlobalText[2'411] );
    TextListColor[TextNum] = 0xFFffffff;
    ++TextNum;
  }
...
void SEASON3B::CNewUICursedTempleSystem::SetCursedTempleSkill( CHARACTER* c, OBJECT* o, DWORD selectcharacterindex )
{
  ...
  if ( m_SkillPoint < MaxKillCount ) {
    g_pSystemLogBox->AddText( GlobalText[2'392], SEASON3B::TYPE_ERROR_MESSAGE );
    MouseRButtonPush = false;
    return;
  }
...
void SEASON3B::CNewUICursedTempleSystem::ReceiveCursedTempleInfo( const BYTE* ReceiveBuffer )
{
  ...
  if ( m_MyTeam == SEASON3A::eTeam_Allied ) {
    if ( m_AlliedPoint != data->btAlliedPoint ) {
      ...
      StartScoreEffect();
      g_pSystemLogBox->AddText( GlobalText[2'360], SEASON3B::TYPE_ERROR_MESSAGE );
    } else if ( m_IllusionPoint != data->btIllusionPoint ) {
      PlayBuffer( SOUND_CURSEDTEMPLE_GAMESYSTEM4 );
      StartScoreEffect();
      g_pSystemLogBox->AddText( GlobalText[2'361], SEASON3B::TYPE_ERROR_MESSAGE );
    }
  } else {
    if ( m_IllusionPoint != data->btIllusionPoint ) {
      PlayBuffer( SOUND_CURSEDTEMPLE_GAMESYSTEM4 );
      StartScoreEffect();
      g_pSystemLogBox->AddText( GlobalText[2'363], SEASON3B::TYPE_ERROR_MESSAGE );
    } else if ( m_AlliedPoint != data->btAlliedPoint ) {
      PlayBuffer( SOUND_CURSEDTEMPLE_GAMESYSTEM4 );
      StartScoreEffect();
      g_pSystemLogBox->AddText( GlobalText[2'364], SEASON3B::TYPE_ERROR_MESSAGE );
    }
  }
...
void SEASON3B::CNewUICursedTempleSystem::ReceiveCursedTempSkillPoint( const BYTE* ReceiveBuffer )
{
  ...
  if ( m_SkillPoint < data->btSkillPoint ) {
    wchar_t message[100];
    memset( &message, 0, sizeof( char ) );
    swprintf( message, GlobalText[2'391], data->btSkillPoint - m_SkillPoint );
