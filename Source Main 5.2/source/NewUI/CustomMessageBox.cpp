...
bool SEASON3B::CUseFruitCheckMsgBox::Create( float fPriority )
{
  ...
  if ( pItem->Type == ITEM_FRUITS ) {
    switch ( pItem->Level ) {
      ...
      case 3:
        ...
        break;
      case 4:
        swprintf( strName, L"%ls", GlobalText[1'900] );
        break;
    }
  }
  ...
  swprintf( strText, L"( %ls%ls )", strName, GlobalText[1'901] );
  AddMsg( strText, RGBA( 255, 255, 0, 255 ), MSGBOX_FONT_BOLD );
  AddMsg( GlobalText[1'902], RGBA( 255, 255, 0, 255 ), MSGBOX_FONT_BOLD );
...
void SEASON3B::CUseFruitCheckMsgBox::SetButtonInfo()
{
  ...
  m_BtnAdd.SetText( GlobalText[1'412] );
  ...
  m_BtnMinus.SetInfo( CNewUIMessageBoxMng::IMAGE_MSGBOX_BTN_EMPTY_SMALL, x, y, width, height,
                      CNewUIMessageBoxButton::MSGBOX_BTN_SIZE_EMPTY_SMALL );
  m_BtnMinus.SetText( GlobalText[1'903] );
...
bool SEASON3B::CGemIntegrationMsgBox::Create( float fPriority )
{
  ...
  AddMsg( GlobalText[1'801], RGBA( 255, 128, 0, 255 ), MSGBOX_FONT_BOLD );

  AddMsg( GlobalText[3'307] );
  AddMsg( GlobalText[3'308] );
                                                                  ...
                                                                  const leaf::xstreambuf&     xParam )
{
  ...
  if ( !COMGEM::FindWantedList() ) {
    g_pSystemLogBox->AddText( GlobalText[1'818], SEASON3B::TYPE_ERROR_MESSAGE );
    return CALLBACK_BREAK;
  }
...
void SEASON3B::CGemIntegrationMsgBox::SetButtonInfo()
{
  ...
  m_BtnUnity.SetText( GlobalText[1'801] );
  ...
  m_BtnDisjoint.SetText( GlobalText[1'800] );
  ...
  m_BtnCancel.SetText( GlobalText[1'002] );
}
...
void SEASON3B::CGemIntegrationUnityMsgBox::SetText( void )
{
  ...
  if ( COMGEM::m_cGemType == COMGEM::NOGEM ) {
    AddMsg( GlobalText[1'801], RGBA( 255, 128, 0, 255 ), MSGBOX_FONT_BOLD );
    AddMsg( GlobalText[3'309] );
  } else {
    AddMsg( GlobalText[1'801], RGBA( 255, 128, 0, 255 ), MSGBOX_FONT_BOLD );
    AddMsg( GlobalText[3'310] );
  }
}
...
void SEASON3B::CGemIntegrationUnityMsgBox::SetButtonInfo()
{
  ...
  int     nNum                            = 0;
  int     nBtnIndex[COMGEM::eGEMTYPE_END] = { 1'806, 1'807, 3'312, 3'313, 3'314, 2'081, 3'315, 3'316, 3'317, 3'318 };
  wchar_t szTemp[256]                     = {
    0,
  };
  ...
  for ( int k = 0; k < (int)COMGEM::eCOMTYPE_END; k++ ) {
    ...
    // 1808 "%d개 조합(%d젠 소요)"
    swprintf( szTemp, GlobalText[1'808], 10 * ( k + 1 ), 500'000 * ( k + 1 ) );
                                                                        ...
                                                                        const leaf::xstreambuf&     xParam )
{
  ...
  if ( pMsgBox ) {
    ...
    swprintf( strText, GlobalText[COMGEM::GetJewelIndex( COMGEM::m_cGemType, 0 )], GlobalText[1'807],
              COMGEM::m_cCount );
    pMsgBox->AddMsg( strText, CLRDW_YELLOW, MSGBOX_FONT_BOLD );

    swprintf( strText, GlobalText[1'810], COMGEM::m_iValue );
    pMsgBox->AddMsg( strText, CLRDW_YELLOW, MSGBOX_FONT_BOLD );
  }
  ...
  if ( COMGEM::CheckInv() ) {
    ...
    if ( pMsgBox ) {
      ...
      if ( COMGEM::m_cGemType == COMGEM::CELE ) {
        swprintf( strText, GlobalText[1'809], GlobalText[1'806], COMGEM::m_cCount );
      } else if ( COMGEM::m_cGemType == COMGEM::SOUL ) {
        swprintf( strText, GlobalText[1'809], GlobalText[1'807], COMGEM::m_cCount );
      }
...
bool SEASON3B::CGemIntegrationDisjointMsgBox::Create( float fPriority )
{
  ...
  AddMsg( GlobalText[1'800], RGBA( 255, 128, 0, 255 ), MSGBOX_FONT_BOLD );
  SetButtonInfo();
  ChangeMiddleFrameBig();
  AddMsg( L" ", RGBA( 255, 128, 0, 255 ), MSGBOX_FONT_BOLD );
  AddMsg( GlobalText[3'311], CLRDW_YELLOW, MSGBOX_FONT_BOLD );
                                                                          ...
                                                                          const leaf::xstreambuf&     xParam )
{
  ...
  if ( pUT ) {
    ...
    if ( pMsgBox ) {
      ...
      COMGEM::SetGem( nIdx );
      swprintf( strText, GlobalText[1'813], GlobalText[COMGEM::GetJewelIndex( nIdx, COMGEM::eGEM_NAME )], iGemLevel );

      pMsgBox->AddMsg( strText, CLRDW_DARKYELLOW, MSGBOX_FONT_BOLD );
      swprintf( strText, GlobalText[1'814], COMGEM::m_iValue );
      pMsgBox->AddMsg( strText, CLRDW_DARKYELLOW, MSGBOX_FONT_BOLD );
    }
  }
...
void SEASON3B::CChaosMixMenuMsgBox::SetButtonInfo()
{
  ...
  m_BtnMix380.SetInfo( CNewUIMessageBoxMng::IMAGE_MSGBOX_BTN_EMPTY, x, y, width, height,
                       CNewUIMessageBoxButton::MSGBOX_BTN_SIZE_EMPTY );
  m_BtnMix380.SetText( GlobalText[2'193] );
...
void SEASON3B::CChaosMixMenuMsgBox::RenderTexts()
{
  ...
  swprintf( szText, GlobalText[1'680], Hero->ID );
  ...
  fPos_y += 85;
  swprintf( szText, GlobalText[2'194], Hero->ID );
  g_pRenderText->RenderText( fPos_x, fPos_y + 0 * 15, szText, MSGBOX_WIDTH - 20.0f, 0, RT3_SORT_CENTER );
}
...
CALLBACK_RESULT SEASON3B::CDialogMsgBox::EndBtnDown( class CNewUIMessageBoxBase* pOwner,
                                                     const leaf::xstreambuf&     xParam )
{
  PlayBuffer( SOUND_CLICK01 );
  g_MessageBox->SendEvent( pOwner, MSGBOX_EVENT_DESTROY );
...
bool SEASON3B::CCursedTempleProgressMsgBox::Update()
{
  ...
  if ( CheckHeroAction() == false ||
       g_pNewUISystem->IsVisible( SEASON3B::INTERFACE_CURSEDTEMPLE_GAMESYSTEM ) == false ) {
    g_MessageBox->SendEvent( this, MSGBOX_EVENT_USER_CUSTOM_PROGRESS_CLOSINGPROCESS );
  }
...
void SEASON3B::CDuelResultMsgBox::RenderTexts()
{
  ...
  g_pRenderText->SetTextColor( 255, 255, 0, 255 );
  g_pRenderText->SetFont( g_hFontBold );
  g_pRenderText->RenderText( GetPos().x, GetPos().y + 100, GlobalText[2'694], MSGBOX_WIDTH, 0, RT3_SORT_CENTER );

  g_pRenderText->SetBgColor( 0, 0, 0, 0 );
  g_pRenderText->SetTextColor( 255, 255, 255, 255 );
  g_pRenderText->SetFont( g_hFont );
  swprintf( strDuelID, GlobalText[2'695], m_szWinnerID );
  g_pRenderText->RenderText( GetPos().x, GetPos().y + 120, strDuelID, MSGBOX_WIDTH, 0, RT3_SORT_CENTER );
  swprintf( strDuelID, GlobalText[2'696], m_szLoserID );
  g_pRenderText->RenderText( GetPos().x, GetPos().y + 135, strDuelID, MSGBOX_WIDTH, 0, RT3_SORT_CENTER );
  g_pRenderText->RenderText( GetPos().x, GetPos().y + 151, GlobalText[2'697], MSGBOX_WIDTH, 0, RT3_SORT_CENTER );
}
...
void CCherryBlossomMsgBox::SetButtonInfo()
{
  ...
  m_BtnExit.SetInfo( CNewUIMessageBoxMng::IMAGE_MSGBOX_BTN_EMPTY_SMALL, x, y, width, height,
                     CNewUIMessageBoxButton::MSGBOX_BTN_SIZE_EMPTY_SMALL );
  // 1002 "닫기"
  m_BtnExit.SetText( GlobalText[1'002] );
}
...
void CCherryBlossomMsgBox::RenderTexts()
{
  ...
  g_pRenderText->SetTextColor( 255, 255, 0, 255 );
  g_pRenderText->SetFont( g_hFontBold );
  swprintf( titleinfo, L"%ls", GlobalText[2'544] );
                                                              ...
                                                              const leaf::xstreambuf&     xParam )
{
  ...
  if ( iInputZen <= CharacterMachine->StorageGold && CharacterMachine->Gold + iInputZen <= 2'000'000'000 ) {
  ...
  } else if ( CharacterMachine->Gold + iInputZen > 2'000'000'000 ) {
  } else {
    SEASON3B::CreateOkMessageBox( GlobalText[423] );
  }
...
bool SEASON3B::CPersonalShopItemValueMsgBoxLayout::SetLayout()
{
  ...
  pMsgBox->SetInputBoxOption( UIOPTION_NUMBERONLY | UIOPTION_PAINTBACK );
  pMsgBox->AddMsg( GlobalText[1'129] );
                                                                         ...
                                                                         const leaf::xstreambuf&     xParam )
{
  ...
  if ( bResult == true ) {
    ...
    if ( lpMsgBox ) {
      ...
      swprintf( strText2, GlobalText[1'132], strText );
      lpMsgBox->AddMsg( strText2, RGBA( 255, 0, 0, 255 ), MSGBOX_FONT_BOLD );
      lpMsgBox->AddMsg( GlobalText[1'133] );
...
bool SEASON3B::CPersonalShopNameMsgBoxLayout::SetLayout()
{
  ...
  if ( false == pMsgBox->Create( MSGBOX_COMMON_TYPE_OKCANCEL, INPUTBOX_TYPE_TEXT, INPUT_WIDTH, INPUT_HEIGHT,
                                 INPUT_TEXTLIMIT ) ) {
    return false;
  }

  pMsgBox->SetInputBoxOption( UIOPTION_PAINTBACK );
  pMsgBox->AddMsg( GlobalText[1'128] );
                                                                    ...
                                                                    const leaf::xstreambuf&     xParam )
{
  ...
  if ( IsCorrectShopTitle( strText ) ) {
    ...
    g_pSystemLogBox->AddText( GlobalText[1'130], SEASON3B::TYPE_SYSTEM_MESSAGE );
  }
...
bool SEASON3B::CCastleWithdrawMsgBoxLayout::SetLayout()
{
  ...
  pMsgBox->AddMsg( GlobalText[1'571] );
  pMsgBox->AddMsg( GlobalText[1'570] );
...
bool SEASON3B::CCrownSwitchPopLayout::SetLayout()
{
  ...
  if ( false == pMsgBox->Create( 3'000 ) ) {
    return false;
  }

  pMsgBox->AddMsg( GlobalText[1'484] );
...
bool SEASON3B::CCrownSwitchPushLayout::SetLayout()
{
  ...
  pMsgBox->AddMsg( GlobalText[1'485] );
...
bool SEASON3B::CSealRegisterSuccessLayout::SetLayout()
{
  ...
  pMsgBox->AddMsg( GlobalText[1'490] );
...
bool SEASON3B::CSealRegisterOtherLayout::SetLayout()
{
  ...
  pMsgBox->AddMsg( GlobalText[1'492] );
...
bool SEASON3B::CSealRegisterOtherCampLayout::SetLayout()
{
  ...
  pMsgBox->AddMsg( GlobalText[1'982] );
...
bool SEASON3B::CCrownDefenseRemoveLayout::SetLayout()
{
  ...
  pMsgBox->AddMsg( GlobalText[1'493] );
...
bool SEASON3B::CCrownDefenseCreateLayout::SetLayout()
{
  ...
  pMsgBox->AddMsg( GlobalText[1'494] );
...
bool SEASON3B::CCursedTempleHolicItemGetLayout::SetLayout()
{
  ...
  if ( false == pMsgBox->Create( 10'000 ) ) {
    return false;
  }

  pMsgBox->AddMsg( GlobalText[2'417] );
...
bool SEASON3B::CCursedTempleHolicItemSaveLayout::SetLayout()
{
  ...
  pMsgBox->AddMsg( GlobalText[2'418] );
...
void SEASON3B::CTrainerMenuMsgBox::RenderTexts()
{
  ...
  g_pRenderText->SetTextColor( 255, 255, 255, 255 );
  g_pRenderText->SetFont( g_hFontBold );
  swprintf( szText, GlobalText[1'227] );
  g_pRenderText->RenderText( fPos_x, fPos_y, szText, MSGBOX_WIDTH - 20.0f, 0, RT3_SORT_CENTER );

  fPos_y += 15;
  g_pRenderText->SetFont( g_hFont );
  swprintf( szText, GlobalText[1'202] );
  g_pRenderText->RenderText( fPos_x, fPos_y + 1 * 18, szText, MSGBOX_WIDTH - 20.0f, 0, RT3_SORT_CENTER );
  swprintf( szText, GlobalText[1'203], Hero->ID );
...
void SEASON3B::CTrainerRecoverMsgBox::SetButtonInfo()
{
  ...
  m_BtnRecoverDarkHorse.SetInfo( CNewUIMessageBoxMng::IMAGE_MSGBOX_BTN_EMPTY, x, y, width, height,
                                 CNewUIMessageBoxButton::MSGBOX_BTN_SIZE_EMPTY );
  m_BtnRecoverDarkHorse.SetText( GlobalText[1'187] );

  y = GetPos().y + 115;
  m_BtnRecoverDarkSpirit.SetInfo( CNewUIMessageBoxMng::IMAGE_MSGBOX_BTN_EMPTY, x, y, width, height,
                                  CNewUIMessageBoxButton::MSGBOX_BTN_SIZE_EMPTY );
  m_BtnRecoverDarkSpirit.SetText( GlobalText[1'214] );
...
void SEASON3B::CTrainerRecoverMsgBox::RenderTexts()
{
  ...
  fPos_y += 15;
  g_pRenderText->SetFont( g_hFont );
  swprintf( szText, GlobalText[1'228] );
  g_pRenderText->RenderText( fPos_x, fPos_y + 1 * 18, szText, MSGBOX_WIDTH - 20.0f, 0, RT3_SORT_CENTER );
...
void SEASON3B::CElpisMsgBox::RenderTexts()
{
  ...
  g_pRenderText->SetTextColor( 255, 255, 255, 255 );
  g_pRenderText->SetFont( g_hFontBold );
  swprintf( szText, GlobalText[2'206] );
  ...
  switch ( m_iMessageType ) {
    case 0:
      swprintf( szText, GlobalText[2'074] );
      g_pRenderText->RenderText( fPos_x, fPos_y + 1 * 18, szText, MSGBOX_WIDTH - 20.0f, 0, RT3_SORT_CENTER );
      break;
    case MSGBOX_EVENT_USER_CUSTOM_ELPIS_ABOUT_REFINARY: {
      wchar_t Textlist[7][100];
      int     lineSize = 0;
      lineSize         = CutText3( GlobalText[2'198], Textlist[0], MSGBOX_WIDTH - 60.0f, 7, 100 );
    ...
    case MSGBOX_EVENT_USER_CUSTOM_ELPIS_ABOUT_JEWELOFHARMONY: {
      wchar_t Textlist[7][100];
      int     lineSize = 0;
      lineSize         = CutText3( GlobalText[2'199], Textlist[0], MSGBOX_WIDTH - 60.0f, 7, 100 );
      for ( int i = 0; i < lineSize; ++i ) {
        g_pRenderText->RenderText( fPos_x + 20, fPos_y + ( i + 1 ) * 18, Textlist[i] );
      }
    } break;
...
void SEASON3B::CSeedMasterMenuMsgBox::RenderTexts()
{
  ...
  g_pRenderText->SetTextColor( 255, 255, 255, 255 );
  g_pRenderText->SetFont( g_hFontBold );
  swprintf( szText, GlobalText[2'666] );
  g_pRenderText->RenderText( fPos_x, fPos_y, szText, MSGBOX_WIDTH - 20.0f, 0, RT3_SORT_CENTER );

  fPos_y += 15;
  g_pRenderText->SetFont( g_hFont );
  swprintf( szText, GlobalText[2'667] );
  g_pRenderText->RenderText( fPos_x, fPos_y + 1 * 18, szText, MSGBOX_WIDTH - 20.0f, 0, RT3_SORT_CENTER );
  swprintf( szText, GlobalText[2'668] );
  g_pRenderText->RenderText( fPos_x, fPos_y + 2 * 18, szText, MSGBOX_WIDTH - 20.0f, 0, RT3_SORT_CENTER );
}
...
void SEASON3B::CSeedInvestigatorMenuMsgBox::RenderTexts()
{
  ...
  g_pRenderText->SetTextColor( 255, 255, 255, 255 );
  g_pRenderText->SetFont( g_hFontBold );
  swprintf( szText, GlobalText[2'671] );
  g_pRenderText->RenderText( fPos_x, fPos_y, szText, MSGBOX_WIDTH - 20.0f, 0, RT3_SORT_CENTER );

  fPos_y += 15;
  g_pRenderText->SetFont( g_hFont );
  swprintf( szText, GlobalText[2'672] );
  g_pRenderText->RenderText( fPos_x, fPos_y + 1 * 18, szText, MSGBOX_WIDTH - 20.0f, 0, RT3_SORT_CENTER );
  swprintf( szText, GlobalText[2'673] );
  g_pRenderText->RenderText( fPos_x, fPos_y + 2 * 18, szText, MSGBOX_WIDTH - 20.0f, 0, RT3_SORT_CENTER );
}
...
void SEASON3B::CResetCharacterPointMsgBox::SetButtonInfo()
{
  ...
  m_ResetCharacterPointBtn.SetInfo( CNewUIMessageBoxMng::IMAGE_MSGBOX_BTN_EMPTY, x, y, width, height,
                                    CNewUIMessageBoxButton::MSGBOX_BTN_SIZE_EMPTY );
  m_ResetCharacterPointBtn.SetText( GlobalText[1'884] ); // "스탯 초기화"
...
void SEASON3B::CResetCharacterPointMsgBox::RenderTexts()
{
  ...
  g_pRenderText->SetTextColor( 255, 255, 255, 255 );
  g_pRenderText->SetFont( g_hFontBold );
  swprintf( szText, GlobalText[1'885] );
  g_pRenderText->RenderText( fPos_x, fPos_y, szText, MSGBOX_WIDTH - 20.0f, 0, RT3_SORT_CENTER );

  fPos_y += 25;
  g_pRenderText->SetFont( g_hFont );
  swprintf( szText, GlobalText[1'886] );
  g_pRenderText->RenderText( fPos_x, fPos_y + 1 * 18, szText, MSGBOX_WIDTH - 20.0f, 0, RT3_SORT_CENTER );
}
                                                                                  ...
                                                                                  const leaf::xstreambuf&     xParam )
{
  ...
  for ( int i = 0; i < MAX_EQUIPMENT; i++ ) {
    if ( CharacterMachine->Equipment[i].Type != -1 ) {
      g_pSystemLogBox->AddText( GlobalText[1'883], SEASON3B::TYPE_ERROR_MESSAGE );
      g_MessageBox->SendEvent( pOwner, MSGBOX_EVENT_DESTROY );
      return CALLBACK_BREAK;
    }
  }
...
void SEASON3B::CGuild_ToPerson_Position::RenderButtons()
{
  ...
  wchar_t strText[256];
  if ( COMGEM::m_cGemType == COMGEM::CELE ) {
    swprintf( strText, GlobalText[1'314], GuildList[DeleteIndex].Name, GlobalText[1'301] );
    AppointType = SUBGUILDMASTER;
  ...
  if ( COMGEM::m_cGemType == COMGEM::SOUL ) {
    swprintf( strText, GlobalText[1'314], GuildList[DeleteIndex].Name, GlobalText[1'302] );
  ...
  m_BtnOk.Render();
  m_BtnCancel.Render();
  AddMsg( GlobalText[1'315], RGBA( 255, 128, 0, 255 ), MSGBOX_FONT_BOLD );
}
