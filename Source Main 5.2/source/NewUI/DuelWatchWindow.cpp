...
bool CNewUIDuelWatchWindow::Create( CNewUIManager* pNewUIMng, int x, int y )
{
  ...
  for ( int i = 0; i < 4; ++i ) {
    ...
    InitButton( m_BtnChannel + i, m_Pos.x + INVENTORY_WIDTH / 2 - 27, m_Pos.y + 100 + i * 90, GlobalText[2'701] );
  }
...
bool CNewUIDuelWatchWindow::Render()
{
  ...
  g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, GlobalText[2'699], 190, 0, RT3_SORT_CENTER );
  ...
  for ( i = 0; i < 4; ++i ) {
    ...
    swprintf( szText, GlobalText[2'700], i + 1 );
  ...
  for ( i = 0; i < 4; ++i ) {
    if ( m_bChannelEnable[i] == TRUE ) {
      ...
      g_pRenderText->SetTextColor( 255, 255, 255, 255 );
      g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y + 35, GlobalText[2'705], 190, 0, RT3_SORT_CENTER );
    }
...
void CNewUIDuelWatchWindow::RenderFrame()
{
  ...
  swprintf( szText, L"%ls", GlobalText[2'698] );
