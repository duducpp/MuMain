...
bool CNewUIEmpireGuardianNPC::Create( CNewUIManager* pNewUIMng, CNewUI3DRenderMng* pNewUI3DRenderMng, int x, int y )
{
  ...
  InitButton( &m_btPositive, x + ( NPC_WINDOW_WIDTH / 2 ) - 27, y + 190, GlobalText[1'593] );
  InitButton( &m_btNegative, x + ( NPC_WINDOW_WIDTH / 2 ) - 27, y + 380, GlobalText[1'002] );
...
bool CNewUIEmpireGuardianNPC::Render()
{
  ...
  g_pRenderText->RenderText( m_Pos.x, Position.y + 50, GlobalText[2'795], 190, 0, RT3_SORT_CENTER );
  wchar_t szTextOut[2][300];
  CutStr( GlobalText[2'796], szTextOut[0], 150, 2, 300 );
  ...
  g_pRenderText->RenderText( m_Pos.x, Position.y + 110, GlobalText[2'797], 190, 0, RT3_SORT_CENTER );

  g_pRenderText->SetFont( g_hFontBold );
  g_pRenderText->SetTextColor( 255, 240, 0, 255 );
  g_pRenderText->RenderText( Position.x - 55, Position.y + 170, GlobalText[2'783], 110, 0, RT3_SORT_CENTER );
  ...
  g_pRenderText->RenderText( Position.x - 55, Position.y + 260, GlobalText[2'223], 110, 0, RT3_SORT_CENTER );

  g_pRenderText->SetFont( g_hFont );
  g_pRenderText->SetTextColor( 220, 220, 220, 255 );
  g_pRenderText->RenderText( Position.x - 100, Position.y + 280, GlobalText[2'835], 200, 0, RT3_SORT_CENTER );
  g_pRenderText->RenderText( Position.x - 100, Position.y + 300, GlobalText[2'836], 200, 0, RT3_SORT_CENTER );
  g_pRenderText->RenderText( Position.x - 100, Position.y + 320, GlobalText[2'837], 200, 0, RT3_SORT_CENTER );
  CutStr( GlobalText[2'838], szTextOut[0], 155, 2, 300 );
...
void CNewUIEmpireGuardianNPC::RenderFrame()
{
  ...
  g_pRenderText->SetTextColor( 220, 220, 220, 255 );
  g_pRenderText->SetBgColor( 0, 0, 0, 0 );
  g_pRenderText->RenderText( m_Pos.x + ( NPC_WINDOW_WIDTH / 2 ) - 55, m_Pos.y + 13, GlobalText[2'794], 110, 0,
                             RT3_SORT_CENTER );
}
