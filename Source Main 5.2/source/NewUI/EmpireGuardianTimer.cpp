...
bool CNewUIEmpireGuardianTimer::Render()
{
  ...
  swprintf( szText, GlobalText[2'805], m_iDay, m_iZone );
  ...
  switch ( m_iType ) {
    ...
    case 1:
      ...
      g_pRenderText->RenderText( m_Pos.x + ( TIMER_WINDOW_WIDTH / 2 ) - 55, m_Pos.y + 38, GlobalText[2'844], 110, 0,
                                 RT3_SORT_CENTER );
      break;
    case 2:
      g_pRenderText->SetTextColor( 255, 150, 0, 255 );
      swprintf( szText, L"%ls (%ls)", GlobalText[865], GlobalText[2'845] );
      g_pRenderText->RenderText( m_Pos.x + ( TIMER_WINDOW_WIDTH / 2 ) - 55, m_Pos.y + 38, szText, 110, 0,
                                 RT3_SORT_CENTER );
      break;
  }

  int iSecond = m_dTime / 1'000;
