...
bool SEASON3B::CNewUIHelpWindow::Render()
{
  ...
  if ( m_iIndex == 0 ) {
    ...
    wcscpy( TextList[iTextNum], GlobalText[2'421] );
    TextListColor[iTextNum] = TEXT_COLOR_BLUE;
    TextBold[iTextNum]      = true;
    iTextNum++;

    for ( int i = 0; i < 24; ++i ) {
      wcscpy( TextList[iTextNum], GlobalText[2'422 + i] );
      TextListColor[iTextNum] = TEXT_COLOR_WHITE;
      TextBold[iTextNum]      = false;
      iTextNum++;
    }
    ...
    wcscpy( TextList[iTextNum], GlobalText[2'446] );
    TextListColor[iTextNum] = TEXT_COLOR_BLUE;
    TextBold[iTextNum]      = true;
    iTextNum++;

    for ( int i = 0; i < 18; ++i ) {
      wcscpy( TextList[iTextNum], GlobalText[2'447 + i] );
      if ( i == 0 || i == 8 || i == 9 ) {
        TextListColor[iTextNum] = TEXT_COLOR_BLUE;
        TextBold[iTextNum]      = true;
        iTextNum++;
