...
bool CNewUIHeroPositionInfo::Create( CNewUIManager* pNewUIMng, int x, int y )
{
  ...
  std::wstring tooltiptext1 = GlobalText[3'561];
  ...
  std::wstring tooltiptext2 = GlobalText[3'562];
  ...
  std::wstring tooltiptext3 = GlobalText[3'563];
  std::wstring btname3      = L"";

  SetButtonInfo( &m_BtnStop, IMAGE_HERO_POSITION_INFO_BASE_WINDOW + 5, x + WidenX + 59, y, 18, 13, 1, 0, 1, 1u, btname2,
                 tooltiptext3, 0 );
...
bool CNewUIHeroPositionInfo::Render()
{
  ...
  //--
  swprintf( szText, L"%ls (%d , %d)", gMapManager.GetMapName( gMapManager.WorldActive ), m_CurHeroPosition.x,
            m_CurHeroPosition.y );
