  ...
  if ( SelectedCharacter >= 0 ) {
    if ( SEASON3B::IsRepeat( VK_MENU ) && SEASON3B::IsRelease( VK_RBUTTON ) && gMapManager.InChaosCastle() == false &&
      ...
      if ( fDistance < 300.f ) {
        ...
        g_pSystemLogBox->AddText( GlobalText[1'388], SEASON3B::TYPE_ERROR_MESSAGE );
...
bool SEASON3B::CNewUIHotKey::UpdateKeyEvent()
{
  ...
  if ( SEASON3B::IsPress( 'F' ) ) {
    ...
    if ( ::IsStrifeMap( gMapManager.WorldActive ) ) {
      if ( g_pSystemLogBox->CheckChatRedundancy( GlobalText[2'989] ) == FALSE ) {
        g_pSystemLogBox->AddText( GlobalText[2'989], SEASON3B::TYPE_SYSTEM_MESSAGE );
      }
    } else {
