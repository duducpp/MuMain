...
#include "GameShop/MsgBoxIGSBuyPackageItem.h"
#include "GameShop/MsgBoxIGSBuySelectItem.h"
...
void CNewUIInGameShop::RenderFrame()
{
  ...
  for ( int cnt = g_InGameShopSystem->GetSizePackageAsDisplayPackage(); cnt < INGAMESHOP_DISPLAY_ITEMLIST_SIZE;
        cnt++ ) {
...
void CNewUIInGameShop::RenderTexts()
{
  ...
  swprintf( szText, GlobalText[2'883], L"" );
  ...
  swprintf( szText, GlobalText[3'145], L"" );
  ...
  swprintf( szText, GlobalText[2'884], L"" );
  g_pRenderText->RenderText( m_Pos.x + TEXT_IGS_CASH_POS_X, m_Pos.y + TEXT_IGS_POINT_POS_Y, szText, TEXT_IGS_CASH_WIDTH,
                             0, RT3_SORT_LEFT );
  ...
  g_pRenderText->RenderText( m_Pos.x + TEXT_IGS_STORAGE_NAME_POS_X, m_Pos.y + TEXT_IGS_STORAGE_NAME_POS_Y,
                             GlobalText[2'951], TEXT_IGS_STORAGE_NAME_WIDTH, 0, RT3_SORT_CENTER );

  g_pRenderText->RenderText( m_Pos.x + TEXT_IGS_STORAGE_TIME_POS_X, m_Pos.y + TEXT_IGS_STORAGE_NAME_POS_Y,
                             GlobalText[2'952], TEXT_IGS_STORAGE_TIME_WIDTH, 0, RT3_SORT_CENTER );
...
bool CNewUIInGameShop::BtnProcess()
{
  ...
  if ( m_CashGiftButton.UpdateMouseEvent() == true ) {
    ...
    pMsgBox->Initialize( GlobalText[2'937], GlobalText[2'938] );
    return true;
  }
  ...
  if ( m_UseButton.UpdateMouseEvent() == true ) {
    if ( m_StorageItemListBox.GetLineNum() <= 0 ) {
      CMsgBoxIGSCommon* pMsgBox = NULL;
      CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
      pMsgBox->Initialize( GlobalText[3'028], GlobalText[3'033] );
      return true;
    }
...
void CNewUIInGameShop::SetBtnInfo()
{
  ...
  m_CloseButton.ChangeToolTipText( GlobalText[1'002], true );
  ...
  strText = GlobalText[2'888];
  TextList.push_back( strText );
  strText = GlobalText[2'889];
  TextList.push_back( strText );
  ...
  for ( int i = 0; i < INGAMESHOP_DISPLAY_ITEMLIST_SIZE; i++ ) {
    ...
    m_ViewDetailButton[i].ChangeText( GlobalText[2'886] );
  }
  ...
  m_CashGiftButton.ChangeToolTipText( GlobalText[2'939] );
  ...
  m_CashChargeButton.ChangeButtonInfo( m_Pos.x + IMAGE_IGS_CASHGIFT_BTN_POS_X, m_Pos.y + IMAGE_IGS_ICON_BTN_POS_Y,
                                       IMAGE_IGS_ICON_BTN_WIDTH, IMAGE_IGS_ICON_BTN_HEIGHT );
  m_CashChargeButton.ChangeToolTipText( GlobalText[2'940] );
  ...
  m_CashRefreshButton.ChangeButtonInfo( m_Pos.x + IMAGE_IGS_REFRESH_BTN_POS_X, m_Pos.y + IMAGE_IGS_ICON_BTN_POS_Y,
                                        IMAGE_IGS_ICON_BTN_WIDTH, IMAGE_IGS_ICON_BTN_HEIGHT );
  m_CashRefreshButton.ChangeToolTipText( GlobalText[2'941] );
  ...
  m_UseButton.ChangeText( GlobalText[2'887] );
...
bool CNewUIInGameShop::IsInGameShopOpen()
{
  ...
  if ( !( Hero->SafeZone ) &&
       !( WD_0LORENCIA == gMapManager.WorldActive && WD_3NORIA == gMapManager.WorldActive &&
          WD_2DEVIAS == gMapManager.WorldActive && WD_51HOME_6TH_CHAR == gMapManager.WorldActive ) ) {
    CMsgBoxIGSCommon* pMsgBox = NULL;
    CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
    pMsgBox->Initialize( GlobalText[3'028], GlobalText[3'051] );
    g_ConsoleDebug->Write( MCD_NORMAL, L"InGameShopStatue.Txt Return - false <%ls>", GlobalText[3'051] );
    return false;
  }
  ...
  if ( g_InGameShopSystem->IsShopOpen() == false ) {
    CMsgBoxIGSCommon* pMsgBox = NULL;
    CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
    pMsgBox->Initialize( GlobalText[3'028], GlobalText[3'035] );
    g_ConsoleDebug->Write( MCD_NORMAL, L"InGameShopStatue.Txt Return - false <%ls>", GlobalText[3'035] );
    return false;
  }
...
bool CNewUIInGameShop::UpdateBanner()
{
  ...
  if ( ( SEASON3B::IsPress( VK_LBUTTON ) ) &&
       ( SEASON3B::CheckMouseIn( IMAGE_IGS_BANNER_POS_X, IMAGE_IGS_BANNER_POS_Y, IMAGE_IGS_BANNER_WIDTH,
                                 IMAGE_IGS_BANNER_HEIGHT ) ) ) {
                                       ...
                                       wchar_t* pszUserName /* = NULL */, wchar_t* pszMessage /* = NULL */ )
{
  ...
  if ( chItemType == 'C' || chItemType == 'c' ) {
    ...
    // Name
    swprintf( Item.m_szName, GlobalText[3'050], szValue );

    // Num
    swprintf( Item.m_szNum, GlobalText[3'043], szValue );
    ...
    if ( iPriceSeq > 0 ) {
      ...
      g_InGameShopSystem->GetProductInfoFromProductSeq( iProductSeq, CInGameShopSystem::IGS_PRODUCT_ATT_TYPE_ITEMCODE,
                                                        iValue, szText );
      Item.m_wItemCode = iValue;
    }
  } else {
