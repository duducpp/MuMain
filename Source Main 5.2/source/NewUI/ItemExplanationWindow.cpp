...
bool SEASON3B::CNewUIItemExplanationWindow::Render()
{
  ...
  if ( ItemHelp == ITEM_BOLT || ItemHelp == ITEM_ARROWS ) {
    ...
    if ( WindowWidth == 640 || WindowWidth == 1'280 ) {
      TabSpace += int( 5'940 / iInfoWidth );
    } else if ( WindowWidth == 800 || WindowWidth == 1'024 ) {
      TabSpace += int( 5'200 / iInfoWidth );
    }
  } else {
    g_pNewUISystem->Hide( SEASON3B::INTERFACE_ITEM_EXPLANATION );
    return true;
  ...
  float fNumAdd = 1.0f;
  if ( !( g_iItemInfo[0][_COLUMN_TYPE_ATTMIN] <= 0 ||
          ( ItemHelp >= ITEM_ETC && ItemHelp < ITEM_ETC + MAX_ITEM_INDEX ) ) ) {
    ++fNumAdd;
  }
  if ( !( g_iItemInfo[0][_COLUMN_TYPE_ATTMAX] <= 0 ||
          ( ItemHelp >= ITEM_ETC && ItemHelp < ITEM_ETC + MAX_ITEM_INDEX ) ) ) {
    ++fNumAdd;
  }
  ...
  if ( HIBYTE( mixLevel)<=3 )
  {
      ...
      if ( g_csItemOption.GetSetItemName( Text, ItemHelp, 1 ) )
      {
          ...
          swprintf(TextList[TextNum],"%ls %ls %ls:(%ls+%d)", Text, GlobalText[1089], GlobalText[591], GlobalText[1090],
  HIBYTE( mixLevel ) );TextNum++;
      }
  }
