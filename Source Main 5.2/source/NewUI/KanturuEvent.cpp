...
void SEASON3B::CNewUIKanturu2ndEnterNpc::CreateMessageBox( BYTE btResult )
{
  ...
  if ( btResult == POPUP_FAILED || btResult == POPUP_FAILED2 ) {
    wcscpy( strMessage, GlobalText[2'170] );
  } else if ( btResult == POPUP_UNIRIA ) {
    wcscpy( strMessage, GlobalText[569] );
  } else if ( btResult == POPUP_CHANGERING ) {
    wcscpy( strMessage, GlobalText[2'175] );
  } else if ( btResult == POPUP_NOT_HELPER ) {
    wcscpy( strMessage, GlobalText[2'176] );
  } else {
    wcscpy( strMessage, GlobalText[2'170 + btResult] );
  }
                                                                ...
                                                                BYTE btUserCount, int iRemainTime )
{
  ...
  if ( btState == KANTURU_STATE_TOWER ) {
    if ( btDetailState == KANTURU_TOWER_REVITALIXATION || btDetailState == KANTURU_TOWER_NOTIFY ) {
      wcscpy( m_strSubject, GlobalText[2'149] );
      wcscpy( m_strStateText[0], GlobalText[2'150] );
      swprintf( m_strStateText[1], GlobalText[2'151], iRemainTime / 3'600 );
      m_iStateTextNum = 2;
    } else {
      wcscpy( m_strSubject, GlobalText[2'174] );
      wcscpy( m_strStateText[0], GlobalText[2'160] );
      wcscpy( m_strStateText[1], GlobalText[2'161] );
      m_iStateTextNum = 2;
    }
    ...
    if ( btDetailState != KANTURU_MAYA_DIRECTION_STANBY1 && btDetailState != KANTURU_MAYA_DIRECTION_STANBY2 &&
      ...
      wcscpy( m_strSubject, GlobalText[2'152] );
      swprintf( m_strStateText[0], GlobalText[2'153], btUserCount );
    } else {
      wcscpy( m_strSubject, GlobalText[2'163] );

      if ( btDetailState == KANTURU_MAYA_DIRECTION_STANBY1 ) {
        if ( btUserCount < 15 ) {
          wcscpy( m_strStateText[0], GlobalText[2'164] );
        } else if ( btUserCount == 15 ) {
          wcscpy( m_strStateText[0], GlobalText[2'172] );
        } else {
        ...
        if ( btUserCount < 15 ) {
          swprintf( m_strStateText[0], GlobalText[2'165], btUserCount );
          swprintf( m_strStateText[1], GlobalText[2'167], 15 - btUserCount );
          m_iStateTextNum = 2;
        } else if ( btUserCount == 15 ) {
          wcscpy( m_strStateText[0], GlobalText[2'168] );
          m_iStateTextNum = 1;
        }
      } else if ( btDetailState == KANTURU_MAYA_DIRECTION_STANBY3 ) {
        if ( btUserCount < 15 ) {
          swprintf( m_strStateText[0], GlobalText[2'166], btUserCount );
          swprintf( m_strStateText[1], GlobalText[2'167], 15 - btUserCount );
          m_iStateTextNum = 2;
        } else if ( btUserCount == 15 ) {
          wcscpy( m_strStateText[0], GlobalText[2'168] );
    ...
    if ( btDetailState == KANTURU_MAYA_DIRECTION_NOTIFY || btDetailState == KANTURU_MAYA_DIRECTION_MONSTER1 ||
      ...
      swprintf( m_strStateText[1], GlobalText[2'154], btUserCount );
      m_iStateTextNum = 2;
      ...
      swprintf( m_strStateText[1], GlobalText[2'155], btUserCount );
      m_iStateTextNum = 2;
      ...
      swprintf( m_strStateText[1], GlobalText[2'156], btUserCount );
      m_iStateTextNum = 2;
    ...
    wcscpy( m_strSubject, GlobalText[2'152] );
    swprintf( m_strStateText[0], GlobalText[2'153], btUserCount );
    swprintf( m_strStateText[1], GlobalText[2'157], btUserCount );
    m_iStateTextNum = 2;
  } else if ( btState == KANTURU_STATE_STANDBY ) {
    wcscpy( m_strSubject, GlobalText[2'158] );
    if ( btDetailState == 1 ) // STANBY_START
    {
      swprintf( m_strStateText[0], GlobalText[2'159], iRemainTime / 60 );
    } else // STANBY_NONE || STANBY_NOTIFY || STANBY_END || STANBY_ENDCYCLE
    {
      swprintf( m_strStateText[0], GlobalText[2'162] );
    }
    swprintf( m_strStateText[1], GlobalText[2'160] );
    swprintf( m_strStateText[2], GlobalText[2'161] );
    m_iStateTextNum = 3;
  } else {
    wcscpy( m_strSubject, GlobalText[2'170] );
  }
...
{
  m_BtnRefresh.ChangeText( GlobalText[2'148] );
  ...
  m_BtnEnter.ChangeText( GlobalText[2'147] );
  ...
  m_BtnClose.ChangeText( GlobalText[1'002] );
...
void SEASON3B::CNewUIKanturuInfoWindow::RenderInfo()
{
  ...
  wchar_t strText[256];
  swprintf( strText, GlobalText[2'180], UserCount );
  g_pRenderText->SetBgColor( 0 );
  ...
  if ( g_Direction.m_CKanturu.m_iMayaState == KANTURU_MAYA_DIRECTION_MAYA1 ||
    ...
    g_pRenderText->RenderText( m_Pos.x + 10, m_Pos.y + 35, GlobalText[2'182] );
  } else {
    swprintf( strText, GlobalText[2'183], MonsterCount );
    g_pRenderText->RenderText( m_Pos.x + 10, m_Pos.y + 35, strText );
  }

  int iCurrentTime = ( GetTickCount() - m_dwSyncTime ) / 1'000;
