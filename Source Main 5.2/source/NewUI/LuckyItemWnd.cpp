...
void CNewUILuckyItemWnd::Render_Frame( void )
{
  ...
  for ( i = 0; i < eFrame_END; i++ ) {
    RenderImage( m_sFrame[i].s_Img.s_nImgIndex, m_sFrame[i].s_ptPos.x, m_sFrame[i].s_ptPos.y, m_sFrame[i].s_Img.s_fWid,
                 m_sFrame[i].s_Img.s_fHgt );
  }
  ...
  for ( i = 0; i < m_nTextMaxLine; i++ ) {
    ...
    g_pRenderText->RenderText( m_ptPos.x + 10, fTextY + 11.0f * i, GlobalText[m_sText[i].s_nTextIndex], m_fSizeX - 20,
                               0, m_sText[i].s_nLine );
  }
}
...
void CNewUILuckyItemWnd::OpeningProcess( void )
{
  ...
  switch ( m_eType ) {
    case eLuckyItemType_Trade:
      swprintf( m_szSubject, L"%ls", GlobalText[3'288] );
      AddText( 3'291, 0xFF0000FF, RT3_SORT_LEFT ), AddText( 0 ), AddText( 0 ), AddText( 3'292 ), AddText( 3'293 ),
          AddText( 3'294 );
      AddText( 0 ), AddText( 0 );
      AddText( 2'223, 0xFF00FFFF );
      AddText( 0 );
      AddText( 3'295, 0xFF0000FF ), AddText( 3'296, 0xFF0000FF );
      m_BtnMix.ChangeToolTipText( GlobalText[591], true ); // 조합
      break;
    case eLuckyItemType_Refinery:
      swprintf( m_szSubject, L"%ls", GlobalText[3'289] );
      AddText( 2'346, 0xFF0000FF, RT3_SORT_LEFT ), AddText( 0 ), AddText( 0 );
      AddText( 3'300 ), AddText( 3'301 );
      AddText( 0 ), AddText( 0 ), AddText( 0 );
      AddText( 3'302, 0xFF0000FF );
      m_BtnMix.ChangeToolTipText( GlobalText[2'061], true ); // 제련
      break;
  }
}
...
bool CNewUILuckyItemWnd::Process_BTN_Action( void )
{
  ...
  if ( !Check_LuckyItem_InWnd() ) {
    g_pChatListBox->AddText( L"", GlobalText[1'817], SEASON3B::TYPE_ERROR_MESSAGE );
    return false;
  }
  if ( !Check_LuckyItem( m_pNewInventoryCtrl->GetItem( 0 ) ) ) {
    g_pChatListBox->AddText( L"", GlobalText[1'812], SEASON3B::TYPE_ERROR_MESSAGE );
    return false;
  }
  ...
  {
    g_pChatListBox->AddText( L"", GlobalText[1'815], SEASON3B::TYPE_ERROR_MESSAGE );
    return false;
  }
