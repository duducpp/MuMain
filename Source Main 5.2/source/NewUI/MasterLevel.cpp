...
bool SEASON3B::CNewUIMasterLevel::Create( CNewUIManager* pNewUIMng )
{
  ...
  this->m_CloseBT.ChangeToolTipText( GlobalText[1'002] );
...
void SEASON3B::CNewUIMasterLevel::OpenMasterSkillTreeData( wchar_t* path )
{
  ...
  if ( dwCheckSum != GenerateCheckSum2( Buffer, 12'288, 0x2BC1 ) ) {
...
void SEASON3B::CNewUIMasterLevel::SetMasterType( CLASS_TYPE Class )
{
  ...
  switch ( Class ) {
    ...
    case CLASS_GRANDMASTER:
      this->CategoryTextIndex  = 1'751;
      this->ClassNameTextIndex = 1'669;
      break;
    case CLASS_KNIGHT:
    case CLASS_BLADEKNIGHT:
    case CLASS_BLADEMASTER:
      this->CategoryTextIndex  = 1'755;
      this->ClassNameTextIndex = 1'668;
      break;
    case CLASS_ELF:
    case CLASS_MUSEELF:
    case CLASS_HIGHELF:
      this->CategoryTextIndex  = 1'759;
      this->ClassNameTextIndex = 1'670;
      break;
    case CLASS_DARK:
    case CLASS_DUELMASTER:
      this->CategoryTextIndex  = 1'763;
      this->ClassNameTextIndex = 1'671;
      break;
    case CLASS_DARK_LORD:
    case CLASS_LORDEMPEROR:
      this->CategoryTextIndex  = 1'767;
      this->ClassNameTextIndex = 1'672;
      break;
    case CLASS_SUMMONER:
    case CLASS_BLOODYSUMMONER:
    case CLASS_DIMENSIONMASTER:
      this->CategoryTextIndex  = 3'136;
      this->ClassNameTextIndex = 1'689;
      break;
    case CLASS_RAGEFIGHTER:
    case CLASS_TEMPLENIGHT:
      this->CategoryTextIndex  = 3'330;
      this->ClassNameTextIndex = 3'151;
      break;
    default:
      return;
  }
...
void SEASON3B::CNewUIMasterLevel::RenderText() const
{
  ...
  if ( SEASON3B::IsPress( VK_LBUTTON ) == false && SEASON3B::CheckMouseIn( 458, 11, 81, 10 ) == true ) {
    ...
    TextBold[0]      = 0;
    TextListColor[0] = 0;
    swprintf( TextList[0], L"%I64d / %I64d", Master_Level_Data.lMasterLevel_Experince,
  ...
  swprintf( Buffer, GlobalText[1'746], Master_Level_Data.nMLevel );

  g_pRenderText->RenderText( 275, 11, Buffer, 0, 0, 1, 0 );

  swprintf( Buffer, GlobalText[1'747], Master_Level_Data.nMLevelUpMPoint );
  ...
  if ( Master_Level_Data.lNext_MasterLevel_Experince != 0 ) {
    ...
    const __int64 iData_Master = // A
        ...
        ( ( (__int64)9 + (__int64)iTOverLevel ) * (__int64)iTOverLevel * (__int64)iTOverLevel * (__int64)1'000 );

    iBaseExperience = ( iData_Master - (__int64)3'892'250'000 ) / (__int64)2; // B
    ...
    swprintf( Buffer, GlobalText[3'335], fExp / fNeedExp * 100.0 );
...
void SEASON3B::CNewUIMasterLevel::RenderToolTip()
{
  for ( auto it = this->map_masterData.begin(); it != this->map_masterData.end(); it++ ) {
    ...
    if ( skillLevel != 0 && skillLevel < it->second.MaxLevel ) {
      swprintf( buffer, GlobalText[3'328] );
    ...
    if ( skillLevel < it->second.MaxLevel ) {
      swprintf( buffer, GlobalText[3'329] );
...
bool SEASON3B::CNewUIMasterLevel::CheckAttributeArea( const _MASTER_SKILLTREE_DATA& skillData )
{
  ...
  if ( !g_csItemOption.IsNonWeaponSkillOrIsSkillEquipped( skillData.Skill ) ) {
    SEASON3B::CreateOkMessageBox( GlobalText[3'336] );
    return true;
  }
  ...
  if ( !this->CheckParentSkill( skillData ) ||
    ...
    SEASON3B::CreateOkMessageBox( GlobalText[3'327] );
                                                   ...
                                                   BYTE skillLevel )
{
  ...
  if ( skillLevel >= skillData.MaxLevel ) {
    SEASON3B::CreateOkMessageBox( GlobalText[3'326] );
    return false;
  }
  ...
  swprintf( Buffer, GlobalText[3'326], skillData.RequiredPoints - mLevelUpPoint );
