        ...
        if ( m_SocketListBox.GetLineNum() == 0 ) {
          ...
          for ( int i = 0; i < g_MixRecipeMgr.GetFirstItemSocketCount(); ++i ) {
            if ( g_MixRecipeMgr.GetFirstItemSocketSeedID( i ) == SOCKET_EMPTY ) {
              swprintf( szSocketText, GlobalText[2'652] );
            } else {
...
void CNewUIMixInventory::RenderFrame()
{
  ...
  switch ( g_MixRecipeMgr.GetMixInventoryType() ) {
    ...
    case SEASON3A::MIXTYPE_GOBLIN_CHAOSITEM:
      ...
      break;
    case SEASON3A::MIXTYPE_GOBLIN_ADD380:
      swprintf( szText, L"%ls", GlobalText[2'193] );
      break;
    case SEASON3A::MIXTYPE_CASTLE_SENIOR:
      fLine_y += 5.0f;
      swprintf( szText, L"%ls", GlobalText[1'640] );
      break;
    case SEASON3A::MIXTYPE_TRAINER:
      swprintf( szText, L"%ls", GlobalText[1'205] );
      break;
    case SEASON3A::MIXTYPE_OSBOURNE:
      fLine_y += 5.0f;
      swprintf( szText, L"%ls", GlobalText[2'061] );
      break;
    case SEASON3A::MIXTYPE_JERRIDON:
      fLine_y += 5.0f;
      swprintf( szText, L"%ls", GlobalText[2'062] );
      break;
    case SEASON3A::MIXTYPE_ELPIS:
      fLine_y += 5.0f;
      swprintf( szText, L"%ls", GlobalText[2'063] );
      break;
    case SEASON3A::MIXTYPE_CHAOS_CARD:
      fLine_y += 5.0f;
      swprintf( szText, L"%ls", GlobalText[2'265] );
      break;
    case SEASON3A::MIXTYPE_CHERRYBLOSSOM:
      fLine_y += 5.0f;
      swprintf( szText, L"%ls", GlobalText[2'563] );
      break;
    case SEASON3A::MIXTYPE_EXTRACT_SEED:
      fLine_y += 5.0f;
      swprintf( szText, L"%ls", GlobalText[2'660] );
      break;
    case SEASON3A::MIXTYPE_SEED_SPHERE:
      fLine_y += 5.0f;
      swprintf( szText, L"%ls", GlobalText[2'661] );
      break;
    case SEASON3A::MIXTYPE_ATTACH_SOCKET:
      fLine_y += 5.0f;
      swprintf( szText, L"%ls", GlobalText[2'662] );
      break;
    case SEASON3A::MIXTYPE_DETACH_SOCKET:
      fLine_y += 5.0f;
      swprintf( szText, L"%ls", GlobalText[2'663] );
      break;
    default:
      fLine_y += 5.0f;
  ...
  switch ( g_MixRecipeMgr.GetMixInventoryType() ) {
    ...
    case SEASON3A::MIXTYPE_GOBLIN_ADD380:
    case SEASON3A::MIXTYPE_TRAINER:
      swprintf( szText, GlobalText[1'623], g_nChaosTaxRate );
      g_pRenderText->RenderText( fPos_x, fPos_y + fLine_y, szText, 160.0f, 0, RT3_SORT_CENTER );
      break;
    default:
      fLine_y -= 5;
      ...
      if ( g_MixRecipeMgr.IsReadyToMix() && g_MixRecipeMgr.GetPlusChaosRate() > 0 &&
        ...
        switch ( g_MixRecipeMgr.GetMixInventoryType() ) {
          ...
          case SEASON3A::MIXTYPE_SEED_SPHERE:
            ...
            break;
          case SEASON3A::MIXTYPE_TRAINER:
            swprintf( szText, GlobalText[584], GlobalText[1'212], g_MixRecipeMgr.GetSuccessRate() );
            break;
          case SEASON3A::MIXTYPE_OSBOURNE:
            swprintf( szText, GlobalText[584], GlobalText[2'061], g_MixRecipeMgr.GetSuccessRate() );
            break;
          case SEASON3A::MIXTYPE_ELPIS:
            swprintf( szText, GlobalText[584], GlobalText[2'063], g_MixRecipeMgr.GetSuccessRate() );
            break;
        }
        g_pRenderText->RenderText( fPos_x, fPos_y + fLine_y, szText );
      }
  ...
  switch ( g_MixRecipeMgr.GetMixInventoryType() ) {
    ...
    case SEASON3A::MIXTYPE_DETACH_SOCKET: {
      ...
      if ( g_MixRecipeMgr.IsReadyToMix() && g_MixRecipeMgr.GetCurRecipe()->m_bRequiredZenType == 'C' ) {
        swprintf( szText, GlobalText[1'636], szGoldText2, szGoldText );
      } else {
        swprintf( szText, GlobalText[1'622], szGoldText2, szGoldText );
      }
  ...
  if ( g_MixRecipeMgr.GetMostSimilarRecipe() != NULL ) {
    ...
    if ( !g_MixRecipeMgr.IsReadyToMix() && g_MixRecipeMgr.GetMostSimilarRecipeName( szTempText[0], 1 ) == TRUE ) {
      swprintf( szText, GlobalText[2'334], szTempText[0] );
    ...
    swprintf( szText, GlobalText[2'346] );
    g_pRenderText->RenderText( fPos_x, fPos_y + fLine_y, szText );
    iTextPos_y++;
  } else {
    g_pRenderText->SetTextColor( 255, 50, 20, 255 );
    ...
    swprintf( szText, GlobalText[2'334], L" " );
    g_pRenderText->RenderText( fPos_x, fPos_y + fLine_y, szText );
  ...
  switch ( g_MixRecipeMgr.GetMixInventoryType() ) {
    case SEASON3A::MIXTYPE_TRAINER:
      m_BtnMix.ChangeToolTipText( GlobalText[1'212], true );
      break;
    case SEASON3A::MIXTYPE_OSBOURNE:
      m_BtnMix.ChangeToolTipText( GlobalText[2'061], true );
      break;
    case SEASON3A::MIXTYPE_JERRIDON:
      m_BtnMix.ChangeToolTipText( GlobalText[2'062], true );
      break;
    case SEASON3A::MIXTYPE_ELPIS:
      m_BtnMix.ChangeToolTipText( GlobalText[2'063], true );
      break;
    case SEASON3A::MIXTYPE_EXTRACT_SEED:
      m_BtnMix.ChangeToolTipText( GlobalText[2'660], true );
      break;
    case SEASON3A::MIXTYPE_SEED_SPHERE:
      m_BtnMix.ChangeToolTipText( GlobalText[2'661], true );
      break;
    case SEASON3A::MIXTYPE_ATTACH_SOCKET:
      m_BtnMix.ChangeToolTipText( GlobalText[2'662], true );
      break;
    case SEASON3A::MIXTYPE_DETACH_SOCKET:
      m_BtnMix.ChangeToolTipText( GlobalText[2'663], true );
      break;
    default:
      m_BtnMix.ChangeToolTipText( GlobalText[591], true );
      break;
...
void CNewUIMixInventory::RenderMixDescriptions( float fPos_x, float fPos_y )
{
  ...
  switch ( g_MixRecipeMgr.GetMixInventoryType() ) {
    ...
    case SEASON3A::MIXTYPE_CASTLE_SENIOR: {
      ...
      for ( int i = 0; i < 6; ++i ) {
        g_pRenderText->RenderText( fPos_x, fPos_y + 270 + i * 13, GlobalText[1'644 + i], 160.0f, 0, RT3_SORT_CENTER );
      }
    } break;
    case SEASON3A::MIXTYPE_TRAINER:
      break;
    case SEASON3A::MIXTYPE_OSBOURNE: {
      ...
      swprintf( szText, GlobalText[2'220] );
      g_pRenderText->RenderText( fPos_x, fPos_y + 250 + 0 * 13, szText, 160.0f, 0, RT3_SORT_CENTER );
      swprintf( szText, GlobalText[2'221] );
      g_pRenderText->RenderText( fPos_x, fPos_y + 250 + 1 * 13, szText, 160.0f, 0, RT3_SORT_CENTER );
      swprintf( szText, GlobalText[2'084], GlobalText[2'061], GlobalText[2'082] );
      g_pRenderText->RenderText( fPos_x, fPos_y + 250 + 2 * 13, szText, 160.0f, 0, RT3_SORT_CENTER );
      swprintf( szText, GlobalText[2'098] );
      g_pRenderText->RenderText( fPos_x, fPos_y + 250 + 3 * 13, szText, 160.0f, 0, RT3_SORT_CENTER );
      g_pRenderText->SetTextColor( 255, 0, 0, 255 );
      swprintf( szText, GlobalText[2'222] );
      g_pRenderText->RenderText( fPos_x, fPos_y + 250 + 4 * 13, szText, 160.0f, 0, RT3_SORT_CENTER );
    } break;
    case SEASON3A::MIXTYPE_JERRIDON: {
      g_pRenderText->SetBgColor( 0, 0, 0, 0 );
      g_pRenderText->SetTextColor( 255, 255, 255, 255 );
      swprintf( szText, GlobalText[2'102] );
      g_pRenderText->RenderText( fPos_x, fPos_y + 250 + 0 * 13, szText, 160.0f, 0, RT3_SORT_CENTER );
      swprintf( szText, GlobalText[2'103] );
      g_pRenderText->RenderText( fPos_x, fPos_y + 250 + 1 * 13, szText, 160.0f, 0, RT3_SORT_CENTER );
      swprintf( szText, GlobalText[2'104] );
      g_pRenderText->RenderText( fPos_x, fPos_y + 250 + 2 * 13, szText, 160.0f, 0, RT3_SORT_CENTER );
      swprintf( szText, GlobalText[2'088] );
      g_pRenderText->RenderText( fPos_x, fPos_y + 250 + 3 * 13, szText, 160.0f, 0, RT3_SORT_CENTER );
      swprintf( szText, GlobalText[2'100] );
      g_pRenderText->RenderText( fPos_x, fPos_y + 250 + 4 * 13, szText, 160.0f, 0, RT3_SORT_CENTER );
      swprintf( szText, GlobalText[2'101] );
      g_pRenderText->RenderText( fPos_x, fPos_y + 250 + 5 * 13, szText, 160.0f, 0, RT3_SORT_CENTER );
    } break;
    case SEASON3A::MIXTYPE_ELPIS:
      g_pRenderText->SetBgColor( 0, 0, 0, 0 );
      g_pRenderText->SetTextColor( 255, 255, 255, 255 );
      swprintf( szText, GlobalText[2'071] );
      g_pRenderText->RenderText( fPos_x, fPos_y + 250 + 0 * 13, szText, 160.0f, 0, RT3_SORT_CENTER );
      swprintf( szText, GlobalText[2'095] );
      g_pRenderText->RenderText( fPos_x, fPos_y + 250 + 1 * 13, szText, 160.0f, 0, RT3_SORT_CENTER );
      swprintf( szText, GlobalText[2'096] );
      g_pRenderText->RenderText( fPos_x, fPos_y + 250 + 2 * 13, szText, 160.0f, 0, RT3_SORT_CENTER );
      break;
    case SEASON3A::MIXTYPE_CHAOS_CARD: {
      g_pRenderText->SetBgColor( 0, 0, 0, 0 );
    ...
    case SEASON3A::MIXTYPE_ATTACH_SOCKET:
      g_pRenderText->SetBgColor( 0, 0, 0, 0 );
      g_pRenderText->SetTextColor( 200, 200, 200, 255 );
      swprintf( szText, GlobalText[2'674] );
    ...
    case SEASON3A::MIXTYPE_DETACH_SOCKET:
      g_pRenderText->SetBgColor( 0, 0, 0, 0 );
      g_pRenderText->SetTextColor( 200, 200, 200, 255 );
      swprintf( szText, GlobalText[2'675] );
      g_pRenderText->RenderText( fPos_x, fPos_y + 280 + 0 * 13, szText, 160.0f, 0, RT3_SORT_CENTER );
      m_SocketListBox.Render();
      break;
    default:
...
bool CNewUIMixInventory::Mix()
{
  ...
  if ( iLevel < g_MixRecipeMgr.GetCurRecipe()->m_iRequiredLevel ) {
    ...
    swprintf( szText, GlobalText[2'347], g_MixRecipeMgr.GetCurRecipe()->m_iRequiredLevel, szText2 );
    g_pSystemLogBox->AddText( szText, SEASON3B::TYPE_ERROR_MESSAGE );
    return false;
  }
  ...
  if ( g_MixRecipeMgr.GetMixInventoryType() == SEASON3A::MIXTYPE_ATTACH_SOCKET ) {
    ...
    for ( int i = 0; i < m_SocketListBox.GetLineNum(); ++i ) {
      ...
      if ( bySocketSeedID != SOCKET_EMPTY ) {
        ...
        if ( bySocketSeedID == bySeedSphereID ) {
          g_pSystemLogBox->AddText( GlobalText[2'683], SEASON3B::TYPE_ERROR_MESSAGE );
          return false;
        }
      }
    }
    ...
    if ( m_SocketListBox.SLGetSelectLineNum() == 0 ) {
      g_pSystemLogBox->AddText( GlobalText[2'676], SEASON3B::TYPE_ERROR_MESSAGE );
      return false;
      ...
      g_pSystemLogBox->AddText( GlobalText[2'677], SEASON3B::TYPE_ERROR_MESSAGE );
      return false;
    }
    ...
    int iSelectedLine = m_SocketListBox.GetLineNum() - m_SocketListBox.SLGetSelectLineNum();
    if ( m_SocketListBox.SLGetSelectLineNum() == 0 ) {
      g_pSystemLogBox->AddText( GlobalText[2'678], SEASON3B::TYPE_ERROR_MESSAGE );
      return false;
    } else if ( iSelectedLine > g_MixRecipeMgr.GetFirstItemSocketCount() ||
                g_MixRecipeMgr.GetFirstItemSocketSeedID( iSelectedLine ) == SOCKET_EMPTY ) {
      g_pSystemLogBox->AddText( GlobalText[2'679], SEASON3B::TYPE_ERROR_MESSAGE );
      return false;
    }
    g_MixRecipeMgr.SetMixSubType( iSelectedLine );
  }
  ...
  if ( g_MixRecipeMgr.GetTotalChaosCharmCount() > 0 && g_MixRecipeMgr.GetTotalCharmCount() > 0 ) {
    g_pSystemLogBox->AddText( GlobalText[3'286], SEASON3B::TYPE_ERROR_MESSAGE );
    return FALSE;
  }
  ...
  if ( m_pNewInventoryCtrl && pPickedItem ) {
    ...
    if ( GetMixState() == MIX_READY && g_MixRecipeMgr.IsMixSource( pPickedItem->GetItem() ) &&
      ...
      if ( SEASON3B::IsPress( VK_LBUTTON ) ) {
        ...
        int iTargetIndex = pPickedItem->GetTargetLinealPos( m_pNewInventoryCtrl );
        if ( iTargetIndex != -1 && m_pNewInventoryCtrl->CanMove( iTargetIndex, pItemObj ) ) {
          SendRequestEquipmentItem( STORAGE_TYPE::INVENTORY, iSourceIndex, pItemObj, iCurInventory, iTargetIndex );
          return true;
        }
      }
    } else {
