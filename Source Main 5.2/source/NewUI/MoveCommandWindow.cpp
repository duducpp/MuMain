      ...
      if ( wcscmp( ( *li )->_ReqInfo.szMainMapName, GlobalText[55] ) == 0 ) {
        ...
        if ( pEquipedHelper->Type == ITEM_HORN_OF_UNIRIA || pEquipedHelper->Type == ITEM_HORN_OF_DINORANT ) {
          ...
          ( *li )->_bCanMove = true;
                  ...
                  ( wcscmp( ( *li )->_ReqInfo.szMainMapName, GlobalText[2'686] ) == 0 ) ) {
        ( *li )->_bCanMove = false;
      } else {
        ( *li )->_bCanMove = true;
      }
...
void SEASON3B::CNewUIMoveCommandWindow::RenderFrame()
{
  ...
  g_pRenderText->RenderText( m_StrifePos.x, m_StartUISubjectName.y + 20, GlobalText[2'988], 0, 0, RT3_WRITE_CENTER );
...
bool SEASON3B::CNewUIMoveCommandWindow::Render()
{
  ...
  for ( int i = 0; i < m_iRenderEndTextIndex; i++, li++ ) {
    ...
    if ( ( *li )->_bCanMove == true ) {
      ...
      if ( ( *li )->_bStrife ) {
        g_pRenderText->RenderText( m_StrifePos.x, iY, GlobalText[2'987], 0, 0, RT3_WRITE_CENTER );
      }
  ...
  g_pRenderText->SetTextColor( 255, 255, 255, 255 );
  g_pRenderText->RenderText( m_MapNameUISize.x / 2, m_MapNameUISize.y - m_iRealFontHeight - 5, GlobalText[1'002], 0, 0,
                             RT3_WRITE_CENTER );
  DisableAlphaBlend();
  return true;
}
