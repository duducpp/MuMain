...
bool CNewUIMyInventory::UpdateMouseEvent()
{
  ...
  if ( pPickedItem && IsPress( VK_LBUTTON ) && CheckMouseIn( 0, 0, GetScreenWidth(), 429 ) ) {
    ...
    if ( pItemObj && pItemObj->Jewel_Of_Harmony_Option != 0 ) {
      g_pSystemLogBox->AddText( GlobalText[2'217], TYPE_ERROR_MESSAGE );
    ...
    if ( pItemObj && IsDropBan( pItemObj ) ) {
      g_pSystemLogBox->AddText( GlobalText[1'915], TYPE_ERROR_MESSAGE );
      CNewUIInventoryCtrl::BackupPickedItem();
...
void CNewUIMyInventory::RenderSocketOption()
{
  ...
  wchar_t strText[128];
  swprintf( strText, L"[%ls]", GlobalText[2'651] );
...
void CNewUIMyInventory::SetButtonInfo()
{
  ...
  m_BtnMyShop.ChangeToolTipText( GlobalText[1'125], true );
  ...
  m_BtnExpand.ChangeToolTipText( GlobalText[3'322], true );
}
...
void CNewUIMyInventory::RenderEquippedItem()
{
  for ( int i = 0; i < MAX_EQUIPMENT_INDEX; i++ ) {
    ...
    RenderImage( m_EquipmentSlots[i].dwBgImage, m_EquipmentSlots[i].x, m_EquipmentSlots[i].y, m_EquipmentSlots[i].width,
...
bool CNewUIMyInventory::EquipmentWindowProcess()
{
  if ( m_iPointedSlot != -1 && IsRelease( VK_LBUTTON ) ) {
    if ( CNewUIPickedItem* pPickedItem = CNewUIInventoryCtrl::GetPickedItem() ) {
      ...
      if ( pItemObj->bPeriodItem && pItemObj->bExpiredPeriod ) {
        g_pSystemLogBox->AddText( GlobalText[2'285], SEASON3B::TYPE_ERROR_MESSAGE );
        CNewUIInventoryCtrl::BackupPickedItem();
      ...
      if ( g_ChangeRingMgr->CheckChangeRing( pPickedItem->GetItem()->Type ) ) {
        ...
        if ( g_ChangeRingMgr->CheckChangeRing( pItemRingLeft->Type ) ||
          ...
          g_pSystemLogBox->AddText( GlobalText[2'285], TYPE_ERROR_MESSAGE );
          CNewUIInventoryCtrl::BackupPickedItem();
  ...
  if ( IsRelease( VK_RBUTTON ) ) {
    ...
    if ( GetRepairMode() != REPAIR_MODE_ON && EquipmentItem == false && pPickedItem == nullptr && iSourceIndex != -1 ) {
      ResetMouseRButton();
      ...
      if ( pEquippedItem->Type >= 0 ) {
        ...
        if ( emptySlotIndex != -1 ) {
          ...
          SendRequestEquipmentItem( STORAGE_TYPE::INVENTORY, iSourceIndex, pEquippedItem, STORAGE_TYPE::INVENTORY,
                                    emptySlotIndex );
          return true;
        }
      }
...
bool CNewUIMyInventory::TryConsumeItem( CNewUIInventoryCtrl* targetControl, ITEM* pItem, const int iIndex )
{
  ...
  if ( ( pItem->Type >= ITEM_POTION + 78 && pItem->Type <= ITEM_POTION + 82 ) ) {
    ...
    CreateOkMessageBox( GlobalText[2'530], RGBA( 255, 30, 0, 255 ) );
    return false;
  }
  ...
  if ( pItem->Type == ITEM_HELPER + 58 && gCharacterManager.GetBaseClass( Hero->Class ) != CLASS_DARK_LORD ) {
    CreateOkMessageBox( GlobalText[1'905] );
    return true;
  }
  ...
  if ( pItem->Type == ITEM_ARMOR_OF_GUARDSMAN ) {
    if ( IsUnitedMarketPlace() ) {
      ...
      swprintf( szOutputText, L"%ls %ls", GlobalText[3'014], GlobalText[3'015] );
    ...
    if ( Hero->SafeZone == false ) {
      CreateOkMessageBox( GlobalText[2'330] );
      return false;
    }
  ...
  if ( pItem->Type == ITEM_HELPER + 48 ) {
    if ( Hero->SafeZone || gMapManager.InHellas() ) {
      g_pSystemLogBox->AddText( GlobalText[1'238], TYPE_ERROR_MESSAGE );
      return false;
    }
  ...
  if ( pItem->Type == ITEM_INVISIBILITY_CLOAK ) {
    if ( pItem->Level == 0 ) {
      g_pSystemLogBox->AddText( GlobalText[2'089], TYPE_ERROR_MESSAGE );
    } else {
  ...
  if ( ( pItem->Type >= ITEM_SCROLL_OF_POISON && pItem->Type < ITEM_ETC + MAX_ITEM_INDEX ) ||
    ...
    if ( bReadBookGem ) {
      ...
      if ( CharacterAttribute->Level >= ItemAttribute[pItem->Type].RequireLevel && Energy >= pItem->RequireEnergy &&
  ...
  if ( pItem->Type == ITEM_FRUITS ) {
    ...
    if ( Level >= 10 ) {
      ...
      CreateOkMessageBox( GlobalText[1'909] );
      return true;
    }
  ...
  if ( pItem->Type == ITEM_HELPER + 69 ) {
    if ( g_PortalMgr.IsRevivePositionSaved() ) {
      if ( g_PortalMgr.IsPortalUsable() ) {
        ...
        CreateOkMessageBox( GlobalText[2'608] );
      }
    }
    ...
    if ( g_PortalMgr.IsPortalUsable() ) {
      if ( pItem->Durability == 2 ) {
        if ( g_PortalMgr.IsPortalPositionSaved() ) {
          CreateOkMessageBox( GlobalText[2'610] );
        } else {
          g_pMyInventory->SetStandbyItemKey( pItem->Key );
...
void CNewUIMyInventory::ChangeMyShopButtonStateClose()
{
  ...
  m_BtnMyShop.ChangeToolTipText( GlobalText[1'127], true );
}
