...
void SEASON3B::CNewUINPCShop::RenderTexts()
{
  ...
  wchar_t strText[256];
  swprintf( strText, GlobalText[1'623], m_iTaxRate );
...
bool SEASON3B::CNewUINPCShop::InventoryProcess()
{
  ...
  if ( SEASON3B::IsRelease( VK_LBUTTON ) == true && m_pNewInventoryCtrl->CheckPtInRect( MouseX, MouseY ) == true &&
       m_bSellingItem == false ) {
    if ( CharacterMachine->Gold + ItemValue( pItem ) > 2'000'000'000 ) {
      g_pSystemLogBox->AddText( GlobalText[3'148], SEASON3B::TYPE_SYSTEM_MESSAGE );

      return true;
    }

    if ( pItem && pItem->Jewel_Of_Harmony_Option != 0 ) {
      g_pSystemLogBox->AddText( GlobalText[2'211], SEASON3B::TYPE_ERROR_MESSAGE );
