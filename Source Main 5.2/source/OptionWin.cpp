...
void COptionWin::SetPosition( int nXCoord, int nYCoord )
{
  ...
  m_aBtn[OW_BTN_CLOSE].SetPosition(
      m_winBack.GetXPos() + ( m_winBack.GetWidth() - m_aBtn[OW_BTN_CLOSE].GetWidth() ) / 2, m_winBack.GetYPos() + 301 );
...
void COptionWin::RenderControls()
{
  ...
  int            nTextPosY;
  const wchar_t* apszSldText[OW_SLD_MAX] = { GlobalText[389], GlobalText[1'840] };
