...
void CPortalMgr::GetPortalPositionText( wchar_t* pszOut )
{
  ...
  if ( m_iPortalWorld == -1 ) {
    ...
    swprintf( pszOut, L"%ls (%d, %d)", gMapManager.GetMapName( m_iPortalWorld ), m_iPortalPosition_x,
              m_iPortalPosition_y );
  }
}
...
void CPortalMgr::GetRevivePositionText( wchar_t* pszOut )
{
  ...
  if ( m_iReviveWorld == -1 ) {
    ...
    swprintf( pszOut, L"%ls (%d, %d)", gMapManager.GetMapName( m_iReviveWorld ), m_iRevivePosition_x,
              m_iRevivePosition_y );
  }
}
