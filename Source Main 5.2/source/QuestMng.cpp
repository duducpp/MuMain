...
#pragma pack( pop )
  ...
  while ( 0 != ::fread( &sQuestWordsHeader, nSize, 1, fp ) ) {
    ...
    CMultiLanguage::ConvertFromUtf8( szWords, rawWords, 1'024 );
...
void CQuestMng::SetCurQuestProgress( DWORD dwQuestIndex )
{
  ...
  if ( LOWORD( dwQuestIndex ) == 0x00FF ) {
    ...
    g_pSystemLogBox->AddText( GlobalText[2'814], SEASON3B::TYPE_ERROR_MESSAGE );
...
bool CQuestMng::GetRequestRewardText( SRequestRewardText* aDest, int nDestCount, DWORD dwQuestIndex )
{
  ...
  if ( pRequestReward->m_byRequestCount + pRequestReward->m_byGeneralRewardCount + pRequestReward->m_byRandRewardCount +
           3 >
       nDestCount ) {
    return false;
  }
  ...
  wcscpy( aDest[nLine++].m_szText, GlobalText[2'809] );
  ...
  for ( i = 0; i < pRequestReward->m_byRequestCount; ++i, ++nLine ) {
    ...
    switch ( pRequestInfo->m_dwType ) {
      case QUEST_REQUEST_NONE:
        ...
        wcscpy( aDest[nLine].m_szText, GlobalText[1'361] );
        break;
      ...
      case QUEST_REQUEST_PVP_POINT:
        ...
        switch ( pRequestInfo->m_dwType ) {
          ...
          case QUEST_REQUEST_LEVEL:
            ::swprintf( aDest[nLine].m_szText, L"Level: %lu %ls", pRequestInfo->m_dwValue, GlobalText[2'812] );
            break;
          case QUEST_REQUEST_ZEN:
            ::swprintf( aDest[nLine].m_szText, L"Zen : %lu", pRequestInfo->m_dwValue );
            break;
          case QUEST_REQUEST_PVP_POINT:
            swprintf( aDest[nLine].m_szText, GlobalText[3'278],
                      MIN( pRequestInfo->m_dwCurValue, pRequestInfo->m_dwValue ), pRequestInfo->m_dwValue );
            break;
        }
        break;
      ...
      case QUEST_REQUEST_MONSTER:
...

        {
          ...
          swprintf( aDest[nLine].m_szText, L"Mon.: %ls x %lu/%lu", text,
      ...
      case QUEST_REQUEST_SKILL:
        ...
        ::swprintf( aDest[nLine].m_szText, L"Skill: %ls", SkillAttribute[pRequestInfo->m_wIndex].Name );
        break;
      ...
      case QUEST_REQUEST_TUTORIAL:
        ...
        switch ( dwQuestIndex ) {
          case 0x10009:
            ::swprintf( aDest[nLine].m_szText, L"%ls", GlobalText[2'819] );
            break;
          case 0x1000F:
            ::swprintf( aDest[nLine].m_szText, L"%ls", GlobalText[2'820] );
            break;
        }
        break;
  ...
  for ( j = 0; j < 2; ++j ) {
    if ( 0 == j && pRequestReward->m_byGeneralRewardCount ) {
      ::wcscpy( aDest[nLine].m_szText, GlobalText[2'810] );
    } else if ( 1 == j && pRequestReward->m_byRandRewardCount ) {
      swprintf( aDest[nLine].m_szText, GlobalText[3'082], pRequestReward->m_byRandGiveCount );
    } else {
      continue;
    }
    aDest[nLine].m_hFont     = g_hFontBold;
    ...
    for ( ; i < byRewardCount; ++i, ++nLine ) {
      ...
      switch ( pRewardInfo->m_dwType ) {
        ...
        case QUEST_REWARD_ITEM:
          ...
          ::swprintf( aDest[nLine].m_szText, L"Item: %ls x %lu", szItemName, pRewardInfo->m_dwValue );
          break;

        case QUEST_REWARD_BUFF: {
          const BuffInfo buffinfo = g_BuffInfo( (eBuffState)pRewardInfo->m_wIndex );
          ::swprintf( aDest[nLine].m_szText, L"Bonus: %ls x %lu%ls", buffinfo.s_BuffName, pRewardInfo->m_dwValue,
                      GlobalText[2'300] );
        } break;

#ifdef ASG_ADD_GENS_SYSTEM
        case QUEST_REWARD_CONTRIBUTE:
          swprintf( aDest[nLine].m_szText, GlobalText[2'994], pRewardInfo->m_dwValue );
          break;
