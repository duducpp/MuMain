...
void CServerListManager::InsertServer( CServerGroup* pServerGroup, int iConnectIndex, int iServerPercent )
{
  ...
  switch ( pServerInfo->m_byNonPvP ) {
    case 0:
      swprintf( pServerInfo->m_bName, L"%ls-%d %ls", pServerGroup->m_szName, pServerInfo->m_iIndex,
                GlobalText[iTextIndex] );
      break;

    case 1:
      swprintf( pServerInfo->m_bName, L"%ls-%d(Non-PVP) %ls", pServerGroup->m_szName, pServerInfo->m_iIndex,
                GlobalText[iTextIndex] );
      break;

    case 2:
      swprintf( pServerInfo->m_bName, L"%ls-%d(Gold PVP) %ls", pServerGroup->m_szName, pServerInfo->m_iIndex,
                GlobalText[iTextIndex] );
      break;

    case 3:
      swprintf( pServerInfo->m_bName, L"%ls-%d(Gold) %ls", pServerGroup->m_szName, pServerInfo->m_iIndex,
                GlobalText[iTextIndex] );
      break;
  }
