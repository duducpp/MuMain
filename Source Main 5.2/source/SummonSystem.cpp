...
BOOL CSummonSystem::SendRequestSummonSkill( int iSkill, CHARACTER* pCharacter, OBJECT* pObject )
{
  ...
  if ( CheckTile( pCharacter, pObject, fDistance ) ) {
    if ( iTargetKey != -1 ) {
      ...
      SendRequestMagicContinue( iSkill, (BYTE)TargetX, (BYTE)TargetY, (BYTE)( pObject->Angle[2] / 360.f * 256.f ), 0, 0,
                                0, 0 );
    }
...
void CSummonSystem::CreateEquipEffect_WristRing( CHARACTER* pCharacter, int iItemType, int iItemLevel,
                                                 int iItemOption1 )
{
  ...
  switch ( iSkill ) {
    case AT_SKILL_SUMMON_EXPLOSION:
      if ( !SearchEffect( MODEL_SUMMONER_EQUIP_HEAD_SAHAMUTT, pObject, 1 ) ) {
        CreateEffect( MODEL_SUMMONER_EQUIP_HEAD_SAHAMUTT, pObject->Position, pObject->Angle, pObject->Light, 1, pObject,
                      -1 );
      }
      break;
    case AT_SKILL_SUMMON_REQUIEM:
