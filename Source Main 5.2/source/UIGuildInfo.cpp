...
void RenderAppoint()
{
  ...
  g_pRenderText->RenderText( s_ptAppointWindow.x + 20, s_ptAppointWindow.y + 24, GlobalText[1'311], 170, 0,
                             RT3_SORT_CENTER );

  RenderGoldRect( s_ptAppointWindow.x + 20, s_ptAppointWindow.y + 19 + 25 - 5, 170, 20.f,
                  ( s_eAppointType == APPOINT_BATTLEMASTSER ? 1 : 0 ) );
  g_pRenderText->RenderText( s_ptAppointWindow.x + 20, s_ptAppointWindow.y + 24 + 25 - 3, GlobalText[1'312], 170, 0,
                             RT3_SORT_CENTER );

  wchar_t Text[64];
  swprintf( Text, GlobalText[1'314], s_szTargetID,
            ( s_eAppointType == APPOINT_SUBGUILDMASTER ? GlobalText[1'301] : GlobalText[1'302] ) );

  g_pRenderText->RenderText( s_ptAppointWindow.x + 20, s_ptAppointWindow.y + 24 + 25 * 2 + 1, Text, 170, 0,
                             RT3_SORT_CENTER );
  g_pRenderText->RenderText( s_ptAppointWindow.x + 20, s_ptAppointWindow.y + 24 + 25 * 2 + 1 + 18, GlobalText[1'315],
...
CUIGuildInfo::CUIGuildInfo()
{
  ...
  m_AppointButton.Init( nButtonID++, GlobalText[1'307] );
  ...
  m_DisbandButton.Init( nButtonID++, GlobalText[1'308] );
  ...
  m_FireButton.Init( nButtonID++, GlobalText[1'309] );
  ...
  m_BanUnionButton.Init( nButtonID++, GlobalText[1'422] );
  ...
  if ( m_BreakUpGuildButton.DoMouseAction() ) {
    if ( IsGuildMaster() ) {
      if ( !wcscmp( GuildMark[Hero->GuildMarkIndex].GuildName, GuildMark[Hero->GuildMarkIndex].UnionName ) ) {
        wchar_t szText[50];
        wcscpy( szText, GlobalText[1'270] );
        m_dwPopupID = g_pUIPopup->SetPopup( szText, 1, 50, POPUP_OK, NULL );
      } else {
        wchar_t szText[4][100];

        wcscpy( szText[0], GlobalText[1'363] );
        wcscpy( szText[1], GlobalText[1'364] );
        wcscpy( szText[2], GlobalText[1'365] );
        wcscpy( szText[3], GlobalText[1'366] );
...
void CUIGuildInfo::RenderGuildInfoTab()
{
  ...
  if ( IsGuildMaster() ) {
    ...
    if ( Class == CLASS_DARK_LORD ) {
      ...
      swprintf( szTemp, GlobalText[1'362], g_nGuildMemberCount, nCount );
    } else {
      swprintf( szTemp, GlobalText[1'362], g_nGuildMemberCount, CharacterAttribute->Level / 10 );
    }
  } else {
    swprintf( szTemp, GlobalText[1'310], g_nGuildMemberCount );
  }
  g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, szTemp, 140 * g_fScreenRate_x, 0, RT3_SORT_LEFT );

  ptOrigin.y += 13;
  swprintf( szTemp, L"%ls : %ls", GlobalText[1'321], m_szRivalGuildName[0] ? m_szRivalGuildName : GlobalText[1'361] );
  g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, szTemp, 140 * g_fScreenRate_x, 0, RT3_SORT_LEFT );

  ptOrigin.x -= 5;
  ptOrigin.y += 33;
  g_pRenderText->RenderText( ptOrigin.x + 5, ptOrigin.y, GlobalText[1'323], 136 * g_fScreenRate_x, 0, RT3_SORT_LEFT );
    ...
    if ( m_DisbandButton.DoMouseAction() ) {
      if ( GUILDLIST_TEXT* pText = m_GuildMemberListBox.GetSelectedText() ) {
        if ( pText->m_GuildStatus == G_SUB_MASTER || pText->m_GuildStatus == G_BATTLE_MASTER ) {
          ...
          swprintf( szText[0], GlobalText[1'367], pText->m_szID );
          wcscpy( szText[1], GlobalText[1'368] );
    ...
    if ( m_FireButton.DoMouseAction() ) {
      if ( GUILDLIST_TEXT* pText = m_GuildMemberListBox.GetSelectedText() ) {
        if ( pText->m_GuildStatus != G_MASTER ) {
          ...
          wchar_t szText[2][64];
          swprintf( szText[0], GlobalText[1'367], pText->m_szID );
          wcscpy( szText[1], GlobalText[1'369] );
  ...
  if ( Hero->GuildStatus == G_MASTER ) {
    if ( GuildMark[Hero->GuildMarkIndex].UnionName[0] ) {
      if ( m_BreakUnionButton.DoMouseAction() ) {
        if ( !wcscmp( GuildMark[Hero->GuildMarkIndex].GuildName, GuildMark[Hero->GuildMarkIndex].UnionName ) ) {
          wchar_t szText[50];
          wcscpy( szText, GlobalText[1'271] );
          m_dwPopupID = g_pUIPopup->SetPopup( szText, 1, 50, POPUP_OK, NULL );
        } else {
      ...
      if ( m_BanUnionButton.DoMouseAction() ) {
        if ( UNIONGUILD_TEXT* pText = m_UnionListBox.GetSelectedText() ) {
          if ( wcscmp( pText->szName, GuildMark[Hero->GuildMarkIndex].GuildName ) ) {
            ...
            swprintf( szText[0], GlobalText[1'423], pText->szName );
            wcscpy( szText[1], GlobalText[1'369] );
...
void CUIGuildInfo::RenderGuildUnionTab()
{
  ...
  if ( GuildMark[Hero->GuildMarkIndex].UnionName[0] == NULL ) {
    ...
    g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, GlobalText[1'257] );
    ptOrigin.y += 15;
    g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, GlobalText[1'258] );
    ptOrigin.y += 15;
    g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, GlobalText[1'259] );
    ptOrigin.y += 15;
    g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, GlobalText[1'260] );
    ptOrigin.y += 15;
    g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, GlobalText[1'261] );

    g_pRenderText->SetTextColor( 0xFFFFFFFF );
    g_pRenderText->SetBgColor( 0xFF0000CC );

    ptOrigin.y += 25;
    g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, GlobalText[1'262] );
    ptOrigin.y += 15;
    g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, GlobalText[1'263] );
    ptOrigin.y += 15;
    g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, GlobalText[1'264] );
    ptOrigin.y += 20;
    g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, GlobalText[1'265] );
    ptOrigin.y += 15;
    g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, GlobalText[1'266] );
    ptOrigin.y += 15;
    g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y, GlobalText[1'267] );
    ...
    if ( Hero->GuildStatus == G_MASTER ) {
      if ( !wcscmp( GuildMark[Hero->GuildMarkIndex].GuildName, GuildMark[Hero->GuildMarkIndex].UnionName ) ) {
        m_BreakUnionButton.SetCaption( GlobalText[1'324] );
        m_BanUnionButton.SetState( UISTATE_NORMAL );
        m_BanUnionButton.Render();
      } else {
        m_BreakUnionButton.SetCaption( GlobalText[1'325] );
...
void CUIGuildInfo::Render()
{
  ...
  g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y + 4 - ( m_nCurrentTab == 1 ? 1 : 0 ), GlobalText[1'330], 52, 0,
                             RT3_SORT_CENTER );
  ...
  g_pRenderText->RenderText( ptOrigin.x, ptOrigin.y + 4 - ( m_nCurrentTab == 2 ? 1 : 0 ), GlobalText[1'352], 52, 0,
                             RT3_SORT_CENTER );
