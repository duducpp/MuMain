...
CUIWindowMgr::CUIWindowMgr()
{
  ...
  SetTimer( g_hWnd, CHATCONNECT_TIMER, 15 * 1'000, 0 );
...
void CUIWindowMgr::OpenMainWnd( int iPos_x, int iPos_y )
{
  ...
  if ( iLevel < 6 ) {
    if ( g_pSystemLogBox->CheckChatRedundancy( GlobalText[1'067] ) == FALSE ) {
      g_pSystemLogBox->AddText( GlobalText[1'067], SEASON3B::TYPE_SYSTEM_MESSAGE );
    }
    return;
  }
...
void CUIWindowMgr::SetServerEnable( BOOL bFlag )
{
  ...
  if ( bFlag == TRUE ) {
    ...
    if ( m_iFriendMainWindowTitleNumber != 1'066 ) {
      m_iFriendMainWindowTitleNumber = 1'066;
      if ( GetFriendMainWindow() != NULL ) {
        GetFriendMainWindow()->SetTitle( GlobalText[m_iFriendMainWindowTitleNumber] );
      }
    }
...
void CUIBaseWindow::Render()
{
  ...
  if ( CheckOption( UIWINDOWSTYLE_RESIZEABLE ) ) {
    RenderBitmap( BITMAP_INTERFACE_EX + 11, (float)m_iPos_x + m_iWidth - 10, (float)m_iPos_y + m_iHeight - 10, (float)9,
                  (float)9, 0.f, 0.f, 9.f / 16.f, 9.f / 16.f );
  }
    ...
    if ( CheckOption( UIWINDOWSTYLE_TITLEBAR ) && CheckMouseIn( m_iPos_x + m_iWidth - 16, m_iPos_y + 8, 9, 9 ) ) {
      ...
      if ( MouseOnWindow == false && MouseLButtonPop == true ) {
        ...
        MouseLButtonPop = false;
                ...
                CheckMouseIn( m_iPos_x + m_iWidth - ( CheckOption( UIWINDOWSTYLE_MAXBUTTON ) ? 38 : 27 ), m_iPos_y + 8,
                              9, 9 ) ) {
...
void CUIChatWindow::ConnectToChatServer( const wchar_t* pszIP, DWORD dwRoomNumber, DWORD dwTicket )
{
  ...
  _connection = new Connection( pszIP, 55'980, true, &HandlePacketS );
        ...
        case 2:
          if ( m_TextInputBox.IsLocked() == FALSE && m_InvitePalListBox.GetSelectedText() != NULL ) {
            if ( m_PalListBox.GetLineNum() <= 1 )
              ...
              AddChatText( 255, GlobalText[1'074], 1, 0 );
            } else {
              SocketClient->ToGameServer()->SendChatRoomInvitationRequest( m_InvitePalListBox.GetSelectedText()->m_szID,
                                                                           m_dwRoomNumber, GetUIID() );
            }
          }
          break;
        default:
...
void CUIPhotoViewer::RenderPhotoCharacter()
{
  ...
  glViewport2( m_iPos_x * g_fScreenRate_x, m_iPos_y * g_fScreenRate_y, m_iWidth * g_fScreenRate_x,
               141 * g_fScreenRate_y );
  gluPerspective2( 1.f, (float)( m_iWidth * g_fScreenRate_x ) / (float)( 141 * g_fScreenRate_y ), 2'000,
                   20'000 ); // CameraViewNear,CameraViewFar);
...
void CUIPhotoViewer::CopyPlayer()
{
  ...
  } else // 변신 상태
  {
    ...
    for ( i = 0; i < 2; ++i ) {
      if ( CompareItemEqual( &m_PhotoChar.Weapon[i], &CharacterMachine->Equipment[EQUIPMENT_WEAPON_RIGHT + i], -1 ) ==
           FALSE ) {
        bChangeWeapon = TRUE;
        break;
      }
...
void CUIPhotoViewer::Render()
{
  ...
  if ( CheckOption( UIPHOTOVIEWER_CANCONTROL ) ) {
    ...
    if ( m_bHelpEnable == FALSE ) {
      ...
      TextNum = 0;
      swprintf( TextList[TextNum], GlobalText[997] );
      TextListColor[TextNum] = 0;
      TextBold[TextNum]      = false;
      TextNum++;
      swprintf( TextList[TextNum], GlobalText[998] );
      TextListColor[TextNum] = 0;
      TextBold[TextNum]      = false;
      TextNum++;
      swprintf( TextList[TextNum], GlobalText[999] );
      TextListColor[TextNum] = 0;
      TextBold[TextNum]      = false;
      TextNum++;
      SIZE TextSize;
...
{
  SIZE size;
  GetTextExtentPoint32( g_pRenderText->GetFontDC(), GlobalText[1'000], GlobalText.GetStringSize( 1'000 ), &size );
...
void CUILetterWriteWindow::RenderSub()
{
  ...
  g_pRenderText->SetTextColor( 230, 220, 200, 255 );
  GetTextExtentPoint32( g_pRenderText->GetFontDC(), GlobalText[1'000], GlobalText.GetStringSize( 1'000 ), &size );
  g_pRenderText->RenderText( RPos_x( 3 ), RPos_y( 3 ), GlobalText[1'000], size.cx / g_fScreenRate_x, 0,
                             RT3_SORT_RIGHT );
  g_pRenderText->RenderText( RPos_x( 3 ), RPos_y( 18 ), GlobalText[1'005], size.cx / g_fScreenRate_x, 0,
                             RT3_SORT_RIGHT );
        ...
        case 1:
          if ( m_bIsSend == FALSE ) {
            ...
            if ( szMailto[0] == '\0' || wcslen( szMailto ) == 0 ) {
              g_pWindowMgr->AddWindow( UIWNDTYPE_OK, UIWND_DEFAULT, UIWND_DEFAULT, GlobalText[1'006] );
              m_MailtoInputBox.GiveFocus();
              m_iLastTabIndex = 0;
              break;
            }
            ...
            if ( szTitle[0] == '\0' || wcslen( szTitle ) == 0 ) {
              g_pWindowMgr->AddWindow( UIWNDTYPE_OK, UIWND_DEFAULT, UIWND_DEFAULT, GlobalText[1'007] );
              m_TitleInputBox.GiveFocus();
              m_iLastTabIndex = 1;
              break;
            }
            ...
            if ( szTempText[0] == '\0' || wcslen( szTempText ) == 0 ) {
              g_pWindowMgr->AddWindow( UIWNDTYPE_OK, UIWND_DEFAULT, UIWND_DEFAULT, GlobalText[1'008] );
              m_TextInputBox.GiveFocus();
              m_iLastTabIndex = 2;
              break;
            }

            wchar_t szText[1'024] = { 0 };
...
BOOL CUILetterWriteWindow::CloseCheck()
{
  ...
  if ( iTextSize == 0 ) {
    ...
    g_pWindowMgr->AddWindow( UIWNDTYPE_QUESTION, UIWND_DEFAULT, UIWND_DEFAULT, GlobalText[1'009], GetUIID() );
    return FALSE;
  }
}
...
void CUILetterReadWindow::RenderSub()
{
  ...
  swprintf( szMailFrom, GlobalText[1'014], m_LetterHead.m_szID, m_LetterHead.m_szDate, m_LetterHead.m_szTime );
    ...
    case UI_MESSAGE_BTNLCLICK: {
      ...
      switch ( m_WorkMessage.m_iParam1 ) {
        case 1: {
          ...
          swprintf( temp, GlobalText[1'071], g_cdwLetterCost );
          ...
          swprintf( szMailTitle, GlobalText[1'016], m_LetterHead.m_szText );
        ...
        case 2: {
          ...
          wcscat( tempTxt, GlobalText[1'017] );
...
int CFriendList::UpdateFriendList( std::deque<GUILDLIST_TEXT>& pDestData, const wchar_t* pszID )
{
  ...
  for ( m_FriendListIter = m_FriendList.begin(); m_FriendListIter != m_FriendList.end(); ++m_FriendListIter, ++i ) {
    ...
    if ( pszID != NULL && wcsncmp( m_FriendListIter->m_szID, pszID, MAX_ID_SIZE ) == 0 ) {
      iResult = i;
    }
  }
  return iResult;
...
void CUIFriendListTabWindow::RenderSub()
{
  ...
  if ( CheckMouseIn( RPos_x( 0 ) + m_PalListBox.GetColumnPos_x( 0 ), RPos_y( 0 ), m_PalListBox.GetColumnWidth( 0 ),
                     19 ) == TRUE ||
       g_pFriendList->GetCurrentSortType() == 0 ) {
    g_pRenderText->SetTextColor( 255, 255, 255, 255 );
    g_pRenderText->RenderText( RPos_x( 4 ) + m_PalListBox.GetColumnPos_x( 0 ), RPos_y( 3 ), GlobalText[1'021] );
    g_pRenderText->SetTextColor( 230, 220, 200, 255 );
  } else {
    g_pRenderText->RenderText( RPos_x( 4 ) + m_PalListBox.GetColumnPos_x( 0 ), RPos_y( 3 ), GlobalText[1'021] );
  }

  if ( CheckMouseIn( RPos_x( 0 ) + m_PalListBox.GetColumnPos_x( 1 ), RPos_y( 0 ), m_PalListBox.GetColumnWidth( 1 ),
                     19 ) == TRUE ||
       g_pFriendList->GetCurrentSortType() == 1 ) {
    g_pRenderText->SetTextColor( 255, 255, 255, 255 );
    g_pRenderText->RenderText( RPos_x( 4 ) + m_PalListBox.GetColumnPos_x( 1 ), RPos_y( 3 ), GlobalText[1'022] );
    g_pRenderText->SetTextColor( 230, 220, 200, 255 );
  } else {
    g_pRenderText->RenderText( RPos_x( 4 ) + m_PalListBox.GetColumnPos_x( 1 ), RPos_y( 3 ), GlobalText[1'022] );
  }
...
BOOL CUIFriendListTabWindow::HandleMessage()
{
  ...
  switch ( m_WorkMessage.m_iMessage ) {
    ...
    case UI_MESSAGE_BTNLCLICK: {
      ...
      switch ( m_WorkMessage.m_iParam1 ) {
        case 1:
          dwUIID = g_pWindowMgr->AddWindow( UIWNDTYPE_TEXTINPUT, UIWND_DEFAULT, UIWND_DEFAULT, GlobalText[1'023],
                                            GetUIID() );
        ...
        case 2: {
          ...
          wchar_t tempTxt[MAX_TEXT_LENGTH + 1] = { 0 };
          swprintf( tempTxt, L"%ls %ls", GlobalText[1'024],
...
void ReceiveChatRoomConnectResult( DWORD dwWindowUIID, const BYTE* ReceiveBuffer )
{
  ...
  switch ( Data->Result ) {
    case 0x00:
      g_pWindowMgr->AddWindow( UIWNDTYPE_OK_FORCE, UIWND_DEFAULT, UIWND_DEFAULT, GlobalText[1'058] );
      break;
    case 0x01:
      break;
    default:
...
void ReceiveChatRoomChatText( DWORD dwWindowUIID, const BYTE* ReceiveBuffer )
{
  ...
  if ( pChatWindow->GetState() == UISTATE_READY ) {
    ...
    g_pSystemLogBox->AddText( GlobalText[1'063], SEASON3B::TYPE_SYSTEM_MESSAGE );
...
//{
//	// TODO: Change that
//	for (m_ChatRoomSocketMapIter = m_ChatRoomSocketMap.begin(); m_ChatRoomSocketMapIter !=
m_ChatRoomSocketMap.end(); ++m_ChatRoomSocketMapIter)
//	{
...
void CUIChatRoomListTabWindow::Init( const wchar_t* pszTitle, DWORD dwParentID )
{
  ...
  m_HideAllButton.Init( 1, GlobalText[1'025] );
...
void CUIChatRoomListTabWindow::RenderSub()
{
  ...
  g_pRenderText->RenderText( RPos_x( 8 ), RPos_y( 3 ), GlobalText[1'026] );
  DisableAlphaBlend();
}
...
void CUILetterBoxTabWindow::RenderSub()
{
  ...
  RenderBitmap( BITMAP_INTERFACE_EX + 14, RPos_x( 1 + 10 ), RPos_y( 3 ), 13.0f, 9.0f, 0.f, 0.f, 13.f / 16.f,
                9.f / 32.f );
  ...
  if ( CheckMouseIn( RPos_x( 0 ) + m_LetterListBox.GetColumnPos_x( 1 ), RPos_y( 0 ),
    ...
    g_pRenderText->SetTextColor( 255, 255, 255, 255 );
    g_pRenderText->RenderText( RPos_x( 4 ) + m_LetterListBox.GetColumnPos_x( 1 ), RPos_y( 3 ), GlobalText[1'028] );
    g_pRenderText->SetTextColor( 230, 220, 200, 255 );
  } else {
    g_pRenderText->RenderText( RPos_x( 4 ) + m_LetterListBox.GetColumnPos_x( 1 ), RPos_y( 3 ), GlobalText[1'028] );
  }
  ...
  if ( CheckMouseIn( RPos_x( 0 ) + m_LetterListBox.GetColumnPos_x( 2 ), RPos_y( 0 ),
    ...
    g_pRenderText->SetTextColor( 255, 255, 255, 255 );
    g_pRenderText->RenderText( RPos_x( 4 ) + m_LetterListBox.GetColumnPos_x( 2 ), RPos_y( 3 ), GlobalText[1'029] );
    g_pRenderText->SetTextColor( 230, 220, 200, 255 );
  } else {
    g_pRenderText->RenderText( RPos_x( 4 ) + m_LetterListBox.GetColumnPos_x( 2 ), RPos_y( 3 ), GlobalText[1'029] );
  }
  ...
  if ( CheckMouseIn( RPos_x( 0 ) + m_LetterListBox.GetColumnPos_x( 3 ), RPos_y( 0 ),
    ...
    g_pRenderText->SetTextColor( 255, 255, 255, 255 );
    g_pRenderText->RenderText( RPos_x( 4 ) + m_LetterListBox.GetColumnPos_x( 3 ), RPos_y( 3 ), GlobalText[1'030] );
    g_pRenderText->SetTextColor( 230, 220, 200, 255 );
  } else {
    g_pRenderText->RenderText( RPos_x( 4 ) + m_LetterListBox.GetColumnPos_x( 3 ), RPos_y( 3 ), GlobalText[1'030] );
  }
    ...
    case UI_MESSAGE_BTNLCLICK: {
      ...
      switch ( m_WorkMessage.m_iParam1 ) {
        ...
        case 3: {
          ...
          wchar_t szMailTitle[MAX_TEXT_LENGTH + 1] = { 0 };
          swprintf( szMailTitle, GlobalText[1'016], GetCurrentSelectedLetter()->m_szText );
          wchar_t szMailTitleResult[32 + 1] = { 0 };
          CutText4( szMailTitle, szMailTitleResult, NULL, 32 );
          ( (CUILetterWriteWindow*)g_pWindowMgr->GetWindow( dwUIID ) )->SetMainTitleText( szMailTitleResult );
        } break;
        case 4: {
          if ( m_LetterListBox.HaveCheckedLine() == FALSE ) {
            dwUIID = g_pWindowMgr->AddWindow( UIWNDTYPE_OK, UIWND_DEFAULT, UIWND_DEFAULT, GlobalText[1'031] );
            break;
          }
          dwUIID =
              g_pWindowMgr->AddWindow( UIWNDTYPE_QUESTION, UIWND_DEFAULT, UIWND_DEFAULT, GlobalText[1'017], GetUIID() );
        } break;
        case 5: {
...
void CUIFriendWindow::Init( const wchar_t* pszTitle, DWORD dwParentID )
{
  ...
  m_FriendListWnd.Init( GlobalText[1'032], GetUIID() );
  m_ChatRoomListWnd.Init( GlobalText[1'033], GetUIID() );
  m_LetterBoxWnd.Init( GlobalText[1'034], GetUIID() );
...
void CUIFriendWindow::RenderSub()
{
  ...
  g_pRenderText->SetTextColor( 230, 220, 200, 255 );
  GetTextExtentPoint32( g_pRenderText->GetFontDC(), GlobalText[1'035], GlobalText.GetStringSize( 1'035 ), &TextSize );
...
void CUIFriendWindow::DoMouseActionSub()
{
  ...
  if ( CheckMouseIn( RPos_x( 0 ), RPos_y( 2 ), 53, 19 ) == TRUE ) {
    ...
    if ( CheckMouseIn( RPos_x( 0 ) + RWidth() - TextSize.cx - 2 - 14, RPos_y( 4 ), TextSize.cx + 2 + 14, 20 ) ==
         TRUE ) {
      if ( MouseLButtonPop ) {
        PlayBuffer( SOUND_CLICK01 );
        if ( g_pWindowMgr->GetChatReject() == TRUE ) {
          ...
          g_pWindowMgr->AddWindow( UIWNDTYPE_QUESTION, UIWND_DEFAULT, UIWND_DEFAULT, GlobalText[1'036], GetUIID() );
        }
        MouseLButtonPop = FALSE;
      }
    }
...
void CUIQuestionWindow::Init( const wchar_t* pszTitle, DWORD dwParentID )
{
  ...
  if ( m_iDialogType == 0 ) {
    m_AddButton.Init( 1, GlobalText[1'037] );
    m_AddButton.SetParentUIID( GetUIID() );
    m_AddButton.SetArrangeType( 0, 18, 40 );
    m_AddButton.SetSize( 50, 20 );

    m_CancelButton.Init( 2, GlobalText[1'038] );
    m_CancelButton.SetParentUIID( GetUIID() );
    m_CancelButton.SetArrangeType( 0, 73, 40 );
    m_CancelButton.SetSize( 50, 20 );
  } else if ( m_iDialogType == 1 ) {
...
void CUIFriendMenu::RenderFriendButton()
{
  ...
  if ( g_pWindowMgr->GetFriendMainWindow() != NULL ) {
    ...
    if ( g_pFriendMenu->IsNewMailAlert() ) {
      ...
      if ( bIsAlertTime ) {
        RenderBitmap( BITMAP_INTERFACE_EX + 16, m_iPos_x + 36, m_iFriendMenuPos_y + 7, 15.f, 9.f, 0.f, 0.f, 15.f / 16.f,
                      9.f / 16.f );
    ...
    if ( g_pFriendMenu->IsNewMailAlert() ) {
      if ( bIsAlertTime ) {
        RenderBitmap( BITMAP_INTERFACE_EX + 16, m_iPos_x + 33, m_iFriendMenuPos_y + 5, 15.f, 9.f, 0.f, 0.f, 15.f / 16.f,
                      9.f / 16.f );
        if ( m_iBlinkTemp % 24 == 11 ) {
          ++m_iLetterBlink;
        }
  ...
  for ( m_WindowListIter = m_WindowList.begin(); m_WindowListIter != m_WindowList.end(); ++m_WindowListIter, ++i ) {
    ...
    if ( pWindow != NULL && pWindow->GetUserCount() > 2 ) {
      ...
      RenderBitmap( BITMAP_INTERFACE_EX + 15, (float)m_iPos_x + m_iWidth - 7,
                    (float)m_iFriendMenuPos_y - ( m_fLineHeight + 4 ) * i + 5, (float)4, (float)6, 0.f, 0.f, 4.f / 8.f,
                    6.f / 8.f );
...
void CUIFriendMenu::AddRequestWindow( const wchar_t* szTargetName )
{
  ...
  if ( wcslen( szTargetName ) > MAX_ID_SIZE ) {
    return;
  }
