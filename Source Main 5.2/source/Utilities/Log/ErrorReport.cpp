...
{
  DWORD   dwNumber;
  wchar_t lpszBuffer[128 * 1'024];
  ReadFile( m_hFile, lpszBuffer, 128 * 1'024 - 1, &dwNumber, NULL );
  // m_iKey = Xor_ConvertBuffer( lpszBuffer, dwNumber);
  lpszBuffer[dwNumber] = '\0';
  wchar_t* lpCut       = CheckHeadToCut( lpszBuffer, dwNumber );
  if ( dwNumber >= 32 * 1'024 - 1 ) {
    lpCut = &lpszBuffer[32 * 1'024 - 1];
  }
  if ( lpCut != lpszBuffer ) {
    CloseHandle( m_hFile );
    DeleteFile( m_lpszFileName );
...
{
  wchar_t lpszBuffer[1'024] = {
    0,
  };
  va_list va;
  va_start( va, lpszFormat );
  vswprintf( lpszBuffer, 1'024, lpszFormat, va );
  va_end( va );
...
void CErrorReport::WriteSystemInfo( ER_SystemInfo* si )
{
  ...
  Write( L"OS \t\t\t: %ls\r\n", si->m_lpszOS );
  Write( L"CPU \t\t\t: %ls\r\n", si->m_lpszCPU );
  Write( L"RAM \t\t\t: %dMB\r\n", 1 + ( si->m_iMemorySize / 1'024 / 1'024 ) );
  AddSeparator();
  Write( L"Direct-X \t\t: %ls\r\n", si->m_lpszDxVersion );
}
...
typedef struct tagSOUNDDEVICEENUM
{
  ...
  tagER_SOUNDDEVICE& operator[]( size_t p ) { return infoSoundDevice[p]; }
  tagER_SOUNDDEVICE& GetNextDevice() { return infoSoundDevice[nDeivceCount]; }
...
void CErrorReport::WriteSoundCardInfo( void )
{
  ...
  for ( unsigned int i = 0; i < sdi.nDeivceCount; ++i ) {
    ...
    Write( L"Sound Card Driver\t: %ls (%d.%d.%d.%d)\r\n", sdi.infoSoundDevice[i].szDriverName, wVersion[0], wVersion[1],
...
void GetOSVersion( ER_SystemInfo* si )
{
  ...
  switch ( iBuildNumberType ) {
    case 0:
      ...
      break;
    case 1:
      swprintf( lpszTemp, L"Build %d.%d.%d ", HIBYTE( HIWORD( osiOne.dwBuildNumber ) ),
