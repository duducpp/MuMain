...
CConsoleWindow::CConsoleWindow()
{
  ...
  m_LimitTimer.SetTimer( 12'000 ); //. 12ÃÊ
}
CConsoleWindow::~CConsoleWindow() {}
...
{
  wchar_t szConsoleTile[1'024] = {
    0,
  };
  ::GetConsoleTitle( szConsoleTile, 1'024 );
...
void CConsoleWindow::ClearScreen()
{
  ...
  ::FillConsoleOutputCharacter( ::GetStdHandle( STD_OUTPUT_HANDLE ), (TCHAR)' ', dwConSize, coordScreen,
                                &cCharsWritten );
...
bool CConsoleWindow::SaveScreenBuffer( const std::wstring& filename )
{
  ...
  if ( ReadConsoleOutput( GetStdHandle( STD_OUTPUT_HANDLE ), pbyCharBuffer, BufferSize, StartPointToWrite,
                          &RectToRead ) ) {
  }
