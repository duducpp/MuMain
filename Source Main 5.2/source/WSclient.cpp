...
#include "Interface/UI/UIGuardsMan.h"
...
void ReceiveMuHelperStatusUpdate( std::span<const BYTE> ReceiveBuffer )
{
  ...
  if ( pMuHelperStatus->Pause ) {
    ...
    if ( pMuHelperStatus->Money > 0 && pMuHelperStatus->ConsumeMoney ) {
      ...
      wchar_t Text[100];
      swprintf( Text, GlobalText[3'586], iTotalCost );
      g_pSystemLogBox->AddText( Text, SEASON3B::TYPE_SYSTEM_MESSAGE );
    }
  }
...
void ReceiveTradeInventoryExtended( std::span<const BYTE> ReceiveBuffer )
{
  ...
  if ( Data->SubCode == 3 ) {
    ...
    g_pSystemLogBox->AddText( GlobalText[1'208], SEASON3B::TYPE_ERROR_MESSAGE );
    PlayBuffer( SOUND_MIX01 );
    PlayBuffer( SOUND_BREAK01 );
    g_pMixInventory->SetMixState( SEASON3B::CNewUIMixInventory::MIX_FINISHED );
    g_pMixInventory->DeleteAllItems();
...
void ReceiveChangePlayer( std::span<const BYTE> ReceiveBuffer )
{
  ...
  g_ConsoleDebug->Write(
      MCD_RECEIVE, L"0x25 ReceiveChangePlayer Key(0x%04X) Item Slot(0x%02X) Group(0x%02X) Number(0x%04X) Level(0x%02X)",
...
void ReceiveCreateTransformViewport( std::span<const BYTE> ReceiveBuffer )
{
  ...
  for ( int i = 0; i < Data->Value; i++ ) {
    ...
    Offset +=
        ( sizeof( PCREATE_TRANSFORM_EXTENDED ) - ( sizeof( BYTE ) * ( MAX_BUFF_SLOT_INDEX - Data2->s_BuffCount ) ) );
  }
...
void ReceiveCreateMonsterViewport( const BYTE* ReceiveBuffer )
{
  ...
  for ( int i = 0; i < Data->Value; i++ ) {
    ...
    else if ( PathFinding2( c->PositionX, c->PositionY, Data2->TargetX, Data2->TargetY, &c->Path, 0.0f,
                            iDefaultWall ) ) {
      c->Movement = true;
    } else {
      c->Movement = false;
                                ...
                                const bool bEndRepeatedly, const bool bDoubleEnable, const bool bComboEnable )
{
  ...
  if ( accumDamage > 0 ) {
    rstDamage = -2;
    if ( accumDamage > 0 && accumDamage < 1'000 ) {
      Vector( 1.f, 0.f, 0.f, Light );
    } else if ( accumDamage >= 1'000 && accumDamage < 3'000 ) {
      Vector( 0.95f, 0.7f, 0.15f, Light );
    } else if ( accumDamage >= 3'000 ) {
      Vector( 0.95f, 0.7f, 0.15f, Light );
      rstScale = 1.5f;
    }
  } else {
...
void ReceiveSkillStatus( const BYTE* ReceiveBuffer )
{
  ...
  if ( Data->State == 1 ) // add
  {
    ...
    if ( g_isCharacterBuff( o, bufftype ) ) {
      if ( ( o->Type >= MODEL_CRYWOLF_ALTAR1 && o->Type <= MODEL_CRYWOLF_ALTAR5 ) || MODEL_SMITH || MODEL_NPC_SERBIS ||
           MODEL_MERCHANT_MAN || MODEL_STORAGE || MODEL_NPC_BREEDER ) {
...
BOOL ReceiveMagic( const BYTE* ReceiveBuffer, int Size, BOOL bEncrypted )
{
  ...
  if ( MagicNumber == AT_SKILL_ATTACK || MagicNumber == AT_SKILL_DEFENSE || MagicNumber == AT_SKILL_ATTACK_STR ||
    ...
    if ( Success == false ) {
      g_pSystemLogBox->AddText( GlobalText[2'249], SEASON3B::TYPE_SYSTEM_MESSAGE );
      return FALSE;
    }
  }
  ...
  switch ( MagicNumber ) {
    ...
    case AT_SKILL_DRAGON_ROAR_STR: {
      ...
      if ( g_CMonkSystem.SetLowerEffEct() ) {
        CreateEffect( MODEL_VOLCANO_OF_MONK, to->Position, to->Angle, vLight, g_CMonkSystem.GetLowerEffCnt(), to, -1, 0,
                      0, 0, 0.8f );
      }
...
void ReceiveChainMagic( const BYTE* ReceiveBuffer )
{
  ...
  for ( int i = 0; i < (int)( pPacketData->byCount ); i++ ) {
    ...
    if ( pTempObject != pTargetObject && pTargetObject != nullptr && pTargetObject->Live == true ) {
      ...
      CreateEffect( MODEL_CHAIN_LIGHTNING, pSourceObject->Position, vAngle, pSourceObject->Light, i, pTempObject, -1, 0,
                    0, 0, 0.f, pPacketData2->wTargetIndex );
    }
...
BOOL ReceiveDieExp( const BYTE* ReceiveBuffer, BOOL bEncrypted )
{
  ...
  if ( Exp > 0 ) {
    wchar_t Text[100];
    if ( gCharacterManager.IsMasterLevel( Hero->Class ) == true ) {
      swprintf( Text, GlobalText[1'750], Exp );
    } else {
...
BOOL ReceiveDieExpLarge( const BYTE* ReceiveBuffer, BOOL bEncrypted )
{
  ...
  if ( addedExperience > 0 ) {
    ...
    if ( experienceType == eExperienceType_Master ) {
      swprintf( Text, GlobalText[1'750], addedExperience );
    } else {
    ...
    case 0: {
      ...
      switch ( g_MixRecipeMgr.GetMixInventoryType() ) {
        ...
        case SEASON3A::MIXTYPE_OSBOURNE:
          swprintf( szText, GlobalText[2'105], GlobalText[2'061] );
          g_pSystemLogBox->AddText( szText, SEASON3B::TYPE_ERROR_MESSAGE );
          break;
        case SEASON3A::MIXTYPE_JERRIDON:
          swprintf( szText, GlobalText[2'105], GlobalText[2'062] );
          g_pSystemLogBox->AddText( szText, SEASON3B::TYPE_ERROR_MESSAGE );
          break;
        case SEASON3A::MIXTYPE_ELPIS:
          swprintf( szText, GlobalText[2'112], GlobalText[2'063] );
          g_pSystemLogBox->AddText( szText, SEASON3B::TYPE_ERROR_MESSAGE );
          break;
        case SEASON3A::MIXTYPE_CHAOS_CARD:
          swprintf( szText, GlobalText[2'112], GlobalText[2'265] );
          g_pSystemLogBox->AddText( szText, SEASON3B::TYPE_ERROR_MESSAGE );
          break;
        case SEASON3A::MIXTYPE_CHERRYBLOSSOM:
          swprintf( szText, GlobalText[2'112], GlobalText[2'560] );
          g_pSystemLogBox->AddText( szText, SEASON3B::TYPE_ERROR_MESSAGE );
          break;
      }
    } break;
    case 1: {
      ...
      switch ( g_MixRecipeMgr.GetMixInventoryType() ) {
        ...
        case SEASON3A::MIXTYPE_SEED_SPHERE:
          ...
          // 				break;
        case SEASON3A::MIXTYPE_OSBOURNE:
          swprintf( szText, GlobalText[2'106], GlobalText[2'061] );
          g_pSystemLogBox->AddText( szText, SEASON3B::TYPE_SYSTEM_MESSAGE );
          break;
        case SEASON3A::MIXTYPE_JERRIDON:
          swprintf( szText, GlobalText[2'106], GlobalText[2'062] );
          g_pSystemLogBox->AddText( szText, SEASON3B::TYPE_SYSTEM_MESSAGE );
          break;
        case SEASON3A::MIXTYPE_ELPIS:
          swprintf( szText, GlobalText[2'113], GlobalText[2'063] );
          g_pSystemLogBox->AddText( szText, SEASON3B::TYPE_SYSTEM_MESSAGE );
          break;
        case SEASON3A::MIXTYPE_CHAOS_CARD:
          swprintf( szText, GlobalText[2'113], GlobalText[2'265] );
          g_pSystemLogBox->AddText( szText, SEASON3B::TYPE_SYSTEM_MESSAGE );
          break;
        case SEASON3A::MIXTYPE_CHERRYBLOSSOM:
          swprintf( szText, GlobalText[2'113], GlobalText[2'560] );
          g_pSystemLogBox->AddText( szText, SEASON3B::TYPE_SYSTEM_MESSAGE );
          break;
      }
...
void ReceivePartyResult( const BYTE* ReceiveBuffer )
{
  ...
  switch ( Data->Value ) {
    ...
    case 5:
      ...
      break;
    case 6:
      g_pSystemLogBox->AddText( GlobalText[2'990], SEASON3B::TYPE_ERROR_MESSAGE );
      break;
    case 7:
      g_pSystemLogBox->AddText( GlobalText[2'997], SEASON3B::TYPE_ERROR_MESSAGE );
      break;
    case 8:
      g_pSystemLogBox->AddText( GlobalText[2'998], SEASON3B::TYPE_ERROR_MESSAGE );
      break;
  }
}
...
void ReceivePartyGetItem( const BYTE* ReceiveBuffer )
{
  ...
  if ( ( Data->ItemInfo & 0x10000 ) ) {
    swprintf( itemName, L"%ls ", GlobalText[620] );
  } else if ( ( Data->ItemInfo & 0x20000 ) ) {
    swprintf( itemName, L"%ls ", GlobalText[1'089] );
  }
...
void ReceiveGuildResult( const BYTE* ReceiveBuffer )
{
  ...
  switch ( Data->Value ) {
    ...
    case 7:
      ...
      break;
    case 0xA1:
      g_pSystemLogBox->AddText( GlobalText[2'992], SEASON3B::TYPE_ERROR_MESSAGE );
      break;
    case 0xA2:
      g_pSystemLogBox->AddText( GlobalText[2'995], SEASON3B::TYPE_ERROR_MESSAGE );
      break;
    case 0xA3:
      g_pSystemLogBox->AddText( GlobalText[2'996], SEASON3B::TYPE_ERROR_MESSAGE );
      break;
  }
}
...
void ReceiveGuildAssign( const BYTE* ReceiveBuffer )
{
  ...
  if ( pData->byResult == 0x01 ) {
    switch ( pData->byType ) {
      case 0x01:
        wcscpy( szTemp, GlobalText[1'374] );
        break;
      case 0x02:
        wcscpy( szTemp, GlobalText[1'375] );
        break;
      case 0x03:
        wcscpy( szTemp, GlobalText[1'376] );
        break;
      default:
        assert( !"Packet(0xE1)" );
        break;
    ...
    switch ( pData->byResult ) {
      ...
      case GUILD_ANS_NOTEXIST_PERMISSION:
        wcscpy( szTemp, GlobalText[1'386] );
        break;
      case GUILD_ANS_NOTEXIST_EXTRA_STATUS:
        wcscpy( szTemp, GlobalText[1'326] );
        break;
      case GUILD_ANS_NOTEXIST_EXTRA_TYPE:
        wcscpy( szTemp, GlobalText[1'327] );
        break;
      default:
        assert( !"Packet(0xE1)" );
        break;
  ...
  if ( pData->byResult == 0x01 ) {
    if ( pData->byRelationShipType == 0x01 ) {
      if ( pData->byRequestType == 0x01 ) {
        wcscpy( szTemp, GlobalText[1'381] );
      } else if ( pData->byRequestType == 0x02 ) {
        wcscpy( szTemp, GlobalText[1'382] );
      } else if ( pData->byRequestType == 0x10 ) {
        wcscpy( szTemp, GlobalText[1'635] );
      }
    } else {
      if ( pData->byRequestType == 0x01 ) {
        wcscpy( szTemp, GlobalText[1'383] );
      } else {
        wcscpy( szTemp, GlobalText[1'384] );
      }
    }
  } else if ( pData->byResult == 0 ) {
    wcscpy( szTemp, GlobalText[1'328] );
  } else {
    switch ( pData->byResult ) {
      case GUILD_ANS_UNIONFAIL_BY_CASTLE:
        wcscpy( szTemp, GlobalText[1'637] );
        break;
      case GUILD_ANS_NOTEXIST_PERMISSION:
        wcscpy( szTemp, GlobalText[1'386] );
        break;
      case GUILD_ANS_EXIST_RELATIONSHIP_UNION:
        wcscpy( szTemp, GlobalText[1'250] );
        break;
      case GUILD_ANS_EXIST_RELATIONSHIP_RIVAL:
        wcscpy( szTemp, GlobalText[1'251] );
        break;
      case GUILD_ANS_EXIST_UNION:
        wcscpy( szTemp, GlobalText[1'252] );
        break;
      case GUILD_ANS_EXIST_RIVAL:
        wcscpy( szTemp, GlobalText[1'253] );
        break;
      case GUILD_ANS_NOTEXIST_UNION:
        wcscpy( szTemp, GlobalText[1'254] );
        break;
      case GUILD_ANS_NOTEXIST_RIVAL:
        wcscpy( szTemp, GlobalText[1'255] );
        break;
      case GUILD_ANS_NOT_UNION_MASTER:
        wcscpy( szTemp, GlobalText[1'333] );
        break;
      case GUILD_ANS_NOT_GUILD_RIVAL:
        wcscpy( szTemp, GlobalText[1'329] );
        break;
      case GUILD_ANS_CANNOT_BE_UNION_MASTER_GUILD:
        wcscpy( szTemp, GlobalText[1'331] );
        break;
      case GUILD_ANS_EXCEED_MAX_UNION_MEMBER:
        wcscpy( szTemp, GlobalText[1'287] );
        break;
      case GUILD_ANS_CANCEL_REQUEST:
        wcscpy( szTemp, GlobalText[1'268] );
        break;
#ifdef ASG_ADD_GENS_SYSTEM
      case GUILD_ANS_UNION_MASTER_NOT_GENS:
        wcscpy( szTemp, GlobalText[2'991] );
        break;
      case GUILD_ANS_GUILD_MASTER_NOT_GENS:
        wcscpy( szTemp, GlobalText[2'992] );
        break;
      case GUILD_ANS_UNION_MASTER_DISAGREE_GENS:
        wcscpy( szTemp, GlobalText[2'993] );
        break;
...
void ReceiveBanUnionGuildResult( const BYTE* ReceiveBuffer )
{
  ...
  if ( pData->byResult == 0x01 ) {
    ...
    g_pSystemLogBox->AddText( GlobalText[1'328], SEASON3B::TYPE_SYSTEM_MESSAGE );
  }
}
...
void Receive_Master_LevelUp( const BYTE* ReceiveBuffer, int Size )
{
  ...
  if ( iExp > 0 ) {
    swprintf( szText, GlobalText[1'750], iExp );
    g_pSystemLogBox->AddText( szText, SEASON3B::TYPE_SYSTEM_MESSAGE );
  }
...
void ReceiveServerCommand( const BYTE* ReceiveBuffer )
{
  ...
  switch ( Data->Cmd1 ) {
    ...
    case 6:
      ...
      break;
    case 13:
      SEASON3B::CreateOkMessageBox( GlobalText[1'826] );
      break;
    case 14: {
      switch ( Data->Cmd2 ) {
        case 0:
        ...
        case 1:
          SEASON3B::CreateOkMessageBox( GlobalText[2'024] );
          break;

        case 2:
          SEASON3B::CreateOkMessageBox( GlobalText[2'022] );
          break;

        case 3:
          SEASON3B::CreateOkMessageBox( GlobalText[2'023] );
          break;
    ...
    case 16: {
      ...
      switch ( Data->Cmd2 ) {
        case 0:
          ...
          pMsgBox->AddMsg( GlobalText[2'588] );
          break;
        case 1:
          SEASON3B::CreateMessageBox( MSGBOX_LAYOUT_CLASS( SEASON3B::CSantaTownSantaMsgBoxLayout ), &pMsgBox );
          pMsgBox->AddMsg( GlobalText[2'585] );
          break;
        case 2:
          SEASON3B::CreateOkMessageBox( GlobalText[2'587] );
          break;
        case 3:
          SEASON3B::CreateOkMessageBox( GlobalText[2'023] );
          break;
    ...
    case 48:
    case 49:
      SEASON3B::CreateOkMessageBox( GlobalText[1'823 + Data->Cmd1 - 47] );
      break;
    case 55: {
      wchar_t strText[128];
      swprintf( strText, GlobalText[2'043], GlobalText[39] );
      SEASON3B::CreateOkMessageBox( strText );
    } break;
    case 56: {
      wchar_t strText[128];
      swprintf( strText, GlobalText[2'043], GlobalText[56] );
      SEASON3B::CreateOkMessageBox( strText );
    } break;
    case 57: {
      wchar_t strText[128];
      swprintf( strText, GlobalText[2'043], GlobalText[57] );
      SEASON3B::CreateOkMessageBox( strText );
    } break;
    case 58: {
...
void ReceiveGemMixResult( const BYTE* ReceiveBuffer )
{
  ...
  switch ( Data->m_iResult ) {
    ...
    case 2:
    case 3: {
      swprintf( sBuf, L"%ls%ls %ls", GlobalText[1'801], GlobalText[1'816], GlobalText[868] );
    ...
    case 4: {
      g_pSystemLogBox->AddText( GlobalText[1'817], SEASON3B::TYPE_SYSTEM_MESSAGE );
      COMGEM::GetBack();
    } break;
    case 5: {
      g_pSystemLogBox->AddText( GlobalText[1'811], SEASON3B::TYPE_SYSTEM_MESSAGE );
      COMGEM::GetBack();
    } break;
  }
}
...
void ReceiveGemUnMixResult( const BYTE* ReceiveBuffer )
{
  ...
  switch ( Data->m_iResult ) {
    case 0:
    case 5: {
      swprintf( sBuf, L"%ls%ls %ls", GlobalText[1'800], GlobalText[1'816], GlobalText[868] );
      g_pSystemLogBox->AddText( sBuf, SEASON3B::TYPE_SYSTEM_MESSAGE );
      COMGEM::GetBack();
    } break;
    case 1: {
    ...
    case 4:
    case 6: {
      g_pSystemLogBox->AddText( GlobalText[1'812], SEASON3B::TYPE_SYSTEM_MESSAGE );
      COMGEM::GetBack();
    } break;
    case 7: {
      g_pSystemLogBox->AddText( GlobalText[1'815], SEASON3B::TYPE_SYSTEM_MESSAGE );
      COMGEM::GetBack();
    } break;
    case 8: {
      g_pSystemLogBox->AddText( GlobalText[1'811], SEASON3B::TYPE_SYSTEM_MESSAGE );
...
void ReceiveMoveToEventMatchResult( const BYTE* ReceiveBuffer )
{
  ...
  switch ( Data->Value ) {
    ...
    case 2: {
      wchar_t strText[128];
      swprintf( strText, GlobalText[852], GlobalText[1'146] );
      SEASON3B::CreateOkMessageBox( strText );
    } break;
    ...
    case 5: {
      wchar_t strText[128];
      swprintf( strText, GlobalText[853], GlobalText[1'146], MAX_BLOOD_CASTLE_MEN );
      SEASON3B::CreateOkMessageBox( strText );
    } break;
    ...
    case 8: {
      wchar_t strText[128];
      swprintf( strText, GlobalText[852], GlobalText[1'147] );
      SEASON3B::CreateOkMessageBox( strText );
    } break;

    case 9: {
      wchar_t strText[128];
      swprintf( strText, GlobalText[853], GlobalText[1'147], MAX_CHAOS_CASTLE_MEN );
      SEASON3B::CreateOkMessageBox( strText );
    } break;
  }
}
...
void ReceiveEventZoneOpenTime( const BYTE* ReceiveBuffer )
{
  ...
  if ( Data->Value == 1 ) {
    ...
    wchar_t strText[256];
    if ( 0 == Data->KeyH ) {
      swprintf( strText, GlobalText[850], GlobalText[1'146] );
    } else {
      swprintf( strText, GlobalText[851], (int)Data->KeyH, GlobalText[1'146] );
    }
    SEASON3B::CreateOkMessageBox( strText );
    ...
    if ( 0 == time ) {
      ...
      swprintf( szOpenTime1, GlobalText[850], GlobalText[1'147] );
      swprintf( szOpenTime2, GlobalText[1'156], GlobalText[1'147], Data->KeyM, 100 );

      GlobalText.Remove( 1'154 );
      GlobalText.Remove( 1'155 );
      GlobalText.Add( 1'154, szOpenTime1 );
      GlobalText.Add( 1'155, szOpenTime2 );

      SEASON3B::CNewUICommonMessageBox* pMsgBox = nullptr;
      SEASON3B::CreateMessageBox( MSGBOX_LAYOUT_CLASS( SEASON3B::CChaosCastleTimeCheckMsgBoxLayout ), &pMsgBox );
      if ( pMsgBox ) {
        pMsgBox->AddMsg( GlobalText[1'154] );
        pMsgBox->AddMsg( GlobalText[1'155] );
      }
    } else {
      wchar_t Text[256];
      ...
      swprintf( szOpenTime, GlobalText[1'164], Hour );
      swprintf( Text, GlobalText[851], Mini, GlobalText[1'147] );
      wcscat( szOpenTime, Text );

      GlobalText.Remove( 1'154 );
      GlobalText.Add( 1'154, szOpenTime );
    ...
    wchar_t strText[256];
    if ( 0 == Data->KeyH ) {
      swprintf( strText, GlobalText[850], GlobalText[2'369] );
    } else {
      swprintf( strText, GlobalText[851], (int)Data->KeyH, GlobalText[2'369] );
    }
    SEASON3B::CreateOkMessageBox( strText );
  }
}
...
void ReceiveDuelStart( const BYTE* ReceiveBuffer )
{
  ...
  if ( Data->nResult == 0 ) {
    ...
    g_DuelMgr.SetDuelPlayer( DUEL_ENEMY, MAKEWORD( Data->bIndexL, Data->bIndexH ), playerName );
    swprintf( szMessage, GlobalText[2'704], 30 );
    g_pSystemLogBox->AddText( szMessage, SEASON3B::TYPE_ERROR_MESSAGE );
  } else if ( Data->nResult == 30 ) {
    g_DuelMgr.SetDuelPlayer( DUEL_ENEMY, MAKEWORD( Data->bIndexL, Data->bIndexH ), playerName );
    swprintf( szMessage, GlobalText[1'811] );
    g_pSystemLogBox->AddText( szMessage, SEASON3B::TYPE_ERROR_MESSAGE );
  }
}
...
void ReceiveDuelResult( const BYTE* ReceiveBuffer )
{
  ...
  wchar_t szMessage[256];
  swprintf( szMessage, GlobalText[2'689], 10 );
  g_pSystemLogBox->AddText( szMessage, SEASON3B::TYPE_SYSTEM_MESSAGE );
...
void ReceivePersonalShopItemList( std::span<const BYTE> ReceiveBuffer )
{
  ...
  if ( Header->byResult == Success ) {
    ...
    switch ( Header->byResult ) {
      case Fail1: {
        g_pSystemLogBox->AddText( GlobalText[1'120], SEASON3B::TYPE_ERROR_MESSAGE );
      } break;
      case Fail2:
      default:
...
void ReceivePurchaseItem( std::span<const BYTE> ReceiveBuffer )
{
  ...
  if ( Header->Result == PURCHASEITEM_RESULTINFO::BoughtSuccessfully ) {
    ...
    g_pSystemLogBox->AddText( GlobalText[1'166], SEASON3B::TYPE_ERROR_MESSAGE );
    g_pNewUISystem->Hide( SEASON3B::INTERFACE_MYSHOP_INVENTORY );
    g_pNewUISystem->Hide( SEASON3B::INTERFACE_PURCHASESHOP_INVENTORY );
  } else {
...
void NotifySoldItem( const BYTE* ReceiveBuffer )
{
  ...
  CMultiLanguage::ConvertFromUtf8( szId, Header->szId, MAX_ID_SIZE );
  wchar_t Text[100];
  swprintf( Text, GlobalText[1'122], szId );
  g_pSystemLogBox->AddText( Text, SEASON3B::TYPE_SYSTEM_MESSAGE );
}
...
void NotifyClosePersonalShop( const BYTE* ReceiveBuffer )
{
  if ( g_IsPurchaseShop == PSHOPWNDTYPE_PURCHASE ) {
    ...
    g_pSystemLogBox->AddText( GlobalText[1'126], SEASON3B::TYPE_ERROR_MESSAGE );
  }
}
...
void ReceiveFriendList( const BYTE* ReceiveBuffer )
{
  ...
  if ( Header->MemoCount > 0 ) {
    ...
    swprintf( temp, GlobalText[1'072], Header->MemoCount, Header->MaxMemo );
...
void ReceiveAddFriendResult( const BYTE* ReceiveBuffer )
{
  ...
  switch ( Data->Result ) {
    case 0x00:
      wcscat( szText, GlobalText[1'047] );
      g_pWindowMgr->AddWindow( UIWNDTYPE_OK_FORCE, UIWND_DEFAULT, UIWND_DEFAULT, szText );
      break;
    case 0x01: {
      g_pSystemLogBox->AddText( GlobalText[1'075], SEASON3B::TYPE_SYSTEM_MESSAGE );
      g_pFriendList->AddFriend( szName, 0, Data->Server );
      g_pFriendList->Sort();
      g_pWindowMgr->RefreshMainWndPalList();
    ...
    case 0x03:
      wcscpy( szText, GlobalText[1'048] );
      g_pWindowMgr->AddWindow( UIWNDTYPE_OK_FORCE, UIWND_DEFAULT, UIWND_DEFAULT, szText );
      break;
    case 0x04:
      wcscat( szText, GlobalText[1'049] );
      g_pWindowMgr->AddWindow( UIWNDTYPE_OK_FORCE, UIWND_DEFAULT, UIWND_DEFAULT, szText );
      break;
    case 0x05:
      wcscpy( szText, GlobalText[1'050] );
      g_pWindowMgr->AddWindow( UIWNDTYPE_OK_FORCE, UIWND_DEFAULT, UIWND_DEFAULT, szText );
      break;
    case 0x06:
      wcscpy( szText, GlobalText[1'068] );
      g_pWindowMgr->AddWindow( UIWNDTYPE_OK_FORCE, UIWND_DEFAULT, UIWND_DEFAULT, szText );
      break;
    default:
      break;
...
void ReceiveRequestAcceptAddFriend( const BYTE* ReceiveBuffer )
{
  ...
  swprintf( szText, L"%ls %ls", szText, GlobalText[1'051] ); // " has requested to list you as a friend."
...
void ReceiveDeleteFriendResult( const BYTE* ReceiveBuffer )
{
  ...
  switch ( Data->Result ) {
    case 0x00:
      g_pWindowMgr->AddWindow( UIWNDTYPE_OK_FORCE, UIWND_DEFAULT, UIWND_DEFAULT, GlobalText[1'052] );
      break;
    case 0x01:
...
void ReceiveLetterSendResult( const BYTE* ReceiveBuffer )
{
  ...
  switch ( Data->Result ) {
    case 0x00:
      ...
      g_pWindowMgr->AddWindow( UIWNDTYPE_OK_FORCE, UIWND_DEFAULT, UIWND_DEFAULT, GlobalText[1'053] );
      break;
    case 0x01: {
      if ( Data->WindowGuid != 0 ) {
      ...
      wchar_t temp[MAX_TEXT_LENGTH + 1];
      swprintf( temp, GlobalText[1'046], g_cdwLetterCost );
      g_pSystemLogBox->AddText( temp, SEASON3B::TYPE_SYSTEM_MESSAGE );
    } break;
    case 0x02:
      if ( Data->WindowGuid != 0 ) {
      ...
      g_pWindowMgr->AddWindow( UIWNDTYPE_OK_FORCE, UIWND_DEFAULT, UIWND_DEFAULT, GlobalText[1'061] );
      break;
    case 0x03:
      if ( Data->WindowGuid != 0 ) {
        ( (CUILetterWriteWindow*)g_pWindowMgr->GetWindow( Data->WindowGuid ) )->SetSendState( FALSE );
      }
      g_pWindowMgr->AddWindow( UIWNDTYPE_OK_FORCE, UIWND_DEFAULT, UIWND_DEFAULT, GlobalText[1'064] );
      break;
    case 0x04:
      if ( Data->WindowGuid != 0 ) {
        ( (CUILetterWriteWindow*)g_pWindowMgr->GetWindow( Data->WindowGuid ) )->SetSendState( FALSE );
      }
      g_pWindowMgr->AddWindow( UIWNDTYPE_OK_FORCE, UIWND_DEFAULT, UIWND_DEFAULT, GlobalText[1'065] );
      break;
    case 0x06:
      if ( Data->WindowGuid != 0 ) {
        ( (CUILetterWriteWindow*)g_pWindowMgr->GetWindow( Data->WindowGuid ) )->SetSendState( FALSE );
      }
      g_pWindowMgr->AddWindow( UIWNDTYPE_OK_FORCE, UIWND_DEFAULT, UIWND_DEFAULT, GlobalText[1'068] );
      break;
    case 0x07:
      if ( Data->WindowGuid != 0 ) {
        ( (CUILetterWriteWindow*)g_pWindowMgr->GetWindow( Data->WindowGuid ) )->SetSendState( FALSE );
...
void ReceiveLetter( const BYTE* ReceiveBuffer )
{
  ...
  switch ( Data->Read ) {
    case 0x02:
      ...
      g_pSystemLogBox->AddText( GlobalText[1'062], SEASON3B::TYPE_SYSTEM_MESSAGE );
  ...
  if ( g_pLetterList->GetLetterCount() >= g_iMaxLetterCount ) {
    g_pSystemLogBox->AddText( GlobalText[1'073], SEASON3B::TYPE_SYSTEM_MESSAGE );
  }
}
...
void ReceiveLetterText( std::span<const BYTE> ReceiveBuffer, bool isCached )
{
  ...
  swprintf( tempTxt, GlobalText[1'054], pLetterHead->m_szText );
  DWORD dwUIID = 0;
  if ( g_iLetterReadNextPos_x == UIWND_DEFAULT ) {
    dwUIID = g_pWindowMgr->AddWindow( UIWNDTYPE_READLETTER, 100, 100, tempTxt );
  } else {
    dwUIID = g_pWindowMgr->AddWindow( UIWNDTYPE_READLETTER, g_iLetterReadNextPos_x, g_iLetterReadNextPos_y, tempTxt, 0,
...
void ReceiveLetterDeleteResult( const BYTE* ReceiveBuffer )
{
  ...
  switch ( Data->Result ) {
    case 0x00:
      g_pWindowMgr->AddWindow( UIWNDTYPE_OK_FORCE, UIWND_DEFAULT, UIWND_DEFAULT, GlobalText[1'055] );
      break;
    case 0x01:
...
void ReceiveCreateChatRoomResult( const BYTE* ReceiveBuffer )
{
  ...
  switch ( Data->Result ) {
    case 0x00:
      ...
      g_pWindowMgr->AddWindow( UIWNDTYPE_OK_FORCE, UIWND_DEFAULT, UIWND_DEFAULT, GlobalText[1'069] );
      break;
    case 0x01:
      g_pFriendMenu->RemoveRequestWindow( szName );
      if ( Data->Type == 0 ) {
    ...
    case 0x02:
      g_pFriendMenu->RemoveRequestWindow( szName );
      g_pWindowMgr->AddWindow( UIWNDTYPE_OK_FORCE, UIWND_DEFAULT, UIWND_DEFAULT, GlobalText[1'070] );
      break;
    default:
      break;
  };
...
void ReceiveChatRoomInviteResult( const BYTE* ReceiveBuffer )
{
  ...
  switch ( Data->Result ) {
    case 0x00:
      pChatWindow->AddChatText( 255, GlobalText[1'056], 1, 0 );
      break;
    case 0x01:
      if ( pChatWindow->GetCurrentInvitePal() != nullptr ) {
        ...
        szText[MAX_ID_SIZE] = '\0';
        wcscat( szText, GlobalText[1'057] );
      ...
      break;
    case 0x03:
      pChatWindow->AddChatText( 255, GlobalText[1'074], 1, 0 );
      break;
    default:
      break;
  };
...
void ReceiveBuffState( const BYTE* ReceiveBuffer )
{
  ...
  if ( Data->byEffectOption == 0 ) {
    ...
    if ( bufftype == eBuff_HelpNpc ) {
      g_pSystemLogBox->AddText( GlobalText[1'828], SEASON3B::TYPE_SYSTEM_MESSAGE );
    }
  } else {
...
void ReceiveQuestCompleteResult( const BYTE* ReceiveBuffer )
{
  ...
  switch ( pData->m_byResult ) {
    ...
    case 2:
      ...
      g_pSystemLogBox->AddText( GlobalText[2'816], SEASON3B::TYPE_ERROR_MESSAGE );
      break;
  ...
  switch ( result ) {
    case 0x00:
      if ( fruit >= 0 && fruit <= 4 ) {
        ...
        swprintf( strText, GlobalText[379], GlobalText[index], point, GlobalText[1'412] );
        SEASON3B::CreateOkMessageBox( strText );
      }
      break;
    ...
    case 0x02: {
      swprintf( strText, GlobalText[1'904], GlobalText[1'412] );
      SEASON3B::CreateOkMessageBox( strText );
    } break;
    case 0x03:
      if ( fruit >= 0 && fruit <= 4 ) {
        ...
        wchar_t strText[128];
        swprintf( strText, GlobalText[379], GlobalText[index], point, GlobalText[1'903] );
        SEASON3B::CreateOkMessageBox( strText );
      }
      break;

    case 0x04:
      SEASON3B::CreateOkMessageBox( GlobalText[1'906] );
      break;

    case 0x05: {
      wchar_t strText[128];
      swprintf( strText, GlobalText[1'904], GlobalText[1'903] );
      SEASON3B::CreateOkMessageBox( strText );
    } break;
    case 0x06:
      if ( fruit >= 0 && fruit <= 4 ) {
        ...
        swprintf( Text, GlobalText[379], GlobalText[index], point, GlobalText[1'903] );
    ...
    case 0x08:
      wchar_t Text[MAX_GLOBAL_TEXT_STRING];
      swprintf( Text, GlobalText[1'904], GlobalText[1'903] );
      SEASON3B::CreateOkMessageBox( Text );
      break;
    case 0x10: {
      SEASON3B::CreateOkMessageBox( GlobalText[1'909] );
    } break;

    case 0x21:
      SEASON3B::CreateOkMessageBox( GlobalText[1'911] );
      break;
    ...
    case 0x26:
      SEASON3B::CreateOkMessageBox( GlobalText[1'912] );
      break;
  }
...
void ReceiveBCStatus( const BYTE* ReceiveBuffer )
{
  ...
  switch ( Data->btResult ) {
    case 0x00:
      g_pSystemLogBox->AddText( GlobalText[1'500], SEASON3B::TYPE_SYSTEM_MESSAGE );
      break;
    case 0x01:
    case 0x02:
      ...
      break;
    case 0x03:
      g_pSystemLogBox->AddText( GlobalText[1'501], SEASON3B::TYPE_SYSTEM_MESSAGE );
      break;
    case 0x04:
      g_pSystemLogBox->AddText( GlobalText[1'502], SEASON3B::TYPE_SYSTEM_MESSAGE );
      break;
  }
}
...
void ReceiveBCReg( const BYTE* ReceiveBuffer )
{
  ...
  switch ( Data->btResult ) {
    case 0x00:
      g_pSystemLogBox->AddText( GlobalText[1'503], SEASON3B::TYPE_SYSTEM_MESSAGE );
      break;
    case 0x01:
      g_GuardsMan.SetRegStatus( 1 );
      g_pSystemLogBox->AddText( GlobalText[1'504], SEASON3B::TYPE_SYSTEM_MESSAGE );
      break;
    case 0x02:
      g_pSystemLogBox->AddText( GlobalText[1'505], SEASON3B::TYPE_SYSTEM_MESSAGE );
      break;
    case 0x03:
      g_pSystemLogBox->AddText( GlobalText[1'506], SEASON3B::TYPE_SYSTEM_MESSAGE );
      break;
    case 0x04:
      g_pSystemLogBox->AddText( GlobalText[1'507], SEASON3B::TYPE_SYSTEM_MESSAGE );
      break;
    case 0x05:
      g_pSystemLogBox->AddText( GlobalText[1'508], SEASON3B::TYPE_SYSTEM_MESSAGE );
      break;
    case 0x06:
      g_pSystemLogBox->AddText( GlobalText[1'509], SEASON3B::TYPE_SYSTEM_MESSAGE );
      break;
    case 0x07:
      g_pSystemLogBox->AddText( GlobalText[1'510], SEASON3B::TYPE_SYSTEM_MESSAGE );
      break;
    case 0x08:
      g_pSystemLogBox->AddText( GlobalText[1'511], SEASON3B::TYPE_SYSTEM_MESSAGE );
      break;
    default:
...
void ReceiveBCGiveUp( const BYTE* ReceiveBuffer )
{
  ...
  switch ( Data->btResult ) {
    case 0x00:
      g_pSystemLogBox->AddText( GlobalText[1'512], SEASON3B::TYPE_SYSTEM_MESSAGE );
      break;
    case 0x01:
      ...
      g_pSystemLogBox->AddText( GlobalText[1'513], SEASON3B::TYPE_SYSTEM_MESSAGE );
      break;
    case 0x02:
      g_pSystemLogBox->AddText( GlobalText[1'514], SEASON3B::TYPE_SYSTEM_MESSAGE );
      break;
    case 0x03:
      g_pSystemLogBox->AddText( GlobalText[1'515], SEASON3B::TYPE_SYSTEM_MESSAGE );
      break;
    default:
...
void ReceiveBCRegMark( const BYTE* ReceiveBuffer )
{
  ...
  switch ( Data->btResult ) {
    case 0x00:
      g_pSystemLogBox->AddText( GlobalText[1'516], SEASON3B::TYPE_SYSTEM_MESSAGE );
      break;
    case 0x01: {
      DWORD dwMarkCount;
      auto* pMarkCount = (BYTE*)&dwMarkCount;
    ...
    case 0x02:
      g_pSystemLogBox->AddText( GlobalText[1'517], SEASON3B::TYPE_SYSTEM_MESSAGE );
      break;
    case 0x03:
      g_pSystemLogBox->AddText( GlobalText[1'518], SEASON3B::TYPE_SYSTEM_MESSAGE );
      break;
  }
}
...
void ReceiveBCNPCBuy( const BYTE* ReceiveBuffer )
{
  ...
  switch ( Data->btResult ) {
    case 0:
      g_pSystemLogBox->AddText( GlobalText[1'519], SEASON3B::TYPE_SYSTEM_MESSAGE );
      break;
    case 1:
      g_SenatusInfo.BuyNewNPC( Data->iNpcNumber, Data->iNpcIndex );
      break;
    case 2:
      g_pSystemLogBox->AddText( GlobalText[1'386], SEASON3B::TYPE_SYSTEM_MESSAGE );
      break;
    case 3:
      g_pSystemLogBox->AddText( GlobalText[1'520], SEASON3B::TYPE_SYSTEM_MESSAGE );
      break;
    case 4:
      g_pSystemLogBox->AddText( GlobalText[1'616], SEASON3B::TYPE_SYSTEM_MESSAGE );
      break;
  }
}
...
void ReceiveBCNPCUpgrade( const BYTE* ReceiveBuffer )
{
  ...
  switch ( Data->btResult ) {
    ...
    case 3:
      ...
      break;
    case 4:
      g_pSystemLogBox->AddText( GlobalText[1'521], SEASON3B::TYPE_SYSTEM_MESSAGE );
      break;
    case 5:
      g_pSystemLogBox->AddText( GlobalText[1'522], SEASON3B::TYPE_SYSTEM_MESSAGE );
      break;
    case 6:
      g_pSystemLogBox->AddText( GlobalText[1'523], SEASON3B::TYPE_SYSTEM_MESSAGE );
      break;
    case 7:
      g_pSystemLogBox->AddText( GlobalText[1'524], SEASON3B::TYPE_SYSTEM_MESSAGE );
      break;
  }
}
...
void ReceiveBCGetTaxInfo( const BYTE* ReceiveBuffer )
{
  ...
  switch ( Data->btResult ) {
    case 0:
      g_pSystemLogBox->AddText( GlobalText[1'525], SEASON3B::TYPE_SYSTEM_MESSAGE );
      break;
    case 1:
...
void ReceiveBCChangeTaxRate( const BYTE* ReceiveBuffer )
{
  ...
  switch ( Data->btResult ) {
    case 0:
      g_pSystemLogBox->AddText( GlobalText[1'526], SEASON3B::TYPE_SYSTEM_MESSAGE );
      break;
    case 1:
      if ( Data->btTaxType == 3 ) {
...
void ReceiveBCWithdraw( const BYTE* ReceiveBuffer )
{
  ...
  switch ( Data->btResult ) {
    case 0:
      g_pSystemLogBox->AddText( GlobalText[1'527], SEASON3B::TYPE_SYSTEM_MESSAGE );
      break;
    case 1:
...
void ReceiveHuntZoneEnter( const BYTE* ReceiveBuffer )
{
  ...
  switch ( pData->m_byResult ) {
    ...
    case 2: {
      g_pUIPopup->CancelPopup();
      g_pUIPopup->SetPopup( GlobalText[1'386], 1, 50, POPUP_OK, nullptr );
    } break;
  }
}
...
void ReceiveBCGuildList( const BYTE* ReceiveBuffer )
{
  ...
  switch ( Data->btResult ) {
    ...
    case 2:
      g_pSystemLogBox->AddText( GlobalText[1'609], SEASON3B::TYPE_SYSTEM_MESSAGE );
      break;
    case 3:
      g_pSystemLogBox->AddText( GlobalText[1'609], SEASON3B::TYPE_SYSTEM_MESSAGE );
      break;
    ...
    case 2: {
      ...
      if ( pMsgBox ) {
        if ( pCha != nullptr && pCha->ID != nullptr ) {
          swprintf( strText, GlobalText[1'486], pCha->ID );
        } else {
          swprintf( strText, GlobalText[1'487] );
        }
        pMsgBox->AddMsg( strText );

        swprintf( strText, GlobalText[1'488], CrownSwitch->ID );
        pMsgBox->AddMsg( strText );
      }
    } break;
  }
...
void ReceiveCrownRegist( const BYTE* ReceiveBuffer )
{
  ...
  switch ( pData->m_byCrownState ) {
    case 0: {
      ...
      if ( pMsgBox ) {
        wchar_t strText[256];
        int     iTime = ( pData->m_dwCrownAccessTime / 1'000 );
        if ( iTime >= 59 ) {
          iTime = 59;
        }
        swprintf( strText, GlobalText[1'980], GlobalText[1'489], iTime );
        pMsgBox->AddMsg( strText );
        pMsgBox->SetElapseTime( 60'000 - pData->m_dwCrownAccessTime );
      }
    } break;
    ...
    case 2: {
      ...
      if ( pMsgBox ) {
        ...
        swprintf( strText, GlobalText[1'980], GlobalText[1'491], iTime );
        pMsgBox->AddMsg( strText );
      }
    } break;
...
void ReceiveBattleCastleProcess( const BYTE* ReceiveBuffer )
{
  ...
  switch ( pData->m_byBasttleCastleState ) {
    case 0: {
      wchar_t Text[100];
      swprintf( Text, GlobalText[1'496], guildName );
      CreateNotice( Text, 1 );
    } break;
    ...
    case 1: {
      ...
      wchar_t Text2[100];
      swprintf( Text2, GlobalText[1'497], guildName );
...
bool ReceiveRequestExChangeLuckyCoin( const BYTE* ReceiveBuffer )
{
  ...
  switch ( _pData->btResult ) {
    ...
    case 1: {
      ...
      g_pSystemLogBox->AddText( GlobalText[1'888], SEASON3B::TYPE_SYSTEM_MESSAGE );
    } break;
    case 2: {
...
bool ReceiveEnterDoppelGangerEvent( const BYTE* ReceiveBuffer )
{
  ...
  switch ( Data->btResult ) {
    ...
    case 1:
      ...
      break;
    case 2:
      swprintf( szText, GlobalText[2'864] );
      g_pSystemLogBox->AddText( szText, SEASON3B::TYPE_ERROR_MESSAGE );
      g_pDoppelGangerWindow->LockEnterButton( TRUE );
      break;
    case 3:
      swprintf( szText, GlobalText[2'865] );
      g_pSystemLogBox->AddText( szText, SEASON3B::TYPE_ERROR_MESSAGE );
      g_pDoppelGangerWindow->LockEnterButton( TRUE );
      break;
    case 4:
...
bool ReceiveDoppelGangerState( const BYTE* ReceiveBuffer )
{
  ...
  switch ( Data->btDoppelgangerState ) {
    ...
    case 2: // ready->play
    {
      ...
      pMsgBox->AddMsg( GlobalText[2'763], RGBA( 255, 255, 255, 255 ), SEASON3B::MSGBOX_FONT_NORMAL );
      pMsgBox->AddMsg( L" " );
      pMsgBox->AddMsg( GlobalText[2'764], RGBA( 255, 255, 255, 255 ), SEASON3B::MSGBOX_FONT_NORMAL );
      pMsgBox->AddMsg( L" " );
      pMsgBox->AddMsg( GlobalText[2'765], RGBA( 255, 255, 255, 255 ), SEASON3B::MSGBOX_FONT_NORMAL );
    } break;
    case 3: // play->end
      break;
  }
...
bool ReceiveDoppelGangerResult( const BYTE* ReceiveBuffer )
{
  ...
  switch ( Data->btResult ) {
    case 0: {
      ...
      SEASON3B::CNewUICommonMessageBox* pMsgBox;
      SEASON3B::CreateMessageBox( MSGBOX_LAYOUT_CLASS( SEASON3B::CDoppelGangerMsgBoxLayout ), &pMsgBox );
      pMsgBox->AddMsg( GlobalText[2'769], RGBA( 255, 255, 255, 255 ), SEASON3B::MSGBOX_FONT_NORMAL );
      pMsgBox->AddMsg( L" " );
      pMsgBox->AddMsg( GlobalText[2'770], RGBA( 255, 255, 255, 255 ), SEASON3B::MSGBOX_FONT_NORMAL );
      // 			pMsgBox->AddMsg(L" ");
      // 			pMsgBox->AddMsg(L" ");
      // 			char szText[256] = { 0, };
    ...
    case 1: {
      SEASON3B::CNewUICommonMessageBox* pMsgBox;
      SEASON3B::CreateMessageBox( MSGBOX_LAYOUT_CLASS( SEASON3B::CDoppelGangerMsgBoxLayout ), &pMsgBox );
      pMsgBox->AddMsg( GlobalText[2'766], RGBA( 255, 255, 255, 255 ), SEASON3B::MSGBOX_FONT_NORMAL );
    } break;
    case 2: {
      SEASON3B::CNewUICommonMessageBox* pMsgBox;
      SEASON3B::CreateMessageBox( MSGBOX_LAYOUT_CLASS( SEASON3B::CDoppelGangerMsgBoxLayout ), &pMsgBox );
      pMsgBox->AddMsg( GlobalText[2'767], RGBA( 255, 255, 255, 255 ), SEASON3B::MSGBOX_FONT_NORMAL );
      pMsgBox->AddMsg( L" " );
      pMsgBox->AddMsg( GlobalText[2'768], RGBA( 255, 255, 255, 255 ), SEASON3B::MSGBOX_FONT_NORMAL );
    } break;
  }
...
bool ReceiveEnterEmpireGuardianEvent( const BYTE* ReceiveBuffer )
{
  ...
  switch ( Data->Result ) {
    ...
    case 1: {
      ...
      pMsgBox->AddMsg( GlobalText[2'798], RGBA( 255, 255, 255, 255 ), SEASON3B::MSGBOX_FONT_NORMAL );
      pMsgBox->AddMsg( L" " );
      wchar_t szText[256] = {
        NULL,
      };
      swprintf( szText, GlobalText[2'799], ( Data->RemainTick / 60'000 ) );
      pMsgBox->AddMsg( szText, RGBA( 255, 255, 255, 255 ), SEASON3B::MSGBOX_FONT_NORMAL );
    } break;
    case 2: {
      SEASON3B::CNewUICommonMessageBox* pMsgBox;
      SEASON3B::CreateMessageBox( MSGBOX_LAYOUT_CLASS( SEASON3B::CEmpireGuardianMsgBoxLayout ), &pMsgBox );
      pMsgBox->AddMsg( GlobalText[2'839], RGBA( 255, 255, 255, 255 ), SEASON3B::MSGBOX_FONT_NORMAL );
    } break;
    case 3: {
      SEASON3B::CNewUICommonMessageBox* pMsgBox;
      SEASON3B::CreateMessageBox( MSGBOX_LAYOUT_CLASS( SEASON3B::CEmpireGuardianMsgBoxLayout ), &pMsgBox );
      pMsgBox->AddMsg( GlobalText[2'841], RGBA( 255, 255, 255, 255 ), SEASON3B::MSGBOX_FONT_NORMAL );
    } break;
    case 4: {
      SEASON3B::CNewUICommonMessageBox* pMsgBox;
      SEASON3B::CreateMessageBox( MSGBOX_LAYOUT_CLASS( SEASON3B::CEmpireGuardianMsgBoxLayout ), &pMsgBox );
      pMsgBox->AddMsg( GlobalText[2'842], RGBA( 255, 255, 255, 255 ), SEASON3B::MSGBOX_FONT_NORMAL );
    } break;
    case 5: {
      SEASON3B::CNewUICommonMessageBox* pMsgBox;
      SEASON3B::CreateMessageBox( MSGBOX_LAYOUT_CLASS( SEASON3B::CEmpireGuardianMsgBoxLayout ), &pMsgBox );
      pMsgBox->AddMsg( GlobalText[2'843], RGBA( 255, 255, 255, 255 ), SEASON3B::MSGBOX_FONT_NORMAL );
    } break;
...
bool ReceiveResultEmpireGuardian( const BYTE* ReceiveBuffer )
{
  ...
  switch ( Data->Result ) {
    case 0: {
      SEASON3B::CNewUICommonMessageBox* pMsgBox;
      SEASON3B::CreateMessageBox( MSGBOX_LAYOUT_CLASS( SEASON3B::CEmpireGuardianMsgBoxLayout ), &pMsgBox );
      pMsgBox->AddMsg( GlobalText[2'803], RGBA( 255, 255, 255, 255 ), SEASON3B::MSGBOX_FONT_NORMAL );
      pMsgBox->AddMsg( GlobalText[2'804], RGBA( 255, 255, 255, 255 ), SEASON3B::MSGBOX_FONT_NORMAL );
    } break;
    case 1: {
      ...
      swprintf( szText, GlobalText[2'801], day );
      pMsgBox->AddMsg( szText, RGBA( 255, 255, 255, 255 ), SEASON3B::MSGBOX_FONT_NORMAL );
      swprintf( szText, L"%d%ls", zone, GlobalText[2'840] );
      pMsgBox->AddMsg( szText, RGBA( 255, 255, 255, 255 ), SEASON3B::MSGBOX_FONT_NORMAL );
    } break;
    case 2: {
      int                               day = g_pEmpireGuardianTimer->GetDay();
      ...
      swprintf( szText, GlobalText[2'801], day );
      pMsgBox->AddMsg( szText, RGBA( 255, 255, 255, 255 ), SEASON3B::MSGBOX_FONT_NORMAL );
      pMsgBox->AddMsg( GlobalText[2'802], RGBA( 255, 255, 255, 255 ), SEASON3B::MSGBOX_FONT_NORMAL );
...
bool ReceiveIGS_BuyItem( const BYTE* pReceiveBuffer )
{
  ...
  switch ( (BYTE)Data->byResultCode ) {
    case -2: {
      ...
      pMsgBox->Initialize( GlobalText[2'902], GlobalText[2'953] );
    } break;
    case -1: {
      CMsgBoxIGSCommon* pMsgBox = nullptr;
      CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
      pMsgBox->Initialize( GlobalText[2'902], GlobalText[2'954] );
    } break;
    case 0: {
      CMsgBoxIGSCommon* pMsgBox = nullptr;
      CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
      pMsgBox->Initialize( GlobalText[2'900], GlobalText[2'901] );
    ...
    case 1: {
      CMsgBoxIGSCommon* pMsgBox = nullptr;
      CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
      pMsgBox->Initialize( GlobalText[2'902], GlobalText[2'903] );
    } break;
    case 2: {
      CMsgBoxIGSCommon* pMsgBox = nullptr;
      CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
      pMsgBox->Initialize( GlobalText[2'902], GlobalText[2'904] );
    } break;
    case 3: {
      CMsgBoxIGSCommon* pMsgBox = nullptr;
      CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
      pMsgBox->Initialize( GlobalText[2'902], GlobalText[2'956] );
    } break;
    case 4: {
      CMsgBoxIGSCommon* pMsgBox = nullptr;
      CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
      pMsgBox->Initialize( GlobalText[2'902], GlobalText[2'957] );
    } break;
    case 5: {
      CMsgBoxIGSCommon* pMsgBox = nullptr;
      CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
      pMsgBox->Initialize( GlobalText[2'902], GlobalText[2'958] );
    } break;
    case 6: {
      CMsgBoxIGSCommon* pMsgBox = nullptr;
      CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
      pMsgBox->Initialize( GlobalText[2'902], GlobalText[3'052] );
    } break;
    case 7: {
      CMsgBoxIGSCommon* pMsgBox = nullptr;
      CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
      pMsgBox->Initialize( GlobalText[2'902], GlobalText[3'053] );
    } break;
    case 8: {
      CMsgBoxIGSCommon* pMsgBox = nullptr;
      CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
      pMsgBox->Initialize( GlobalText[2'902], GlobalText[3'054] );
    } break;
    case 9: {
      CMsgBoxIGSCommon* pMsgBox = nullptr;
      CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
      pMsgBox->Initialize( GlobalText[2'902], GlobalText[3'264] );
    } break;
    default: {
      CMsgBoxIGSCommon* pMsgBox = nullptr;
      CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
      pMsgBox->Initialize( GlobalText[2'945], GlobalText[890] );
    } break;
  }
...
bool ReceiveIGS_SendItemGift( const BYTE* pReceiveBuffer )
{
  ...
  switch ( (BYTE)Data->byResultCode ) {
    case -2: {
      CMsgBoxIGSCommon* pMsgBox = nullptr;
      CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
      pMsgBox->Initialize( GlobalText[2'912], GlobalText[2'953] );
    } break;
    case -1: {
      CMsgBoxIGSCommon* pMsgBox = nullptr;
      CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
      pMsgBox->Initialize( GlobalText[2'912], GlobalText[2'954] );
    } break;
    case 0: {
      CMsgBoxIGSCommon* pMsgBox = nullptr;
      CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
      pMsgBox->Initialize( GlobalText[2'910], GlobalText[2'911] );
    ...
    case 1: {
      CMsgBoxIGSCommon* pMsgBox = nullptr;
      CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
      pMsgBox->Initialize( GlobalText[2'912], GlobalText[2'913] );
    } break;
    case 2: {
      CMsgBoxIGSCommon* pMsgBox = nullptr;
      CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
      pMsgBox->Initialize( GlobalText[2'912], GlobalText[2'914] );
    } break;
    case 3: {
      CMsgBoxIGSCommon* pMsgBox = nullptr;
      CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
      pMsgBox->Initialize( GlobalText[2'912], GlobalText[2'915] );
    } break;
    case 4: {
      CMsgBoxIGSCommon* pMsgBox = nullptr;
      CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
      pMsgBox->Initialize( GlobalText[2'912], GlobalText[2'956] );
    } break;
    case 5: {
      CMsgBoxIGSCommon* pMsgBox = nullptr;
      CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
      pMsgBox->Initialize( GlobalText[2'912], GlobalText[2'958] );
    } break;
    case 6: {
      CMsgBoxIGSCommon* pMsgBox = nullptr;
      CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
      pMsgBox->Initialize( GlobalText[2'945], GlobalText[2'958] );
    } break;
    case 7: {
      CMsgBoxIGSCommon* pMsgBox = nullptr;
      CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
      pMsgBox->Initialize( GlobalText[2'912], GlobalText[2'959] );
    } break;
    case 8: {
      CMsgBoxIGSCommon* pMsgBox = nullptr;
      CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
      pMsgBox->Initialize( GlobalText[2'912], GlobalText[2'960] );
    } break;
    case 9: {
      CMsgBoxIGSCommon* pMsgBox = nullptr;
      CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
      pMsgBox->Initialize( GlobalText[2'912], GlobalText[2'961] );
    } break;
    case 10: {
      CMsgBoxIGSCommon* pMsgBox = nullptr;
      CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
      pMsgBox->Initialize( GlobalText[2'912], GlobalText[3'264] );
    } break;
    case 20: {
      CMsgBoxIGSCommon* pMsgBox = nullptr;
      CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
      pMsgBox->Initialize( GlobalText[2'912], GlobalText[3'263] );
    } break;
    default: {
      CMsgBoxIGSCommon* pMsgBox = nullptr;
      CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
...
bool ReceiveIGS_UseStorageItem( const BYTE* pReceiveBuffer )
{
  ...
  switch ( (BYTE)Data->byResult ) {
    case -2: {
      CMsgBoxIGSCommon* pMsgBox = nullptr;
      CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
      pMsgBox->Initialize( GlobalText[2'928], GlobalText[2'967] );
    } break;
    case -1: {
      CMsgBoxIGSCommon* pMsgBox = nullptr;
      CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
      pMsgBox->Initialize( GlobalText[2'928], GlobalText[2'966] );
    } break;
    case 0: {
      CMsgBoxIGSCommon* pMsgBox = nullptr;
      CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
      pMsgBox->Initialize( GlobalText[2'924], GlobalText[2'925] );
    ...
    case 1: {
      CMsgBoxIGSCommon* pMsgBox = nullptr;
      CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
      pMsgBox->Initialize( GlobalText[2'928], GlobalText[2'962] );
    } break;
    case 2: {
      CMsgBoxIGSCommon* pMsgBox = nullptr;
      CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
      pMsgBox->Initialize( GlobalText[2'928], GlobalText[2'963] );
    } break;
    case 3: {
      CMsgBoxIGSCommon* pMsgBox = nullptr;
      CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
      pMsgBox->Initialize( GlobalText[2'928], GlobalText[2'964] );
    } break;
    case 4: {
      CMsgBoxIGSCommon* pMsgBox = nullptr;
      CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
      pMsgBox->Initialize( GlobalText[2'928], GlobalText[2'965] );
    } break;
    case 21: {
      CMsgBoxIGSCommon* pMsgBox = nullptr;
      CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
      pMsgBox->Initialize( GlobalText[2'928], GlobalText[2'284] );
    } break;
    case 22: {
      CMsgBoxIGSCommon* pMsgBox = nullptr;
      CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
      pMsgBox->Initialize( GlobalText[2'928], GlobalText[3'036] );
    } break;
#ifdef LEM_FIX_SERVERMSG_SEALITEM
    case 24: {
      CMsgBoxIGSCommon* pMsgBox = NULL;
      CreateMessageBox( MSGBOX_LAYOUT_CLASS( CMsgBoxIGSCommonLayout ), &pMsgBox );
      pMsgBox->Initialize( GlobalText[2'928], GlobalText[2'610] );
    } break;
    ...
    switch ( buff ) {
      ...
      case eBuff_LuckOfSanta: {
        ...
        if ( buff == eBuff_BlessingOfXmax ) {
          g_pSystemLogBox->AddText( GlobalText[2'591], SEASON3B::TYPE_SYSTEM_MESSAGE );
          CharacterMachine->CalculateDamage();
          CharacterMachine->CalculateDefense();
        } else if ( buff == eBuff_StrengthOfSanta ) {
          swprintf( _Temp, GlobalText[2'594], 30 );
          g_pSystemLogBox->AddText( _Temp, SEASON3B::TYPE_SYSTEM_MESSAGE );

          CharacterMachine->CalculateDamage();
        } else if ( buff == eBuff_DefenseOfSanta ) {
          swprintf( _Temp, GlobalText[2'595], 100 );
          g_pSystemLogBox->AddText( _Temp, SEASON3B::TYPE_SYSTEM_MESSAGE );

          CharacterMachine->CalculateDefense();
        } else if ( buff == eBuff_QuickOfSanta ) {
          swprintf( _Temp, GlobalText[2'598], 15 );
          g_pSystemLogBox->AddText( _Temp, SEASON3B::TYPE_SYSTEM_MESSAGE );
        } else if ( buff == eBuff_LuckOfSanta ) {
          swprintf( _Temp, GlobalText[2'599], 10 );
          g_pSystemLogBox->AddText( _Temp, SEASON3B::TYPE_SYSTEM_MESSAGE );
        } else if ( buff == eBuff_CureOfSanta ) {
          swprintf( _Temp, GlobalText[2'592], 500 );
          g_pSystemLogBox->AddText( _Temp, SEASON3B::TYPE_SYSTEM_MESSAGE );
        } else if ( buff == eBuff_SafeGuardOfSanta ) {
          swprintf( _Temp, GlobalText[2'593], 500 );
          g_pSystemLogBox->AddText( _Temp, SEASON3B::TYPE_SYSTEM_MESSAGE );
        }
      } break;
