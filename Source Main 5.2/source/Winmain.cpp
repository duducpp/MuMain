...
LRESULT CALLBACK WndProc( HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam )
{
  switch ( msg ) {
    ...
    case WM_NPROTECT_EXIT_TWO:
      ...
      SetTimer( g_hWnd, WINDOWMINIMIZED_TIMER, 1 * 1'000, nullptr );
...
int APIENTRY WinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR szCmdLine, int nCmdShow )
{
  ...
  DWORD dwDisp;
  DWORD dwSize;
  if ( ERROR_SUCCESS == RegCreateKeyEx( HKEY_CURRENT_USER, L"SOFTWARE\\Webzen\\Mu\\Config", 0, nullptr,
                                        REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, nullptr, &hKey, &dwDisp ) ) {
    dwSize = sizeof( m_ID );
    if ( RegQueryValueEx( hKey, L"ID", nullptr, nullptr, (LPBYTE)m_ID, &dwSize ) != ERROR_SUCCESS ) {
    }
    dwSize = sizeof( m_EncryptedPassword );
    if ( RegQueryValueEx( hKey, L"Password", nullptr, nullptr, (LPBYTE)m_EncryptedPassword, &dwSize ) !=
         ERROR_SUCCESS ) {
    } else {
      DATA_BLOB dataIn;
      DATA_BLOB dataOut;
    ...
    dwSize = sizeof( int );
    if ( RegQueryValueEx( hKey, L"WindowMode", nullptr, nullptr, (LPBYTE)&g_bUseWindowMode, &dwSize ) !=
         ERROR_SUCCESS ) {
      g_bUseWindowMode = FALSE;
    }

    dwSize = MAX_LANGUAGE_NAME_LENGTH;
    if ( RegQueryValueEx( hKey, L"LangSelection", nullptr, nullptr, (LPBYTE)g_aszMLSelection, &dwSize ) !=
         ERROR_SUCCESS ) {
  ...
  if ( g_bUseWindowMode == TRUE ) {
    ...
    AdjustWindowRect( &rc, WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX | WS_BORDER | WS_CLIPCHILDREN,
                      NULL );
    g_hWnd = CreateWindow(
        windowName, windowName, WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX | WS_BORDER | WS_CLIPCHILDREN,
  ...
  SetTimer( g_hWnd, HACK_TIMER, 20 * 1'000, nullptr );
  ...
  ItemAttRibuteMemoryDump = new ITEM_ATTRIBUTE[MAX_ITEM + 1'024]{};
  ItemAttribute           = ( (ITEM_ATTRIBUTE*)ItemAttRibuteMemoryDump ) + rand() % 1'024;
