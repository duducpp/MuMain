  ...
  for ( int y = 0; y < TERRAIN_SIZE; y++ ) {
    for ( int x = 0; x < TERRAIN_SIZE; x++ ) {
      ...
      Vector( ( ( x + 1 ) * TERRAIN_SCALE ), ( y * TERRAIN_SCALE ),
              BackTerrainHeight[TERRAIN_INDEX_REPEAT( ( x + 1 ), y )], v1 );
      ...
      Vector( ( x * TERRAIN_SCALE ), ( ( y + 1 ) * TERRAIN_SCALE ),
              BackTerrainHeight[TERRAIN_INDEX_REPEAT( x, ( y + 1 ) )], v3 );
      vec3_t face_normal;
...
bool OpenTerrainHeight( wchar_t* filename )
{
  ...
  if ( fileSize < 0 || fileSize < RequiredSize ) {
    fclose( fp );
    wchar_t Text[256];
    swprintf_s( Text, std::size( Text ), L"%ls is too small (%ld bytes, needs %ld).", FileName, fileSize,
                RequiredSize );
    g_ErrorReport.Write( Text );
    g_ErrorReport.Write( L"\r\n" );
    MessageBox( g_hWnd, Text, NULL, MB_OK );
  ...
  if ( readBytes != Size ) {
    delete[] Buffer;
    wchar_t Text[256];
    swprintf_s( Text, std::size( Text ), L"Failed to read %ls (expected %d bytes, got %zu).", FileName, Size,
                readBytes );
    g_ErrorReport.Write( Text );
    g_ErrorReport.Write( L"\r\n" );
    MessageBox( g_hWnd, Text, NULL, MB_OK );
...
void InterpolationHeight( int lod, int x, int y, int xd, float* TerrainHeight )
{
  ...
  if ( lod >= 2 ) {
    TerrainHeight[TERRAIN_INDEX_REPEAT( x + xd, y + 1 )] =
        ...
        0.5f;
    TerrainHeight[TERRAIN_INDEX_REPEAT( x + xd, y + 3 )] = ( TerrainHeight[TERRAIN_INDEX_REPEAT( x + xd, y + 2 )] +
                                                             TerrainHeight[TERRAIN_INDEX_REPEAT( x + xd, y + 4 )] ) *
                                                           0.5f;
  }
}
...
void InterpolationWidth( int lod, int x, int y, int yd, float* TerrainHeight )
{
  ...
  if ( lod >= 2 ) {
    TerrainHeight[TERRAIN_INDEX_REPEAT( x + 1, y + yd )] =
        ...
        0.5f;
    TerrainHeight[TERRAIN_INDEX_REPEAT( x + 3, y + yd )] = ( TerrainHeight[TERRAIN_INDEX_REPEAT( x + 2, y + yd )] +
                                                             TerrainHeight[TERRAIN_INDEX_REPEAT( x + 4, y + yd )] ) *
                                                           0.5f;
  }
}
...
void InitTerrainLight()
{
  ...
  if ( EnableEvent == 0 ) {
    WindScale = 10.f;
    WindSpeed = (int)WorldTime % ( 360'000 * 2 ) * ( 0.002f );
  } else {
    WindScale = 10.f;
    WindSpeed = (int)WorldTime % 36'000 * ( 0.01f );
  }
#ifdef ASG_ADD_MAP_KARUTAN
  ...
  if ( IsKarutanMap() ) {
    WindScale1 = 15.f;
    WindSpeed1 = (int)WorldTime % 36'000 * ( 0.008f );
  }
#endif // ASG_ADD_MAP_KARUTAN
  yi = FrustrumBoundMinY;
...
void RenderTerrain( bool EditFlag )
{
  if ( !EditFlag ) {
    if ( gMapManager.WorldActive == WD_8TARKAN ) {
      WaterMove = (float)( (int)( WorldTime ) % 40'000 ) * 0.000025f;
    } else if ( gMapManager.WorldActive == WD_42CHANGEUP3RD_2ND ) {
      int iWorldTime = (int)WorldTime;
      int iRemainder = iWorldTime % 50'000;
      WaterMove      = (float)iRemainder * 0.00002f;
    } else {
      WaterMove = (float)( (int)( WorldTime ) % 20'000 ) * 0.00005f;
    }
  }
