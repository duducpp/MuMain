...
void SetEffectVolumeLevel( int level )
{
  ...
  if ( level == 0 ) {
    SetMasterVolume( -10'000 );
  } else {
    long vol = -2'000 * log10( 10.f / float( level ) );
    SetMasterVolume( vol );
  }
}
  ...
  if ( message == MESSAGE_USE_STATE || message == MESSAGE_USE_STATE2 ) {
    ...
    if ( TargetItem.Type == ITEM_FRUITS ) {
      switch ( TargetItem.Level ) {
        case 0:
          swprintf( Name, L"%ls", GlobalText[168] );
          break;
        case 1:
          swprintf( Name, L"%ls", GlobalText[169] );
          break;
        case 2:
          swprintf( Name, L"%ls", GlobalText[167] );
          break;
        case 3:
          swprintf( Name, L"%ls", GlobalText[166] );
          break;
        case 4:
          swprintf( Name, L"%ls", GlobalText[1'900] );
          break;
      }
    }

    if ( message == MESSAGE_USE_STATE2 ) {
      swprintf( g_lpszMessageBoxCustom[0], L"( %ls%ls )", Name, GlobalText[1'901] );
    } else {
    ...
    if ( COMGEM::isComMode() ) {
      if ( COMGEM::m_cGemType == 0 ) {
        swprintf( tBuf, GlobalText[1'809], GlobalText[1'806], COMGEM::m_cCount );
      } else {
        swprintf( tBuf, GlobalText[1'809], GlobalText[1'807], COMGEM::m_cCount );
      }
      ...
      swprintf( g_lpszMessageBoxCustom[g_iNumLineMessageBoxCustom], GlobalText[1'810], COMGEM::m_iValue );
      ++g_iNumLineMessageBoxCustom;
    } else {
      int t_GemLevel = COMGEM::GetUnMixGemLevel() + 1;
      if ( COMGEM::m_cGemType == 0 ) {
        swprintf( tBuf, GlobalText[1'813], GlobalText[1'806], t_GemLevel );
      } else {
        swprintf( tBuf, GlobalText[1'813], GlobalText[1'807], t_GemLevel );
      }
      ...
      swprintf( g_lpszMessageBoxCustom[g_iNumLineMessageBoxCustom], GlobalText[1'814], COMGEM::m_iValue );
      ++g_iNumLineMessageBoxCustom;
    }
  } else if ( message == MESSAGE_CANCEL_SKILL ) {
    wchar_t tBuf[MAX_GLOBAL_TEXT_STRING];
    swprintf( tBuf, L"%ls%ls", SkillAttribute[index].Name, GlobalText[2'046] );
...
float  CameraWalk[] = {
  0.f, -1000.f, 500.f, -80.f, 0.f, 0.f, 0.f, -1100.f, 500.f, -80.f, 0.f, 0.f, 0.f,   -1100.f, 500.f, -80.f, 0.f, 0.f,
  0.f, -1100.f, 500.f, -80.f, 0.f, 0.f, 0.f, -1100.f, 500.f, -80.f, 0.f, 0.f, 200.f, -800.f,  250.f, -87.f, 0.f, -10.f,
};
...
bool NewRenderCharacterScene( HDC hDC )
{
  ...
  if ( SelectedHero != -1 && o->Live ) {
    ...
    float Rotation = (int)WorldTime % 3'600 / (float)10.f;
...
void CreateLogInScene()
{
  ...
  CCameraMove::GetInstancePtr()->PlayCameraWalk( Hero->Object.Position, 1'000 );
    ...
    if ( battleCastle::InBattleCastle2( Hero->Object.Position ) ) {
      ...
      switch ( g_shCameraLevel ) {
        ...
        case 1:
          ...
          break;
        case 2:
          CameraViewFar = 2600.f;
          break;
        case 3:
          CameraViewFar = 2950.f;
          break;
        case 5:
        case 4:
          CameraViewFar = 3200.f;
...
void MoveMainScene()
{
  ...
  if ( ( gMapManager.WorldActive == WD_0LORENCIA && HeroTile != 4 ) ||
       ...
       gMapManager.InDevilSquare() == true || gMapManager.WorldActive == WD_10HEAVEN ||
       gMapManager.InChaosCastle() == true || gMapManager.InBattleCastle() == true ||
       M31HuntingGround::IsInHuntingGround() == true || M33Aida::IsInAida() == true ||
       M34CryWolf1st::IsCyrWolf1st() == true || gMapManager.WorldActive == WD_42CHANGEUP3RD_2ND || IsIceCity() ||
...
bool RenderMainScene()
{
  ...
  if ( ( gMapManager.WorldActive == WD_2DEVIAS && HeroTile != 3 && HeroTile < 10 ) || IsIceCity() || IsSantaTown() ||
...
void UpdateSceneState()
{
  ...
  if ( GrabEnable ) {
    SYSTEMTIME st;
    GetLocalTime( &st );
    swprintf( GrabFileName, L"Screen(%02d_%02d-%02d_%02d)-%04d.jpg", st.wMonth, st.wDay, st.wHour, st.wMinute,
              GrabScreen );
...
void MainScene( HDC hDC )
{
  ...
  constexpr double timePerFrame           = 1'000 / REFERENCE_FPS;
  ...
  try {
    ...
    swprintf( szCamera3D, L"Camera3D : %.1f %.1f:%.1f:%.1f", CameraFOV, CameraAngle[0], CameraAngle[1],
              CameraAngle[2] );
