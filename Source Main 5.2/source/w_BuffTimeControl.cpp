...
eBuffTimeType BuffTimeControl::CheckBuffTimeType( eBuffState bufftype )
{
  ...
  return eBuffTimeType( 1'005 + bInfo.s_BuffEffectType );
}
...
void BuffTimeControl::RegisterBuffTime( eBuffState bufftype, DWORD curbufftime )
{
  ...
  buffinfo.s_CurBuffTime   = curbufftime * 1'000;
...
void BuffTimeControl::GetStringTime( DWORD time, std::wstring& timeText, bool isSecond )
{
  ...
  if ( isSecond ) {
    ...
    if ( day != 0 ) {
      swprintf( buffer, L"%d %ls %d %ls %d %ls %d %ls", day, GlobalText[2'298], oClock, GlobalText[2'299], minutes,
                GlobalText[2'300], second, GlobalText[2'301] );
      timeText = buffer;
    } else if ( day == 0 && oClock != 0 ) {
      swprintf( buffer, L"%d %ls %d %ls %d %ls", oClock, GlobalText[2'299], minutes, GlobalText[2'300], second,
                GlobalText[2'301] );
      timeText = buffer;
    } else if ( day == 0 && oClock == 0 && minutes != 0 ) {
      swprintf( buffer, L"%d %ls %d %ls", minutes, GlobalText[2'300], second, GlobalText[2'301] );
      timeText = buffer;
    } else if ( day == 0 && oClock == 0 && minutes == 0 ) {
      swprintf( buffer, L"%ls", GlobalText[2'308] );
      timeText = buffer;
    }
  } else {
    ...
    if ( day != 0 ) {
      swprintf( buffer, L"%d %ls %d %ls %d %ls", day, GlobalText[2'298], oClock, GlobalText[2'299], minutes,
                GlobalText[2'300] );
      timeText = buffer;
    } else if ( day == 0 && oClock != 0 ) {
      swprintf( buffer, L"%d %ls %d %ls", oClock, GlobalText[2'299], minutes, GlobalText[2'300] );
      timeText = buffer;
    } else if ( day == 0 && oClock == 0 && minutes != 0 ) {
      swprintf( buffer, L"%d %ls", minutes, GlobalText[2'300] );
      timeText = buffer;
    }
  }
}
